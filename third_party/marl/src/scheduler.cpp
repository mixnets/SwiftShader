...
_When_(return == true,
              _Acquires_lock_(work.mutex)) bool Scheduler::Worker::tryLock() {
...
_Requires_lock_held_(work.mutex) _Releases_lock_(
    work.mutex) void Scheduler::Worker::enqueueAndUnlock(Task&& task) {
...
_Requires_lock_held_(work.mutex) void Scheduler::Worker::waitForWork() {
...
_Requires_lock_held_(
    work.mutex) void Scheduler::Worker::enqueueFiberTimeouts() {
...
_Requires_lock_held_(work.mutex) void Scheduler::Worker::changeFiberState(
    Fiber* fiber,
    Fiber::State from,
...
_Requires_lock_held_(work.mutex) void Scheduler::Worker::setFiberState(
    Fiber* fiber,
    Fiber::State to) const {
...
_Requires_lock_held_(work.mutex) void Scheduler::Worker::runUntilIdle() {
  ASSERT_FIBER_STATE(currentFiber, Fiber::State::Running);
  MARL_ASSERT(work.num == work.fibers.size() + work.tasks.size(),
              "work.num out of sync");
