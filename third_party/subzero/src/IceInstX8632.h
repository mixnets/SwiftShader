...
struct InstImpl {
  ...
  private:
    static constexpr const char *Opcode = nullptr;
    ...
    Inst *deoptLeaToAddOrNull(const Cfg *Func) const {
      ...
      if (auto *MemOp = llvm::dyn_cast<X86OperandMem>(this->getSrc(0))) {
        if (getFlags().getAggressiveLea() &&
          ...
          auto *Add = InstImpl::InstX86Add::create(
  ...
  public:
    static InstX86Pcmpeq *create(Cfg *Func, Variable *Dest, Operand *Source,
      ...
      const Type Ty = ArithmeticTypeOverride == IceType_void
                          ...
                          : ArithmeticTypeOverride;
      (void)Ty;
      assert((Ty != IceType_f64 && Ty != IceType_i64)/**  FIXME(capn)  ||
  ...
  public:
    static InstX86Pcmpgt *create(Cfg *Func, Variable *Dest, Operand *Source) {
      assert(Dest->getType() != IceType_f64/**  FIXME(capn)  ||
             InstX86Base::getTarget(Func)->getInstructionSet() >=
                 Traits::SSE4_1**/);
  ...
  public:
    static InstX86Pinsr *create(Cfg *Func, Variable *Dest, Operand *Source1,
      ...
      assert(
          Dest->getType() == IceType_v8i16 || Dest->getType() == IceType_v8i1/**  FIXME(capn)  ||
  ...
  public:
    static InstX86Pextr *create(Cfg *Func, Variable *Dest, Operand *Source0,
      ...
      assert(Source0->getType() == IceType_v8i16 ||
             Source0->getType() == IceType_v8i1/**  FIXME(capn)  ||
             InstX86Base::getTarget(Func)->getInstructionSet() >=
                 Traits::SSE4_1**/);
...
struct Insts {
  using FakeRMW = typename InstImpl::InstX86FakeRMW;
  using Label = typename InstImpl::InstX86Label;

  using Call = typename InstImpl::InstX86Call;

  using Br = typename InstImpl::InstX86Br;
  using Jmp = typename InstImpl::InstX86Jmp;
  using Bswap = typename InstImpl::InstX86Bswap;
  using Neg = typename InstImpl::InstX86Neg;
  using Bsf = typename InstImpl::InstX86Bsf;
  using Bsr = typename InstImpl::InstX86Bsr;
  using Lea = typename InstImpl::InstX86Lea;
  using Cbwdq = typename InstImpl::InstX86Cbwdq;
  using Movsx = typename InstImpl::InstX86Movsx;
  using Movzx = typename InstImpl::InstX86Movzx;
  using Movd = typename InstImpl::InstX86Movd;
  using Movmsk = typename InstImpl::InstX86Movmsk;
  using Sqrt = typename InstImpl::InstX86Sqrt;
  using Mov = typename InstImpl::InstX86Mov;
  using Movp = typename InstImpl::InstX86Movp;
  using Movq = typename InstImpl::InstX86Movq;
  using Add = typename InstImpl::InstX86Add;
  using AddRMW = typename InstImpl::InstX86AddRMW;
  using Addps = typename InstImpl::InstX86Addps;
  using Adc = typename InstImpl::InstX86Adc;
  using AdcRMW = typename InstImpl::InstX86AdcRMW;
  using Addss = typename InstImpl::InstX86Addss;
  using Andnps = typename InstImpl::InstX86Andnps;
  using Andps = typename InstImpl::InstX86Andps;
  using Padd = typename InstImpl::InstX86Padd;
  using Padds = typename InstImpl::InstX86Padds;
  using Paddus = typename InstImpl::InstX86Paddus;
  using Sub = typename InstImpl::InstX86Sub;
  using SubRMW = typename InstImpl::InstX86SubRMW;
  using Subps = typename InstImpl::InstX86Subps;
  using Subss = typename InstImpl::InstX86Subss;
  using Sbb = typename InstImpl::InstX86Sbb;
  using SbbRMW = typename InstImpl::InstX86SbbRMW;
  using Psub = typename InstImpl::InstX86Psub;
  using Psubs = typename InstImpl::InstX86Psubs;
  using Psubus = typename InstImpl::InstX86Psubus;
  using And = typename InstImpl::InstX86And;
  using AndRMW = typename InstImpl::InstX86AndRMW;
  using Pand = typename InstImpl::InstX86Pand;
  using Pandn = typename InstImpl::InstX86Pandn;
  using Or = typename InstImpl::InstX86Or;
  using Orps = typename InstImpl::InstX86Orps;
  using OrRMW = typename InstImpl::InstX86OrRMW;
  using Por = typename InstImpl::InstX86Por;
  using Xor = typename InstImpl::InstX86Xor;
  using Xorps = typename InstImpl::InstX86Xorps;
  using XorRMW = typename InstImpl::InstX86XorRMW;
  using Pxor = typename InstImpl::InstX86Pxor;
  using Maxss = typename InstImpl::InstX86Maxss;
  using Minss = typename InstImpl::InstX86Minss;
  using Maxps = typename InstImpl::InstX86Maxps;
  using Minps = typename InstImpl::InstX86Minps;
  using Imul = typename InstImpl::InstX86Imul;
  using ImulImm = typename InstImpl::InstX86ImulImm;
  using Mulps = typename InstImpl::InstX86Mulps;
  using Mulss = typename InstImpl::InstX86Mulss;
  using Pmull = typename InstImpl::InstX86Pmull;
  using Pmulhw = typename InstImpl::InstX86Pmulhw;
  using Pmulhuw = typename InstImpl::InstX86Pmulhuw;
  using Pmaddwd = typename InstImpl::InstX86Pmaddwd;
  using Pmuludq = typename InstImpl::InstX86Pmuludq;
  using Divps = typename InstImpl::InstX86Divps;
  using Divss = typename InstImpl::InstX86Divss;
  using Rol = typename InstImpl::InstX86Rol;
  using Shl = typename InstImpl::InstX86Shl;
  using Psll = typename InstImpl::InstX86Psll;
  using Psrl = typename InstImpl::InstX86Psrl;
  using Shr = typename InstImpl::InstX86Shr;
  using Sar = typename InstImpl::InstX86Sar;
  using Psra = typename InstImpl::InstX86Psra;
  using Pcmpeq = typename InstImpl::InstX86Pcmpeq;
  using Pcmpgt = typename InstImpl::InstX86Pcmpgt;
  using MovssRegs = typename InstImpl::InstX86MovssRegs;
  using Idiv = typename InstImpl::InstX86Idiv;
  using Div = typename InstImpl::InstX86Div;
  using Insertps = typename InstImpl::InstX86Insertps;
  using Pinsr = typename InstImpl::InstX86Pinsr;
  using Shufps = typename InstImpl::InstX86Shufps;
  using Blendvps = typename InstImpl::InstX86Blendvps;
  using Pblendvb = typename InstImpl::InstX86Pblendvb;
  using Pextr = typename InstImpl::InstX86Pextr;
  using Pshufd = typename InstImpl::InstX86Pshufd;
  using Lockable = typename InstImpl::InstX86BaseLockable;
  using Mul = typename InstImpl::InstX86Mul;
  using Shld = typename InstImpl::InstX86Shld;
  using Shrd = typename InstImpl::InstX86Shrd;
  using Cmov = typename InstImpl::InstX86Cmov;
  using Cmpps = typename InstImpl::InstX86Cmpps;
  using Cmpxchg = typename InstImpl::InstX86Cmpxchg;
  using Cmpxchg8b = typename InstImpl::InstX86Cmpxchg8b;
  using Cvt = typename InstImpl::InstX86Cvt;
  using Round = typename InstImpl::InstX86Round;
  using Icmp = typename InstImpl::InstX86Icmp;
  using Ucomiss = typename InstImpl::InstX86Ucomiss;
  using UD2 = typename InstImpl::InstX86UD2;
  using Int3 = typename InstImpl::InstX86Int3;
  using Test = typename InstImpl::InstX86Test;
  using Mfence = typename InstImpl::InstX86Mfence;
  using Store = typename InstImpl::InstX86Store;
  using StoreP = typename InstImpl::InstX86StoreP;
  using StoreQ = typename InstImpl::InstX86StoreQ;
  using StoreD = typename InstImpl::InstX86StoreD;
  using Nop = typename InstImpl::InstX86Nop;
  using Pop = typename InstImpl::InstX86Pop;
  using Push = typename InstImpl::InstX86Push;
  using Ret = typename InstImpl::InstX86Ret;
  using Setcc = typename InstImpl::InstX86Setcc;
  using Xadd = typename InstImpl::InstX86Xadd;
  using Xchg = typename InstImpl::InstX86Xchg;

  using IacaStart = typename InstImpl::InstX86IacaStart;
  using IacaEnd = typename InstImpl::InstX86IacaEnd;

  using Pshufb = typename InstImpl::InstX86Pshufb;
  using Punpckl = typename InstImpl::InstX86Punpckl;
  using Punpckh = typename InstImpl::InstX86Punpckh;
  using Packss = typename InstImpl::InstX86Packss;
  using Packus = typename InstImpl::InstX86Packus;
};
...
/* In-place ops */
template <>
constexpr const char *InstImpl::InstX86Bswap::Base::Opcode = "bswap";
template <> constexpr const char *InstImpl::InstX86Neg::Base::Opcode = "neg";
/* Unary ops */
template <> constexpr const char *InstImpl::InstX86Bsf::Base::Opcode = "bsf";
template <> constexpr const char *InstImpl::InstX86Bsr::Base::Opcode = "bsr";
template <> constexpr const char *InstImpl::InstX86Lea::Base::Opcode = "lea";
template <> constexpr const char *InstImpl::InstX86Movd::Base::Opcode = "movd";
template <> constexpr const char *InstImpl::InstX86Movsx::Base::Opcode = "movs";
template <> constexpr const char *InstImpl::InstX86Movzx::Base::Opcode = "movz";
template <> constexpr const char *InstImpl::InstX86Sqrt::Base::Opcode = "sqrt";
template <>
constexpr const char *InstImpl::InstX86Cbwdq::Base::Opcode = "cbw/cwd/cdq";
/* Mov-like ops */
template <> constexpr const char *InstImpl::InstX86Mov::Base::Opcode = "mov";
template <>
constexpr const char *InstImpl::InstX86Movp::Base::Opcode = "movups";
template <> constexpr const char *InstImpl::InstX86Movq::Base::Opcode = "movq";
/* Binary ops */
template <> constexpr const char *InstImpl::InstX86Add::Base::Opcode = "add";
template <> constexpr const char *InstImpl::InstX86AddRMW::Base::Opcode = "add";
template <> constexpr const char *InstImpl::InstX86Addps::Base::Opcode = "add";
template <> constexpr const char *InstImpl::InstX86Adc::Base::Opcode = "adc";
template <> constexpr const char *InstImpl::InstX86AdcRMW::Base::Opcode = "adc";
template <> constexpr const char *InstImpl::InstX86Addss::Base::Opcode = "add";
template <>
constexpr const char *InstImpl::InstX86Andnps::Base::Opcode = "andn";
template <> constexpr const char *InstImpl::InstX86Andps::Base::Opcode = "and";
template <> constexpr const char *InstImpl::InstX86Maxss::Base::Opcode = "max";
template <> constexpr const char *InstImpl::InstX86Minss::Base::Opcode = "min";
template <> constexpr const char *InstImpl::InstX86Maxps::Base::Opcode = "max";
template <> constexpr const char *InstImpl::InstX86Minps::Base::Opcode = "min";
template <> constexpr const char *InstImpl::InstX86Padd::Base::Opcode = "padd";
template <>
constexpr const char *InstImpl::InstX86Padds::Base::Opcode = "padds";
template <>
constexpr const char *InstImpl::InstX86Paddus::Base::Opcode = "paddus";
template <> constexpr const char *InstImpl::InstX86Sub::Base::Opcode = "sub";
template <> constexpr const char *InstImpl::InstX86SubRMW::Base::Opcode = "sub";
template <> constexpr const char *InstImpl::InstX86Subps::Base::Opcode = "sub";
template <> constexpr const char *InstImpl::InstX86Subss::Base::Opcode = "sub";
template <> constexpr const char *InstImpl::InstX86Sbb::Base::Opcode = "sbb";
template <> constexpr const char *InstImpl::InstX86SbbRMW::Base::Opcode = "sbb";
template <> constexpr const char *InstImpl::InstX86Psub::Base::Opcode = "psub";
template <>
constexpr const char *InstImpl::InstX86Psubs::Base::Opcode = "psubs";
template <>
constexpr const char *InstImpl::InstX86Psubus::Base::Opcode = "psubus";
template <> constexpr const char *InstImpl::InstX86And::Base::Opcode = "and";
template <> constexpr const char *InstImpl::InstX86AndRMW::Base::Opcode = "and";
template <> constexpr const char *InstImpl::InstX86Pand::Base::Opcode = "pand";
template <>
constexpr const char *InstImpl::InstX86Pandn::Base::Opcode = "pandn";
template <> constexpr const char *InstImpl::InstX86Or::Base::Opcode = "or";
template <> constexpr const char *InstImpl::InstX86Orps::Base::Opcode = "or";
template <> constexpr const char *InstImpl::InstX86OrRMW::Base::Opcode = "or";
template <> constexpr const char *InstImpl::InstX86Por::Base::Opcode = "por";
template <> constexpr const char *InstImpl::InstX86Xor::Base::Opcode = "xor";
template <> constexpr const char *InstImpl::InstX86Xorps::Base::Opcode = "xor";
template <> constexpr const char *InstImpl::InstX86XorRMW::Base::Opcode = "xor";
template <> constexpr const char *InstImpl::InstX86Pxor::Base::Opcode = "pxor";
template <> constexpr const char *InstImpl::InstX86Imul::Base::Opcode = "imul";
template <>
constexpr const char *InstImpl::InstX86ImulImm::Base::Opcode = "imul";
template <> constexpr const char *InstImpl::InstX86Mulps::Base::Opcode = "mul";
template <> constexpr const char *InstImpl::InstX86Mulss::Base::Opcode = "mul";
template <>
constexpr const char *InstImpl::InstX86Pmull::Base::Opcode = "pmull";
template <>
constexpr const char *InstImpl::InstX86Pmulhw::Base::Opcode = "pmulhw";
template <>
constexpr const char *InstImpl::InstX86Pmulhuw::Base::Opcode = "pmulhuw";
template <>
constexpr const char *InstImpl::InstX86Pmaddwd::Base::Opcode = "pmaddwd";
template <>
constexpr const char *InstImpl::InstX86Pmuludq::Base::Opcode = "pmuludq";
template <> constexpr const char *InstImpl::InstX86Div::Base::Opcode = "div";
template <> constexpr const char *InstImpl::InstX86Divps::Base::Opcode = "div";
template <> constexpr const char *InstImpl::InstX86Divss::Base::Opcode = "div";
template <> constexpr const char *InstImpl::InstX86Idiv::Base::Opcode = "idiv";
template <> constexpr const char *InstImpl::InstX86Rol::Base::Opcode = "rol";
template <> constexpr const char *InstImpl::InstX86Shl::Base::Opcode = "shl";
template <> constexpr const char *InstImpl::InstX86Psll::Base::Opcode = "psll";
template <> constexpr const char *InstImpl::InstX86Shr::Base::Opcode = "shr";
template <> constexpr const char *InstImpl::InstX86Sar::Base::Opcode = "sar";
template <> constexpr const char *InstImpl::InstX86Psra::Base::Opcode = "psra";
template <> constexpr const char *InstImpl::InstX86Psrl::Base::Opcode = "psrl";
template <>
constexpr const char *InstImpl::InstX86Pcmpeq::Base::Opcode = "pcmpeq";
template <>
constexpr const char *InstImpl::InstX86Pcmpgt::Base::Opcode = "pcmpgt";
template <>
constexpr const char *InstImpl::InstX86MovssRegs::Base::Opcode = "movss";
/* Ternary ops */
template <>
constexpr const char *InstImpl::InstX86Insertps::Base::Opcode = "insertps";
template <>
constexpr const char *InstImpl::InstX86Round::Base::Opcode = "round";
template <>
constexpr const char *InstImpl::InstX86Shufps::Base::Opcode = "shufps";
template <>
constexpr const char *InstImpl::InstX86Pinsr::Base::Opcode = "pinsr";
template <>
constexpr const char *InstImpl::InstX86Blendvps::Base::Opcode = "blendvps";
template <>
constexpr const char *InstImpl::InstX86Pblendvb::Base::Opcode = "pblendvb";
/* Three address ops */
template <>
constexpr const char *InstImpl::InstX86Pextr::Base::Opcode = "pextr";
template <>
constexpr const char *InstImpl::InstX86Pshufd::Base::Opcode = "pshufd";
template <>
constexpr const char *InstImpl::InstX86Pshufb::Base::Opcode = "pshufb";
template <>
constexpr const char *InstImpl::InstX86Punpckl::Base::Opcode = "punpckl";
template <>
constexpr const char *InstImpl::InstX86Punpckh::Base::Opcode = "punpckh";
template <>
constexpr const char *InstImpl::InstX86Packss::Base::Opcode = "packss";
template <>
constexpr const char *InstImpl::InstX86Packus::Base::Opcode = "packus";
/* Inplace GPR ops */
template <>
constexpr const InstImpl::Assembler::GPREmitterOneOp
    InstImpl::InstX86Bswap::Base::Emitter = {
        &InstImpl::Assembler::bswap, nullptr /* only a reg form exists */
};
template <>
constexpr const InstImpl::Assembler::GPREmitterOneOp
    InstImpl::InstX86Neg::Base::Emitter = {&InstImpl::Assembler::neg,
                                           &InstImpl::Assembler::neg};
/* Unary GPR ops */
/* uses specialized emitter. */
template <>
constexpr const InstImpl::Assembler::GPREmitterRegOp
    InstImpl::InstX86Cbwdq::Base::Emitter = {nullptr, nullptr, nullptr};
template <>
constexpr const InstImpl::Assembler::GPREmitterRegOp
    InstImpl::InstX86Bsf::Base::Emitter = {&InstImpl::Assembler::bsf,
                                           &InstImpl::Assembler::bsf, nullptr};
template <>
constexpr const InstImpl::Assembler::GPREmitterRegOp
    InstImpl::InstX86Bsr::Base::Emitter = {&InstImpl::Assembler::bsr,
                                           &InstImpl::Assembler::bsr, nullptr};
template <>
constexpr const InstImpl::Assembler::GPREmitterRegOp
    InstImpl::InstX86Lea::Base::Emitter = {
...
/* uses specialized emitter. */
template <>
constexpr const InstImpl::Assembler::XmmEmitterRegOp
...
template <>
constexpr const InstImpl::Assembler::GPREmitterRegOp
    InstImpl::InstX86Add::Base::Emitter = {&InstImpl::Assembler::add,
                                           &InstImpl::Assembler::add,
                                           &InstImpl::Assembler::add};
template <>
constexpr const InstImpl::Assembler::GPREmitterAddrOp
    InstImpl::InstX86AddRMW::Base::Emitter = {&InstImpl::Assembler::add,
                                              &InstImpl::Assembler::add};
template <>
constexpr const InstImpl::Assembler::GPREmitterRegOp
    InstImpl::InstX86Adc::Base::Emitter = {&InstImpl::Assembler::adc,
...
template <>
constexpr const InstImpl::Assembler::GPREmitterRegOp
    InstImpl::InstX86And::Base::Emitter = {&InstImpl::Assembler::And,
...
template <>
constexpr const InstImpl::Assembler::GPREmitterRegOp
    InstImpl::InstX86Or::Base::Emitter = {&InstImpl::Assembler::Or,
...
template <>
constexpr const InstImpl::Assembler::GPREmitterRegOp
    InstImpl::InstX86Sbb::Base::Emitter = {&InstImpl::Assembler::sbb,
...
template <>
constexpr const InstImpl::Assembler::GPREmitterRegOp
    InstImpl::InstX86Sub::Base::Emitter = {&InstImpl::Assembler::sub,
...
template <>
constexpr const InstImpl::Assembler::GPREmitterRegOp
    InstImpl::InstX86Xor::Base::Emitter = {&InstImpl::Assembler::Xor,
...
template <>
constexpr const InstImpl::Assembler::GPREmitterShiftOp
...
template <>
constexpr const InstImpl::Assembler::XmmEmitterRegOp
    InstImpl::InstX86Por::Base::Emitter = {&InstImpl::Assembler::por,
                                           &InstImpl::Assembler::por};
template <>
constexpr const InstImpl::Assembler::XmmEmitterRegOp
...
template <>
constexpr const InstImpl::Assembler::XmmEmitterShiftOp
