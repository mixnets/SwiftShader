    ...
    if (Offset) {
      if (!llvm::isa<ConstantRelocatable>(Offset)) {
        if (llvm::isa<ConstantInteger64>(Offset)) {
          ...
          // During memory operand building, we do not blind or pool the
          // constant
          // offset, we will work on the whole memory operand later as one
          // entity later, this save one instruction. By turning blinding and
          // pooling off, we guarantee legalize(Offset) will return a Constant*.
          BoolFlagSaver B(RandomizationPoolingPaused, true);
