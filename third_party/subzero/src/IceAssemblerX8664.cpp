...
//
/// \file
/// \brief Implements the AssemblerX8664 template class, which is the base
...
template <typename, typename> void AssemblerX8664::pushal() {
  AssemblerBuffer::EnsureCapacity ensured(&Buffer);
  emitUint8(0x60);
}

template <typename, typename> void AssemblerX8664::popal() {
  AssemblerBuffer::EnsureCapacity ensured(&Buffer);
  emitUint8(0x61);
}
