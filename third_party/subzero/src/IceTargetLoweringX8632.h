...
protected:
  ...
  void initRebasePtr() {
    ...
    dispatchToConcrete(&Traits::TargetLowering::initRebasePtr);
  }
  ...
  void initSandbox() {
    assert(SandboxingType != ST_None);
    dispatchToConcrete(&Traits::TargetLowering::initSandbox);
  }
  ...
  bool legalizeOptAddrForSandbox(OptAddr *Addr) {
    return dispatchToConcrete(
        &Traits::TargetLowering::legalizeOptAddrForSandbox, std::move(Addr));
  }
  ...
  void emitGetIP(CfgNode *Node) {
    dispatchToConcrete(&Traits::TargetLowering::emitGetIP, std::move(Node));
  }
  /// Emit a sandboxed return sequence rather than a return.
  void emitSandboxedReturn() {
    dispatchToConcrete(&Traits::TargetLowering::emitSandboxedReturn);
  }

  void emitStackProbe(size_t StackSizeBytes) {
    dispatchToConcrete(&Traits::TargetLowering::emitStackProbe,
  ...
  void _add_sp(Operand *Adjustment) {
    dispatchToConcrete(&Traits::TargetLowering::_add_sp, std::move(Adjustment));
  }
  ...
  void _link_bp() { dispatchToConcrete(&Traits::TargetLowering::_link_bp); }
  void _push_reg(RegNumT RegNum) {
    dispatchToConcrete(&Traits::TargetLowering::_push_reg, std::move(RegNum));
  }
  void _pop_reg(RegNumT RegNum) {
    dispatchToConcrete(&Traits::TargetLowering::_pop_reg, std::move(RegNum));
  }
  ...
  void _mov_sp(Operand *NewValue) {
    dispatchToConcrete(&Traits::TargetLowering::_mov_sp, std::move(NewValue));
  }
  ...
  X86OperandMem *_sandbox_mem_reference(X86OperandMem *Mem) {
    return dispatchToConcrete(&Traits::TargetLowering::_sandbox_mem_reference,
                              std::move(Mem));
  }
  ...
  void _sub_sp(Operand *Adjustment) {
    dispatchToConcrete(&Traits::TargetLowering::_sub_sp, std::move(Adjustment));
  }
  ...
  void _unlink_bp() { dispatchToConcrete(&Traits::TargetLowering::_unlink_bp); }
...
class TargetX8632 final : public ::Ice::X8632::TargetX8632<X8632::Traits> {
  TargetX8632() = delete;
  TargetX8632(const TargetX8632 &) = delete;
  TargetX8632 &operator=(const TargetX8632 &) = delete;
...
private:
  ENABLE_MAKE_UNIQUE;
  friend class X8632::TargetX8632<X8632::Traits>;

  explicit TargetX8632(Cfg *Func) : TargetX8632(Func) {}
