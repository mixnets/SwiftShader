...
void BoolFolding<Traits>::init(CfgNode *Node) {
  Producers.clear();
...
void TargetX8632<TraitsType>::initNodeForLowering(CfgNode *Node) {
  FoldingInfo.init(Node);
  FoldingInfo.dump(Func);
}

TargetX8632<TraitsType>::TargetX8632(Cfg *Func)
...
void TargetX8632<TraitsType>::staticInit(GlobalContext *Ctx) {
...
bool TargetX8632<TraitsType>::shouldBePooled(const Constant *C) {
...
::Ice::Type TargetX8632<TraitsType>::getPointerType() {
  if (!Traits::Is64Bit ||
...
void TargetX8632<TraitsType>::translateO2() {
...
void TargetX8632<TraitsType>::translateOm1() {
...
bool isSameMemAddressOperand(const Operand *A, const Operand *B) {
  if (A == B)
    return true;
  if (auto *MemA =
          llvm::dyn_cast<typename TargetX8632<TraitsType>::X86OperandMem>(A)) {
    if (auto *MemB =
            llvm::dyn_cast<typename TargetX8632<TraitsType>::X86OperandMem>(
                B)) {
...
void TargetX8632<TraitsType>::findRMW() {
...
void TargetX8632<TraitsType>::doLoadOpt() {
...
bool TargetX8632<TraitsType>::doBranchOpt(Inst *I, const CfgNode *NextNode) {
...
Variable *TargetX8632<TraitsType>::getPhysicalRegister(RegNumT RegNum,
                                                       Type Ty) {
  if (Ty == IceType_void)
    Ty = IceType_i32;
...
const char *TargetX8632<TraitsType>::getRegName(RegNumT RegNum, Type Ty) const {
  return Traits::getRegName(Traits::getGprForType(Ty, RegNum));
}

void TargetX8632<TraitsType>::emitVariable(const Variable *Var) const {
  if (!BuildDefs::dump())
    return;
...
typename TargetX8632<TraitsType>::X86Address
TargetX8632<TraitsType>::stackVarToAsmOperand(const Variable *Var) const {
  if (Var->hasReg())
...
void TargetX8632<TraitsType>::addProlog(CfgNode *Node) {
  // Stack frame layout:
  //
  // +------------------------+  ^ +
  // | 1. return address      |  |
...
void TargetX8632<TraitsType>::finishArgumentLowering(Variable *Arg,
                                                     Variable *FramePtr,
...
void TargetX8632<TraitsType>::addEpilog(CfgNode *Node) {
...
Type TargetX8632<TraitsType>::stackSlotType() { return Traits::WordType; }

template <typename T>
typename std::enable_if<!T::Is64Bit, Operand>::type *
TargetX8632<TraitsType>::loOperand(Operand *Operand) {
...
template <typename T>
typename std::enable_if<!T::Is64Bit, Operand>::type *
TargetX8632<TraitsType>::hiOperand(Operand *Operand) {
  assert(Operand->getType() == IceType_i64 ||
         Operand->getType() == IceType_f64);
  if (Operand->getType() != IceType_i64 && Operand->getType() != IceType_f64)
    return Operand;
...
SmallBitVector
TargetX8632<TraitsType>::getRegisterSet(RegSetMask Include,
                                        RegSetMask Exclude) const {
  return Traits::getRegisterSet(getFlags(), Include, Exclude);
}

void TargetX8632<TraitsType>::lowerAlloca(const InstAlloca *Instr) {
...
void TargetX8632<TraitsType>::lowerArguments() {
  const bool OptM1 = Func->getOptLevel() == Opt_m1;
...
bool TargetX8632<TraitsType>::optimizeScalarMul(Variable *Dest, Operand *Src0,
                                                int32_t Src1) {
...
void TargetX8632<TraitsType>::lowerShift64(InstArithmetic::OpKind Op,
...
void TargetX8632<TraitsType>::lowerArithmetic(const InstArithmetic *Instr) {
  Variable *Dest = Instr->getDest();
...
void TargetX8632<TraitsType>::lowerAssign(const InstAssign *Instr) {
  Variable *Dest = Instr->getDest();
  if (Dest->isRematerializable()) {
    Context.insert<InstFakeDef>(Dest);
    return;
...
void TargetX8632<TraitsType>::lowerBr(const InstBr *Br) {
...
void TargetX8632<TraitsType>::lowerCall(const InstCall *Instr) {
...
void TargetX8632<TraitsType>::lowerCast(const InstCast *Instr) {
...
void TargetX8632<TraitsType>::lowerExtractElement(
...
void TargetX8632<TraitsType>::lowerFcmp(const InstFcmp *Fcmp) {
...
void TargetX8632<TraitsType>::lowerFcmpAndConsumer(const InstFcmp *Fcmp,
                                                   const Inst *Consumer) {
  Operand *Src0 = Fcmp->getSrc(0);
  Operand *Src1 = Fcmp->getSrc(1);
  Variable *Dest = Fcmp->getDest();
...
void TargetX8632<TraitsType>::lowerFcmpVector(const InstFcmp *Fcmp) {
  Operand *Src0 = Fcmp->getSrc(0);
  Operand *Src1 = Fcmp->getSrc(1);
  Variable *Dest = Fcmp->getDest();
...
void TargetX8632<TraitsType>::lowerIcmpAndConsumer(const InstIcmp *Icmp,
                                                   const Inst *Consumer) {
...
void TargetX8632<TraitsType>::lowerIcmpVector(const InstIcmp *Icmp) {
  Operand *Src0 = legalize(Icmp->getSrc(0));
  Operand *Src1 = legalize(Icmp->getSrc(1));
  Variable *Dest = Icmp->getDest();
...
TargetX8632<TraitsType>::lowerIcmp64(const InstIcmp *Icmp,
                                     const Inst *Consumer) {
...
void TargetX8632<TraitsType>::setccOrConsumer(BrCond Condition, Variable *Dest,
                                              const Inst *Consumer) {
  if (Consumer == nullptr) {
...
void TargetX8632<TraitsType>::movOrConsumer(bool IcmpResult, Variable *Dest,
                                            const Inst *Consumer) {
  if (Consumer == nullptr) {
...
void TargetX8632<TraitsType>::lowerArithAndConsumer(const InstArithmetic *Arith,
                                                    const Inst *Consumer) {
  Variable *T = nullptr;
...
void TargetX8632<TraitsType>::lowerInsertElement(
...
void TargetX8632<TraitsType>::lowerIntrinsic(const InstIntrinsic *Instr) {
  ...
  case Intrinsics::NaClReadTP: {
    if (NeedSandboxing) {
      Operand *Src =
          dispatchToConcrete(&TargetLowering::createNaClReadTPSrcOperand);
      Variable *Dest = Instr->getDest();
      Variable *T = nullptr;
      _mov(T, Src);
      _mov(Dest, T);
...
void TargetX8632<TraitsType>::lowerAtomicCmpxchg(Variable *DestPrev,
                                                 Operand *Ptr,
                                                 Operand *Expected,
                                                 Operand *Desired) {
...
bool TargetX8632<TraitsType>::tryOptimizedCmpxchgCmpBr(Variable *Dest,
                                                       Operand *PtrToMem,
                                                       Operand *Expected,
                                                       Operand *Desired) {
...
void TargetX8632<TraitsType>::lowerAtomicRMW(Variable *Dest, uint32_t Operation,
  ...
  case Intrinsics::AtomicAdd: {
    if (!Traits::Is64Bit && Dest->getType() == IceType_i64) {
      ...
      // for asserting.
      NeedsCmpxchg = true;
      Op_Lo = &TargetX8632<TraitsType>::_add;
      Op_Hi = &TargetX8632<TraitsType>::_adc;
      break;
    }
  ...
  case Intrinsics::AtomicSub: {
    if (!Traits::Is64Bit && Dest->getType() == IceType_i64) {
      NeedsCmpxchg = true;
      Op_Lo = &TargetX8632<TraitsType>::_sub;
      Op_Hi = &TargetX8632<TraitsType>::_sbb;
      break;
    }
    X86OperandMem *Addr = formMemoryOperand(Ptr, Dest->getType());
    constexpr bool Locked = true;
  ...
  case Intrinsics::AtomicOr:
    ...
    // vs an atomic store.
    NeedsCmpxchg = true;
    Op_Lo = &TargetX8632<TraitsType>::_or;
    Op_Hi = &TargetX8632<TraitsType>::_or;
    break;
  case Intrinsics::AtomicAnd:
    NeedsCmpxchg = true;
    Op_Lo = &TargetX8632<TraitsType>::_and;
    Op_Hi = &TargetX8632<TraitsType>::_and;
    break;
  case Intrinsics::AtomicXor:
    NeedsCmpxchg = true;
    Op_Lo = &TargetX8632<TraitsType>::_xor;
    Op_Hi = &TargetX8632<TraitsType>::_xor;
    break;
...
void TargetX8632<TraitsType>::expandAtomicRMWAsCmpxchg(LowerBinOp Op_Lo,
                                                       LowerBinOp Op_Hi,
                                                       Variable *Dest,
                                                       Operand *Ptr,
                                                       Operand *Val) {
...
void TargetX8632<TraitsType>::lowerCountZeros(bool Cttz, Type Ty,
                                              Variable *Dest, Operand *FirstVal,
                                              Operand *SecondVal) {
...
void TargetX8632<TraitsType>::typedLoad(Type Ty, Variable *Dest, Variable *Base,
                                        Constant *Offset) {
...
void TargetX8632<TraitsType>::typedStore(Type Ty, Variable *Value,
...
void TargetX8632<TraitsType>::copyMemory(Type Ty, Variable *Dest, Variable *Src,
                                         int32_t OffsetAmt) {
...
void TargetX8632<TraitsType>::lowerMemcpy(Operand *Dest, Operand *Src,
                                          Operand *Count) {
...
void TargetX8632<TraitsType>::lowerMemmove(Operand *Dest, Operand *Src,
                                           Operand *Count) {
  // There is a load and store for each chunk in the unroll
  constexpr uint32_t BytesPerStorep = 16;
...
void TargetX8632<TraitsType>::lowerMemset(Operand *Dest, Operand *Val,
                                          Operand *Count) {
  constexpr uint32_t BytesPerStorep = 16;
...
typename TargetX8632<TypeTraits>::X86OperandMem *
TargetX8632<TypeTraits>::computeAddressOpt(const Inst *Instr, Type MemType,
                                           Operand *Addr) {
...
void TargetX8632<TraitsType>::doMockBoundsCheck(Operand *Opnd) {
...
void TargetX8632<TraitsType>::lowerLoad(const InstLoad *Load) {
...
void TargetX8632<TraitsType>::doAddressOptOther() {
...
void TargetX8632<TraitsType>::doAddressOptLoad() {
  Inst *Instr = iteratorToInst(Context.getCur());
...
void TargetX8632<TraitsType>::doAddressOptLoadSubVector() {
...
void TargetX8632<TraitsType>::lowerPhi(const InstPhi * /*Instr*/) {
  Func->setError("Phi found in regular instruction list");
}

void TargetX8632<TraitsType>::lowerRet(const InstRet *Instr) {
...
Variable *TargetX8632<TraitsType>::lowerShuffleVector_AllFromSameSrc(
...
Variable *TargetX8632<TraitsType>::lowerShuffleVector_TwoFromSameSrc(
...
Variable *TargetX8632<TraitsType>::lowerShuffleVector_UnifyFromDifferentSrcs(
...
GlobalString TargetX8632<TraitsType>::lowerShuffleVector_NewMaskName() {
...
ConstantRelocatable *
TargetX8632<TraitsType>::lowerShuffleVector_CreatePshufbMask(
...
void TargetX8632<TraitsType>::lowerShuffleVector_UsingPshufb(
...
void TargetX8632<TraitsType>::lowerShuffleVector(
...
void TargetX8632<TraitsType>::lowerSelect(const InstSelect *Select) {
...
void TargetX8632<TraitsType>::lowerSelectMove(Variable *Dest, BrCond Cond,
  ...
  if (llvm::isa<Constant>(SrcT) && !llvm::isa<Constant>(SrcF)) {
    std::swap(SrcT, SrcF);
    Cond = InstImpl::InstX86Base::getOppositeCondition(Cond);
  }
  if (!Traits::Is64Bit && DestTy == IceType_i64) {
...
void TargetX8632<TraitsType>::lowerSelectIntMove(Variable *Dest, BrCond Cond,
                                                 Operand *SrcT, Operand *SrcF) {
  Variable *T = nullptr;
  SrcF = legalize(SrcF);
  _mov(T, SrcF);
...
void TargetX8632<TraitsType>::lowerMove(Variable *Dest, Operand *Src,
                                        bool IsRedefinition) {
  assert(Dest->getType() == Src->getType());
  assert(!Dest->isRematerializable());
  if (!Traits::Is64Bit && Dest->getType() == IceType_i64) {
...
bool TargetX8632<TraitsType>::lowerOptimizeFcmpSelect(
...
void TargetX8632<TraitsType>::lowerIcmp(const InstIcmp *Icmp) {
  Variable *Dest = Icmp->getDest();
  if (isVectorType(Dest->getType())) {
...
void TargetX8632<TraitsType>::lowerSelectVector(const InstSelect *Instr) {
  Variable *Dest = Instr->getDest();
  Type DestTy = Dest->getType();
...
void TargetX8632<TraitsType>::lowerStore(const InstStore *Instr) {
...
void TargetX8632<TraitsType>::doAddressOptStore() {
...
void TargetX8632<TraitsType>::doAddressOptStoreSubVector() {
  auto *Intrinsic = llvm::cast<InstIntrinsic>(Context.getCur());
...
void TargetX8632<TraitsType>::lowerIndirectJump(Variable *JumpTarget) {
  // Without std::move below, the compiler deduces that the argument to
  // lowerIndirectJmp is a Variable *&, not a Variable *.
  dispatchToConcrete(&Traits::TargetLowering::lowerIndirectJump,
                     std::move(JumpTarget));
}

Operand *TargetX8632<TraitsType>::lowerCmpRange(Operand *Comparison,
...
void TargetX8632<TraitsType>::lowerCaseCluster(const CaseCluster &Case,
                                               Operand *Comparison,
                                               bool DoneCmp,
...
void TargetX8632<TraitsType>::lowerSwitch(const InstSwitch *Instr) {
...
void TargetX8632<TraitsType>::eliminateNextVectorSextInstruction(
...
void TargetX8632<TraitsType>::lowerUnreachable(
...
void TargetX8632<TraitsType>::lowerBreakpoint(
    const InstBreakpoint * /*Instr*/) {
  _int3();
}

void TargetX8632<TraitsType>::lowerRMW(const InstX86FakeRMW *RMW) {
...
void TargetX8632<TraitsType>::lowerOther(const Inst *Instr) {
...
void TargetX8632<TraitsType>::prelowerPhis() {
  ...
  PhiLowering::prelowerPhis32Bit<TargetX8632<TraitsType>>(
      this, Context.getNode(), Func);
}

void TargetX8632<TraitsType>::genTargetHelperCallFor(Inst *Instr) {
...
uint32_t TargetX8632<TraitsType>::getCallStackArgumentsSizeBytes(
...
uint32_t
TargetX8632<TraitsType>::getCallStackArgumentsSizeBytes(const InstCall *Instr) {
...
Variable *TargetX8632<TraitsType>::makeZeroedRegister(Type Ty, RegNumT RegNum) {
...
Variable *TargetX8632<TraitsType>::makeVectorOfZeros(Type Ty, RegNumT RegNum) {
  return makeZeroedRegister(Ty, RegNum);
}

Variable *TargetX8632<TraitsType>::makeVectorOfMinusOnes(Type Ty,
                                                         RegNumT RegNum) {
...
Variable *TargetX8632<TraitsType>::makeVectorOfOnes(Type Ty, RegNumT RegNum) {
...
Variable *TargetX8632<TraitsType>::makeVectorOfHighOrderBits(Type Ty,
                                                             RegNumT RegNum) {
...
Variable *TargetX8632<TraitsType>::makeVectorOfFabsMask(Type Ty,
                                                        RegNumT RegNum) {
...
typename TargetX8632<TraitsType>::X86OperandMem *
TargetX8632<TraitsType>::getMemoryOperandForStackSlot(Type Ty, Variable *Slot,
                                                      uint32_t Offset) {
...
Variable *TargetX8632<TraitsType>::copyToReg8(Operand *Src, RegNumT RegNum) {
  Type Ty = Src->getType();
...
Variable *TargetX8632<TraitsType>::copyToReg(Operand *Src, RegNumT RegNum) {
  Type Ty = Src->getType();
  Variable *Reg = makeReg(Ty, RegNum);
  if (isVectorType(Ty)) {
    _movp(Reg, Src);
...
Operand *TargetX8632<TraitsType>::legalize(Operand *From, LegalMask Allowed,
                                           RegNumT RegNum) {
...
Variable *TargetX8632<TraitsType>::legalizeToReg(Operand *From,
                                                 RegNumT RegNum) {
...
Operand *TargetX8632<TraitsType>::legalizeUndef(Operand *From, RegNumT RegNum) {
...
Operand *TargetX8632<TraitsType>::legalizeSrc0ForCmp(Operand *Src0,
                                                     Operand *Src1) {
...
typename TargetX8632<TraitsType>::X86OperandMem *
TargetX8632<TraitsType>::formMemoryOperand(Operand *Opnd, Type Ty,
                                           bool DoLegalize) {
...
Variable *TargetX8632<TraitsType>::makeReg(Type Type, RegNumT RegNum) {
...
Type TargetX8632<TraitsType>::largestTypeInSize(uint32_t Size,
                                                uint32_t MaxSize) {
  assert(Size != 0);
...
Type TargetX8632<TraitsType>::firstTypeThatFitsSize(uint32_t Size,
                                                    uint32_t MaxSize) {
  assert(Size != 0);
  uint32_t TyIndex = llvm::findLastSet(Size, llvm::ZB_Undefined);
...
void TargetX8632<TraitsType>::postLower() {
  if (Func->getOptLevel() == Opt_m1)
    return;
  markRedefinitions();
  Context.availabilityUpdate();
...
void TargetX8632<TraitsType>::emit(const ConstantInteger32 *C) const {
  if (!BuildDefs::dump())
    return;
  Ostream &Str = Ctx->getStrEmit();
  Str << "$" << C->getValue();
...
void TargetX8632<TraitsType>::emit(const ConstantInteger64 *C) const {
  if (!Traits::Is64Bit) {
...
void TargetX8632<TraitsType>::emit(const ConstantFloat *C) const {
  if (!BuildDefs::dump())
    return;
  Ostream &Str = Ctx->getStrEmit();
  Str << C->getLabelName();
...
void TargetX8632<TraitsType>::emit(const ConstantDouble *C) const {
  if (!BuildDefs::dump())
    return;
  Ostream &Str = Ctx->getStrEmit();
  Str << C->getLabelName();
...
void TargetX8632<TraitsType>::emit(const ConstantUndef *) const {
  llvm::report_fatal_error("undef value encountered by emitter.");
}

template <class Machine>
void TargetX8632<Machine>::emit(const ConstantRelocatable *C) const {
  if (!BuildDefs::dump())
    return;
...
void TargetX8632<TraitsType>::emitJumpTable(
...
template <typename T>
void TargetDataX8632<TraitsType>::emitConstantPool(GlobalContext *Ctx) {
  if (!BuildDefs::dump())
    return;
  Ostream &Str = Ctx->getStrEmit();
  Type Ty = T::Ty;
...
void TargetDataX8632<TraitsType>::lowerConstants() {
...
void TargetDataX8632<TraitsType>::lowerJumpTables() {
...
void TargetDataX8632<TraitsType>::lowerGlobals(
...
createTargetDataLowering(::Ice::GlobalContext *Ctx) {
  return ::Ice::X8632::TargetDataX8632<::Ice::X8632::TargetX8632Traits>::create(
      Ctx);
}
...
std::array<SmallBitVector, RCX86_NUM>
    TargetX8632<X8632::Traits>::TypeToRegisterSet = {{}};

template <>
std::array<SmallBitVector, RCX86_NUM>
    TargetX8632<X8632::Traits>::TypeToRegisterSetUnfiltered = {{}};

template <>
std::array<SmallBitVector,
           TargetX8632<X8632::Traits>::Traits::RegisterSet::Reg_NUM>
    TargetX8632<X8632::Traits>::RegisterAliases = {{}};

template <>
FixupKind TargetX8632<X8632::Traits>::PcRelFixup =
    TargetX8632<X8632::Traits>::Traits::FK_PcRel;

template <>
FixupKind TargetX8632<X8632::Traits>::AbsFixup =
    TargetX8632<X8632::Traits>::Traits::FK_Abs;
