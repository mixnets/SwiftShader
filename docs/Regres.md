...
* [Local dEQP test runner](#local-dEQP-test-runner) Provides a local tool for
  efficiently running a number of dEQP tests based on fuzzy name name matching.
...
reference known-passing test lists updated by the [daily-run](#daily-run-continuous-integration-testing), so that failing tests for incomplete functionality are skipped, but tests that
pass for new functionality *are tested* to ensure they do not regres.

Additional tests found in the
[`<swiftshader>/tests/regres/full-tests.json`](https://github.com/google/swiftshader/blob/master/tests/regres/full-tests.json) file can be explicitly included in the change's presubmit run
...
local dEQP test runner with a wild-card based test name matcher.
...
* The above is repeated until it is time to perform a daily run, upon which:
  ...
  7. Stages 3 - 5 is repeated for both the LLVM and Subzero backends.
...
The cache needs to consider more than just the change identifier. The test
lists and version of dEQP are dictated by the change being tested, and so are
also part of the cache key for test results.
...
If you used 1 bit for each source line per test, a per-line source coverage per
test would require over 11GiB of storage for a single code coverage snapshot.
...
For example, given the fully qualified test name:

`dEQP-VK.fragment_shader_interlock.basic.discard.ssbo.sample_unordered.4xaa.sample_shading.16x16`

We can break this down into the following groups and leaf test name:
...
These test groups provide a natural way to structure coverage data, as tests of
the same group are likely to have similar coverage spans.

For each source file in the codebase, we create a tree, with a root node and
each test subgroup being a sub-node in the tree.
