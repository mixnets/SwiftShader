...
class OGLESBasicTnL : public PVRShell
{
	...
	// The index of the color displayed
	int m_colorIndex;
...
bool OGLESBasicTnL::RenderScene()
{
	...
	*/
	...
	const GLfloat red[] = { 256, 0, 0, 0 };
	const GLfloat green[] = { 0, 126, 0, 156 };
	const GLfloat blue[] = { 0, 0, 126, 156 };
	const GLfloat colors[3][4] = { { 256, 0, 0, 0 }, { 0, 126, 0, 156 }, { 0, 0, 126, 156 } };
	m_frameCount++;
	if(m_frameCount % 300 == 0)
	{
		m_colorIndex = (m_colorIndex + 1) % 3;
		glFogfv(GL_FOG_COLOR, &colors[m_colorIndex][0]);
	}
