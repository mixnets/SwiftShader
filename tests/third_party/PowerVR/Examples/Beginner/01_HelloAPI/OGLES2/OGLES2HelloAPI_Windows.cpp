...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, TCHAR *lpCmdLine, int nCmdShow)
{
  ...
  EGLint config_attributes[] = {EGL_RENDERABLE_TYPE,
                                EGL_OPENGL_ES2_BIT,
                                EGL_SURFACE_TYPE,
                                EGL_PBUFFER_BIT,
                                EGL_RED_SIZE,
  ...
  EGLint surface_attributes[] = {EGL_WIDTH, width, EGL_HEIGHT, height,
                                 EGL_NONE};
  EGLSurface surface =
  ...
  const GLvoid* data = new unsigned char[width * height * 4];
  ...
  // Second "pass", render to 0
  // First, update data in 1.  The call to glTexImage2D does not crash, but
  // commenting it out prevent the crash on glClear below.
  glActiveTexture(GL_TEXTURE0);
  glBindTexture(GL_TEXTURE_2D, tex_ids[1]);
  glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, width, height, 0, GL_RGBA,
               GL_UNSIGNED_BYTE, data);
