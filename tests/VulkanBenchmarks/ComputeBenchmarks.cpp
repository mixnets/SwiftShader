...
void CppCompute(benchmark::State &state, Init init, Func op)
{
	...
	float *bufferIn = (float *)malloc(range * sizeof(float));
	float *bufferOut = (float *)malloc(range * sizeof(float));
...
BENCHMARK_CAPTURE(CppCompute, mul, zero, [](float x) { return x * x; })->Arg(4 * 1024 * 1024)->Unit(benchmark::kMillisecond);
