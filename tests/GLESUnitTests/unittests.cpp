...
protected:
	...
	void checkCompileFails(std::string s)
	{
		...
		(void) checkCompileFails(vs, GL_VERTEX_SHADER);
		(void) checkCompileFails(fs, GL_FRAGMENT_SHADER);
	}
...
TEST_F(SwiftShaderTest, MatrixInStruct)
{
	...
	GLuint fragmentShader = MakeShader(fs, GL_FRAGMENT_SHADER);
	EXPECT_NO_GL_ERROR();
	(void) fragmentShader;
...
TEST_F(SwiftShaderTest, TransformFeedback_DrawArraysInstanced)
{
	...
	GLuint vert = MakeShader(data1, GL_VERTEX_SHADER);
	GLuint frag = MakeShader(data0, GL_FRAGMENT_SHADER);
