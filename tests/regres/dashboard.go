...
// Dashboard updates a Google sheets document with the latest test results
package main
...
func createSheetsService(authdir string) (*sheets.Service, error) {
	...
	if err != nil {
		return nil, cause.Wrap(err, "Unable to read client secret file '%v'", credentialsPath)
	}
