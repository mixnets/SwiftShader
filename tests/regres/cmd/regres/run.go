...
func main() {
	...
	shell.MaxProcMemory = *parse.MaxProcMemory

	r := regres{
		cacheRoot:     *parse.CacheDir,
		gerritEmail:   os.ExpandEnv(*parse.GerritEmail),
		gerritUser:    os.ExpandEnv(*parse.GerritUser),
		gerritPass:    os.ExpandEnv(*parse.GerritPass),
		githubUser:    os.ExpandEnv(*parse.GithubUser),
		githubPass:    os.ExpandEnv(*parse.GithubPass),
		keepCheckouts: *parse.KeepCheckouts,
		dryRun:        *parse.DryRun,
		dailyNow:      *parse.DailyNow,
		dailyOnly:     *parse.DailyOnly,
		dailyChange:   *parse.DailyChange,
...
func (t *test) run(testLists testlist.Lists, d deqpBuild) (*deqp.Results, error) {
    ...
    if *parse.Limit != 0 {
		...
		for i := range testLists {
			testLists[i] = testLists[i].Limit(*parse.Limit)
		}
	}
