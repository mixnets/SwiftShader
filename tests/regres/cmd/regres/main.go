...
type regres struct {
	...
	copy          string          // path to copy executable
	diff          string          // path to diff executable
...
func copyFileIfDifferent(src, dst string) (bool, error) {
	...
	// Remove all Windows line endings from file
	srcLinuxLE := bytes.ReplaceAll(srcContents, []byte("\r"), []byte(""))
	...
	dstLinuxLE := bytes.ReplaceAll(dstContents, []byte("\r"), []byte(""))

	if bytes.Equal(srcLinuxLE, dstLinuxLE) {
		return false, nil
	} else {
		if err := os.WriteFile(dst, srcLinuxLE, 0640); err != nil {
			return false, err
		}
	}
