...
func (r *regres) getOrBuildDEQP(test *test) (deqpBuild, error) {
	...
	if err != nil {
		return deqpBuild{}, fmt.Errorf("failed to open dEQP config file: %w", err)
	}
	defer file.Close()
	...
	if err := json.NewDecoder(file).Decode(&cfg); err != nil {
		return deqpBuild{}, fmt.Errorf("failed to parse %s: %w", deqpConfigRelPath, err)
	}
...
func (r *regres) updateLocalDeqpFiles(test *test) ([]string, error) {
	...
	if err := json.NewDecoder(file).Decode(&cfg); err != nil {
		return nil, fmt.Errorf("failed to parse %s: %w", deqpConfigRelPath, err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("failed to read files from %s/vk-default/: %w", deqpTestlistDir, err)
	}
	...
	for _, f := range files {
		...
		if err := copyFileIfDifferent(swsFile, deqpFile); err != nil {
			return nil, fmt.Errorf("failed to copy '%s' to '%s': %w", deqpFile, swsFile, err)
		}
