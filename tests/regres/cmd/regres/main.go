...
func (r *regres) run() error {
	...
	for {
		...
		changeName := fmt.Sprintf("%s~%s~%s", change.project, change.branch, change.id)
		if r.dryRun {
			...
			_, _, err = client.Changes.SetReview(changeName, change.latest.String(), &gerrit.ReviewInput{
				Message: msg,
				Tag:     "autogenerated:regress",
				Notify:  notify,
			})
			if err != nil {
				return cause.Wrap(err, "Failed to post comments on change '%s'", changeName)
			}
		}
		change.pending = false
	}
...
func queryChanges(client *gerrit.Client, changes map[string]*changeInfo) error {
	...
	ids := map[string]ChangeLocation{}
	for _, r := range *results {
		ids[r.ChangeID] = ChangeLocation{r.Project, r.Branch}
	}
