...
var (
	...
	priority      = flag.Int("priority", 0, "Prioritize a single change with the given id")
...
type regres struct {
	...
	priority      int             // Prioritize a single change with the given id
}
...
func (r *regres) run() error {
	...
	changes := map[int]*changeInfo{} // Change ID -> changeInfo
