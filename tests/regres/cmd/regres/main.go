...
func (r *regres) run() error {
	...
	for {
		...
		if r.dryRun {
			...
			_, _, err = client.Changes.SetReview(change.number, change.latest.String(), &gerrit.ReviewInput{
				Message: msg,
				Tag:     "autogenerated:regress",
				Notify:  notify,
			})
			if err != nil {
				return cause.Wrap(err, "Failed to post comments on change '%s'", change.number)
			}
		}
		change.pending = false
	}
...
func (r *regres) postMostCommonFailures(client *gerrit.Client, change *gerrit.ChangeInfo, results *deqp.Results) error {
	...
	changeNumber := fmt.Sprintf("%d", change.Number)
	if r.dryRun {
		log.Printf("DRY RUN: add most common failures to '%v':\n%v\n", changeNumber, msg)
	} else {
		log.Printf("Posting most common failures to '%s'\n", changeNumber)
		_, _, err := client.Changes.SetReview(changeNumber, change.CurrentRevision, &gerrit.ReviewInput{
			Message: msg,
			Tag:     "autogenerated:regress",
		})
		if err != nil {
			return cause.Wrap(err, "Failed to post comments on change '%s'", changeNumber)
		}
	}
	return nil
}
...
type changeInfo struct {
	...
	number        string    // The numeric ID gerrit assigned to the change
...
func queryChanges(client *gerrit.Client, changes map[string]*changeInfo) error {
	...
	for id, number := range ids {
		if _, found := changes[id]; !found {
			...
			changes[id] = &changeInfo{id: id, number: fmt.Sprint("%d", number)}
		}
	}
...
func (c *changeInfo) update(client *gerrit.Client) error {
	change, _, err := client.Changes.GetChange(fmt.Sprintf("%d", c.number), &gerrit.ChangeOptions{
		AdditionalFields: []string{"CURRENT_REVISION", "CURRENT_COMMIT", "MESSAGES", "LABELS", "DETAILED_ACCOUNTS"},
	})
	if err != nil {
		return cause.Wrap(err, "Getting info for change '%d'", c.number)
	}
