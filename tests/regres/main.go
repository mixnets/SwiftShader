...
func main() {
	...
	// as this may need more memory than the limit allows.
	//
	...
	// the call to the test executable with another layer of the regres exe in a
	// special --exec mode:
	//
	// [regres] -> [regres --exec <test-exe N args...>] -> [test-exe]
	//                      (calls rlimit() with memory limit of N)
	if len(os.Args) > 3 && os.Args[1] == "--exec" {
		// Limit the amount of virtual memory each test can allocate.
		// This can prevent the whole system swapping itself to death.
		exe := os.Args[2]
		limit, err := strconv.ParseUint(os.Args[3], 10, 64)
		if err != nil {
			log.Fatalf("Expected memory limit as 2nd argument. %v\n", err)
		}
		if limit > 0 {
...
func (r *regres) run() error {
	...
	for {
		...
		// changeInfo.update() uses this to detect whether a patchset has
...
func (t *test) checkout() (err error) {
	...
	for _, cmds := range [][]string{
		...
		if err := t.r.shell(t.r.git, t.srcDir, cmds...); err != nil {
			os.RemoveAll(t.srcDir)
			return err
		}
	}
...
func (t *test) build() error {
	...
	if err := t.r.shell(t.r.cmake, t.buildDir, "-DCMAKE_BUILD_TYPE=Release", ".."); err != nil {
		return err
	}

	if err := t.r.shell(t.r.make, t.buildDir, fmt.Sprintf("-j%d", runtime.NumCPU())); err != nil {
		return err
	}
...
func compare(old, new *CommitTestResults) string {
	...
	for test, new := range new.Tests {
		...
		case (found && old.Status == Fail) && new.Status == Fail:
	...
	if len(broken) == 0 && len(fixed) == 0 && len(removed) == 0 {
		sb.WriteString(fmt.Sprintf("--- No change in test results ---\n"))
	}

	if n := len(failing); n > 0 {
		sort.Strings(failing)
		sb.WriteString(fmt.Sprintf("--- %d tests continue to fail: ---\n", n))
		list(failing)
		sb.WriteString("\n\n")
	}
...
func (t *test) deqpTestRoutine(exe string, tests <-chan string, results chan<- TestResult) {
	for name := range tests {
		...
		out, err := t.r.exec(exe, filepath.Dir(exe), env, "--deqp-surface-type=pbuffer", "-n="+name)
		if err != nil {
			results <- TestResult{
				Test:   name,
				Status: Fail,
...
// directory wd.
func (r *regres) shell(exe, wd string, args ...string) error {
	if out, err := r.exec(exe, wd, nil, args...); err != nil {
...
func (r *regres) exec(exe, wd string, env []string, args ...string) ([]byte, error) {
