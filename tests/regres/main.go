...
func (r *regres) testLatest(change *changeInfo, test *test, d deqp) (*deqprun.TestResults, testlist.Lists, error) {
	...
	if results, err := deqprun.LoadTestResults(cachePath); err == nil {
...
func (r *regres) testParent(change *changeInfo, testlists testlist.Lists, d deqp) (*deqprun.TestResults, error) {
...
func (r *regres) postMostCommonFailures(client *gerrit.Client, change *gerrit.ChangeInfo, results *deqprun.TestResults) error {
	const limit = 25
...
func (t *test) buildAndRun(testLists testlist.Lists, d deqp) *deqprun.TestResults {
	// Build the parent change.
	if err := t.build(); err != nil {
		msg := fmt.Sprintf("Failed to build '%s'", t.commit)
		log.Println(cause.Wrap(err, msg))
		return &deqprun.TestResults{Error: msg}
	}
...
func (t *test) run(testLists testlist.Lists, d deqp) (*deqprun.TestResults, error) {
...
func (t *test) writeTestListsByStatus(testLists testlist.Lists, results *deqprun.TestResults) ([]string, error) {
	out := []string{}
...
func commonFailures(results *deqprun.TestResults) []commonFailure {
...
// gerrit code review.
func compare(old, new *deqprun.TestResults) string {
	if old.Error != "" {
		return old.Error
	}
	if new.Error != "" {
