...
func (r *regres) resolveDirs() error {
	for _, path := range []*string{
		...
		if err != nil {
			return wrap(err, "Couldn't find path '%s'", path)
		}
		if _, err := os.Stat(abs); err != nil {
			return wrap(err, "Couldn't find path '%s'", abs)
		}
		*path = abs
	}
	return nil
...
func (r *regres) run() error {
	...
	client, _ := gerrit.NewClient(gerritURL, nil)
	...
	for {
		...
		mostRecent, _, err := client.Changes.QueryChanges(&gerrit.QueryChangeOptions{
		...
		if err != nil {
			log.Println(wrap(err, "Failed to get list of changes"))
			time.Sleep(time.Minute) // Might be a transient issue. Wait and retry.
			continue
		}

		// Fetch change info for any changes that aren't already in the changes
		// map.
		seen := map[string]bool{}
		for _, change := range *mostRecent {
			changeID := change.ChangeID
			seen[changeID] = true
			if _, found := changes[changeID]; !found {
				log.Printf("Requesting info for new change '%v'\n", changeID)
				ci, err := fetchChangeInfo(client, changeID)
				if err != nil {
					log.Println(wrap(err, "Couldn't fetch info for change '%s'", changeID))
					continue
				}
				changes[changeID] = ci
			}
		...
		// Reap any changes that are no longer in mostRecent.
		...
		// Pick a change that needs testing.
		// TODO: Prioritize changes?
		...
		if change == nil {
			...
			// And check if there's anything else to do.
			continue
		}

		log.Printf("%d changes in pending testing. Testing change '%s'\n", pending, change.id)
		...
		// Compare the latest patchset to the change's parent changelist.
		msg := compare(parent, latest)

		// Always include the reportHeader in the message.
		// fetchChangeInfo() uses this to detect whether a patchset has already
		// got a test result.
		msg = reportHeader + "\n" + msg
...
type changeInfo struct {
	...
	hasReport bool   // Has the latest patchset already got a regres report?
	latest    Hash   // Git hash of the latest patchset in the change.
	parent    Hash   // Git hash of the changelist this change is based on.
}

// fetchChangeInfo queries gerrit for the important information about the change
// with the given identifier.
func fetchChangeInfo(client *gerrit.Client, id string) (*changeInfo, error) {
	change, _, err := client.Changes.GetChange(id, &gerrit.ChangeOptions{
		AdditionalFields: []string{"CURRENT_REVISION", "CURRENT_COMMIT", "MESSAGES"},
	})
	if err != nil {
		return nil, wrap(err, "Getting info for change '%s'", id)
	}

	current, ok := change.Revisions[change.CurrentRevision]
	if !ok {
		return nil, fmt.Errorf("Couldn't find current revision for change '%s'", id)
	}

	if len(current.Commit.Parents) == 0 {
		return nil, fmt.Errorf("Couldn't find current commit for change '%s' has no parents(?)", id)
	}
	...
	out := &changeInfo{
		id:        id,
		hasReport: hasReport,
		latest:    plumbing.NewHash(change.CurrentRevision),
		parent:    plumbing.NewHash(current.Commit.Parents[0].Commit),
	}
	return out, nil
}
...
func (t *test) run() (*CommitTestResults, error) {
	if isDir(t.srcDir) && t.r.keepCheckouts {
		...
		if err := t.checkout(); err != nil {
			return nil, wrap(err, "Checking out commit '%s'")
		}
...
func (t *test) checkout() (err error) {
	...
	for _, cmds := range [][]string{
		...
		if err := shell(t.r.git, t.srcDir, cmds...); err != nil {
			os.RemoveAll(t.srcDir)
			return err
		}
	}
...
func (t *test) build() error {
	...
	if err := shell(t.r.cmake, t.buildDir, "-DCMAKE_BUILD_TYPE=Release", ".."); err != nil {
		return err
	}

	if err := shell(t.r.make, t.buildDir, fmt.Sprintf("-j%d", runtime.NumCPU())); err != nil {
		return err
	}
...
func (t *test) runTests() (*CommitTestResults, error) {
	...
	for _, api := range apis {
		...
		case vulkan:
			exe = filepath.Join(t.r.deqpBuild, "external", "vulkancts", "vulkan", "deqp-vk")
		default:
	...
	close(results) // Unblock the go-routine above.
...
func compare(old, new *CommitTestResults) string {
	...
	totalTestsPassing, totalTests := 0, 0

	broken, fixed, removed := []string{}, []string{}, []string{}

	for test, new := range new.Tests {
		old, found := old.Tests[test]
		switch {
		case (!found || old.Status == Pass) && new.Status == Fail:
			broken = append(broken, test)
		case (found && old.Status == Fail) && new.Status == Pass:
	...
	sb.WriteString(fmt.Sprintf("%d out of %d tests passing (%d%%)\n",
		totalTestsPassing, totalTests, (100*totalTestsPassing)/totalTests))

	if n := len(broken); n > 0 {
		sort.Strings(broken)
		sb.WriteString(fmt.Sprintf("--- This change breaks %d tests ---\n", n))
		list(broken)
		sb.WriteString("\n\n")
	}
	if n := len(fixed); n > 0 {
		sort.Strings(fixed)
		sb.WriteString(fmt.Sprintf("--- This change fixes %d tests ---\n", n))
		list(fixed)
		sb.WriteString("\n\n")
	}
	if n := len(removed); n > 0 {
		sort.Strings(removed)
		sb.WriteString(fmt.Sprintf("--- This change removes %d tests ---\n", n))
		list(removed)
		sb.WriteString("\n\n")
	}

	if len(broken) == 0 && len(fixed) == 0 && len(removed) == 0 {
		sb.WriteString(fmt.Sprintf("--- No changes in test results ---\n"))
	}
...
var parseRE = regexp.MustCompile(`(Fail|Pass) \((.*)\)`)
...
func (t *test) deqpTestRoutine(exe string, tests <-chan string, results chan<- TestResult) {
	for name := range tests {
		c := exec.Command(exe, "-n="+name)
		c.Env = []string{
			"LD_LIBRARY_PATH=" + t.buildDir,
		...
		if len(toks) < 3 {
			results <- TestResult{Test: name, Status: Fail, Err: fmt.Errorf("Couldn't parse test output:\n%s", string(out))}
			continue
		}
		switch toks[1] {
		case "Pass":
...
func (t *test) loadTestList() ([]TestList, error) {
	...
	for i, group := range groups {
		...
		out[i] = TestList{
			Name:  group.Name,
			API:   API(group.API),
			Tests: strings.Split(string(tests), "\n"),
		}
	}
...
// directory wd.
func shell(exe, wd string, args ...string) error {
