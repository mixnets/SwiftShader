...
const (
	...
	deqpConfigRelPath       = "tests/regres/regres.json"
)
...
func (r *regres) getOrBuildDEQP(test *test) (string, error) {
	srcDir := test.srcDir
	...
	if err != nil {
		return "", cause.Wrap(err, "Couldn't open dEQP config file")
	}
	defer file.Close()
	...
	if err := json.NewDecoder(file).Decode(&cfg); err != nil {
		return "", cause.Wrap(err, "Couldn't parse %s", deqpConfigRelPath)
	}

	hash := sha1.New()
	if err := json.NewEncoder(hash).Encode(&cfg); err != nil {
		return "", cause.Wrap(err, "Couldn't re-encode %s", deqpConfigRelPath)
	}
	cacheDir := path.Join(r.cacheRoot, "deqp", hex.EncodeToString(hash.Sum(nil)))
	buildDir := path.Join(cacheDir, "build")
	if !isDir(cacheDir) {
		if err := os.MkdirAll(cacheDir, 0777); err != nil {
			return "", cause.Wrap(err, "Couldn't make deqp cache directory '%s'", cacheDir)
		}
		...
		if err := git.Checkout(cacheDir, cfg.Remote, git.ParseHash(cfg.SHA)); err != nil {
			return "", cause.Wrap(err, "Couldn't build deqp %s @ %s", cfg.Remote, cfg.SHA)
		}
		...
		if err := shell.Shell(buildTimeout, r.python, cacheDir, "external/fetch_sources.py"); err != nil {
			return "", cause.Wrap(err, "Couldn't fetch deqp sources %s @ %s", cfg.Remote, cfg.SHA)
		}
		...
		for _, patch := range cfg.Patches {
			...
			if err := git.Apply(cacheDir, fullPath); err != nil {
				return "", cause.Wrap(err, "Couldn't apply deqp patch %s for %s @ %s", patch, cfg.Remote, cfg.SHA)
			}
		}
		...
		if err := shell.Shell(buildTimeout, r.cmake, buildDir, ".."); err != nil {
			return "", cause.Wrap(err, "Couldn't generate build rules for deqp %s @ %s", cfg.Remote, cfg.SHA)
		}
...
func (r *regres) testLatest(change *changeInfo, test *test, deqp string) (*CommitTestResults, testlist.Lists, error) {
	...
	results := test.buildAndRun(testlists, deqp)
...
func (r *regres) testParent(change *changeInfo, testlists testlist.Lists, deqp string) (*CommitTestResults, error) {
...
func (r *regres) updateTestLists(client *gerrit.Client) error {
	...
	deqp, err := r.getOrBuildDEQP(test)
	if err != nil {
	...
	results, err := test.run(testLists, deqp)
	if err != nil {
...
func (t *test) buildAndRun(testLists testlist.Lists, deqp string) *CommitTestResults {
	...
	results, err := t.run(testLists, deqp)
	if err != nil {
...
func (t *test) run(testLists testlist.Lists, deqp string) (*CommitTestResults, error) {
	...
	for _, list := range testLists {
		...
		switch list.API {
		case testlist.EGL:
			exe = filepath.Join(deqp, "modules", "egl", "deqp-egl")
		case testlist.GLES2:
			exe = filepath.Join(deqp, "modules", "gles2", "deqp-gles2")
		case testlist.GLES3:
			exe = filepath.Join(deqp, "modules", "gles3", "deqp-gles3")
		case testlist.Vulkan:
			exe = filepath.Join(deqp, "external", "vulkancts", "modules", "vulkan", "deqp-vk")
		default:
