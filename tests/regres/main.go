...
func (c *changeInfo) update(client *gerrit.Client) error {
	...
	c.pending = canTest
	c.latest = plumbing.NewHash(change.CurrentRevision)
	c.parent = plumbing.NewHash(current.Commit.Parents[0].Commit)
...
var parseRE = regexp.MustCompile(`(Fail|Pass|NotSupported|CompatibilityWarning|QualityWarning) \((.*)\)`)
