...
func (r *regres) updateTestLists(client *gerrit.Client) error {
	...
	if existingChange != nil {
		...
		commitMsg.WriteString("Change-Id: " + existingChange.ID)
	}
...
func (r *regres) postMostCommonFailures(client *gerrit.Client, change *gerrit.ChangeInfo, results *CommitTestResults) error {
	...
	for _, f := range failures {
		...
		if len(lines) == 1 {
			line := lines[0]
			if line != "" {
				sb.WriteString(fmt.Sprintf("%d occurances: %v: %v\n", f.count, f.status, line))
			} else {
				sb.WriteString(fmt.Sprintf("%d occurances: %v\n", f.count, f.status))
			}
		} else {
			sb.WriteString(fmt.Sprintf("%d occurances: %v:\n", f.count, f.status))
			for _, l := range lines {
	...
	if r.dryRun {
		...
		_, _, err := client.Changes.SetReview(change.ID, change.CurrentRevision, &gerrit.ReviewInput{
			Message: msg,
			Tag:     "autogenerated:regress",
		})
		if err != nil {
