...
// Package deqprun provides functions for running dEQP, as well as loading and storing the results.
package deqprun
...
// RunConfig contains the inputs required for running dEQP on a group of test lists.
...
// TestResults holds the results of the tests across all APIs.
// The TestResults structure may be serialized to cache the results.
...
// LoadTestResults loads cached test results from disk.
func LoadTestResults(path string) (*TestResults, error) {
	f, err := os.Open(path)
	if err != nil {
...
func (r *TestResults) Save(path string) error {
...
func (c *RunConfig) Run() (*TestResults, error) {
...
func (c *RunConfig) TestRoutine(exe string, tests <-chan string, results chan<- TestResult) {
nextTest:
