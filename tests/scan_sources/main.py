	...
	for fn in filelist:
		with open(fn, encoding="utf-8") as f:
			...
			if memset_template_check.search(contents) != None:
				print(f"ERROR: {fn} has illegal memset<> template use: must always be the first base class.")
				retval = 1
			if memset_call_check.search(contents) != None:
				print(f"ERROR: {fn} has illegal memset<> template use: not called as the first initializer.")
				retval = 1
