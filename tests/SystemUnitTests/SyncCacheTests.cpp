...
TYPED_TEST(SyncCacheTest, SingleThread_Empty)
{
	...
	ASSERT_EQ("", cache.lookup("one"));
}

TYPED_TEST(SyncCacheTest, SingleThread_AddLookup)
{
	typename TestFixture::SyncCache cache(8);
	cache.add("one", "1");
	cache.add("two", "2");
	cache.add("three", "3");
	ASSERT_EQ("1", cache.lookup("one"));
	ASSERT_EQ("2", cache.lookup("two"));
	ASSERT_EQ("3", cache.lookup("three"));
	ASSERT_EQ("", cache.lookup("four"));
}
...
TYPED_TEST(SyncCacheTest, SingleThread_AddAll)
{
	...
	ASSERT_EQ("1", cacheB.lookup("one"));
	ASSERT_EQ("2", cacheB.lookup("two"));
	ASSERT_EQ("3", cacheB.lookup("three"));
	ASSERT_EQ("10", cacheB.lookup("ten"));
}
...
TYPED_TEST(SyncCacheTest, SingleThread_Clear)
{
	...
	ASSERT_EQ("", cache.lookup("one"));
	ASSERT_EQ("", cache.lookup("two"));
	ASSERT_EQ("", cache.lookup("three"));
}
...
TYPED_TEST(SyncCacheTest, SingleThread_LookupOrCreate)
{
	typename TestFixture::SyncCache cache(8);
	ASSERT_EQ("meow", cache.lookupOrCreate("cat", [] { return "meow"; }));
	ASSERT_EQ("meow", cache.lookupOrCreate("cat", [] {
		ADD_FAILURE() << "create unexpectedly called";
		return "xxx";
	}));
	ASSERT_EQ("woof", cache.lookupOrCreate("dog", [] { return "woof"; }));
	ASSERT_EQ("woof", cache.lookupOrCreate("dog", [] {
		ADD_FAILURE() << "create unexpectedly called";
		return "xxx";
	}));
}

TYPED_TEST(SyncCacheTest, MultiThread_AddLookup)
{
	typename TestFixture::SyncCache cache(8);
...
TYPED_TEST(SyncCacheTest, MultiThread_GetOrCreate)
{
	...
	marl::schedule([wg, unblock, &cache] {
		ASSERT_EQ("blub", cache.lookupOrCreate("fish", [=] {
			wg.done();
			unblock.wait();
			return "blub";
		}));
