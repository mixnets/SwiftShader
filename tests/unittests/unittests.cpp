...
protected:
	...
	ProgramHandles createProgram(const std::string& vs, const std::string& fs)
	{
		...
		glGetShaderiv(ph.vertexShader, GL_COMPILE_STATUS, &compileStatus);
		...
		EXPECT_GLENUM_EQ(GL_NONE, glGetError());
		glGetShaderiv(ph.fragmentShader, GL_COMPILE_STATUS, &compileStatus);
		EXPECT_EQ(compileStatus, GL_TRUE);
