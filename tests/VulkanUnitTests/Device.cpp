		...
		Driver const *driver, VkInstance instance, Device *outDevice)
{
    ...
    for (auto physicalDevice : physicalDevices)
    {
		...
		*outDevice = Device(driver, device, physicalDevice, static_cast<uint32_t>(queueFamilyIndex));
        return VK_SUCCESS;
    }
...
VkResult Device::CreateShaderModule(
		const std::vector<uint32_t>& code, VkShaderModule* out) const
{
...
VkResult Device::CreateDescriptorPool(VkDescriptorPool* out) const
{
...
VkResult Device::AllocateMemory(size_t size, int flags, VkDeviceMemory* out) const
{
