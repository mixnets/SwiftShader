...
std::vector<uint32_t> compileSpirv(const char* assembly)
{
    ...
    if (disassembled != assembly)
    {
        ...
        auto splitLines = [](std::stringstream str) -> std::vector<std::string>
        {
        ...
        auto srcLines = splitLines(std::stringstream(std::string(assembly)));
        auto disLines = splitLines(std::stringstream(disassembled));
...
INSTANTIATE_TEST_CASE_P(Foobar, SwiftShaderVulkanComputeTest, testing::Values(
    ComputeParams{1, 1, 1},
    ComputeParams{2, 1, 1},
    ComputeParams{4, 1, 1},
    ComputeParams{8, 1, 1},
