...
protected:
    void SetUp() override
    {
        EXPECT_TRUE(ssDriver.loadSwiftShader());
        sysDriver.loadSystem();
    }
...
{
    Driver driver;
    ASSERT_TRUE(ssDriver.loadSwiftShader());
    if(!ssDriver.isLoaded())
    {
        return;
    }

    auto createInstance = ssDriver.vk_icdGetInstanceProcAddr(VK_NULL_HANDLE, "vkCreateInstance");
    EXPECT_NE(createInstance, nullptr);

    auto enumerateInstanceExtensionProperties =
        ssDriver.vk_icdGetInstanceProcAddr(VK_NULL_HANDLE, "vkEnumerateInstanceExtensionProperties");
    EXPECT_NE(enumerateInstanceExtensionProperties, nullptr);

    auto enumerateInstanceLayerProperties =
        ssDriver.vk_icdGetInstanceProcAddr(VK_NULL_HANDLE, "vkEnumerateInstanceLayerProperties");
    EXPECT_NE(enumerateInstanceLayerProperties, nullptr);

    auto enumerateInstanceVersion = ssDriver.vk_icdGetInstanceProcAddr(VK_NULL_HANDLE, "vkEnumerateInstanceVersion");
    EXPECT_NE(enumerateInstanceVersion, nullptr);

    auto bad_function = ssDriver.vk_icdGetInstanceProcAddr(VK_NULL_HANDLE, "bad_function");
...
TEST_F(SwiftShaderVulkanTest, Version)
{
    ...
    VkResult result = ssDriver.vkEnumerateInstanceVersion(&apiVersion);
    ...
    result = ssDriver.vkCreateInstance(&createInfo, nullptr, &instance);
    EXPECT_EQ(result, VK_SUCCESS);

    ASSERT_TRUE(ssDriver.resolve(instance));

    uint32_t pPhysicalDeviceCount = 0;
    result = ssDriver.vkEnumeratePhysicalDevices(instance, &pPhysicalDeviceCount, nullptr);
    EXPECT_EQ(result, VK_SUCCESS);
    EXPECT_EQ(pPhysicalDeviceCount, 1U);

    VkPhysicalDevice pPhysicalDevice = VK_NULL_HANDLE;
    result = ssDriver.vkEnumeratePhysicalDevices(instance, &pPhysicalDeviceCount, &pPhysicalDevice);
    EXPECT_EQ(result, VK_SUCCESS);
    EXPECT_NE(pPhysicalDevice, (VkPhysicalDevice)VK_NULL_HANDLE);

    VkPhysicalDeviceProperties physicalDeviceProperties;
    ssDriver.vkGetPhysicalDeviceProperties(pPhysicalDevice, &physicalDeviceProperties);
...
{
    auto& driver = sysDriver;
    // auto& driver = ssDriver;
    ...
    auto code = compile(
        ...
        "             %19 = OpLoad %uint %18 \n"
        "             %27 = OpAccessChain %_ptr_Uniform_int %inArray %int_0 %19 \n"
        "             %28 = OpLoad %int %27 \n"
        "             %31 = OpAccessChain %_ptr_Uniform_int %outArray %int_0 %19 \n"
        "                   OpStore %31 %28 \n"
        "                   OpReturn \n"
        "                   OpFunctionEnd \n");
    ...
    driver.vkCmdDispatch(commandBuffer, NUM_ELEMENTS, 1, 1);
