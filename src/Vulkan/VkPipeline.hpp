...
private:
	std::shared_ptr<sw::SpirvShader>& createShader(const PipelineCache::SpirvShaderKey& key, const ShaderModule *module);
...
protected:
	const std::shared_ptr<sw::SpirvShader>& createShader(const PipelineCache::SpirvShaderKey& key, const ShaderModule *module);
	const std::shared_ptr<sw::ComputeProgram>& createProgram(const PipelineCache::ComputeProgramKey& key);
