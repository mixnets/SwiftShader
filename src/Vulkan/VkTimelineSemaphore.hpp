...
private:
	// The Shared struct tracks the 64-bit payload. Timeline Semaphores have a shared_ptr<Shared>
                                 ...
                                 const std::chrono::time_point<Clock, Duration> timeout)
{
	...
	if(!shared->cv.wait_until(lock, timeout, [&]() REQUIRES(shared->mutex) { return shared->counter == value; }))
	{
		return VK_TIMEOUT;
	}
	return VK_SUCCESS;
