...
public:
	...
	public:
		...
		// Creates an empty WaitForAny object. Call addWait() to populate it before calling wait().
		WaitForAny();
		...
		// Adds a semaphore and a value to the set of semaphores to wait for. Returns kAdded if the
		// `semaphore` and `waitValue` were added, kAlreadyAdded if `semaphore` already exists in
		// this wait object, or kAlreadySignaled if `semaphore` is already signaled at at least
		// `waitValue`. If it is, then the condition for the wait is already reached.
		Result addWait(TimelineSemaphore &semaphore, uint64_t waitValue);
	...
	private:
		Result addWaitInternal(TimelineSemaphore &semaphore, uint64_t waitValue);
		// Removes all semaphores from the wait. Called automatically during destruction.
		void removeAllWaits();
