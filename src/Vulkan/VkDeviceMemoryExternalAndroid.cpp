...
{
	__android_log_print(ANDROID_LOG_INFO, "SwiftShader", "`AHB::~AHB`, line %d", __LINE__);
	if(ahb)
	{
		__android_log_print(ANDROID_LOG_INFO, "SwiftShader", "`AHB::~AHB` rendernodeFD = %d", rendernodeFD);
		const native_handle_t *h = AHardwareBuffer_getNativeHandle(ahb);

		cros_gralloc_handle const *cros_handle = reinterpret_cast<cros_gralloc_handle const *>(h);
		uint32_t primeHandle;
		int ret = drmPrimeFDToHandle(rendernodeFD, cros_handle->fds[0], &primeHandle);
		...
		if(pDeviceHandleMap->decrementReference(primeHandle) == 0)
		{
			...
			__android_log_print(ANDROID_LOG_INFO, "SwiftShader", "`AHB::~AHB` prime_handle = %d", primeHandle);
			if(ret)
			{
				__android_log_print(ANDROID_LOG_INFO, "SwiftShader", "%s: DRM_IOCTL_PRIME_FD_TO_HANDLE failed: %s (errno %d)\n",
			...
			ret = drmIoctl(rendernodeFD, DRM_IOCTL_GEM_CLOSE, &gem_close);
			if(ret)
			{
				__android_log_print(ANDROID_LOG_INFO, "SwiftShader", "%s: DRM_IOCTL_GEM_CLOSE  failed: %s (errno %d)\n",
				                    __func__, strerror(errno), errno);
			}
		}
		close(rendernodeFD);
...
VkResult AHardwareBufferExternalMemory::allocate(size_t size, void **pBuffer)
{
	...
	{
		__android_log_print(ANDROID_LOG_INFO, "SwiftShader", "`AHB::allocate - import`, line %d", __LINE__);
		...
		__android_log_print(ANDROID_LOG_INFO, "SwiftShader", "`AHB::allocate - import` rendernodeFD = %d", rendernodeFD);
		__android_log_print(ANDROID_LOG_INFO, "SwiftShader", "`AHB::allocate - import` primeHandle = %d", primeHandle);
		if(ret)
		{
			__android_log_print(ANDROID_LOG_INFO, "SwiftShader", "%s: DRM_IOCTL_PRIME_FD_TO_HANDLE failed: %s (errno %d)\n",
			                    __func__, strerror(errno), errno);
		...
		if(pAddress != nullptr)
		{
			__android_log_print(ANDROID_LOG_INFO, "SwiftShader", "`AHB::allocate - import`, ptr found: %p", pAddress);
			*pBuffer = pAddress;
		...
		else
		{
			...
			ret = drmIoctl(rendernodeFD, DRM_IOCTL_VIRTGPU_MAP, &map);
			if(ret)
			{
				__android_log_print(ANDROID_LOG_INFO, "SwiftShader", "%s: failed with %d mapping command response buffer (%s)\n", __func__, ret, strerror(errno));
				return VK_ERROR_INVALID_EXTERNAL_HANDLE;
			}
			...
			if(ptr == MAP_FAILED)
			{
				__android_log_print(ANDROID_LOG_INFO, "SwiftShader", "%s: failed with %d mmap'ing command response buffer (%s)\n",
			...
			*pBuffer = pDeviceHandleMap->query(primeHandle);
			__android_log_print(ANDROID_LOG_INFO, "SwiftShader", "`AHB::allocate - import`, no ptr found: %p", pBuffer);
		}
	}
	else
	{
		...
		__android_log_print(ANDROID_LOG_INFO, "SwiftShader", "`AHB::allocate - export`, line %d", __LINE__);
		// Outline ahbDesc
		...
		if(allocateInfo.imageHandle)
		{
			...
			if(ahbDesc.format == AHARDWAREBUFFER_FORMAT_R16G16B16A16_FLOAT)
			{
				ahbDesc.usage = AHARDWAREBUFFER_USAGE_CPU_READ_OFTEN | AHARDWAREBUFFER_USAGE_CPU_WRITE_OFTEN | AHARDWAREBUFFER_USAGE_GPU_SAMPLED_IMAGE;
			}
			else
			{
				ahbDesc.usage = AHARDWAREBUFFER_USAGE_CPU_READ_OFTEN | AHARDWAREBUFFER_USAGE_CPU_WRITE_OFTEN | AHARDWAREBUFFER_USAGE_GPU_SAMPLED_IMAGE | AHARDWAREBUFFER_USAGE_GPU_FRAMEBUFFER;
			}
		}
		else
		{
		...
		rendernodeFD = drmOpenRender(128);
		int ret = drmPrimeFDToHandle(rendernodeFD, crosHandle->fds[0], &primeHandle);
		__android_log_print(ANDROID_LOG_INFO, "SwiftShader", "`AHB::allocate - export` rendernodeFD = %d", rendernodeFD);
		__android_log_print(ANDROID_LOG_INFO, "SwiftShader", "`AHB::allocate - export` primeHandle = %d", primeHandle);
		if(ret)
		{
			__android_log_print(ANDROID_LOG_INFO, "SwiftShader", "%s: DRM_IOCTL_PRIME_FD_TO_HANDLE failed: %s (errno %d)\n",
			                    __func__, strerror(errno), errno);
		...
		if(pAddress != nullptr)
		{
			__android_log_print(ANDROID_LOG_INFO, "SwiftShader", "`AHB::allocate - export`, ptr found: %p", pAddress);
			*pBuffer = pAddress;
			pDeviceHandleMap->incrementReference(primeHandle);
		}
		else
		{
			...
			pDeviceHandleMap->add(primeHandle, ptr);
			*pBuffer = pDeviceHandleMap->query(primeHandle);
			__android_log_print(ANDROID_LOG_INFO, "SwiftShader", "`AHB::allocate - export`, no ptr found: %p", pBuffer);
			__android_log_print(ANDROID_LOG_INFO, "SwiftShader", "`AHB::allocate - export`, no ptr found: %p", *pBuffer);
			if(*pBuffer == nullptr)
				__android_log_print(ANDROID_LOG_INFO, "SwiftShader", "`AHB::allocate - export`, *pBuffer is nullptr");
			else
				__android_log_print(ANDROID_LOG_INFO, "SwiftShader", "`AHB::allocate - export`, *pBuffer is not nullptr");
		}
	}
	return VK_SUCCESS;
}
...
VkResult AHardwareBufferExternalMemory::exportAndroidHardwareBuffer(struct AHardwareBuffer **pAhb) const
{
	...
	__android_log_print(ANDROID_LOG_INFO, "SwiftShader", "`AHB::exportAhb`, line %d", __LINE__);
	AHardwareBuffer_acquire(ahb);
	*pAhb = ahb;
	return VK_SUCCESS;
}
...
{
	__android_log_print(ANDROID_LOG_INFO, "SwiftShader", "`AHB::deallocate`, line %d", __LINE__);
}
