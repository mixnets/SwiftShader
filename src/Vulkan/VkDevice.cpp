...
Device::Device(const Device::DeviceCreateInfo* info, char* membersMemory)
	: physicalDevice(info->pPhysicalDevice), queues(reinterpret_cast<VkQueue*>(membersMemory))
{
	...
	membersMemory += sizeof(Queue*) * queueCount;
	...
	for(uint32_t i = 0; i < pCreateInfo->queueCreateInfoCount; i++)
	{
		...
		for(uint32_t j = 0; j < queueCreateInfo.queueCount; j++, queueID++, membersMemory += sizeof(Queue))
		{
			queues[queueID] = reinterpret_cast<VkQueue>(membersMemory);
...
size_t Device::ComputeRequiredAllocationSize(const Device::DeviceCreateInfo* info)
{
	...
	return (sizeof(Queue*) + sizeof(Queue)) * queueCount;
}
