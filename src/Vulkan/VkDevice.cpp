...
VkResult Device::waitForFences(uint32_t fenceCount, const VkFence* pFences, VkBool32 waitAll, uint64_t timeout)
{
	...
	if(waitAll) // All fences must be signaled
	{
		for(uint32_t i = 0; i < fenceCount; i++)
		{
			...
			{
				auto now_ns = now();
				if(Cast(pFences[i])->waitFor((now_ns > end_ns) ? std::chrono::nanoseconds(0) : end_ns - now_ns) != VK_SUCCESS) // At least one fence is not signaled
				{
					return VK_TIMEOUT;
				}
			}
	...
	else // At least one fence must be signaled
	{
		...
		if(timeout > 0)
		{
			for(uint32_t i = 0; i < fenceCount; i++)
			{
				...
				{
					auto now_ns = now();
					if(Cast(pFences[i])->waitFor((now_ns > end_ns) ? std::chrono::nanoseconds(0) : end_ns - now_ns) == VK_SUCCESS) // At least one fence is signaled
					{
						return VK_SUCCESS;
					}
				}
