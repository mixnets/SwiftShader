	...
	if(pCreateInfo->bindingCount > 0)
	{
		...
		while(layoutInfo && !hasVariableSizedDescriptor)
		{
			if(layoutInfo->sType == VK_STRUCTURE_TYPE_DESCRIPTOR_SET_LAYOUT_BINDING_FLAGS_CREATE_INFO)
			{
				...
				for(uint32_t i = 0; (i < bindingFlagsCreateInfo->bindingCount) && !hasVariableSizedDescriptor; i++)
				{
					hasVariableSizedDescriptor |= !!(bindingFlagsCreateInfo->pBindingFlags[i] & VK_DESCRIPTOR_BINDING_VARIABLE_DESCRIPTOR_COUNT_BIT);
				}
			}
			else
			{
		...
		while(layoutSupport)
		{
			if(layoutSupport->sType == VK_STRUCTURE_TYPE_DESCRIPTOR_SET_VARIABLE_DESCRIPTOR_COUNT_LAYOUT_SUPPORT)
			{
				...
				variableDescriptorCountLayoutSupport->maxVariableDescriptorCount =
				    hasVariableSizedDescriptor ? ((highestNumberedBinding.descriptorType == VK_DESCRIPTOR_TYPE_INLINE_UNIFORM_BLOCK) ? vk::MAX_INLINE_UNIFORM_BLOCK_SIZE : UINT32_MAX) : 0;
			}
			else
			{
