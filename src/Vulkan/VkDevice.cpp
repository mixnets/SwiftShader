...
Device2::Device2(VkPhysicalDevice pPhysicalDevice, uint32_t pQueueCount, VkQueue* pQueues)
	: physicalDevice(pPhysicalDevice), queueCount(pQueueCount), queues(pQueues)
{
}

void Device2::destroy(const VkAllocationCallbacks* pAllocator)
{
	DestroyQueues(pAllocator, queueCount, queues);
}

VkResult Device2::AllocateQueues(const VkAllocationCallbacks* pAllocator, const VkDeviceCreateInfo* pCreateInfo,
							    uint32_t& queueCount, VkQueue** queues)
{
	for(uint32_t i = 0; i < pCreateInfo->queueCreateInfoCount; i++)
	{
...
void Device2::DestroyQueues(const VkAllocationCallbacks* pAllocator, const uint32_t& queuesToDestroy, VkQueue* queues)
{
	// Release queues
	for(uint32_t q = 0; q < queuesToDestroy; ++q)
	{
...
VkQueue Device2::getQueue(uint32_t queueFamilyIndex, uint32_t queueIndex) const
{
	return queues[queueIndex];
}

int Device2::virtualFunctionToForceCreatingVTable()
{
	return 77;
}
