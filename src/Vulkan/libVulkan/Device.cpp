...
namespace vulkan
{
	...
	SwDevice::SwDevice(Context *context) : Renderer(context, OpenGL, true), context(context)
	{
	...
	void* SwDevice::operator new(size_t size)
	{
		ASSERT(size == sizeof(SwDevice)); // This operator can't be called from a derived class
		return sw::allocate(sizeof(SwDevice), 16);
	}

	void SwDevice::operator delete(void * mem)
	{
		sw::deallocate(mem);
	}

	void SwDevice::clearColor(float red, float green, float blue, float alpha, unsigned int rgbaMask)
	{
		if(!rgbaMask)
		{
			return;
	...
	void SwDevice::clearDepth(float z)
	{
		if(!depthBuffer)
		{
			return;
	...
	void SwDevice::clearStencil(unsigned int stencil, unsigned int mask)
	{
		if(!stencilBuffer)
		{
			return;
	...
	sw::Surface *SwDevice::createDepthStencilSurface(unsigned int width, unsigned int height, sw::Format format, int multiSampleDepth, bool discard)
	{
	...
	//	case FORMAT_D32_LOCKABLE:
		...
		sw::Surface *surface = new sw::Surface(nullptr, width, height, multiSampleDepth, format, lockable, true);
	...
	sw::Surface *SwDevice::createRenderTarget(unsigned int width, unsigned int height, sw::Format format, int multiSampleDepth, bool lockable)
	{
		if(height > OUTLINE_RESOLUTION)
		{
			ERR("Invalid parameters: %dx%d", width, height);
	...
	void SwDevice::drawIndexedPrimitive(sw::DrawType type, unsigned int indexOffset, unsigned int primitiveCount)
	{
	...
	void SwDevice::drawPrimitive(sw::DrawType type, unsigned int primitiveCount)
	{
		if(!bindResources() || !primitiveCount)
		{
			return;
	...
	void SwDevice::setPixelShader(PixelShader *pixelShader)
	{
		this->pixelShader = pixelShader;
		pixelShaderDirty = true;
	}

	void SwDevice::setPixelShaderConstantF(unsigned int startRegister, const float *constantData, unsigned int count)
	{
	...
	void SwDevice::setScissorEnable(bool enable)
	{
		scissorEnable = enable;
	}

	void SwDevice::setRenderTarget(int index, sw::Surface *renderTarget)
	{
	...
	void SwDevice::setDepthBuffer(sw::Surface *depthBuffer)
	{
	...
	void SwDevice::setStencilBuffer(sw::Surface *stencilBuffer)
	{
	...
	void SwDevice::setScissorRect(const sw::Rect &rect)
	{
		scissorRect = rect;
	}

	void SwDevice::setVertexShader(VertexShader *vertexShader)
	{
		this->vertexShader = vertexShader;
		vertexShaderDirty = true;
	}

	void SwDevice::setVertexShaderConstantF(unsigned int startRegister, const float *constantData, unsigned int count)
	{
	...
	void SwDevice::copyBuffer(sw::byte *sourceBuffer, sw::byte *destBuffer, unsigned int width, unsigned int height, unsigned int sourcePitch, unsigned int destPitch, unsigned int bytes, bool flipX, bool flipY)
	{
		if(flipX)
		{
			if(flipY)
	...
	bool SwDevice::stretchRect(sw::Surface *source, const sw::SliceRect *sourceRect, sw::Surface *dest, const sw::SliceRect *destRect, unsigned char flags)
	{
		if(!source || !dest)
		{
		...
		bool isDepth = (flags & SwDevice::DEPTH_BUFFER) && sw::Surface::isDepth(source->getInternalFormat());
		bool isStencil = (flags & SwDevice::STENCIL_BUFFER) && (sw::Surface::isDepth(source->getInternalFormat()) || sw::Surface::isStencil(source->getInternalFormat()));
		bool isColor = (flags & SwDevice::COLOR_BUFFER) == SwDevice::COLOR_BUFFER;
		...
		if((isDepth || isStencil) && !scaling && equalFormats && (!hasQuadLayout || fullCopy))
		{
			if(source->hasDepth() && isDepth)
			{
				...
				copyBuffer(sourceBuffer, destBuffer, dRect.width(), dRect.height(), source->getInternalPitchB(), dest->getInternalPitchB(), sw::Surface::bytes(source->getInternalFormat()), flipX, flipY);
			...
			if(source->hasStencil() && isStencil)
			{
				...
				copyBuffer(sourceBuffer, destBuffer, source->getWidth(), source->getHeight(), source->getStencilPitchB(), dest->getStencilPitchB(), sw::Surface::bytes(source->getStencilFormat()), flipX, flipY);

				source->unlockStencil();
				dest->unlockStencil();
			}
		}
		else if((flags & SwDevice::COLOR_BUFFER) && !scaling && equalFormats && (!hasQuadLayout || fullCopy))
		{
			...
			copyBuffer(sourceBytes, destBytes, width, height, sourcePitch, destPitch, sw::Surface::bytes(source->getInternalFormat()), flipX, flipY);
		...
		else if(isColor || isDepth || isStencil)
		{
			...
			blit(source, sRect, dest, dRect, scaling && (flags & SwDevice::USE_FILTER), isStencil);
		}
		else
		{
			UNREACHABLE(false);
	...
	bool SwDevice::stretchCube(sw::Surface *source, sw::Surface *dest)
	{
		if(!source || !dest || sw::Surface::isDepth(source->getInternalFormat()) || sw::Surface::isStencil(source->getInternalFormat()))
		{
			ERR("Invalid parameters");
			return false;
		}
		...
		if(!scaling && equalFormats)
		{
			unsigned int sourcePitch = source->getInternalPitchB();
			unsigned int destPitch = dest->getInternalPitchB();
			unsigned int bytes = dWidth * sw::Surface::bytes(source->getInternalFormat());
	...
	void SwDevice::bindShaderConstants()
	{
		if(pixelShaderDirty)
		{
			if(pixelShader)
	...
	bool SwDevice::validRectangle(const sw::Rect *rect, sw::Surface *surface)
	{
		if(!rect)
		{
			return true;
