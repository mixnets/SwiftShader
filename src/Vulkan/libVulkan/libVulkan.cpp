...
namespace vulkan
{
	...
	VkResult CreateBuffer(VkDevice device, const VkBufferCreateInfo * pCreateInfo, const VkAllocationCallbacks * pAllocator, VkBuffer * pBuffer)
	{
		...
		if (pAllocator == NULL)
		{
			buffer = reinterpret_cast<Buffer *>(vkutils::Alloc(&myDevice->alloc, pAllocator, pCreateInfo->size, ALIGNMENT, VK_SYSTEM_ALLOCATION_SCOPE_OBJECT));
		}
		else
		{
			buffer = reinterpret_cast<Buffer *>(pAllocator->pfnAllocation(myDevice, pCreateInfo->size, ALIGNMENT, VK_SYSTEM_ALLOCATION_SCOPE_OBJECT));
		}
	...
	VkResult AllocateMemory(VkDevice device, const VkMemoryAllocateInfo * pAllocateInfo, const VkAllocationCallbacks * pAllocator, VkDeviceMemory * pMemory)
	{
		...
		if (pAllocator == NULL)
		{
			memory = reinterpret_cast<DeviceMemory *>(vkutils::Alloc(&myDevice->alloc, pAllocator, sizeof(*memory), ALIGNMENT, VK_SYSTEM_ALLOCATION_SCOPE_OBJECT));
		}
		else
		{
			memory = reinterpret_cast<DeviceMemory *>(pAllocator->pfnAllocation(myDevice, sizeof(*memory), ALIGNMENT, VK_SYSTEM_ALLOCATION_SCOPE_OBJECT));
		}
	...
	VkResult CreateImage(VkDevice device, const VkImageCreateInfo * pCreateInfo, const VkAllocationCallbacks * pAllocator, VkImage * pImage)
	{
		...
		if (pAllocator == NULL)
		{
			image = reinterpret_cast<Image *>(vkutils::Alloc(&myDevice->alloc, pAllocator, sizeof(*image), ALIGNMENT, VK_SYSTEM_ALLOCATION_SCOPE_OBJECT));
		}
		else
		{
			image = reinterpret_cast<Image *>(pAllocator->pfnAllocation(nullptr, sizeof(*image), ALIGNMENT, VK_SYSTEM_ALLOCATION_SCOPE_OBJECT));
		}
	...
	VkResult CreateRenderPass(VkDevice device, const VkRenderPassCreateInfo * pCreateInfo, const VkAllocationCallbacks * pAllocator, VkRenderPass * pRenderPass)
	{
		...
		if (pAllocator == NULL)
		{
			pRender = reinterpret_cast<RenderPass *>(vkutils::Alloc(&myDevice->alloc, pAllocator, sizeof(*pRender), ALIGNMENT, VK_SYSTEM_ALLOCATION_SCOPE_OBJECT));
		}
		else
		{
			pRender = reinterpret_cast<RenderPass *>(pAllocator->pfnAllocation(myDevice, sizeof(*pRender), ALIGNMENT, VK_SYSTEM_ALLOCATION_SCOPE_OBJECT));
		}
	...
	VkResult CreateImageView(VkDevice device, const VkImageViewCreateInfo * pCreateInfo, const VkAllocationCallbacks * pAllocator, VkImageView * pView)
	{
		...
		if (pAllocator == NULL)
		{
			view = reinterpret_cast<ImageView *>(vkutils::Alloc(&myDevice->alloc, pAllocator, sizeof(*view), ALIGNMENT, VK_SYSTEM_ALLOCATION_SCOPE_OBJECT));
		}
		else
		{
			view = reinterpret_cast<ImageView *>(pAllocator->pfnAllocation(myDevice, sizeof(*view), ALIGNMENT, VK_SYSTEM_ALLOCATION_SCOPE_OBJECT));
		}
	...
	VkResult CreatePipelineLayout(VkDevice device, const VkPipelineLayoutCreateInfo * pCreateInfo, const VkAllocationCallbacks * pAllocator, VkPipelineLayout * pPipelineLayout)
	{
		...
		if (pAllocator == NULL)
		{
			pipelineLayout = reinterpret_cast<PipelineLayout *>(vkutils::Alloc(&myDevice->alloc, pAllocator, sizeof(*pipelineLayout), ALIGNMENT, VK_SYSTEM_ALLOCATION_SCOPE_OBJECT));
		}
		else
		{
			pipelineLayout = reinterpret_cast<PipelineLayout *>(pAllocator->pfnAllocation(myDevice, sizeof(*pipelineLayout), ALIGNMENT, VK_SYSTEM_ALLOCATION_SCOPE_OBJECT));
		}
	...
	VkResult CreateGraphicsPipelines(VkDevice device, VkPipelineCache pipelineCache, uint32_t createInfoCount, const VkGraphicsPipelineCreateInfo * pCreateInfos, const VkAllocationCallbacks * pAllocator, VkPipeline * pPipelines)
	{
		...
		if (pAllocator == NULL)
		{
			pipeline = reinterpret_cast<Pipeline *>(vkutils::Alloc(&myDevice->alloc, pAllocator, sizeof(*pipeline), ALIGNMENT, VK_SYSTEM_ALLOCATION_SCOPE_OBJECT));
		}
		else
		{
			pipeline = reinterpret_cast<Pipeline *>(pAllocator->pfnAllocation(myDevice, sizeof(*pipeline), ALIGNMENT, VK_SYSTEM_ALLOCATION_SCOPE_OBJECT));
		}
	...
	VkResult CreateFramebuffer(VkDevice device, const VkFramebufferCreateInfo * pCreateInfo, const VkAllocationCallbacks * pAllocator, VkFramebuffer * pFramebuffer)
	{
		...
		if (pAllocator == NULL)
		{
			framebuffer = reinterpret_cast<Framebuffer *>(vkutils::Alloc(&myDevice->alloc, pAllocator, sizeof(*framebuffer), ALIGNMENT, VK_SYSTEM_ALLOCATION_SCOPE_OBJECT));
		}
		else
		{
			framebuffer = reinterpret_cast<Framebuffer *>(pAllocator->pfnAllocation(&myDevice, sizeof(*framebuffer), ALIGNMENT, VK_SYSTEM_ALLOCATION_SCOPE_OBJECT));
		}
	...
	VkResult CreateCommandPool(VkDevice device, const VkCommandPoolCreateInfo * pCreateInfo, const VkAllocationCallbacks * pAllocator, VkCommandPool * pCommandPool)
	{
		...
		if (pAllocator == NULL)
		{
			cmdPool = reinterpret_cast<CommandPool *>(vkutils::Alloc(&myDevice->alloc, pAllocator, sizeof(*cmdPool), ALIGNMENT, VK_SYSTEM_ALLOCATION_SCOPE_OBJECT));
		}
		else
		{
			cmdPool = reinterpret_cast<CommandPool *>(pAllocator->pfnAllocation(myDevice, sizeof(*cmdPool), ALIGNMENT, VK_SYSTEM_ALLOCATION_SCOPE_OBJECT));
		}
	...
	VkResult AllocateCommandBuffers(VkDevice device, const VkCommandBufferAllocateInfo * pAllocateInfo, VkCommandBuffer * pCommandBuffers)
	{
		...
		CommandBuffer *cmdBuf = reinterpret_cast<CommandBuffer *>(vkutils::Allocate(&myDevice->alloc, sizeof(*cmdBuf), ALIGNMENT, VK_SYSTEM_ALLOCATION_SCOPE_OBJECT));
	...
	VkResult CreateFence(VkDevice device, const VkFenceCreateInfo * pCreateInfo, const VkAllocationCallbacks * pAllocator, VkFence * pFence)
	{
		...
		if (pAllocator == NULL)
		{
			fence = reinterpret_cast<Fence *>(vkutils::Alloc(&myDevice->alloc, pAllocator, sizeof(*fence), ALIGNMENT, VK_SYSTEM_ALLOCATION_SCOPE_OBJECT));
		}
		else
		{
			fence = reinterpret_cast<Fence *>(pAllocator->pfnAllocation(myDevice, sizeof(*fence), ALIGNMENT, VK_SYSTEM_ALLOCATION_SCOPE_OBJECT));
		}
