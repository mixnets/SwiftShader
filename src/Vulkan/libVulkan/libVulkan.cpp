...
namespace vulkan
{
	...
	{
    uint32_t globalExtSize = sizeof(global_ext) / sizeof(global_ext[0]);
	...
	VkResult CreateInstance(const VkInstanceCreateInfo * pCreateInfo, const VkAllocationCallbacks * pAllocator, VkInstance * pInstance)
	{
		...
		for (uint32_t i = 0; i < pCreateInfo->enabledExtensionCount; ++i)
		{
			...
			for (uint32_t j = 0; j < sizeof(global_ext) / sizeof(global_ext[0]); ++j)
			{
		...
		instance->physicalDevice.name = "SwiftShader Device";
	...
	{
		uint32_t deviceExtSize = sizeof(device_extensions) / sizeof(device_extensions[0]);
