...
namespace vkutils
{
	...
	struct outarray
	{
		struct OutArray base;
		T meta[];
	};

	static inline void OutArrayInit(struct OutArray *pArray, void *pData, uint32_t *pLength)
	{
		pArray->pData = pData;
		pArray->capacity = *pLength;
		pArray->pFilledLength = pLength;
		*pArray->pFilledLength = 0;
		pArray->desiredLength = 0;

		if (pArray->pData == NULL)
		{
			pArray->capacity = UINT32_MAX;
		}
	}
