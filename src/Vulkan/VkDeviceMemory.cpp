...
void DeviceMemory::notify(DescriptorView::AccessType accessType)
{
	// If the buffer was modified, the associated image using the same device memory must be notified
	if(accessType & DescriptorView::AccessType::WRITE_BUFFER_ACCESS)
	{
		accessType = static_cast<DescriptorView::AccessType>(accessType | DescriptorView::WRITE_IMAGE_ACCESS);
	}
	for(auto image : images)
	{
