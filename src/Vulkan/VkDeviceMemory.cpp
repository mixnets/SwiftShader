...
class HostMappedForeignMemory : public vk::DeviceMemory::ExternalBase
{
public:
	struct AllocateInfo
	{
			...
			while (createInfo)
			{
				switch (createInfo->sType)
				{
					case VK_STRUCTURE_TYPE_IMPORT_MEMORY_HOST_POINTER_INFO_EXT:
					{
						...
						if (importInfo->handleType != VK_EXTERNAL_MEMORY_HANDLE_TYPE_HOST_MAPPED_FOREIGN_MEMORY_BIT_EXT)
						{
	...
	static const VkExternalMemoryHandleTypeFlagBits typeFlagBit = VK_EXTERNAL_MEMORY_HANDLE_TYPE_HOST_MAPPED_FOREIGN_MEMORY_BIT_EXT;
	...
	explicit HostMappedForeignMemory(const VkMemoryAllocateInfo *pAllocateInfo)
...
#endif
	if(parseCreateInfo<HostMappedForeignMemory>(pAllocateInfo, pTraits))
	{
		return;
	}
