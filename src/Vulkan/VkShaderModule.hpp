...
{
public:
	ShaderModule(char* pCode);
	~ShaderModule() = delete;
	void destroy(const VkAllocationCallbacks* pAllocator);

	static VkResult AllocateCode(const VkAllocationCallbacks* pAllocator,
		const VkShaderModuleCreateInfo* pCreateInfo, char** pCode);
	static void DestroyCode(const VkAllocationCallbacks* pAllocator, char* pCode);
