...
namespace vk
{
	                               ...
	                               void* pData, VkDeviceSize stride, VkQueryResultFlags flags) const
	{
		...
		size_t copySize = (flags & VK_QUERY_RESULT_64_BIT) ? sizeof(uint64_t) : sizeof(uint32_t);
		...
		for(uint32_t i = firstQuery; i < (firstQuery + queryCount); i++, data += stride)
		{
			...
			{
				static Query::State* queryState = nullptr;
				std::unique_lock<std::mutex> lk(pool[i].mutex);
				queryState = &pool[i].state;
				pool[i].cv.wait(lk, [] { return *queryState != Query::ACTIVE; });
			}

			std::lock_guard<std::mutex> lk(pool[i].mutex);
			...
			else
			{
				...
				if(writeResult)
				{
					*result32 = static_cast<uint32_t>(pool[i].data);
				}
				if(flags & VK_QUERY_RESULT_WITH_AVAILABILITY_BIT) // Output query availablity
				{
					result32++;
					*result32 = pool[i].state;
				}
			}
		}
