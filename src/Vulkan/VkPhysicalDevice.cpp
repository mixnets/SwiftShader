...
PhysicalDevice::PhysicalDevice() : name("SwiftShader Device")
{
}

PhysicalDevice::operator VkPhysicalDevice()
{
	return reinterpret_cast<VkPhysicalDevice>(this);
}

void* PhysicalDevice::operator new(size_t count, const VkAllocationCallbacks* pAllocator)
{
	return vk::allocate(count, pAllocator, VK_SYSTEM_ALLOCATION_SCOPE_DEVICE);
}

void PhysicalDevice::operator delete(void* ptr, const VkAllocationCallbacks* pAllocator)
{
...
const VkPhysicalDeviceLimits& PhysicalDevice::getLimits() const
{
	...
	static const VkPhysicalDeviceLimits limits = {
		(1 << vk::MaxImageLevels1D), // maxImageDimension1D
		(1 << vk::MaxImageLevels2D), // maxImageDimension2D
		(1 << vk::MaxImageLevels3D), // maxImageDimension3D
		(1 << vk::MaxImageLevelsCube), // maxImageDimensionCube
		(1 << vk::MaxImageArrayLayers), // maxImageArrayLayers
...
{
	pImageFormatProperties->maxArrayLayers = 1 << vk::MaxImageArrayLayers;

	switch(type)
	{
	case VK_IMAGE_TYPE_1D:
		pImageFormatProperties->maxMipLevels = vk::MaxImageLevels1D;
		pImageFormatProperties->maxExtent.width = 1 << vk::MaxImageLevels1D;
	...
	case VK_IMAGE_TYPE_2D:
		...
		{
			pImageFormatProperties->maxMipLevels = vk::MaxImageLevelsCube;
			pImageFormatProperties->maxExtent.width = 1 << vk::MaxImageLevelsCube;
			pImageFormatProperties->maxExtent.height = 1 << vk::MaxImageLevelsCube;
			pImageFormatProperties->maxExtent.depth = 1;
		}
		else
		{
			pImageFormatProperties->maxMipLevels = vk::MaxImageLevels2D;
			pImageFormatProperties->maxExtent.width = 1 << vk::MaxImageLevels2D;
			pImageFormatProperties->maxExtent.height = 1 << vk::MaxImageLevels2D;
			pImageFormatProperties->maxExtent.depth = 1;
		}
		break;
	case VK_IMAGE_TYPE_3D:
		pImageFormatProperties->maxMipLevels = vk::MaxImageLevels3D;
		pImageFormatProperties->maxExtent.width = 1 << vk::MaxImageLevels3D;
		pImageFormatProperties->maxExtent.height = 1 << vk::MaxImageLevels3D;
		pImageFormatProperties->maxExtent.depth = 1 << vk::MaxImageLevels3D;
		break;
	default:
		UNREACHABLE(type);
		break;
