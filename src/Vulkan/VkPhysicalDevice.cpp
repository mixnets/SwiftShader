...
void PhysicalDevice::getProperties(VkImageUsageFlags vkUsageFlags, VkImageCreateFlags vkCreateFlags, VkAndroidHardwareBufferUsageANDROID *ahbProperties) const
{
	...
	ASSERT((vkCreateFlags & VK_IMAGE_CREATE_PROTECTED_BIT) == 0);
	...
	if((vkUsageFlags & VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT) || (vkUsageFlags & VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT))
	{
		ahbUsage |= AHARDWAREBUFFER_USAGE_GPU_FRAMEBUFFER;
	}

	if(vkCreateFlags & VK_IMAGE_CREATE_CUBE_COMPATIBLE_BIT)
	{
		ahbUsage |= AHARDWAREBUFFER_USAGE_GPU_CUBE_MAP;
	}

	if(vkCreateFlags & VK_IMAGE_CREATE_PROTECTED_BIT)
	{
