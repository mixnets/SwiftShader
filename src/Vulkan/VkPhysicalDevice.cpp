...
void PhysicalDevice::getFeatures(VkPhysicalDeviceVulkan11Features *features) const
{
	...
	features->multiviewGeometryShader = VK_FALSE;
	features->multiviewTessellationShader = VK_FALSE;
	features->variablePointerStorageBuffer = VK_FALSE;
	features->variablePointers = VK_FALSE;
	features->protectedMemory = VK_FALSE;
	features->samplerYcbcrConversion = VK_TRUE;
	features->shaderDrawParameters = VK_FALSE;
...
{
	features->sampleMirrorClampToEdge = VK_FALSE;
	features->drawIndirectCount = VK_FALSE;
	features->storageBuffer8BitAccess = VK_FALSE;
	features->uniformAndStorageBuffer8BitAccess = VK_FALSE;
	features->storagePushConstant8 = VK_FALSE;
	features->shaderBufferInt64Atomics = VK_FALSE;
	features->shaderShaderdInt64Atomics = VK_FALSE;
	...
	features->subgroupBroadcastDyanmicId = VK_FALSE;
}

void PhysicalDevice::getFeatures(VkPhysicalDevicePhysicalFeatures2 *features) const
{
	features->features = getFeatures();
	VkBaseInStructure *curExtension = features->pNext;
	while(curExtension != nullptr)
	{
		switch(curExtension->sType)
		{
			...
			case VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_SHADER_DRAW_PARAMETERS_FEATURES:
				getFeatures(reinterpret_cast<VkPhysicalDeviceShaderDrawParameters *>(curExtension));
				break;
			default:
		...
		curExtension = curExtension->pNext;
	}
}
