...
#	error "This header should only be included when building SwiftShader for Fuchsia!"
#endif
...
//       small negotiation protocol between all users of the buffer collection:
//
...
// A function that takes a Fuchsia service name (e.g. "fuchsia.sysmem") and tries
// to connect to it. The second argument is a Zircon channel handle passed to the
// service. This is roughly equivalent to fdio_service_connect(), except that
// this function is not available inside a Vulkan ICD.
//
// This is a private implementation detail between the Fuchsia Vulkan loader
// library and the ICD implementation. Hence it does not appear in
// <vulkan/vulkan_fuchsia.h> or <vulkan/vulkan_fuchsia_extras.h>.
using PFN_vkConnectToServiceAddr = VkResult (*)(const char *, zx_handle_t);
...
public:
	...
	// Return VMO handle and start offset for the n-th buffer in the collection.
	// Return an error if the index is invalid, or the buffers were not readied by
	// the client yet.
	VkResult takeBufferVmo(uint32_t index, zx_handle_t *vmoHandle, uint64_t *vmoUsableStart);

private:
	VkResult getInfo2() const;

	fuchsia::sysmem::AllocatorSyncPtr &sysmemAllocator;
	fuchsia::sysmem::BufferCollectionSyncPtr sysmemCollection;
	mutable bool hasInfo2 = false;
	mutable fuchsia::sysmem::BufferCollectionInfo_2 info2 = {};
};
