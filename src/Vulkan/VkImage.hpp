...
private:
	...
	VkExtent3D adjustExtent(const VkExtent3D& extent, VkImageAspectFlagBits aspect) const;
	VkOffset3D adjustOffset(const VkOffset3D& offset, VkImageAspectFlagBits aspect) const;
	VkExtent2D adjustBufferExtent(const VkExtent2D& extent, const VkBufferImageCopy& region) const;
