...
public:
	...
	static VkResult Create(const VkAllocationCallbacks* pAllocator, const CreateInfo* pCreateInfo, VkT* outObject)
	{
		*outObject = VK_NULL_HANDLE;
		...
		if(sizes.device)
		{
			mem.device = static_cast<char*>(vk::allocate(sizes.device, DEVICE_MEMORY, GetAllocationScope()));
			if(!mem.device)
			{
		...
		if(sizes.host)
		{
			mem.host = static_cast<char*>(vk::allocate(sizes.host, pAllocator, GetAllocationScope()));
			if(!mem.host)
			{
