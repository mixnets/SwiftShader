...
PipelineLayout::PipelineLayout(const VkPipelineLayoutCreateInfo* pCreateInfo, char* membersMemory) :
	setLayoutCount(pCreateInfo->setLayoutCount), pushConstantRangeCount(pCreateInfo->pushConstantRangeCount)
{
	...
	setLayouts = reinterpret_cast<VkDescriptorSetLayout*>(membersMemory);
	...
	pushConstantRanges = reinterpret_cast<VkPushConstantRange*>(membersMemory + setLayoutsSize);
