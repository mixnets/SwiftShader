    ...
    , pushConstantRangeCount(pCreateInfo->pushConstantRangeCount)
{
	...
	for(uint32_t i = 0; i < pCreateInfo->setLayoutCount; i++)
	{
		...
		descriptorSets[i] = bindingStorage;
		...
		{
			descriptorSets[i][j].descriptorType = setLayout->getDescriptorType(j);
			descriptorSets[i][j].offset = setLayout->getBindingOffset(j);
			descriptorSets[i][j].dynamicOffsetIndex = dynamicOffsetIndex;

			if(DescriptorSetLayout::IsDescriptorDynamic(descriptorSets[i][j].descriptorType))
			{
...
{
	vk::deallocate(descriptorSets[0], pAllocator);  // pushConstantRanges are in the same allocation
}
...
{
	ASSERT(setNumber < descriptorSetCount /* && sets[setNumber]*/);x
	return descriptorSets[setNumber][bindingNumber].dynamicOffsetIndex;
}

uint32_t PipelineLayout::getBindingOffset(uint32_t setNumber, uint32_t bindingNumber) const
{
	ASSERT(setNumber < descriptorSetCount /* && sets[setNumber]*/);x
	return descriptorSets[setNumber][bindingNumber].offset;
}

VkDescriptorType PipelineLayout::getDescriptorType(uint32_t setNumber, uint32_t bindingNumber) const
{
	ASSERT(setNumber < descriptorSetCount /* && sets[setNumber]*/);x
	return descriptorSets[setNumber][bindingNumber].descriptorType;
}
