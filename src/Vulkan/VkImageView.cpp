...
Identifier::Identifier(const VkImageViewCreateInfo *pCreateInfo)
{
	...
	VkFormat viewFormat = static_cast<VkFormat>(GetImageViewFormat(pCreateInfo).getAspectFormat(subresource.aspectMask));
	const Image *sampledImage = image->getSampledImage(viewFormat);

	VkFormat samplingFormat = (image == sampledImage) ? viewFormat : sampledImage->getFormat().getAspectFormat(subresource.aspectMask);
	pack({ pCreateInfo->viewType, samplingFormat, ResolveComponentMapping(pCreateInfo->components, samplingFormat) });
}
...
{
	imageViewType = static_cast<uint32_t>(data.imageViewType);
	format = Format::mapTo8bit(data.format);
	r = static_cast<uint32_t>(data.mapping.r);
