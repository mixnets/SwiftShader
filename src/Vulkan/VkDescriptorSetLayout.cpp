...
void DescriptorSetLayout::WriteDescriptorSet(Device* device, DescriptorSet *dstSet, VkDescriptorUpdateTemplateEntry const &entry, char const *src)
{
		...
		for(uint32_t i = 0; i < entry.descriptorCount; i++)
		{
			...
			else
			{
				for(int mipmapLevel = 0; mipmapLevel < sw::MIPMAP_LEVELS; mipmapLevel++)
				{
					...
					int sliceP = imageView->getLayerOffset(aspect, level, ImageView::SAMPLING) / bytes;
					int samplePitchP = imageView->getMipLevelSize(aspect, level, ImageView::SAMPLING) / bytes;

					WriteTextureLevelInfo(texture, mipmapLevel, width, height, depth, pitchP, sliceP, samplePitchP);
				}
			}
		}
	}
	         ...
	         entry.descriptorType == VK_DESCRIPTOR_TYPE_INPUT_ATTACHMENT)
	{
		...
		for(uint32_t i = 0; i < entry.descriptorCount; i++)
		{
			...
			descriptor[i].samplePitchBytes = imageView->getMipLevelSize(VK_IMAGE_ASPECT_COLOR_BIT, 0);
			...
			if (imageView->getFormat().isStencil())
			{
				...
				descriptor[i].stencilSamplePitchBytes = imageView->getMipLevelSize(VK_IMAGE_ASPECT_STENCIL_BIT, 0);
...
void DescriptorSetLayout::WriteTextureLevelInfo(sw::Texture *texture, int level, int width, int height, int depth, int pitchP, int sliceP, int samplePitchP)
{
	if(level == 0)
	{
