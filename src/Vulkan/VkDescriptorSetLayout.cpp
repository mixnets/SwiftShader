...
void DescriptorSetLayout::Notify(void* descriptorSet, DescriptorView::AccessType accessType)
{
	if(descriptorSet)
	{
		DescriptorSet* set = static_cast<DescriptorSet*>(descriptorSet);
		DescriptorSetLayout* layout = set->header.layout;
		if(layout)
		{
			auto it = layout->descriptorMetaData.find(set);
...
void DescriptorSetLayout::Unregister(void* descriptorSet)
{
	if(descriptorSet)
	{
		DescriptorSet* set = static_cast<DescriptorSet*>(descriptorSet);
