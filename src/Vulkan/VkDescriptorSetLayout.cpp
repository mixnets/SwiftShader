...
static bool IsReadOnlyInShader(VkDescriptorType descriptorType)
{
	...
	case VK_DESCRIPTOR_TYPE_STORAGE_BUFFER_DYNAMIC:
		return false;
	default:
		UNREACHABLE("descriptorType: %d", int(descriptorType));
		break;
	}
...
static DescriptorSet* GetDescriptorSetFromData(void* descriptorSetData)
{
	return reinterpret_cast<DescriptorSet*>(reinterpret_cast<uintptr_t>(descriptorSetData) - sizeof(vk::DescriptorSetHeader));
}
...
{
	for(auto metadata : descriptorMetaData)
	{
		DescriptorSet* set = metadata.first;
		set->header.layout = nullptr;
	}
...
void DescriptorSetLayout::Notify(uint8_t* descriptorSetData, DescriptorView::AccessType accessType)
{
	...
	{
		DescriptorSet* descriptorSet = GetDescriptorSetFromData(descriptorSetData);
		DescriptorSetLayout* layout = descriptorSet->header.layout;
		if(layout)
		{
			auto it = layout->descriptorMetaData.find(descriptorSet);
			if(it != layout->descriptorMetaData.end())
			{
				DescriptorSetMetaData metadata = it->second;
				for(auto bindingData : metadata)
				{
...
void DescriptorSetLayout::Unregister(DescriptorSet* descriptorSet)
{
	if(descriptorSet)
	{
		DescriptorSetLayout* layout = descriptorSet->header.layout;
		if(layout)
		{
			...
			if(it != layout->descriptorMetaData.end())
			{
				layout->descriptorMetaData.erase(it);
			}
		}
	}
}
...
void DescriptorSetLayout::WriteDescriptorSet(Device *device, DescriptorSet *dstSet, VkDescriptorUpdateTemplateEntry const &entry, char const *src)
{
	...
	DescriptorSetMetaData& metaData = dstLayout->getOrCreateDescriptorSetMetaData(dstSet);
	bool readOnlyInShader = IsReadOnlyInShader(entry.descriptorType);
	...
	else if(entry.descriptorType == VK_DESCRIPTOR_TYPE_UNIFORM_TEXEL_BUFFER)
	{
		...
		for(uint32_t i = 0; i < entry.descriptorCount; i++)
		{
			...
			metaData[bindingNumber][arrayElement] = { bufferView, readOnlyInShader };
			dstLayout->increment(bindingNumber, arrayElement);
		}
	}
	        ...
	        entry.descriptorType == VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE)
	{
		...
		for(uint32_t i = 0; i < entry.descriptorCount; i++)
		{
			...
			if(format.isYcbcrFormat())
			{
				...
				texture->mipmap[0].buffer = imageView->getOffsetPointer(offset, VK_IMAGE_ASPECT_PLANE_0_BIT, level, 0, false, ImageView::SAMPLING);
				texture->mipmap[1].buffer = imageView->getOffsetPointer(offset, VK_IMAGE_ASPECT_PLANE_1_BIT, level, 0, false, ImageView::SAMPLING);
				if(format.getAspects() & VK_IMAGE_ASPECT_PLANE_2_BIT)
				{
					texture->mipmap[2].buffer = imageView->getOffsetPointer(offset, VK_IMAGE_ASPECT_PLANE_2_BIT, level, 0, false, ImageView::SAMPLING);
				}
			...
			else
			{
				for(int mipmapLevel = 0; mipmapLevel < sw::MIPMAP_LEVELS; mipmapLevel++)
				{
					   ...
					   (imageView->getType() == VK_IMAGE_VIEW_TYPE_CUBE_ARRAY))
					{
						...
						mipmap.buffer = imageView->getOffsetPointer(offset, aspect, level, 0, false, ImageView::SAMPLING);
					}
					else
					{
						VkOffset3D offset = { 0, 0, 0 };
			...
			metaData[bindingNumber][arrayElement] = { imageView, readOnlyInShader };
			dstLayout->increment(bindingNumber, arrayElement);
		}
	}
	        ...
	        entry.descriptorType == VK_DESCRIPTOR_TYPE_INPUT_ATTACHMENT)
	{
		...
		for(uint32_t i = 0; i < entry.descriptorCount; i++)
		{
			...
			descriptor[i].ptr = imageView->getOffsetPointer({ 0, 0, 0 }, VK_IMAGE_ASPECT_COLOR_BIT, 0, 0, false);
			...
			{
				descriptor[i].stencilPtr = imageView->getOffsetPointer({ 0, 0, 0 }, VK_IMAGE_ASPECT_STENCIL_BIT, 0, 0, false);
	        ...
	        entry.descriptorType == VK_DESCRIPTOR_TYPE_STORAGE_BUFFER_DYNAMIC)
	{
		...
		for(uint32_t i = 0; i < entry.descriptorCount; i++)
		{
			...
			metaData[bindingNumber][arrayElement] = { buffer, readOnlyInShader };
			dstLayout->increment(bindingNumber, arrayElement);
		}
	}
}
...
void DescriptorSetLayout::CopyDescriptorSet(const VkCopyDescriptorSet &descriptorCopies)
{
	...
	DescriptorSetMetaData& srcMetaData = srcLayout->getOrCreateDescriptorSetMetaData(srcSet);
	DescriptorSetMetaData& dstMetaData = dstLayout->getOrCreateDescriptorSetMetaData(dstSet);
	for(uint32_t i = 0; i < descriptorCopies.descriptorCount; ++i)
	{
		dstMetaData[dstBinding][dstArrayElement] = srcMetaData[srcBinding][srcArrayElement];
		srcLayout->increment(srcBinding, srcArrayElement);
		dstLayout->increment(dstBinding, dstArrayElement);
	}
}

DescriptorSetLayout::DescriptorSetMetaData& DescriptorSetLayout::getOrCreateDescriptorSetMetaData(DescriptorSet *set)
{
	DescriptorSetLayout *layout = set->header.layout;
	ASSERT(layout == this); // Descriptor sets shouldn't change layouts

	auto it = descriptorMetaData.find(set);
	if(it != descriptorMetaData.end())
	{
		return it->second;
	}

	auto& metadata = descriptorMetaData[set];
	metadata.resize(bindingsArraySize);
	for(uint32_t i = 0; i < bindingsArraySize; i++)
	{
		metadata[i].resize(bindings[i].descriptorCount);
		for(uint32_t j = 0; j < bindings[i].descriptorCount; j++)
		{
			metadata[i][j] = { nullptr, 0 };
		}
	}
