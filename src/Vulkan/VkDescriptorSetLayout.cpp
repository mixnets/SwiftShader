...
void DescriptorSetLayout::WriteDescriptorSet(Device *device, DescriptorSet *dstSet, VkDescriptorUpdateTemplateEntry const &entry, char const *src)
{
	...
	else if(entry.descriptorType == VK_DESCRIPTOR_TYPE_INLINE_UNIFORM_BLOCK_EXT)
	{
		...
		auto update = reinterpret_cast<const VkWriteDescriptorSetInlineUniformBlockEXT *>(src + entry.offset);
		ASSERT(update->dataSize <= MAX_INLINE_UNIFORM_BLOCK_SIZE);
		uint32_t size = sw::min(update->dataSize, MAX_INLINE_UNIFORM_BLOCK_SIZE);
		memcpy(&(inlineUniformBlockDescriptor->data[0]), update->pData, size);
