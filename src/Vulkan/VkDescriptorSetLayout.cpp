...
void DescriptorSetLayout::WriteDescriptorSet(Device *device, DescriptorSet *dstSet, VkDescriptorUpdateTemplateEntry const &entry, char const *src)
{
	...
	if(entry.descriptorType == VK_DESCRIPTOR_TYPE_SAMPLER)
	{
		...
		for(uint32_t i = 0; i < entry.descriptorCount; i++)
		{
			...
			dstSet->header.storeMemoryOwner(bindingNumber, arrayElement, nullptr, readOnlyResource);
	        ...
	        entry.descriptorType == VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE)
	{
		...
		for(uint32_t i = 0; i < entry.descriptorCount; i++)
		{
			...
			dstSet->header.storeMemoryOwner(bindingNumber, arrayElement, imageView, readOnlyResource);
			dstLayout->increment(bindingNumber, arrayElement);
		}
	}
...
void DescriptorSetLayout::CopyDescriptorSet(const VkCopyDescriptorSet &descriptorCopies)
{
	...
	srcSet->header.copyTo(descriptorCopies.srcBinding, descriptorCopies.srcArrayElement, dstSet->header, descriptorCopies.dstBinding, descriptorCopies.dstArrayElement, descriptorCopies.descriptorCount);
}
