	         ...
	         entry.descriptorType == VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE)
	{
		...
		for(uint32_t i = 0; i < entry.descriptorCount; i++)
		{
			...
			if(isYCbCr)
			{
				...
				WriteTextureLevelInfo(texture, 0, width / 2, height / 2, 1, pitchP1, 0);
			}
			else
			{
				for(int mipmapLevel = 0; mipmapLevel < sw::MIPMAP_LEVELS; mipmapLevel++)
				{
					...
					if(imageView->getType() == VK_IMAGE_VIEW_TYPE_CUBE)
					{
						for(int face = 0; face < 6; face++)
						{
							...
							texture->mipmap[level].buffer[face] = imageView->getOffsetPointer(offset, aspect, level, face, ImageView::SAMPLING);
						}
					}
					else
					{
						VkOffset3D offset = {0, 0, 0};
						texture->mipmap[level].buffer[0] = imageView->getOffsetPointer(offset, aspect, level, 0, ImageView::SAMPLING);
					}
					...
					WriteTextureLevelInfo(texture, level, width, height, depth, pitchP, sliceP);
				}
			}
		}
	}
