	...
	for(uint32_t i = 0; i < submitCount; i++)
	{
		    ...
		    extension != nullptr; extension = reinterpret_cast<const VkBaseInStructure *>(extension->pNext))
		{
			switch(extension->sType)
			{
				case VK_STRUCTURE_TYPE_TIMELINE_SEMAPHORE_SUBMIT_INFO:
				{
					// Deep copy the timeline info, including the wait and signal semaphore lists
...
void Queue::submitQueue(const Task &task)
{
	...
	for(uint32_t i = 0; i < task.submitCount; i++)
	{
		...
		{
			if(auto *sem = vk::DynamicCast<BinarySemaphore>(submitInfo.pWaitSemaphores[j]))
			{
				sem->wait(submitInfo.pWaitDstStageMask[j]);
			}
			else if(auto *sem = vk::DynamicCast<TimelineSemaphore>(submitInfo.pWaitSemaphores[j]))
			{
		...
		{
			if(auto *sem = vk::DynamicCast<BinarySemaphore>(submitInfo.pSignalSemaphores[j]))
			{
				sem->signal();
			}
			else if(auto *sem = vk::DynamicCast<TimelineSemaphore>(submitInfo.pSignalSemaphores[j]))
			{
				sem->signal(timelineInfo->pWaitSemaphoreValues[j]);
			}
			else
			{
				UNSUPPORTED("Unknown semaphore type");
