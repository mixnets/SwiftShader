...
void Queue::submitQueue(const Task &task)
{
	...
	for(uint32_t i = 0; i < task.submitCount; i++)
	{
		...
		for(uint32_t j = 0; j < submitInfo.waitSemaphoreCount; j++)
		{
			...
			{
				vk::Cast(submitInfo.pWaitSemaphores[j])->as<TimelineSemaphore>()->wait(semaphoreInfo->pWaitSemaphoreValues[j]);
			}
			else
			{
				vk::Cast(submitInfo.pWaitSemaphores[j])->as<BinarySemaphore>()->wait(submitInfo.pWaitDstStageMask[j]);
			}
		}
		...
		for(uint32_t j = 0; j < submitInfo.signalSemaphoreCount; j++)
		{
			...
			{
				vk::Cast(submitInfo.pSignalSemaphores[j])->as<TimelineSemaphore>()->signal(semaphoreInfo->pSignalSemaphoreValues[j]);
			}
			else
			{
				vk::Cast(submitInfo.pSignalSemaphores[j])->as<BinarySemaphore>()->signal();
			}
		}
	}
...
VkResult Queue::present(const VkPresentInfoKHR *presentInfo)
{
	...
	{
		vk::Cast(presentInfo->pWaitSemaphores[i])->as<BinarySemaphore>()->wait();
	}
