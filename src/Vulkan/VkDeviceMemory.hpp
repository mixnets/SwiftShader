...
{
public:
	DeviceMemory(const VkMemoryAllocateInfo* pAllocateInfo) :
		size(pAllocateInfo->allocationSize)
	{
		// buffer is "GPU memory", so we don't use the allocation callbacks for it
		buffer = vk::allocate(size, INTERNAL_MEMORY, GetAllocationScope());
	}
