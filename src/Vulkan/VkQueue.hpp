...
{
public:
	static constexpr size_t Unbounded = 0;

	Chan(size_t maxSize = Unbounded);
...
template <typename T>
Chan<T>::Chan(size_t maxSize /* = Unbounded */) : maxSize(maxSize) {}
...
{
	std::unique_lock<std::mutex> lock(mutex);
	if (maxSize != Unbounded && queue.size() >= maxSize)
	{
		// Chan full. Wait for item to be removed.
		removed.wait(lock, [this] { return queue.size() < maxSize; });
	}
	queue.push(item);
	lock.unlock();
	added.notify_one();
...
private:
	...
	std::mutex submitLock; // Used to prevent submissions during waitIdle().
};
