...
// Context requires locking before accessing any state. The lock is non-rentrant
// and careful use is required to prevent accidentical double-locking by the
// same thread.
class Context
{
	class Impl;
...
public:
	...
	public:
		...
		// createFrame() returns a new scope for the given file.
		...
		// the given name which has not get been created with a call to
