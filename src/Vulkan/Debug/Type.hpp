...
public:
	...
	inline Type(Kind kind, const std::shared_ptr<Type>& elem);

	// string() returns a string representation of the type.
	std::string string() const;

	const Kind kind;                   // Type kind.
	const std::shared_ptr<Type> elem;  // Element type if pointer.
};

Type::Type(Kind kind) :
    kind(kind) {}

Type::Type(Kind kind, const std::shared_ptr<Type>& elem) :
    kind(kind),
    elem(elem) {}
