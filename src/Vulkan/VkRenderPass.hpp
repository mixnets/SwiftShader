...
class RenderPass : public VkObject<RenderPass, VkRenderPass>
{
public:
	...
	{
		size_t attachmentSize = attachmentCount * sizeof(VkAttachmentDescription);
		attachments = reinterpret_cast<VkAttachmentDescription*>(
			vk::allocate(attachmentSize, pAllocator, GetAllocationScope()));
		if(attachments)
		{
			memcpy(attachments, pCreateInfo->pAttachments, attachmentSize);
		}

		size_t subpassesSize = subpassCount * sizeof(VkSubpassDescription);
		subpasses = reinterpret_cast<VkSubpassDescription*>(
			vk::allocate(subpassesSize, pAllocator, GetAllocationScope()));
		if(subpasses)
		{
			memcpy(subpasses, pCreateInfo->pSubpasses, subpassesSize);
		}

		size_t dependenciesSize = dependencyCount * sizeof(VkSubpassDependency);
		dependencies = reinterpret_cast<VkSubpassDependency*>(
			vk::allocate(dependenciesSize, pAllocator, GetAllocationScope()));
		if(dependencies)
		{
