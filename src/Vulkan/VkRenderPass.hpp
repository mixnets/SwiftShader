...
{
public:
	RenderPass(const VkRenderPassCreateInfo* pCreateInfo, const Memory& mem);
	~RenderPass() = delete;
	void destroy(const VkAllocationCallbacks* pAllocator) override;

	static MemorySize ComputeRequiredAllocationSize(const VkRenderPassCreateInfo* pCreateInfo);
