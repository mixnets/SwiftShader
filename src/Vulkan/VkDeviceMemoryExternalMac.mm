...
IOSurfaceExternalMemory::IOSurfaceExternalMemory(const VkMemoryAllocateInfo* pAllocateInfo)
{
	...
	{
		const VkIOSurfaceMemoryInfo* ioSurfaceMemoryInfoPtr = reinterpret_cast<const VkIOSurfaceMemoryInfo*>(pAllocateInfo->pNext);
		if (ioSurfaceMemoryInfoPtr->sType == VK_STRUCTURE_TYPE_IOSURFACE_MEMORY_INFO)
		{
			ioSurfaceMemoryInfo = *ioSurfaceMemoryInfoPtr;
		}
	}
}
...
{
	// ANGLE handles deallocation
}

VkResult IOSurfaceExternalMemory::allocate(size_t size, void** pBuffer)
{
	if (!ioSurfaceMemoryInfo.pIOSurfaceRef)
	{
		return VK_ERROR_OUT_OF_DEVICE_MEMORY;
	}
	*pBuffer = ioSurfaceMemoryInfo.pIOSurfaceRef;
	return VK_SUCCESS;
}
