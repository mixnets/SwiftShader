...
class DescriptorPool : public VkObject<DescriptorPool, VkDescriptorPool>
{
public:
	struct Pool
	{
	...
	DescriptorPool(const VkDescriptorPoolCreateInfo* pCreateInfo, Pool* pPools, DescriptorSet* pSets);
	~DescriptorPool() = delete;
	void destroy(const VkAllocationCallbacks* pAllocator) override;

	static VkResult AllocatePoolsAndSets(const VkAllocationCallbacks* pAllocator, const VkDescriptorPoolCreateInfo* pCreateInfo,
	                                     Pool** pPools, DescriptorSet** pSets);
	static void DestroyPoolsAndSets(const VkAllocationCallbacks* pAllocator, Pool* pPools, DescriptorSet* pSets);
