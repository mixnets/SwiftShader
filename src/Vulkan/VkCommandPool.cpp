...
void CommandPool::destroy(const VkAllocationCallbacks *pAllocator)
{
	...
	{
		vk::destroy(commandBuffer, pAllocator);
	}
}
...
VkResult CommandPool::allocateCommandBuffers(Device *device, VkCommandBufferLevel level, uint32_t commandBufferCount, VkCommandBuffer *pCommandBuffers)
{
	for(uint32_t i = 0; i < commandBufferCount; i++)
	{
		...
		void *objectMemory = vk::allocate(sizeof(DispatchableCommandBuffer), REQUIRED_MEMORY_ALIGNMENT);
		ASSERT(objectMemory);
		DispatchableCommandBuffer *commandBuffer = new(objectMemory) DispatchableCommandBuffer(device, level);
