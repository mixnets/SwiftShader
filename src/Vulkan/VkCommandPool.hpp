...
VkClass(CommandPool) {
public:
	CommandPool(VkCommandPoolCreateFlags pFlags, uint32_t pQueueFamilyIndex) :
		flags(pFlags), queueFamilyIndex(pQueueFamilyIndex)
	{
	}
	...
	void allocateCommandBuffers(VkCommandBufferLevel level, uint32_t commandBufferCount, VkCommandBuffer* pCommandBuffers)
	{
		...
		{
			pCommandBuffers[i] = Cast(new (nullptr) CommandBuffer(level));
		}
	}
