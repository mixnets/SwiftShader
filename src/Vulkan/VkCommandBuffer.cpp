...
struct BindDescriptorSet : public CommandBuffer::Command
{
	...
	void play(CommandBuffer::ExecutionState& executionState)
	{
		...
		binding.dynamicOffsets = dynamicOffsets.get() != nullptr ? &dynamicOffsets.get()[0] : nullptr;
	...
	uint32_t dynamicOffsetCount, const uint32_t* pDynamicOffsets)
{
	...
	for(uint32_t i = 0; i < descriptorSetCount; i++)
	{
		...
		// pre-allocated buffer large enough to fit all dynamic offsets
		...
		// various sizes.
		auto dynamicOffsets = std::shared_ptr<uint32_t>(
			new uint32_t[numDynamicBindings],
			[](uint32_t* ptr) { delete [] ptr; });
		for (size_t i = 0; i < numDynamicBindings; i++)
		{
			dynamicOffsets.get()[i] = pDynamicOffsets[i];
		}
