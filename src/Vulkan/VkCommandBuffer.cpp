...
public:
	...
	virtual void play(CommandBuffer::RenderingState& renderingState) = 0;
	virtual ~Command() {}
};
...
protected:
	void play(CommandBuffer::RenderingState& renderingState)
	{
...
protected:
	void play(CommandBuffer::RenderingState& renderingState)
	{
		Cast(renderingState.renderpass)->end();
	}
...
protected:
	void play(CommandBuffer::RenderingState& renderingState)
	{
		renderingState.pipelines[pipelineBindPoint] = pipeline;
	}
...
struct VertexBufferBind : public CommandBuffer::Command
{
	...
	void play(CommandBuffer::RenderingState& renderingState)
	{
		renderingState.vertexInputBindings[binding] = { buffer, offset };
	}
...
struct Draw : public CommandBuffer::Command
{
	...
	void play(CommandBuffer::RenderingState& renderingState)
	{
		GraphicsPipeline* pipeline = static_cast<GraphicsPipeline*>(
			Cast(renderingState.pipelines[VK_PIPELINE_BIND_POINT_GRAPHICS]));

		sw::Context context = pipeline->getContext();
		for(uint32_t i = 0; i < MAX_VERTEX_INPUT_BINDINGS; i++)
		{
			const auto& vertexInput = renderingState.vertexInputBindings[i];
			Buffer* buffer = Cast(renderingState.vertexInputBindings[i].buffer);
...
void CommandBuffer::submit(CommandBuffer::RenderingState& renderingState)
{
