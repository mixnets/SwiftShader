...
public:
	          ...
	          uint32_t count, uint32_t instanceCount, uint32_t first, int32_t vertexOffset, uint32_t firstInstance)
	{
		...
		{
			// TODO(b/139862810): Reconsider instances/views nesting.
...
private:
	vk::Buffer *const dstBuffer;
	const VkDeviceSize dstOffset;
	const std::vector<uint8_t> data;  // TODO(b/119409619): Replace this vector by an allocator so we can control all memory allocations.
};
...
public:
	...
	void execute(vk::CommandBuffer::ExecutionState &executionState) override
	{
		if(stage & ~(VK_PIPELINE_STAGE_2_TOP_OF_PIPE_BIT | VK_PIPELINE_STAGE_2_DRAW_INDIRECT_BIT))
		{
			...
			// `bottom of pipe`.
			//
			// TODO(b/197691918): Do this in a task so we don't have to stall here.
			if(executionState.renderer_) executionState.renderer_->synchronize();
		}
...
{
	// TODO(b/119409619): Replace this vector by an allocator so we can control all memory allocations.
	commands.clear();
