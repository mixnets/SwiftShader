...
struct WriteTimeStamp : public CommandBuffer::Command
{
	...
	void play(CommandBuffer::ExecutionState& executionState)
	{
		...
		{
			// The `top of pipe` and `draw indirect` stages are handled in command processing so a timestamp write
			// done in those stages can just be done directly in the command buffer processing without any additional
			// synchronization. Everything else is deferred to the Renderer; we will treat those stages all as if they were
			// `bottom of pipe`.
			//
