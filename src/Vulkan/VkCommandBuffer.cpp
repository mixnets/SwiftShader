...
public:
	          ...
	          uint32_t count, uint32_t instanceCount, uint32_t first, int32_t vertexOffset, uint32_t firstInstance)
	{
		...
		VkExtent3D extent = {};
		VkOffset2D offset = {};
		executionState.getRenderArea(&extent, &offset);
			...
			while(viewMask)
			{
				...
				for(auto indexBuffer : indexBuffers)
				{
					executionState.renderer->draw(pipeline, executionState.dynamicState, indexBuffer.first, vertexOffset,
					                              ...
					                              extent, offset, executionState.pushConstants);
				}
			}
...
void CommandBuffer::ExecutionState::getRenderArea(VkExtent3D *extent, VkOffset2D *offset) const
{
	...
	else if(dynamicRendering)
	{
		...
		*extent = { renderArea.extent.width, renderArea.extent.height, dynamicRendering->getLayerCount() };
