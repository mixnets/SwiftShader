...
public:
	...
	void execute(vk::CommandBuffer::ExecutionState &executionState) override
	{
		...
		VkAttachmentLoadOp stencilLoadOp =
		    stencilAttachment ? stencilAttachment->loadOp : VK_ATTACHMENT_LOAD_OP_NONE_EXT;
		for(uint32_t i = 0; i < dynamicRendering.getColorAttachmentCount(); i++)
		{
			...
			if(imageView)
			{
				VkImageAspectFlags clearMask =
				    vk::Framebuffer::GetClearMask(colorAttachment->loadOp, stencilLoadOp, imageView->getFormat());
				...
				imageView->clearWithLayerMask(colorAttachment->clearValue, clearMask,
				                              dynamicRendering.getRenderArea(),
				                              dynamicRendering.getViewMask());
			}
		}
	}
	...
	void execute(vk::CommandBuffer::ExecutionState &executionState) override
	{
		...
		else if(executionState.dynamicRendering)
		{
			if(attachment.aspectMask & VK_IMAGE_ASPECT_COLOR_BIT)
			{
				...
				vk::Cast(colorAttachment->imageView)->clear(attachment.clearValue, VK_IMAGE_ASPECT_COLOR_BIT, rect);
			}
			...
			if(attachment.aspectMask & VK_IMAGE_ASPECT_DEPTH_BIT)
			{
				...
				vk::Cast(depthAttachment->imageView)->clear(attachment.clearValue, VK_IMAGE_ASPECT_DEPTH_BIT, rect);
			}
			...
			if(attachment.aspectMask & VK_IMAGE_ASPECT_STENCIL_BIT)
			{
				...
				vk::Cast(stencilAttachment->imageView)->clear(attachment.clearValue, VK_IMAGE_ASPECT_STENCIL_BIT, rect);
			}
		}
	}
...
const GraphicsState::DynamicRendering &CommandBuffer::ExecutionState::getDynamicRenderingState() const
{
	auto *pipeline = static_cast<vk::GraphicsPipeline *>(pipelineState[VK_PIPELINE_BIND_POINT_GRAPHICS].pipeline);
	return pipeline->getState(dynamicState).getDynamicRendering();
}
