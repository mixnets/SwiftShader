...
public:
	          ...
	          uint32_t count, uint32_t instanceCount, uint32_t first, int32_t vertexOffset, uint32_t firstInstance)
	{
		...
		bool hadDynamicVertexStride = pipeline->hasDynamicVertexStride();
		...
		inputs.bindVertexInputs(firstInstance, hadDynamicVertexStride);
		...
		for(uint32_t instance = firstInstance; instance != firstInstance + instanceCount; instance++)
		{
			...
			inputs.advanceInstanceAttributes(hadDynamicVertexStride);
		}
	}
};
