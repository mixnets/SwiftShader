...
struct ClearAttachment : public CommandBuffer::Command
{
	...
	void play(CommandBuffer::ExecutionState& executionState) override
	{
		...
		if (executionState.renderPass->isMultiView())
		{
			// if the renderpass is multiview, then the passed in clear rect describes only layer zero.
			// we must convert that into individual clears for all layers corresponding to views
			// enabled in the current subpass's view mask.
			VkClearRect r = rect;
			auto viewMask = executionState.renderPass->getViewMask();
			while (viewMask)
			{
				r.baseArrayLayer = sw::log2i(viewMask);
				viewMask &= ~(1 << r.baseArrayLayer);
				executionState.renderPassFramebuffer->clear(executionState.renderPass, attachment, r);
			}
		}
		else
		{
