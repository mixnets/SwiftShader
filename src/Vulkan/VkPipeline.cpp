    ...
    bool optimize)
{
	...
	if(specializationInfo)
	{
		...
		const uint32_t *specializationData = static_cast<const uint32_t *>(specializationInfo->pData);
		...
		for(uint32_t i = 0; i < specializationInfo->mapEntryCount; i++)
		{
			...
			const uint32_t *value_ptr = specializationData + entry.offset / sizeof(uint32_t);
			specializations.emplace(entry.constantID, std::vector<uint32_t>(value_ptr, value_ptr + entry.size / sizeof(uint32_t)));
		}
...
void ComputePipeline::compileShaders(const VkAllocationCallbacks *pAllocator, const VkComputePipelineCreateInfo *pCreateInfo, PipelineCache *pPipelineCache)
{
	...
	auto shader = createShader(shaderKey, module, spirv, robustBufferAccess, dbgctx);
