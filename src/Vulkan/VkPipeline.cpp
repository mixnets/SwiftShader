...
void GraphicsPipeline::compileShaders(const VkAllocationCallbacks *pAllocator, const VkGraphicsPipelineCreateInfo *pCreateInfo, PipelineCache *pPipelineCache)
{
	...
	for(auto pStage = pCreateInfo->pStages; pStage != pCreateInfo->pStages + pCreateInfo->stageCount; pStage++)
	{
		...
		auto moduleSerialID = module->getSerialID();
		stages.emplace_back(Environment::Stage{ *pStage, std::move(key), moduleSerialID });
	}

	ASSERT(!env);
	env = std::make_unique<Environment>(Environment{ /* stages */ std::move(stages),
	                                                 /* pipelineCache */ pPipelineCache });

	marl::schedule([=] { getOrBuild(CompileOptions{}); });
}
...
GraphicsPipeline::Shaders GraphicsPipeline::getOrBuild(const CompileOptions &options)
{
	return shaders.getOrCreate(options, [&] {
		...
		for(auto &stage : env->stages)
		{
			...
			{
				shader = env->pipelineCache->getOrCreateShader(stage.key, [&] {
					return createShader(stage.key, stage.moduleSerialID, robustBufferAccess, device->getDebuggerContext());
				});
			}
			else
			{
				shader = createShader(stage.key, stage.moduleSerialID, robustBufferAccess, device->getDebuggerContext());
			}

			switch(stage.createInfo.stage)
			{
...
void ComputePipeline::compileShaders(const VkAllocationCallbacks *pAllocator, const VkComputePipelineCreateInfo *pCreateInfo, PipelineCache *pPipelineCache)
{
	...
	const ShaderModule *module = vk::Cast(pCreateInfo->stage.module);
	auto dbgCtx = device->getDebuggerContext();

	ASSERT(!env);
	env = std::make_unique<Environment>(Environment{
	    /* createInfo */ stage,
	    /* key */ PipelineCache::SpirvShaderKey(stage.stage, stage.pName, module->getCode(), nullptr, 0, stage.pSpecializationInfo),
	    /* pipelineCache */ pPipelineCache,
...
std::shared_ptr<sw::ComputeProgram> ComputePipeline::getOrBuild(const CompileOptions &options)
{
	return programs.getOrCreate(options, [&] {
		if(env->pipelineCache)
		{
			auto shader = env->pipelineCache->getOrCreateShader(env->key, [&] {
				return createShader(env->key, env->moduleSerialID, robustBufferAccess, device->getDebuggerContext());
			});

			const PipelineCache::ComputeProgramKey programKey(shader.get(), layout);
			auto program = env->pipelineCache->getOrCreateComputeProgram(programKey, [&] {
		...
		{
			auto shader = createShader(env->key, env->moduleSerialID, robustBufferAccess, device->getDebuggerContext());
			const PipelineCache::ComputeProgramKey programKey(shader.get(), layout);
			auto program = createProgram(std::move(shader), layout);
