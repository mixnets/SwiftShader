    ...
    , state(device, pCreateInfo, layout, robustBufferAccess)
{
	...
	for(uint32_t i = 0; i < vertexInputState->vertexAttributeDescriptionCount; i++)
	{
		...
		sw::Stream &input = inputs.stream[desc.location];
...
void GraphicsPipeline::getIndexBuffers(uint32_t count, uint32_t first, bool indexed, std::vector<std::pair<uint32_t, void *>> *indexBuffers)
{
