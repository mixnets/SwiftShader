		...
		VkSpecializationInfo const *specializationInfo)
{
	...
	opt.SetMessageConsumer([](spv_message_level_t level, const char*, const spv_position_t& p, const char* m) {
		switch (level)
		{
		case SPV_MSG_FATAL:          category = vk::warn("SPIR-V FATAL: %d:%d %s", int(p.line), int(p.column), m);
		case SPV_MSG_INTERNAL_ERROR: category = vk::warn("SPIR-V INTERNAL_ERROR: %d:%d %s", int(p.line), int(p.column), m);
		case SPV_MSG_ERROR:          category = vk::warn("SPIR-V ERROR: %d:%d %s", int(p.line), int(p.column), m);
		case SPV_MSG_WARNING:        category = vk::warn("SPIR-V WARNING: %d:%d %s", int(p.line), int(p.column), m);
		case SPV_MSG_INFO:           category = vk::trace("SPIR-V INFO: %d:%d %s", int(p.line), int(p.column), m);
		case SPV_MSG_DEBUG:          category = vk::trace("SPIR-V DEBUG: %d:%d %s", int(p.line), int(p.column), m);
		default:                     category = vk::trace("SPIR-V MESSAGE: %d:%d %s", int(p.line), int(p.column), m);
		}
	});
