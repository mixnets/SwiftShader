...
// Find a struct in the pNext chain.
template<typename T>
const T *getChained(const void *pNext, VkStructureType sType)
{
	const VkBaseInStructure *extensionCreateInfo = reinterpret_cast<const VkBaseInStructure *>(pNext);
	while(extensionCreateInfo)
	{
		if(extensionCreateInfo->sType == sType)
		{
			return reinterpret_cast<const T *>(extensionCreateInfo);
		}
...
private:
	...
	{
		return getChained<VkPipelineCreationFeedbackCreateInfo>(pNext, VK_STRUCTURE_TYPE_PIPELINE_CREATION_FEEDBACK_CREATE_INFO);
	}
...
const VkPipelineRobustnessCreateInfoEXT *getPipelineRobustnessCreateInfo(const void *pNext)
{
	return getChained<VkPipelineRobustnessCreateInfoEXT>(pNext, VK_STRUCTURE_TYPE_PIPELINE_ROBUSTNESS_CREATE_INFO_EXT);
}
...
{
	const VkPipelineRobustnessCreateInfoEXT *overrideRobustness = getPipelineRobustnessCreateInfo(pNext);
