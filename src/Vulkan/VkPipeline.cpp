...
void GraphicsPipeline::getIndexBuffers(uint32_t count, uint32_t first, bool indexed, const VertexInputBinding &binding, VkIndexType type, std::vector<std::pair<uint32_t, void *>> *indexBuffers)
{
	indexBuffer.getIndexBuffers(state.getTopology(), count, first, indexed, binding, type, state.hasPrimitiveRestartEnable(), indexBuffers);
}
