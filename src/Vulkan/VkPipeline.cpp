	...
	: Pipeline(vk::Cast(pCreateInfo->layout), device)
{
	...
	if(rasterizationState->pNext)
	{
		...
		while(extensionCreateInfo)
		{
			...
			{
			case VK_STRUCTURE_TYPE_PIPELINE_PROVOKING_VERTEX_MODE_CREATE_INFO_EXT:
			{
				const VkPipelineProvokingVertexModeCreateInfoEXT* provokingVertexModeCreateInfo =
					reinterpret_cast<const VkPipelineProvokingVertexModeCreateInfoEXT*>(extensionCreateInfo);
