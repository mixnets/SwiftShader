...
Framebuffer::Framebuffer(const VkFramebufferCreateInfo* pCreateInfo, char* membersMemory) :
	renderPass(pCreateInfo->renderPass), attachmentCount(pCreateInfo->attachmentCount),
	attachments(reinterpret_cast<VkImageView*>(membersMemory)), width(pCreateInfo->width),
