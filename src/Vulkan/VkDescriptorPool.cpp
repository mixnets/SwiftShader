	...
	// Attempt to allocate single chunk of memory
	{
		...
		if(memory)
		{
			for(uint32_t i = 0; i < numAllocs; i++)
			{
				...
				vk::Cast(pDescriptorSets[i])->header.layout = nullptr;
...
{
	vk::DescriptorSetLayout::Unregister(vk::Cast(descriptorSet));
...
VkResult DescriptorPool::reset()
{
	for(auto node : nodes)
	{
		vk::DescriptorSetLayout::Unregister(vk::Cast(asDescriptorSet(node.set)));
	}
