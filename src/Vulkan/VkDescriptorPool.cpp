...
DescriptorPool::DescriptorPool(const VkDescriptorPoolCreateInfo* pCreateInfo, const Memory& mem) :
...
MemorySize DescriptorPool::ComputeRequiredAllocationSize(const VkDescriptorPoolCreateInfo* pCreateInfo)
{
	size_t poolsSize = sizeof(Pool) * pCreateInfo->poolSizeCount;
	size_t setsSize = sizeof(DescriptorSet) * pCreateInfo->maxSets;
	size_t usedSetsSize = sizeof(bool) * pCreateInfo->maxSets;
	return MemorySize(poolsSize + setsSize + usedSetsSize, 0);
}
