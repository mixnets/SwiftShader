...
{
	marl::lock lock(mutex);
	auto it = descriptorSetMetaData.find(descriptorSet);
	if(it != descriptorSetMetaData.end())
	{
...
void DescriptorPool::storeMetaData(DescriptorSet *descriptorSet, uint32_t bindingNumber, uint32_t arrayElement, ImageView *imageView, bool readOnly)
{
	...
	if(it == descriptorSetMetaData.end())
	{
		descriptorSetMetaData.insert({ descriptorSet, { descriptorSet->header.layout } });
		it = descriptorSetMetaData.find(descriptorSet);
		ASSERT(it != descriptorSetMetaData.end());
	}
...
void DescriptorPool::copyMetadata(DescriptorSet *srcSet, uint32_t srcBinding, uint32_t srcArrayElement, DescriptorSet *dstSet, uint32_t dstBinding, uint32_t dstArrayElement, uint32_t descriptorCount)
{
	...
	auto itSrc = descriptorSetMetaData.find(srcSet);
	if(itSrc == descriptorSetMetaData.end())
	{
		descriptorSetMetaData.insert({ srcSet, { srcSet->header.layout } });
		itSrc = descriptorSetMetaData.find(srcSet);
		ASSERT(itSrc != descriptorSetMetaData.end());
	}
	...
	if(dstPool)
	{
		if(srcPool == dstPool)
		{
			...
			auto itDst = descriptorSetMetaData.find(dstSet);
			if(itDst == descriptorSetMetaData.end())
			{
				descriptorSetMetaData.insert({ dstSet, { dstSet->header.layout } });
				itDst = descriptorSetMetaData.find(dstSet);
				ASSERT(itDst != descriptorSetMetaData.end());
			}
		...
		else
		{
			...
			// Ensure destination metadata is initialized
			auto itDst = dstPool->descriptorSetMetaData.find(dstSet);
			if(itDst == dstPool->descriptorSetMetaData.end())
			{
				dstPool->descriptorSetMetaData.insert({ dstSet, { dstSet->header.layout } });
				itDst = dstPool->descriptorSetMetaData.find(dstSet);
				ASSERT(itDst != dstPool->descriptorSetMetaData.end());
			}
...
void DescriptorPool::freeMetaData(DescriptorSet *descriptorSet)
{
	...
	if(it != descriptorSetMetaData.end())
	{
		descriptorSetMetaData.erase(it);
	}
}
