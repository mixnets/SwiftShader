...
void DescriptorPool::storeMetaData(DescriptorSet *descriptorSet, uint32_t bindingNumber, uint32_t arrayElement, ImageView *imageView, bool readOnly)
{
	...
	if(it == descriptorSetMemoryOwners.end())
	{
		descriptorSetMemoryOwners.insert({ descriptorSet, { descriptorSet->header.layout } });
		it = descriptorSetMemoryOwners.find(descriptorSet);
		ASSERT(it != descriptorSetMemoryOwners.end());
	}
...
void DescriptorPool::copyMetadata(DescriptorSet *srcSet, uint32_t srcBinding, uint32_t srcArrayElement, DescriptorSet *dstSet, uint32_t dstBinding, uint32_t dstArrayElement, uint32_t descriptorCount)
{
	...
	{
		descriptorSetMemoryOwners.insert({ srcSet, { srcSet->header.layout } });
		itSrc = descriptorSetMemoryOwners.find(srcSet);
		ASSERT(itSrc != descriptorSetMemoryOwners.end());
	}
	...
	if(dstPool)
	{
		if(srcPool == dstPool)
		{
			...
			{
				descriptorSetMemoryOwners.insert({ dstSet, { dstSet->header.layout } });
				itDst = descriptorSetMemoryOwners.find(dstSet);
				ASSERT(itDst != descriptorSetMemoryOwners.end());
			}
		...
		else
		{
			...
			{
				dstPool->descriptorSetMemoryOwners.insert({ dstSet, { dstSet->header.layout } });
				itDst = dstPool->descriptorSetMemoryOwners.find(dstSet);
				ASSERT(itDst != dstPool->descriptorSetMemoryOwners.end());
			}
