...
VkResult DescriptorPool::allocate(uint32_t descriptorSetCount, const VkDescriptorSetLayout* pSetLayouts, VkDescriptorSet* pDescriptorSets)
{
	...
	return allocate(&(layoutSizes[0]), descriptorSetCount, pDescriptorSets);
}
...
VkResult DescriptorPool::allocate(size_t* sizes, uint32_t numAllocs, VkDescriptorSet* pDescriptorSets)
{
	size_t totalSize = 0;
...
void DescriptorPool::free(const VkDescriptorSet descriptorSet)
{
	const auto itEnd = nodes.end();
