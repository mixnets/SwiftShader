...
{
	marl::lock lock(shared->mutex);
	shared->cv.wait(lock, [&]() REQUIRES(shared->mutex) { return shared->counter == value; });
}
...
{
	marl::lock lock(shared->mutex);
	auto mapPos = shared->waitMap.find(id);
	ASSERT(mapPos == shared->waitMap.end());

	shared->waitMap.insert(mapPos, std::pair<int, uint64_t>(id, waitValue));
}
