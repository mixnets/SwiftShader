...
std::function<bool()> TimelineSemaphore::Shared::pred(uint64_t value)
{
	return [&]() REQUIRES(mutex) { return counter == value; };
}
...
void TimelineSemaphore::Shared::addDependency(int id, uint64_t waitValue)
{
	...
	waitMap.insert(mapPos, std::pair<int, uint64_t>(id, waitValue));
}
