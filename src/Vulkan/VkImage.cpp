...
void Image::copyTo(VkImage dstImage, const VkImageCopy& pRegion)
{
	...
	if(isSingleLine) // Copy one line
	{
		...
		ASSERT((srcMem + copySize) <= lastByte());
		ASSERT((dstMem + copySize) <= dst->lastByte());
...
void Image::copy(VkBuffer buf, const VkBufferImageCopy& region, bool bufferIsSource)
{
	...
	for(uint32_t i = 0; i < region.imageSubresource.layerCount; i++)
	{
		...
		{
			ASSERT(((bufferIsSource ? dstMemory : srcMemory) + copySize) <= lastByte());
			ASSERT(((bufferIsSource ? srcMemory : dstMemory) + copySize) <= buffer->lastByte());
		...
		else if(isEntireLine) // Copy plane by plane
		{
			...
			{
				ASSERT(((bufferIsSource ? dstPlaneMemory : srcPlaneMemory) + copySize) <= lastByte());
				ASSERT(((bufferIsSource ? srcPlaneMemory : dstPlaneMemory) + copySize) <= buffer->lastByte());
...
uint8_t* Image::lastByte() const
{
	return reinterpret_cast<uint8_t*>(deviceMemory->getOffsetPointer(deviceMemory->getCommittedMemoryInBytes()));
}
