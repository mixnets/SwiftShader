...
void Image::copyTo(VkBuffer dstBuffer, const VkBufferImageCopy& pRegion)
{
	...
	Cast(dstBuffer)->copyTo(Cast(deviceMemory)->getOffsetPointer(0),
		sw::Surface::sliceB(pRegion.imageExtent.width, pRegion.imageExtent.height,
		                    getBorder(), static_cast<sw::Format>(format), false) * pRegion.imageExtent.depth,
...
void Image::getSubresourceLayout(const VkImageSubresource* pSubresource, VkSubresourceLayout* pLayout) const
{
	...
	else
	{
		...
		sw::Format swFormat = static_cast<sw::Format>(format);
		pLayout->offset = 0;
		pLayout->rowPitch = sw::Surface::pitchB(extent.width, getBorder(), swFormat, false);
		pLayout->depthPitch = sw::Surface::sliceB(extent.width, extent.height, getBorder(), swFormat, false);
...
VkDeviceSize Image::getSize() const
{
	...
	return numPixels * sw::Surface::bytes(static_cast<sw::Format>(format));
}
...
void Image::clear(const VkClearValue& pClearValues, const VkRect2D& pRenderArea, unsigned int rgbaMask)
{
	...
	sw::Format swFormat = static_cast<sw::Format>(format);
	if(sw::Surface::isSignedNonNormalizedInteger(swFormat))
	{
		clearFormat = sw::FORMAT_A32B32G32R32I;
	}
	else if(sw::Surface::isUnsignedNonNormalizedInteger(swFormat))
	{
	...
	sw::Blitter blitter;
	blitter.clear((void*)pClearValues.color.float32, clearFormat, scopedSurface.get(), dRect, rgbaMask);
}

Image::ScopedSurface::ScopedSurface(Image* image)
{
	sw::Format swFormat = static_cast<sw::Format>(image->format);
	surface = sw::Surface::create(
		image->extent.width, image->extent.height, image->extent.depth, swFormat, Cast(image->deviceMemory)->getOffsetPointer(0),
		sw::Surface::pitchB(image->extent.width, image->getBorder(), swFormat, false),
		sw::Surface::sliceB(image->extent.width, image->extent.height, image->getBorder(), swFormat, false));
}
