...
{
	return Cast(deviceMemory)->getOffsetPointer(texelOffsetBytesInStorage(offset, baseArrayLayer, flags) + getMemoryOffset(flags));
}
...
void Image::clear(const VkClearValue& clearValue, const VkRect2D& renderArea, const VkImageSubresourceRange& subresourceRange)
{
	...
	sw::Surface* surface = sw::Surface::create(extent.width, extent.height, extent.depth,
		getFormat(subresourceRange.aspectMask), Cast(deviceMemory)->getOffsetPointer(memoryOffset),
