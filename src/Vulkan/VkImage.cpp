...
void Image::markClean(const VkImageSubresource& subresource)
{
...
bool Image::isDirty(const VkImageSubresource& subresource)
{
	auto it = dirtySubresource.find(subresource);
	return (it != dirtySubresource.end());
}
...
void Image::prepareForReadAccess(const VkImageSubresourceRange &subresourceRange)
{
	...
	// FIXME: Remove "preparingForReadAccess" member if this assert is never hit
	ASSERT(!preparingForReadAccess);
