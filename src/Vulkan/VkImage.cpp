...
void Image::blit(VkImage dstImage, const VkImageBlit& region, VkFilter filter)
{
	...
	sw::Surface* srcSurface = sw::Surface::create(extent.width, extent.height, extent.depth, getFormat(srcFlags),
		deviceMemory->getOffsetPointer(memoryOffset), rowPitchBytes(srcFlags), slicePitchBytes(srcFlags));

	Image* dst = Cast(dstImage);
	sw::Surface* dstSurface = sw::Surface::create(dst->extent.width, dst->extent.height, dst->extent.depth,
		dst->getFormat(dstFlags), dst->deviceMemory->getOffsetPointer(memoryOffset),
		dst->rowPitchBytes(dstFlags), dst->slicePitchBytes(dstFlags));
