...
VkDeviceSize Image::computeNumberOfPixels(uint32_t width, uint32_t height, uint32_t depth) const
{
	...
	VkDeviceSize numPixels = samples *
		(arrayLayers > MaxImageArrayLayers) ? MaxImageArrayLayers : arrayLayers;
...
VkDeviceSize Image::getSize()
{
	...
	if(isCube)
	{
		...
		if(levels > MaxImageLevelsCube) { levels = MaxImageLevelsCube; }
	}
	else
	{
	...
	{
	case VK_IMAGE_TYPE_3D:
		if(levels > MaxImageLevels3D) { levels = MaxImageLevels3D;  }
		break;
	case VK_IMAGE_TYPE_2D:
		if(levels > MaxImageLevels2D) { levels = MaxImageLevels2D; }
		break;
	case VK_IMAGE_TYPE_1D:
		if(levels > MaxImageLevels1D) { levels = MaxImageLevels1D; }
		break;
	default:
		UNIMPLEMENTED();
	}
