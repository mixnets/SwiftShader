...
public:
	...
	ImageUpdater()
	{
		thread = std::thread([&] {
			while(true)
			{
				...
				WARN("Updating %d images", (int)tasks.size());
				if(!keep_going)
				{
					return;
				}
	...
	void add(const ImageUpdater::Task &task)
	{
		std::unique_lock<std::mutex> lock(mutex);
	...
	{
		std::unique_lock<std::mutex> lock(mutex);
		std::vector<ImageUpdater::Task> newTasks;
		for(auto item : tasks)
		{
			if(item.image != image)
			{
				newTasks.push_back(item);
			}
		}
...
private:
	...
	std::mutex mutex;
	std::vector<ImageUpdater::Task> tasks;
	std::thread thread;
};
...
const VkMemoryRequirements Image::getMemoryRequirements() const
{
	...
	memoryRequirements.alignment = 0x100;  //vk::REQUIRED_MEMORY_ALIGNMENT;
...
void Image::copyTo(Buffer *dstBuffer, const VkBufferImageCopy &region)
{
	...
	//ImageUpdater::INSTANCE.add({ this, [=] { copy(dstBuffer, region, false); } });
}

void Image::copyFrom(Buffer *srcBuffer, const VkBufferImageCopy &region)
{
	copy(srcBuffer, region, true);
	//ImageUpdater::INSTANCE.add({ this, [=] { copy(srcBuffer, region, true); } });
}
