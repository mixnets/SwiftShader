...
void Image::copyTo(VkImage dstImage, const VkImageCopy& pRegion)
{
	...
	VkExtent3D copyExtent = adjustExtent(pRegion.extent, srcAspect);
...
void Image::copy(VkBuffer buf, const VkBufferImageCopy& region, bool bufferIsSource)
{
	...
	VkExtent3D imageExtent = adjustExtent(region.imageExtent, aspect);
	VkExtent2D bufferExtent = adjustBufferExtent({ imageExtent.width, imageExtent.height }, region);
...
VkExtent3D Image::adjustExtent(const VkExtent3D& extent, VkImageAspectFlagBits aspect) const
{
...
VkOffset3D Image::adjustOffset(const VkOffset3D& offset, VkImageAspectFlagBits aspect) const
{
...
VkExtent2D Image::adjustBufferExtent(const VkExtent2D& extent, const VkBufferImageCopy& region) const
{
...
{
	VkImageAspectFlagBits aspect = static_cast<VkImageAspectFlagBits>(subresource.aspectMask);
	VkOffset3D adjustedOffset = adjustOffset(offset, aspect);
	return adjustedOffset.z * slicePitchBytes(aspect, subresource.mipLevel) +
	       ...
	       (adjustedOffset.x + (isCube() ? 1 : 0)) * format.bytesPerBlock();
}
