...
void Image::copyTo(VkBuffer dstBuffer, const VkBufferImageCopy& pRegion)
{
	...
	{
		dst->copyFrom(srcMem->getOffsetPointer(srcOffset), copySize, dstOffset);
	}
}
...
void Image::copyFrom(VkBuffer srcBuffer, const VkBufferImageCopy& pRegion)
{
	...
	for(uint32_t layer = pRegion.imageSubresource.baseArrayLayer; layer <= lastLayer; layer++, srcOffset += layerSize, dstOffset += layerSize)
	{
		src->copyTo(dstMem->getOffsetPointer(dstOffset), copySize, srcOffset);
	}
}

void* Image::getTexelPointer(const VkOffset3D& offset, uint32_t baseArrayLayer) const
{
	return Cast(deviceMemory)->getOffsetPointer(texelOffsetBytesInStorage(offset, baseArrayLayer) + memoryOffset);
}
