...
void Image::blitTo(Image *dstImage, const VkImageBlit &region, VkFilter filter)
{
	device->getBlitter()->blit(this, dstImage, region, filter);
}

void Image::copyTo(uint8_t *dst, unsigned int dstPitch)
{
	device->getBlitter()->copy(this, dst, dstPitch);
}

void Image::resolveTo(Image *dstImage, const VkImageResolve &region)
{
	VkImageBlit blitRegion;
