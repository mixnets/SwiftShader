...
{
	return layer * getLayerOffset(aspect) + getMemoryOffset(aspect, mipLevel);
}
...
VkDeviceSize Image::getLayerOffset(VkImageAspectFlagBits aspect) const
{
	if(is3DSlice())
	{
		...
		{
			layerSize += slicePitchBytes(aspect, mipLevel);
		}
