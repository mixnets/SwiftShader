...
int Image::rowPitchBytes(const VkImageAspectFlags& flags, uint32_t mipLevel) const
{
	...
	return sw::Surface::pitchB(getMipLevelExtent(mipLevel).width, getBorder(), getFormat(flags), false);
}
...
int Image::slicePitchBytes(const VkImageAspectFlags& flags, uint32_t mipLevel) const
{
	...
	return sw::Surface::sliceB(mipLevelExtent.width, mipLevelExtent.height, getBorder(), getFormat(flags), false);
}
...
void Image::clear(const VkClearColorValue& color, const VkImageSubresourceRange& subresourceRange)
{
	...
	{
		uint32_t lastLevel = (subresourceRange.levelCount > mipLevels) ? mipLevels - 1 :
