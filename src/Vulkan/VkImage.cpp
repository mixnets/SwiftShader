...
VkResult Image::prepareForExternalUseANDROID() const
{
	...
	const AHardwareBuffer_Desc ahbDesc{
		...
		.layers = 1,
		.format = static_cast<uint32_t>(backingMemory.nativeBufferInfo.format),
		.usage = static_cast<uint64_t>(backingMemory.nativeBufferInfo.usage),
		.stride = static_cast<uint32_t>(backingMemory.nativeBufferInfo.stride),
	};
	...
	if(AHardwareBuffer_lockPlanes(ahb, AHARDWAREBUFFER_USAGE_CPU_WRITE_OFTEN, -1, &ahbRect, &ahbPlanes) != 0)
	{
		return VK_ERROR_OUT_OF_DATE_KHR;
	}
