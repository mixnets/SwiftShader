...
VkClass(Device) {
public:
	constexpr static VkSystemAllocationScope kAllocationScope = VK_SYSTEM_ALLOCATION_SCOPE_DEVICE;
		...
		: physicalDevice(Cast(pPhysicalDevice))
	{
		...
		queues = reinterpret_cast<VkQueue*>(
			vk::allocate(sizeof(Queue*) * queueCount, pAllocator, Queue::kAllocationScope));
