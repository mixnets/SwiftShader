...
std::string Stringify(VkStructureType value)
{
	...
	switch(static_cast<int>(value))
	{
		...
		default:
		{
			...
			std::string ret = vkhpp::to_string(sType);
			// If, for any reason, vulkan.hpp doesn't have the name, return the
			// number so users can look up the structure themselves.
			if(ret.compare("invalid") == 0)
			{
				ret = std::to_string(static_cast<int>(value));
			}
			return ret;
		}
	}
