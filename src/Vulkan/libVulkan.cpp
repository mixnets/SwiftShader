...
void ValidateRenderPassPNextChain(VkDevice device, const T *pCreateInfo)
{
	...
	while(extensionCreateInfo)
	{
		switch(extensionCreateInfo->sType)
		{
			...
			break;
			default:
				LOG_ASSERT("pCreateInfo->pNext sType = %s", vk::Stringify(extensionCreateInfo->sType).c_str());
				break;
		}
	...
	if(pCreateInfo->pNext)
	{
		...
		switch(createInfo->sType)
		{
			case VK_STRUCTURE_TYPE_LOADER_INSTANCE_CREATE_INFO:
				...
				break;
			default:
				LOG_ASSERT("pCreateInfo->pNext sType = %s", vk::Stringify(createInfo->sType).c_str());
				break;
		}
	}
...
VKAPI_ATTR VkResult VKAPI_CALL vkCreateDevice(VkPhysicalDevice physicalDevice, const VkDeviceCreateInfo *pCreateInfo, const VkAllocationCallbacks *pAllocator, VkDevice *pDevice)
{
	...
	for(uint32_t i = 0; i < pCreateInfo->queueCreateInfoCount; i++)
	{
		...
		while(extInfo)
		{
			LOG_ASSERT("pCreateInfo->pQueueCreateInfos[%d].pNext sType = %s", i, vk::Stringify(extInfo->sType).c_str());
...
VKAPI_ATTR VkResult VKAPI_CALL vkAllocateMemory(VkDevice device, const VkMemoryAllocateInfo *pAllocateInfo, const VkAllocationCallbacks *pAllocator, VkDeviceMemory *pMemory)
{
	...
	while(allocationInfo)
	{
		switch(allocationInfo->sType)
		{
			...
			default:
				LOG_ASSERT("pAllocateInfo->pNext sType = %s", vk::Stringify(allocationInfo->sType).c_str());
				break;
		}
...
VKAPI_ATTR VkResult VKAPI_CALL vkCreateFence(VkDevice device, const VkFenceCreateInfo *pCreateInfo, const VkAllocationCallbacks *pAllocator, VkFence *pFence)
{
	...
	while(nextInfo)
	{
		LOG_ASSERT("pCreateInfo->pNext sType = %s", vk::Stringify(nextInfo->sType).c_str());
...
VKAPI_ATTR VkResult VKAPI_CALL vkCreateEvent(VkDevice device, const VkEventCreateInfo *pCreateInfo, const VkAllocationCallbacks *pAllocator, VkEvent *pEvent)
{
	...
	while(extInfo)
	{
		...
		LOG_ASSERT("pCreateInfo->pNext sType = %s", vk::Stringify(extInfo->sType).c_str());
		extInfo = extInfo->pNext;
	}
...
VKAPI_ATTR VkResult VKAPI_CALL vkAllocateDescriptorSets(VkDevice device, const VkDescriptorSetAllocateInfo *pAllocateInfo, VkDescriptorSet *pDescriptorSets)
{
	...
	while(extInfo)
	{
		LOG_ASSERT("pAllocateInfo->pNext sType = %s", vk::Stringify(extInfo->sType).c_str());
		extInfo = extInfo->pNext;
	}
...
VKAPI_ATTR VkResult VKAPI_CALL vkAllocateCommandBuffers(VkDevice device, const VkCommandBufferAllocateInfo *pAllocateInfo, VkCommandBuffer *pCommandBuffers)
{
	...
	while(nextInfo)
	{
		LOG_ASSERT("pAllocateInfo->pNext sType = %s", vk::Stringify(nextInfo->sType).c_str());
		nextInfo = nextInfo->pNext;
	}
...
VKAPI_ATTR VkResult VKAPI_CALL vkBeginCommandBuffer(VkCommandBuffer commandBuffer, const VkCommandBufferBeginInfo *pBeginInfo)
{
	...
	while(nextInfo)
	{
		LOG_ASSERT("pBeginInfo->pNext sType = %s", vk::Stringify(nextInfo->sType).c_str());
		nextInfo = nextInfo->pNext;
	}
	...
	while(renderPassBeginInfo)
	{
		switch(renderPassBeginInfo->sType)
		{
			case VK_STRUCTURE_TYPE_DEVICE_GROUP_RENDER_PASS_BEGIN_INFO:
				...
				break;
			default:
				LOG_ASSERT("pRenderPassBegin->pNext sType = %s", vk::Stringify(renderPassBeginInfo->sType).c_str());
				break;
		}
...
VKAPI_ATTR VkResult VKAPI_CALL vkBindBufferMemory2(VkDevice device, uint32_t bindInfoCount, const VkBindBufferMemoryInfo *pBindInfos)
{
	...
	for(uint32_t i = 0; i < bindInfoCount; i++)
	{
		...
		while(extInfo)
		{
			LOG_ASSERT("pBindInfos[%d].pNext sType = %s", i, vk::Stringify(extInfo->sType).c_str());
			extInfo = extInfo->pNext;
		}
...
VKAPI_ATTR void VKAPI_CALL vkGetImageMemoryRequirements2(VkDevice device, const VkImageMemoryRequirementsInfo2 *pInfo, VkMemoryRequirements2 *pMemoryRequirements)
{
	...
	while(extInfo)
	{
		LOG_ASSERT("pInfo->pNext sType = %s", vk::Stringify(extInfo->sType).c_str());
		extInfo = extInfo->pNext;
	}
	...
	while(extensionRequirements)
	{
		switch(extensionRequirements->sType)
		{
			...
			break;
			default:
				LOG_ASSERT("pMemoryRequirements->pNext sType = %s", vk::Stringify(extensionRequirements->sType).c_str());
				break;
		}
...
VKAPI_ATTR void VKAPI_CALL vkGetImageSparseMemoryRequirements2(VkDevice device, const VkImageSparseMemoryRequirementsInfo2 *pInfo, uint32_t *pSparseMemoryRequirementCount, VkSparseImageMemoryRequirements2 *pSparseMemoryRequirements)
{
	...
	while(extensionRequirements)
	{
		LOG_ASSERT("pSparseMemoryRequirements->pNext sType = %s", vk::Stringify(extensionRequirements->sType).c_str());
		extensionRequirements = extensionRequirements->pNext;
	}
	...
	while(extensionFeatures)
	{
		switch((long)(extensionFeatures->sType))
		{
			...
			case VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_PERFORMANCE_QUERY_FEATURES_KHR:
				...
				break;
			default:
				LOG_ASSERT("pFeatures->pNext sType = %s", vk::Stringify(extensionFeatures->sType).c_str());
				break;
		}
...
VKAPI_ATTR void VKAPI_CALL vkGetPhysicalDeviceProperties2(VkPhysicalDevice physicalDevice, VkPhysicalDeviceProperties2 *pProperties)
{
	...
	while(extensionProperties)
	{
		...
		switch((long)(extensionProperties->sType))
		{
			...
			default:
				// "the [driver] must skip over, without processing (other than reading the sType and pNext members) any structures in the chain with sType values not defined by [supported extenions]"
				LOG_ASSERT("pProperties->pNext sType = %s", vk::Stringify(extensionProperties->sType).c_str());
				break;
		}
...
VKAPI_ATTR void VKAPI_CALL vkGetPhysicalDeviceFormatProperties2(VkPhysicalDevice physicalDevice, VkFormat format, VkFormatProperties2 *pFormatProperties)
{
	...
	while(extInfo)
	{
		LOG_ASSERT("pFormatProperties->pNext sType = %s", vk::Stringify(extInfo->sType).c_str());
		extInfo = extInfo->pNext;
	}
...
VKAPI_ATTR VkResult VKAPI_CALL vkGetPhysicalDeviceImageFormatProperties2(VkPhysicalDevice physicalDevice, const VkPhysicalDeviceImageFormatInfo2 *pImageFormatInfo, VkImageFormatProperties2 *pImageFormatProperties)
{
	...
	while(extensionFormatInfo)
	{
		switch(extensionFormatInfo->sType)
		{
			...
			break;
			default:
				LOG_ASSERT("pImageFormatInfo->pNext sType = %s", vk::Stringify(extensionFormatInfo->sType).c_str());
				break;
		}
	...
	while(extensionProperties)
	{
		switch(extensionProperties->sType)
		{
			...
			break;
			default:
				LOG_ASSERT("pImageFormatProperties->pNext sType = %s", vk::Stringify(extensionProperties->sType).c_str());
				break;
		}
...
VKAPI_ATTR void VKAPI_CALL vkGetPhysicalDeviceQueueFamilyProperties2(VkPhysicalDevice physicalDevice, uint32_t *pQueueFamilyPropertyCount, VkQueueFamilyProperties2 *pQueueFamilyProperties)
{
	...
	if(pQueueFamilyProperties)
	{
		...
		while(extInfo)
		{
			LOG_ASSERT("pQueueFamilyProperties->pNext sType = %s", vk::Stringify(extInfo->sType).c_str());
			extInfo = extInfo->pNext;
		}
	}
...
VKAPI_ATTR void VKAPI_CALL vkGetPhysicalDeviceMemoryProperties2(VkPhysicalDevice physicalDevice, VkPhysicalDeviceMemoryProperties2 *pMemoryProperties)
{
	...
	while(extInfo)
	{
		LOG_ASSERT("pMemoryProperties->pNext sType = %s", vk::Stringify(extInfo->sType).c_str());
		extInfo = extInfo->pNext;
	}
...
VKAPI_ATTR void VKAPI_CALL vkGetPhysicalDeviceSparseImageFormatProperties2(VkPhysicalDevice physicalDevice, const VkPhysicalDeviceSparseImageFormatInfo2 *pFormatInfo, uint32_t *pPropertyCount, VkSparseImageFormatProperties2 *pProperties)
{
	...
	if(pProperties)
	{
		...
		while(extInfo)
		{
			LOG_ASSERT("pProperties->pNext sType = %s", vk::Stringify(extInfo->sType).c_str());
			extInfo = extInfo->pNext;
		}
	}
...
VKAPI_ATTR void VKAPI_CALL vkGetDeviceQueue2(VkDevice device, const VkDeviceQueueInfo2 *pQueueInfo, VkQueue *pQueue)
{
	...
	while(extInfo)
	{
		LOG_ASSERT("pQueueInfo->pNext sType = %s", vk::Stringify(extInfo->sType).c_str());
		extInfo = extInfo->pNext;
	}
