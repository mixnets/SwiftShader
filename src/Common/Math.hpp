...
namespace sw
{
	...
	public:
		RGB9E5Data(float red, float green, float blue)
		{
			...
			const float red_c = sw::max(0.0f, sw::min(g_sharedexp_max, red));
			const float green_c = sw::max(0.0f, sw::min(g_sharedexp_max, green));
			const float blue_c = sw::max(0.0f, sw::min(g_sharedexp_max, blue));

			const float max_c = sw::max(sw::max(red_c, green_c), blue_c);
			const float exp_p = sw::max(-g_sharedexp_bias - 1.0f, floor(log(max_c))) + 1.0f + g_sharedexp_bias;
			...
			R = static_cast<unsigned int>(floor((red_c / (pow(2.0f, exp_s - g_sharedexp_bias - g_sharedexp_mantissabits))) + 0.5f));
			G = static_cast<unsigned int>(floor((green_c / (pow(2.0f, exp_s - g_sharedexp_bias - g_sharedexp_mantissabits))) + 0.5f));
			B = static_cast<unsigned int>(floor((blue_c / (pow(2.0f, exp_s - g_sharedexp_bias - g_sharedexp_mantissabits))) + 0.5f));
			E = exp_s;
		}

		void toBGRFloats(float *blue, float *green, float *red) const
		{
			*red = R * pow(2.0f, (int)E - g_sharedexp_bias - g_sharedexp_mantissabits);
			*green = G * pow(2.0f, (int)E - g_sharedexp_bias - g_sharedexp_mantissabits);
			*blue = B * pow(2.0f, (int)E - g_sharedexp_bias - g_sharedexp_mantissabits);
		}
