...
namespace sw
{
	...
	{
		void *buffer = source->lockInternal(0, 0, 0, sw::LOCK_READONLY, sw::PUBLIC);
		Format sourceFormat = source->getInternalFormat();
		size_t sourceStride = source->getInternalPitchB();
		copy(buffer, sourceFormat, sourceStride);
