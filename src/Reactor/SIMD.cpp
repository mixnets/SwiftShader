...
SIMD::Int::Int(RValue<packed::Int4> rhs)
    : XYZW(this)
{
	for(int i = 0; i < SIMD::Width / 4; i++)
	{
		*this = Insert128(*this, rhs, i);
	}
}
...
SIMD::UInt::UInt(RValue<packed::UInt4> rhs)
    : XYZW(this)
{
	for(int i = 0; i < SIMD::Width / 4; i++)
	{
...
//{
//	ASSERT(SIMD::Width == 4);
//	return SIMD::Float(Sqrt(Extract128(x, 0)));
// }
...
{
	ASSERT(SIMD::Width == 4);
	return SIMD::UInt(Ctlz(Extract128(x, 0), isZeroUndef));
}

RValue<SIMD::UInt> Cttz(RValue<SIMD::UInt> x, bool isZeroUndef)
{
	ASSERT(SIMD::Width == 4);
	return SIMD::UInt(Cttz(Extract128(x, 0), isZeroUndef));
}

RValue<SIMD::Int> MulHigh(RValue<SIMD::Int> x, RValue<SIMD::Int> y)
{
	ASSERT(SIMD::Width == 4);
	return SIMD::Int(MulHigh(Extract128(x, 0), Extract128(y, 0)));
}

RValue<SIMD::UInt> MulHigh(RValue<SIMD::UInt> x, RValue<SIMD::UInt> y)
{
	ASSERT(SIMD::Width == 4);
	return SIMD::UInt(MulHigh(Extract128(x, 0), Extract128(y, 0)));
}
...
rr::Bool SIMD::Pointer::hasSequentialOffsets(unsigned int step) const
{
	ASSERT_MSG(isBasePlusOffset, "No offsets for this type of pointer");
	// if(hasDynamicOffsets)
	//{
...
rr::Bool SIMD::Pointer::hasEqualOffsets() const
{
	ASSERT_MSG(isBasePlusOffset, "No offsets for this type of pointer");
	// if(hasDynamicOffsets)
	//{
...
bool SIMD::Pointer::hasStaticEqualOffsets() const
{
	...
	for(int i = 1; i < SIMD::Width; i++)
	{
		if(staticOffsets[i - 1] != staticOffsets[i])
		{
			return false;
		}
	}
...
Pointer<Byte> SIMD::Pointer::getUniformPointer() const
{
	if(isBasePlusOffset)
	{
	...
	else
	{
		for(int i = 1; i < SIMD::Width; i++)
		{
			Assert(pointers[i - 1] == pointers[i]);
		}
	}
...
Pointer<Byte> SIMD::Pointer::getPointerForLane(int lane) const
{
	if(isBasePlusOffset)
	{
