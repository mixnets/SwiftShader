...
public:
	...
	// values.
	std::unique_ptr<Stream<Return>> operator()(Name, Arguments...);
...
Coroutine<Return(Arguments...)>::operator()(Name name, Arguments... args)
{
	finalize(std::move(name));
...
{
	return (*this)(Name{ "coroutine" }, std::forward<Arguments>(args)...);
}
