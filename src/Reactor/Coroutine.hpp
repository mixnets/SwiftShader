...
private:
	...
	//   (2) Coroutines can call Yield() to suspend execution of the coroutine
	//       and pass a value to the caller. Yield can be called multiple times
	//       in a single execution of a coroutine.
...
#undef Yield
	...
	template<typename T>
	inline void Yield(LValue<T> val) { Nucleus::yield(val.loadValue()); }
