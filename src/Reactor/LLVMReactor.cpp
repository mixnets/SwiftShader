	...
	public:
		LLVMReactorJIT(const char *arch, const llvm::SmallVectorImpl<std::string>& mattrs,
			...
			targetMachine(llvm::EngineBuilder()
				.setOptLevel(llvm::CodeGenOpt::None /*di != nullptr ? llvm::CodeGenOpt::None : llvm::CodeGenOpt::Default*/)
				.setMArch(arch)
				.setMAttrs(mattrs)
			...
			objLayer(
				...
				[this](llvm::orc::VModuleKey, const llvm::object::ObjectFile &Obj, const llvm::RuntimeDyld::LoadedObjectInfo &L) {
					if (di != nullptr)
					{
...
namespace rr
{
	...
	{
		if (di != nullptr) { di->EmitLocation(); }
	}

	void EmitDebugVariable(Value* value)
	{
		if (di != nullptr) { di->EmitVariable(value); }
	}
