...
namespace
{
	...
	class JITGlobals : LLVMInitialzier
	{
	public:
		...
		const llvm::DataLayout dataLayout = getDataLayout();

	private:
		std::vector<std::string> getMattrs() const;
	...
	std::vector<std::string> JITGlobals::getMattrs() const
	{
...
(defined(__linux__) && (defined(__arm__) || defined(__aarch64__)))
	...
	const char* JITGlobals::getArch() const
	{
	...
	llvm::TargetOptions JITGlobals::getTargetOptions() const
	{
	...
	llvm::DataLayout JITGlobals::getDataLayout() const
	{
	...
	// JITSession holds a LLVM JIT session, compiler and object layer.
	// A JITSession is created for each acquired rr::LLVMRoutine as each routine
	// may want different target machine settings, and no Reactor routine
	// directly links against another.
	// The JITSession is reference counted, and is automatically deleted once
	// the last LLVMRoutine is released.
	class JITSession : public RefCounted
	{
	...
	public:
		JITSession(rr::OptimizationLevel optLevel) :
		...
		rr::LLVMRoutine *acquireRoutine(std::unique_ptr<llvm::Module> module, llvm::Function **funcs, size_t count)
		{
			...
			{
				{
					std::unique_lock<std::mutex> lock(this->compileLayerMutex);
					llvm::cantFail(this->compileLayer.removeModule(moduleKey));
				}
				// Release the lock before calling release - prevents
				// 'mutex destroyed while busy' CRT errors.
				this->release();
			});
		}
	...
	public:
		...
		rr::LLVMRoutine *acquireRoutine(rr::OptimizationLevel optLevel, llvm::Function **funcs, size_t count)
		{
			ASSERT(module);
			auto session = new JITSession(optLevel);
			auto routine = session->acquireRoutine(std::move(module), funcs, count);
			session->release();
			return routine;
		}
...
namespace rr
{
	...
	Routine *Nucleus::acquireRoutine(const char *name, OptimizationLevel optimizationLevel)
	{
		...
		LLVMRoutine *routine = jit->acquireRoutine(optimizationLevel, &jit->function, 1);
		jit.reset();
...
Routine* Nucleus::acquireCoroutine(const char *name, OptimizationLevel optimizationLevel)
{
	...
	Routine *routine = jit->acquireRoutine(optimizationLevel, funcs, Nucleus::CoroutineEntryCount);
	jit.reset();
