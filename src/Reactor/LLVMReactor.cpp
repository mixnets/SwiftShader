...
SIMD::Int::Int(RValue<rr::Int> rhs)
{
	RR_DEBUG_INFO_UPDATE_LOC();
	Value *vector = loadValue();
	Value *insert = Nucleus::createInsertElement(vector, rhs.value(), 0);
...
SIMD::UInt::UInt(RValue<rr::UInt> rhs)
{
	RR_DEBUG_INFO_UPDATE_LOC();
	Value *vector = loadValue();
	Value *insert = Nucleus::createInsertElement(vector, rhs.value(), 0);
...
Type *SIMD::UInt::type()
{
	return T(llvm::VectorType::get(T(rr::UInt::type()), SIMD::Width, false));
}

SIMD::Float::Float(RValue<rr::Float> rhs)
{
	RR_DEBUG_INFO_UPDATE_LOC();
	Value *vector = loadValue();
	Value *insert = Nucleus::createInsertElement(vector, rhs.value(), 0);
...
RValue<SIMD::Float> Sin(RValue<SIMD::Float> v)
{
	RR_DEBUG_INFO_UPDATE_LOC();
	auto func = llvm::Intrinsic::getDeclaration(jit->module.get(), llvm::Intrinsic::sin, { V(v.value())->getType() });
	return RValue<SIMD::Float>(V(jit->builder->CreateCall(func, V(v.value()))));
}

RValue<SIMD::Float> Cos(RValue<SIMD::Float> v)
{
	RR_DEBUG_INFO_UPDATE_LOC();
	auto func = llvm::Intrinsic::getDeclaration(jit->module.get(), llvm::Intrinsic::cos, { V(v.value())->getType() });
	return RValue<SIMD::Float>(V(jit->builder->CreateCall(func, V(v.value()))));
...
RValue<SIMD::Float> Tan(RValue<SIMD::Float> v)
{
	RR_DEBUG_INFO_UPDATE_LOC();
	return Sin(v) / Cos(v);
}

static RValue<SIMD::Float> TransformPerElement(RValue<SIMD::Float> v, const char *name)
{
	auto funcTy = llvm::FunctionType::get(T(Float::type()), llvm::ArrayRef<llvm::Type *>(T(Float::type())), false);
	auto func = jit->module->getOrInsertFunction(name, funcTy);
	llvm::Value *out = llvm::UndefValue::get(T(SIMD::Float::type()));
	for(uint64_t i = 0; i < 4; i++)
	{
		auto el = jit->builder->CreateCall(func, V(Nucleus::createExtractElement(v.value(), rr::Float::type(), i)));
		out = V(Nucleus::createInsertElement(V(out), V(el), i));
	}
	return RValue<SIMD::Float>(V(out));
}

RValue<SIMD::Float> Asin(RValue<SIMD::Float> v)
{
	RR_DEBUG_INFO_UPDATE_LOC();
	return TransformPerElement(v, "asinf");
}

RValue<SIMD::Float> Acos(RValue<SIMD::Float> v)
{
	RR_DEBUG_INFO_UPDATE_LOC();
	return TransformPerElement(v, "acosf");
}

RValue<SIMD::Float> Atan(RValue<SIMD::Float> v)
{
	RR_DEBUG_INFO_UPDATE_LOC();
	return TransformPerElement(v, "atanf");
}

RValue<SIMD::Float> Sinh(RValue<SIMD::Float> v)
{
	RR_DEBUG_INFO_UPDATE_LOC();
	return TransformPerElement(v, "sinhf");
}

RValue<SIMD::Float> Cosh(RValue<SIMD::Float> v)
{
	RR_DEBUG_INFO_UPDATE_LOC();
	return TransformPerElement(v, "coshf");
}

RValue<SIMD::Float> Tanh(RValue<SIMD::Float> v)
{
	RR_DEBUG_INFO_UPDATE_LOC();
	return TransformPerElement(v, "tanhf");
}

RValue<SIMD::Float> Asinh(RValue<SIMD::Float> v)
{
	RR_DEBUG_INFO_UPDATE_LOC();
	return TransformPerElement(v, "asinhf");
}

RValue<SIMD::Float> Acosh(RValue<SIMD::Float> v)
{
	RR_DEBUG_INFO_UPDATE_LOC();
	return TransformPerElement(v, "acoshf");
}

RValue<SIMD::Float> Atanh(RValue<SIMD::Float> v)
{
	RR_DEBUG_INFO_UPDATE_LOC();
	return TransformPerElement(v, "atanhf");
}
...
RValue<SIMD::Float> Atan2(RValue<SIMD::Float> x, RValue<SIMD::Float> y)
{
	...
	for(uint64_t i = 0; i < 4; i++)
	{
		auto el = jit->builder->CreateCall(func, { V(Nucleus::createExtractElement(x.value(), rr::Float::type(), i)),
		                                           V(Nucleus::createExtractElement(y.value(), rr::Float::type(), i)) });
		out = V(Nucleus::createInsertElement(V(out), V(el), i));
	}
	return RValue<SIMD::Float>(V(out));
}
...
{
	RR_DEBUG_INFO_UPDATE_LOC();
	auto func = llvm::Intrinsic::getDeclaration(jit->module.get(), llvm::Intrinsic::pow, { T(SIMD::Float::type()) });
	return RValue<SIMD::Float>(V(jit->builder->CreateCall(func, { V(x.value()), V(y.value()) })));
}

RValue<SIMD::Float> Exp(RValue<SIMD::Float> v)
{
	RR_DEBUG_INFO_UPDATE_LOC();
	auto func = llvm::Intrinsic::getDeclaration(jit->module.get(), llvm::Intrinsic::exp, { T(SIMD::Float::type()) });
	return RValue<SIMD::Float>(V(jit->builder->CreateCall(func, V(v.value()))));
}

RValue<SIMD::Float> Log(RValue<SIMD::Float> v)
{
	RR_DEBUG_INFO_UPDATE_LOC();
	auto func = llvm::Intrinsic::getDeclaration(jit->module.get(), llvm::Intrinsic::log, { T(SIMD::Float::type()) });
	return RValue<SIMD::Float>(V(jit->builder->CreateCall(func, V(v.value()))));
}

RValue<SIMD::Float> Exp2(RValue<SIMD::Float> v)
{
	RR_DEBUG_INFO_UPDATE_LOC();
	auto func = llvm::Intrinsic::getDeclaration(jit->module.get(), llvm::Intrinsic::exp2, { T(SIMD::Float::type()) });
	return RValue<SIMD::Float>(V(jit->builder->CreateCall(func, V(v.value()))));
}

RValue<SIMD::Float> Log2(RValue<SIMD::Float> v)
{
	RR_DEBUG_INFO_UPDATE_LOC();
	auto func = llvm::Intrinsic::getDeclaration(jit->module.get(), llvm::Intrinsic::log2, { T(SIMD::Float::type()) });
	return RValue<SIMD::Float>(V(jit->builder->CreateCall(func, V(v.value()))));
}

Type *SIMD::Float::type()
{
	return T(llvm::VectorType::get(T(rr::Float::type()), SIMD::Width, false));
}
