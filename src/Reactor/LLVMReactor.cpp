...
static RValue<SIMD::Float> TransformPerElement(RValue<SIMD::Float> v, const char *name)
{
	...
	for(uint64_t i = 0; i < 4; i++)
	{
		auto el = jit->builder->CreateCall(func, V(Nucleus::createExtractElement(v.value(), scalar::Float::type(), i)));
		out = V(Nucleus::createInsertElement(V(out), V(el), i));
	}
...
RValue<SIMD::Float> Atan2(RValue<SIMD::Float> x, RValue<SIMD::Float> y)
{
	...
	for(uint64_t i = 0; i < 4; i++)
	{
		auto el = jit->builder->CreateCall(func, { V(Nucleus::createExtractElement(x.value(), scalar::Float::type(), i)),
		                                           V(Nucleus::createExtractElement(y.value(), scalar::Float::type(), i)) });
		out = V(Nucleus::createInsertElement(V(out), V(el), i));
	}
	return RValue<SIMD::Float>(V(out));
}
