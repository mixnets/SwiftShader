...
namespace rr
{
	...
	void DebugInfo::EmitVariable(Value *variable)
	{
		...
		for (int i = backtrace.size() - 1; i >= 0; i--)
		{
			...
			scope.pending.insertAfter = block->size() > 0 ? &block->back() : nullptr;
	...
	void DebugInfo::emitPending(Scope &scope, IRBuilder *builder, llvm::DIBuilder *diBuilder)
	{
		...
		auto insertAfter = (pending.insertAfter != nullptr) ? pending.insertAfter : &pending.block->front();
		...
		if (!isAlloca)
		{
			...
			auto store = new llvm::StoreInst(value, alloca);
			entryBlock.getInstList().push_front(alloca);
			store->setDebugLoc(diLoc);
			store->insertAfter(insertAfter);
			value = alloca;
			insertAfter = store;
		}
		...
		auto di = diBuilder->insertDeclare(value, diVar, diBuilder->createExpression(), diLoc, pending.block);
		di->moveAfter(insertAfter);
