...
namespace rr
{
	...
	void DebugInfo::Finalize()
	{
		...
		{
			diScope.back().emitPending(builder, diBuilder);
			diScope.pop_back();
		}
		diBuilder->finalize();
	}
	...
	{
		auto const& backtrace = getCalleeBacktrace();
		syncScope(backtrace);
	...
	void DebugInfo::syncScope(Backtrace const& backtrace)
	{
		auto shrink = [this](size_t newsize)
		{
			while (diScope.size() > newsize)
			{
				...
				scope.emitPending(builder, diBuilder);
				diScope.pop_back();
			}
		};
		...
		while (backtrace.size() > diScope.size())
		{
			...
			auto file = getOrCreateFile(location.function.file.c_str());
			auto sameFunction = (i > 0 && backtrace[i-1].function == location.function);
			if (sameFunction)
			{
				auto const& parent = diScope[i-1];
				auto di = diBuilder->createLexicalBlock(parent.di, file, location.line, 0);
				diScope.push_back({location, di, parent.symbols});
				LOG("+ STACK(%d): di: %p, parent: %p, location: %s:%d", int(i), di,
					parent.di, location.function.file.c_str(), int(location.line));
			}
			else
			{
				auto di = diBuilder->createLexicalBlock(diFunction, file, location.line, 0);
				diScope.push_back({location, di});
				LOG("+ STACK(%d): di: %p, parent: <none>, location: %s:%d", int(i), di,
	...
	void DebugInfo::EmitVariable(Value *variable)
	{
		auto const& backtrace = getCalleeBacktrace();
		syncScope(backtrace);
		auto const &location = backtrace.back();
		auto &scope = diScope.back();
		...
		scope.pending.block = builder->GetInsertBlock();
		scope.pending.type = getOrCreateType(isAlloca ? value->getType()->getPointerElementType() : value->getType());
		scope.pending.scope = scope.di;
		scope.pending.file = getOrCreateFile(location.function.file.c_str());
	}

	void DebugInfo::Scope::emitPending(IRBuilder *builder, llvm::DIBuilder *diBuilder)
	{
		if (pending.value == nullptr) { return; }

		if (!symbols.emplace(pending.name).second)
		{
			return;
		}
		...
		LOG("  EMIT(%s): di: %p, location: %s:%d, isAlloca: %s", pending.name.c_str(), di,
			pending.location.function.file.c_str(), pending.location.line, isAlloca ? "true" : "false");

		pending.value->setName(pending.name);

		auto dbgVar = diBuilder->createAutoVariable(di, pending.name, pending.file, location.line, pending.type);
		auto dbgLoc = llvm::DebugLoc::get(location.line, 0, di);

		auto di = isAlloca
			? diBuilder->insertDeclare(pending.value, dbgVar, diBuilder->createExpression(), dbgLoc, pending.block)
			: diBuilder->insertDbgValueIntrinsic(pending.value, dbgVar, diBuilder->createExpression(), dbgLoc, pending.block);

		if (auto instruction = llvm::dyn_cast<llvm::Instruction>(pending.value))
		{
	...
	{
		return getCalleeBacktrace(1)[0];
	}

	DebugInfo::Backtrace DebugInfo::getCalleeBacktrace(size_t limit /* = 0 */) const
	{
		struct callbacks
		{
