...
namespace rr
{
	...
	void DebugInfo::EmitVariable(Value *variable)
	{
		...
		{
			// Already got debug info for this symbol.
			if (it->second.location != location)
			{
				return;
			}

			// Same line. Assume we're building the initial value for the variable.
			it->second.instruction->eraseFromParent();
		}
		...
		auto dbgTy = getOrCreateType(isAlloca ? type->getPointerElementType() : type);
		auto *dbgVar = diBuilder->createAutoVariable(
			scope.di, name, file, location.line, dbgTy
		);

		auto dbgLoc = llvm::DebugLoc::get(location.line, 0, scope.di);
		if (isAlloca)
		{
			auto di = diBuilder->insertDeclare(
				value, dbgVar, diBuilder->createExpression(), dbgLoc, llvm::cast<llvm::Instruction>(value)->getNextNode());
			scope.symbols.emplace(name, Symbol{location, di});
		}
		else
		{
			auto di = diBuilder->insertDbgValueIntrinsic(
				value, dbgVar, diBuilder->createExpression(), dbgLoc, builder->GetInsertBlock());
			scope.symbols.emplace(name, Symbol{location, di});
		}
	}
