...
namespace
{
	...
	public:
		void optimize(Ice::Cfg *function)
		{
			...
			//		if(isStore(inst))
			//		{
			//			if(!uses[inst.getAddr()].areOnlyLoadStore())
			//			{
			//				continue;
			//			}

			//			// New store found. If we had a previous one, eliminate it.
			//			if(stores.find(inst.getAddr()) != stores.end())
			//			{
			//				deleteInstruction(stores[inst.getAddr()]);
			//			}

			//			stores[inst.getAddr()] = &inst;
			//		}
			//		else if(isLoad(inst))
			//		{
			//			Ice::Inst *load = &inst;

			//			if(stores.find(load.getAddr()) == stores.end())
			//			{
			//				continue;
			//			}

			//			replace(load, stores[load.getAddr()]->getData());
			//		}
			//	}
			//}
		}
