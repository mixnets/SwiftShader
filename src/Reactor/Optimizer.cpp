...
void Optimizer::run(Ice::Cfg *function)
{
	...
	// subsequent optimiztaion passes.
	eliminateDeadCode();
...
// Replace arrays with individual elements if only statically indexed.
void Optimizer::scalarReplAggregates()
{
			...
			for(auto *use : uses)
			{
				...
				else  // Statically indexed use
				{
					...
					if(arithmetic->getOp() == Ice::InstArithmetic::Add)
					{
						...
						assert(false && "Mismatch between isStaticallyIndexedArray() and scalarReplAggregates()");
				}
			}
		}
	}
...
bool Optimizer::isStaticallyIndexedArray(Ice::Operand *allocaAddress)
{
	...
	// No uses, don't perform redundant transformations on it.
	return false;
}
