...
// access TLS directly, but it calls __emutls_get_address() to retrieve the memory
// associated with a __emutls_control structure with a name starting with
// "__emutls_v." that represents the TLS.
...
static void *getTLSAddress(__emutls_control *control)
{
...
JITGlobals *JITGlobals::get()
{
	static JITGlobals instance = [] {
		...
		ASSERT(!__has_feature(memory_sanitizer) || (jtmb->getOptions().ExplicitEmulatedTLS && jtmb->getOptions().EmulatedTLS));
