...
bool emitAsmFile(const std::string &filename, llvm::orc::JITTargetMachineBuilder builder, llvm::Module &module, const rr::Config &config)
{
...
// NOTE: Doing this is error-prone. It would be nice if LLVM's asm output included at
// least the 0-based relative address of each instruction.
...
#ifdef ENABLE_RR_EMIT_ASM_FILE
		const auto asmFilename = AsmFile::generateFilename(name);
		AsmFile::emitAsmFile(asmFilename, JITGlobals::get()->getTargetMachineBuilder(config.getOptimization().getLevel()), *module, config);
#endif
...
#ifdef ENABLE_RR_EMIT_ASM_FILE
		AsmFile::fixupAsmFile(asmFilename, addresses);
#endif
	}
