...
#if defined(LINUX_ENABLE_NAMED_MMAP) || defined(__APPLE__)
...
void *allocateExecutable(size_t bytes, int permissions, bool need_exec)
{
	...
	void *mapping = nullptr;
		...
		int prot = permissionsToMmapProt()
		...
		protectExecutable(mapping, length, permissions);
	#endif
...
void protectExecutable(void *memory, size_t bytes, int permissions)
{
		...
		VirtualProtect(memory, bytes, permissionsToProtectMode(permissions), &oldProtection);
	#elif defined(__Fuchsia__)
		size_t pageSize = memoryPageSize();
		size_t length = roundUp(bytes, pageSize);
