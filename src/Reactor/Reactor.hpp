...
//	const Array<T> &operator--(Array<T> &val);   // Pre-decrement
	...
	{
	public:
		IfData(RValue<Bool> cmp) : loopOnce(0)
		{
			condition = cmp.value;
		...
		IfData &operator++()
		{
			/*BasicBlock *dom = Nucleus::getInsertBlock();
...
//	}
	...
	#define If(cond) \
	for(IfData if__(cond); if__ < 3; ++if__) \
	if(if__ == 0)
