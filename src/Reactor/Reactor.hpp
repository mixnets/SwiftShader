...
struct Pointer4
{
	...
	Pointer4(std::array<Pointer<Byte>, PWidth> pointers);
	...
	inline Pointer<Byte> getBase() const;
	inline std::array<Pointer<Byte>, PWidth> getPointers() const;
	inline void offsetBase(int offset);
	inline void offsetBase(Int offset);
	inline void offsetBase(Int4 offset);

private:
	// Base address for the pointer, common across all lanes.
	Pointer<Byte> base;
	// Per-lane address for dealing with non-uniform data
	std::array<Pointer<Byte>, PWidth> pointers;
...
public:
	...
	std::array<int32_t, PWidth> staticOffsets;
...
{
	this->Store(T(val), robustness, mask, atomic, order);
}

inline Pointer<Byte> Pointer4::getBase() const
{
	assert(isBasePlusOffset);
	return base;
}
inline std::array<Pointer<Byte>, PWidth> Pointer4::getPointers() const
{
	assert(!isBasePlusOffset);
	return pointers;
}

inline void Pointer4::offsetBase(int offset)
{
	if(isBasePlusOffset)
	{
		base += offset;
...
inline void Pointer4::offsetBase(Int4 offset)
{
	if(isBasePlusOffset)
	{
