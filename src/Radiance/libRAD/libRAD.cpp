...
{
public:
	DrawElements(RADprimitiveType mode, RADindexType type, RADsizei count, rad::Buffer *indexBuffer, RADuint offset)
	{
		this->mode = mode;
		this->type = type;
		this->count = count;
...
{
public:
	BindGroup(RADbitfield stages, RADuint group, RADuint count, rad::Buffer *buffer, RADuint offset)
	{
		this->stages = stages;
		this->group = group;
		buffer->reference();
...
public:
	...
	void bindPipeline(RADpipelineType pipelineType, rad::Pipeline *pipeline)
	{
		if(pipelineType == RAD_PIPELINE_TYPE_GRAPHICS)
		{
			ASSERT(!graphicsPipeline);   // FIXME
			pipeline->reference();
...
{
	rad::Queue *radQueue = reinterpret_cast<rad::Queue*>(queue);
	rad::Buffer *indexBuffer = reinterpret_cast<rad::Buffer*>(indexHandle);
...
{
	rad::Queue *radQueue = reinterpret_cast<rad::Queue*>(queue);
	rad::Buffer *groupBuffer = reinterpret_cast<rad::Buffer*>(groupHandle);
