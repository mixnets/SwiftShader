...
bool Surface::reset(int backBufferWidth, int backBufferHeight)
{
	...
	backBuffer = ::createBackBuffer(backBufferWidth, backBufferHeight);
    ...
    {
        mDepthStencil = ::createDepthStencil(backBufferWidth, backBufferHeight, sw::FORMAT_D24S8, 1, false);
...
bool Surface::checkForResize()
{
    ...
    if(sizeDirty)
    {
        ...
        if(gl::getCurrentDrawSurface() == this)
        {
			gl::getCurrentContext()->makeCurrent(this);
        }
