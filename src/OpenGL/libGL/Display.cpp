...
gl::Context *Display::createContext(EGLConfig configHandle, const gl::Context *shareContext)
{
    const egl::Config *config = mConfigSet.get(configHandle);

    gl::Context *context = new gl::Context(config, shareContext);
...
egl::Surface *Display::getPrimarySurface()
{
    if(mSurfaceSet.size() == 0)
	{
		const Config *configuration = mConfigSet.get((void*)1);

		Surface *surface = new Surface(this, configuration, WindowFromDC(displayId));
