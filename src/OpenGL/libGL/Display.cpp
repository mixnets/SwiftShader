...
egl::Surface *Display::getPrimarySurface()
{
    if(mSurfaceSet.size() == 0)
	{
		...
		EGLConfig config;
		EGLint numConfigs;
		eglChooseConfig(this, attribList, &config, 1, &numConfigs);
		EGLSurface surface = eglCreateWindowSurface(this, config, WindowFromDC(displayId), attribList);
		egl::setCurrentDrawSurface(surface);
	}
