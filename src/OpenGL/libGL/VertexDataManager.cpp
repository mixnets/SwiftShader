...
std::map<std::pair<int, int>, unsigned int> mapWriteOffset;
                ...
                else
                {
					...
					if(needsWriting)
					{
						if(currentList != 0)
						{
							...
							mapWriteOffset[std::make_pair(currentList, i)] = writePos;
						}
						else
						{
					...
					{
						mStreamingBuffer->setWritePosition(mapWriteOffset[std::make_pair(currentList, i)] + start);
						//streamOffset = mapWriteOffset[currentList];
