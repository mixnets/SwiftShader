...
bool Display::isValidWindow(EGLNativeWindowType window)
{
		...
		if (!window)
		{
			ALOGE("%s called with window==NULL %s:%d", __FUNCITON__, __FILE__, __LINE__);
			return false;
		}
		if (static_cast<ANativeWindow*>(window)->common.magic != ANDROID_NATIVE_WINDOW_MAGIC)
		{
			ALOGE("%s called with window==%p bad magic %s:%d", __FUNCITON__, window, __FILE__, __LINE__);
			return false;
		}
		return true;
    #else
...
DisplayMode Display::getDisplayMode() const
{
	DisplayMode displayMode = {0};
		...
		ALOGI("Returning framebuffer config width=%d height=%d, format=%d",
