...
bool Surface::initialize()
{
	...
	if(libGLESv2)
	{
		backBuffer = libGLESv2->createBackBuffer(width, height, getInternalFormat(), config->mSamples);
	}
	else if(libGLES_CM)
	{
...
void* Surface::setBoundTexture(egl::Texture *texture)
{
	...
	if(clientBuffer && backBuffer)
	{
		if((this->texture == nullptr) && (texture != nullptr))
		{
			...
			externalBuffer = IOSurfaceGetBaseAddress(ioSurface);
#else
			externalBuffer = clientBuffer;
#endif // __APPLE__
		}
		if((this->texture != nullptr) && (texture == nullptr))
		{
