	...
	void OutputASM::argument(sw::Shader::SourceParameter &parameter, TIntermNode *argument, int index)
	{
		if(argument)
		{
			...
			if(arg->getQualifier() == EvqConstExpr)
			{
				...
				if(constants->getType() == EbtInt || constants->getType() == EbtUInt)
				{
					for(int i = 0; i < 4; i++)
					{
						if(size == 1)   // Replicate
						{
							parameter.integer[i] = constants[component + 0].getIConst();
						}
						else if(i < size)
						{
							parameter.integer[i] = constants[component + i].getIConst();
						}
						else
						{
					...
					else if(left->isArray() || left->isMatrix())
					{
						...
						else if(dst.rel.index != registerIndex(&address))   // Move the previous index register to the address register
						{
							if(scale == 1)
							{
								...
								Instruction *mad = emit(sw::Shader::OPCODE_MADI, &address, &address, &oldScale, right);
							...
							{
								Constant oldScale((int)dst.rel.scale);
								Instruction *mul = emit(sw::Shader::OPCODE_MULI, &address, &address, &oldScale);
								mul->src[0].index = dst.rel.index;
								mul->src[0].type = dst.rel.type;

								Constant newScale(scale);
								emit(sw::Shader::OPCODE_MADI, &address, right, &newScale, &address);
							}
						...
						else   // Just add the new index to the address register
						{
							if(scale == 1)
							{
								emit(sw::Shader::OPCODE_ADDI, &address, &address, right);
							}
							else
							{
								Constant newScale(scale);
