...
namespace glsl
{
	...
	sw::Shader::Opcode OutputASM::getOpcode(sw::Shader::Opcode op, TIntermTyped *in) const
	{
		...
		case sw::Shader::OPCODE_NEG:
			...
			{
			case EbtInt:
				return sw::Shader::OPCODE_NEGI;
			case EbtUInt:
				return sw::Shader::OPCODE_NEGU;
			case EbtFloat:
			default:
				return op;
			}
		case sw::Shader::OPCODE_ADD:
			...
			{
			case EbtInt:
				return sw::Shader::OPCODE_ADDI;
			case EbtUInt:
				return sw::Shader::OPCODE_ADDU;
			case EbtFloat:
			default:
				return op;
			}
		case sw::Shader::OPCODE_SUB:
			...
			{
			case EbtInt:
				return sw::Shader::OPCODE_SUBI;
			case EbtUInt:
				return sw::Shader::OPCODE_SUBU;
			case EbtFloat:
			default:
				return op;
			}
		case sw::Shader::OPCODE_MUL:
			...
			{
			case EbtInt:
				return sw::Shader::OPCODE_MULI;
			case EbtUInt:
				return sw::Shader::OPCODE_MULU;
			case EbtFloat:
			default:
				return op;
			}
		case sw::Shader::OPCODE_DIV:
			...
			{
			case EbtInt:
				return sw::Shader::OPCODE_DIVI;
			case EbtUInt:
				return sw::Shader::OPCODE_DIVU;
			case EbtFloat:
			default:
				return op;
			}
		case sw::Shader::OPCODE_MODI:
			return baseType == EbtUInt ? sw::Shader::OPCODE_MODU : op;
		case sw::Shader::OPCODE_LSI:
			return baseType == EbtUInt ? sw::Shader::OPCODE_LSU : op;
		case sw::Shader::OPCODE_RSI:
			return baseType == EbtUInt ? sw::Shader::OPCODE_RSU : op;
		case sw::Shader::OPCODE_MIN:
			switch(baseType)
			{
			case EbtInt:
				return sw::Shader::OPCODE_MINI;
			case EbtUInt:
				return sw::Shader::OPCODE_MINU;
			case EbtFloat:
			default:
				return op;
			}
		case sw::Shader::OPCODE_MAX:
			...
			{
			case EbtInt:
				return sw::Shader::OPCODE_MAXI;
			case EbtUInt:
				return sw::Shader::OPCODE_MAXU;
			case EbtFloat:
			default:
				return op;
			}
	...
	bool OutputASM::visitBinary(Visit visit, TIntermBinary *node)
	{
		...
		case EOpIModAssign:          if(visit == PostVisit) emitAssign(getOpcode(sw::Shader::OPCODE_MODI, result), result, left, left, right); break;
		case EOpIMod:                if(visit == PostVisit) emitBinary(getOpcode(sw::Shader::OPCODE_MODI, result), result, left, right);       break;
		case EOpBitShiftLeftAssign:  if(visit == PostVisit) emitAssign(getOpcode(sw::Shader::OPCODE_LSI, result), result, left, left, right); break;
		case EOpBitShiftLeft:        if(visit == PostVisit) emitBinary(getOpcode(sw::Shader::OPCODE_LSI, result), result, left, right);       break;
		case EOpBitShiftRightAssign: if(visit == PostVisit) emitAssign(getOpcode(sw::Shader::OPCODE_RSI, result), result, left, left, right); break;
		case EOpBitShiftRight:       if(visit == PostVisit) emitBinary(getOpcode(sw::Shader::OPCODE_RSI, result), result, left, right);       break;
	...
	bool OutputASM::visitUnary(Visit visit, TIntermUnary *node)
	{
		...
		switch(node->getOp())
		{
		case EOpNegative:         if(visit == PostVisit) emit(getOpcode(sw::Shader::OPCODE_NEG, arg), result, arg); break;
	...
	{
		if(dst->getBasicType() == EbtFloat && src->getBasicType() == EbtBool)
		{
			return emit(sw::Shader::OPCODE_B2F, dst, src);
		}
		if(dst->getBasicType() == EbtBool && src->getBasicType() == EbtFloat)
		{
			return emit(sw::Shader::OPCODE_F2B, dst, src);
		}
		if(dst->getBasicType() == EbtFloat && src->getBasicType() == EbtInt)
		{
			return emit(sw::Shader::OPCODE_I2F, dst, src);
		}
		if(dst->getBasicType() == EbtInt && src->getBasicType() == EbtFloat)
		{
			return emit(sw::Shader::OPCODE_F2I, dst, src);
		}
		if(dst->getBasicType() == EbtFloat && src->getBasicType() == EbtUInt)
		{
			return emit(sw::Shader::OPCODE_U2F, dst, src);
		}
		if(dst->getBasicType() == EbtUInt && src->getBasicType() == EbtFloat)
		{
			return emit(sw::Shader::OPCODE_F2U, dst, src);
		}
