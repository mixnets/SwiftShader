		...
		else
		{
			...
			if(cmpOp == sw::Shader::CONTROL_EQ)
			{
				for(int i = 1; i < type.totalRegisterCount(); i++)
				{
					...
					cmp = emit(opcode, &equal, 0, left, index, right, index + i);
					cmp->control = sw::Shader::CONTROL_EQ;
					emit(sw::Shader::OPCODE_AND, dst, dst, &equal);
				}
			}
			else if(cmpOp == sw::Shader::CONTROL_NE)
			{
				for(int i = 1; i < type.totalRegisterCount(); i++)
				{
					...
					cmp = emit(opcode, &notEqual, 0, left, index, right, index + i);
					cmp->control = sw::Shader::CONTROL_NE;
					emit(sw::Shader::OPCODE_OR, dst, dst, &notEqual);
				}
			}
		}
	}
