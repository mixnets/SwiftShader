...
namespace glsl
{
	...
	int OutputASM::uniformRegister(TIntermTyped *uniform)
	{
		...
		TInterfaceBlock *block = type.isInterfaceBlock() ? type.getInterfaceBlock() : nullptr;
	...
	int OutputASM::lookup(VariableArray &list, TIntermTyped *variable)
	{
		...
		TInterfaceBlock *varBlock = variable->getType().isInterfaceBlock() ? variable->getType().getInterfaceBlock() : nullptr;
		...
		else if(varBlock)
		{
			for(unsigned int i = 0; i < list.size(); i++)
			{
				if(list[i])
				{
					TInterfaceBlock *listBlock = list[i]->getType().isInterfaceBlock() ? list[i]->getType().getInterfaceBlock() : nullptr;
