...
namespace glsl
{
	...
	bool OutputASM::visitSwitch(Visit visit, TIntermSwitch *node)
	{
		...
		//   else // default case
		//   ...
		//   condition = false // unconditionally exit loop
		// end for
		Constant True(true);
		Constant False(false);
		Temporary iterate(this);
		emit(sw::Shader::OPCODE_MOV, &iterate, &True);
		emit(sw::Shader::OPCODE_WHILE, 0, &iterate);
		...
		emit(sw::Shader::OPCODE_TEST);
		emit(sw::Shader::OPCODE_MOV, &iterate, &False); // Unconditionally exit the while statement
		emit(sw::Shader::OPCODE_ENDWHILE);
