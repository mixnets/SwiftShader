...
namespace glsl
{
	...
	void OutputASM::declareUniform(const TType &type, const TString &name, int registerIndex, int blockId, BlockLayoutEncoder* encoder)
	{
		...
		if(!structure && !block)
		{
			...
			activeUniforms.push_back(Uniform(glVariableType(type), glVariablePrecision(type), name.c_str(), type.getArraySize(), registerIndex,
			                                 blockId, encoder ? encoder->encodeType(type) : BlockMemberInfo::getDefaultBlockInfo()));
		...
		else if(block)
		{
			...
			activeUniformBlocks.push_back(UniformBlock(block->name().c_str(), block->hasInstanceName() ? block->instanceName().c_str() : std::string(), 0,
			                                           block->arraySize(), block->blockStorage(), isRowMajor, registerIndex, blockId));

			const TFieldList& fields = block->fields();
			const bool containerHasName = block->hasInstanceName();
			const TString &containerName = containerHasName ? block->instanceName() : TString();
			if(type.isArray() && type.isInterfaceBlock())
			{
				for(int i = 0; i < type.getArraySize(); i++)
				{
					...
					for(size_t j = 0; j < fields.size(); j++)
					{
						...
						declareUniform(fieldType, uniformName, currentBlockEncoder.getCurrentRegister(), blockId, &currentBlockEncoder);
					}
			...
			if(type.isArray() && (structure || type.isInterfaceBlock()))
			{
				for(int i = 0; i < type.getArraySize(); i++)
				{
					...
					for(size_t j = 0; j < fields.size(); j++)
					{
						...
						if(encoder)
						{
							fieldRegisterIndex = encoder->getCurrentRegister();
						}
