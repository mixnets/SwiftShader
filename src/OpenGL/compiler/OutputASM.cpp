...
namespace glsl
{
	...
	int OutputASM::samplerRegister(TIntermTyped *sampler)
	{
		...
		else if(binary)
		{
			...
			case EOpIndexDirectInterfaceBlock:
				{
					int reg = samplerRegister(binary->getLeft());
					...
					return reg + right->getAsConstantUnion()->getIConst(0);
				}
			case EOpIndexIndirect:
				return -1;   // Temporary register
			default:
	...
	{
		if(!operand || !IsSampler(operand->getBasicType()))
		{
			return false;
		}

		const TType &type = operand->getType();
		bool notParameter = (type.getQualifier() == EvqUniform || type.getQualifier() == EvqGlobal || type.getQualifier() == EvqTemporary);

		return notParameter && samplerRegister(operand) >= 0;
	}
