...
namespace glsl
{
	...
	bool OutputASM::visitLoop(Visit visit, TIntermLoop *node)
	{
		...
		{
			 uniformLoopIndexIds.insert(loop.index->getId());
		}
		...
		if(loop.isDeterministic())
		{
			 uniformLoopIndexIds.erase(loop.index->getId());
		}
					...
					else if(left->isArray() || left->isMatrix())
					{
						...
						if(rel.type == sw::Shader::PARAMETER_VOID)   // Use the index register as the relative address directly
						{
							if(left->totalRegisterCount() > 1)
							{
								...
								rel.scale = scale;
								rel.dynamic = (right->getQualifier() != EvqUniform) && (uniformLoopIndexIds.count(indexId) == 0);
							}
						}
	...
	// Sets iterations to ~0u if no static loop count could be determined.
	OutputASM::LoopInfo::LoopInfo(TIntermLoop *node)
	{
		...
		{
			// Check the loop body for return statements or changes to the index variable that make it non-unrollable.
