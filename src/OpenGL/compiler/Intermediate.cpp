...
bool TIntermBinary::promote(TInfoSink& infoSink)
{
    ...
    int matrixSize = std::max(left->getMatrixSize(), right->getMatrixSize()); // FIMXE: This will have to change for NxM matrices
    ...
    if (left->getMatrixSize() != right->getMatrixSize()) {
        // Operator cannot be of type pure assignment.
        if (op == EOpAssign || op == EOpInitialize)
            return false;
    }
...
TIntermTyped* TIntermediate::promoteConstantUnion(TBasicType promoteTo, TIntermConstantUnion* node)
{
    ...
    return addConstantUnion(leftUnionArray, TType(promoteTo, t.getPrecision(), t.getQualifier(), t.getNominalSize(), t.getMatrixSize(), t.isArray()), node->getLine());
}
