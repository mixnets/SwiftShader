...
bool TParseContext::areAllChildConst(TIntermAggregate* aggrNode)
{
    ...
    for (TIntermSequence::iterator p = sequence.begin(); p != sequence.end(); ++p) {
		TIntermConstantUnion *constantNode = (*p)->getAsTyped()->getAsConstantUnion();
        if (!constantNode)
            return false;
		if (!constantNode->getUnionArrayPointer())
            return false;
    }
...
bool TParseContext::areAllChildConstExpr(TIntermAggregate* aggrNode)
{
    TIntermSequence &sequence = aggrNode->getSequence() ;
    for (TIntermSequence::iterator p = sequence.begin(); p != sequence.end(); p++)
	{
		if((*p)->getAsTyped()->getQualifier() != EvqConstExpr)
		{
			return false;
		}
    }
...
TIntermTyped* TParseContext::addConstructor(TIntermNode* arguments, const TType* type, TOperator op, TFunction* fnCall, const TSourceLoc &line)
{
	...
	if(areAllChildConstExpr(constructor))
	{
		constructor->getTypePointer()->setQualifier(EvqConstExpr);
	}
		...
		if(fnCandidate)
		{
			...
			if(builtIn && op != EOpNull)
			{
				...
				else
				{
					...
					if(areAllChildConstExpr(aggregate))
					{
						aggregate->getTypePointer()->setQualifier(EvqConstExpr);
					}
