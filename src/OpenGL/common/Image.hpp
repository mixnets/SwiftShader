...
protected:
	...
	// Render target
	Image(GLsizei width, GLsizei height, GLint format, int multiSampleDepth, bool lockable)
		: sw::Surface(nullptr, width, height, 1, 0, multiSampleDepth, SelectInternalFormat(format), lockable, true),
		  width(width), height(height), depth(1), internalformat(format), implementationFormat(SelectInternalFormat(format)),
		  parentTexture(nullptr)
	{
		shared = false;
