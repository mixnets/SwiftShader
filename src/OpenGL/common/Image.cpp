...
namespace
{
	...
	template<DataType dataType>
	void SaveImageData(GLint xoffset, GLint yoffset, GLint zoffset, GLsizei width, GLsizei height, GLsizei depth, int inputPitch, int inputHeight, int destPitch, GLsizei destHeight, const void *buffer, void *output)
	{
		for(int z = 0; z < depth; ++z)
		{
			const unsigned char *inputStart = static_cast<const unsigned char*>(buffer) + (z * inputPitch * inputHeight);
			unsigned char *destStart = static_cast<unsigned char*>(output) + ((zoffset + z) * destPitch * destHeight);
	...
	template<DataType dataType>
	void TransferImageData(bool isWriteToTexture, GLint xoffset, GLint yoffset, GLint zoffset, GLsizei width, GLsizei height, GLsizei depth, int inputPitch, int inputHeight, int destPitch, GLsizei destHeight, const void *input, void *output)
	{
		for(int z = 0; z < depth; ++z)
		{
			const unsigned char *inputStart = static_cast<const unsigned char*>(input) + ((z + (isWriteToTexture ? 0 : zoffset)) * inputPitch * inputHeight);
			unsigned char *destStart = static_cast<unsigned char*>(output) + ((z + (isWriteToTexture ? zoffset : 0)) * destPitch * destHeight);
			for(int y = 0; y < height; ++y)
			{
				if (isWriteToTexture)
				{
...
namespace egl
{
	...
	{
	    transferImageData(true, context, xoffset, yoffset, zoffset, width, height, depth, format, type, unpackInfo, const_cast<void*>(pixels));
	}
	...
	{
	    transferImageData(false, context, xoffset, yoffset, zoffset, width, height, depth, format, type, unpackInfo, pixels);
	}

	void Image::transferImageData(bool isWrite, Context *context, GLint xoffset, GLint yoffset, GLint zoffset, GLsizei width, GLsizei height, GLsizei depth, GLenum format, GLenum type, const UnpackInfo& unpackInfo, void *pixels)
	{
		...
		if(selectedInternalFormat == internalFormat)
		{
			...
			if(buffer)
			{
				const void* input = isWrite ? pixels : buffer;
				void* output = isWrite ? buffer : pixels;
				switch(type)
				{
				case GL_BYTE:
					switch(format)
					...
					case GL_LUMINANCE:
					case GL_LUMINANCE8_EXT:
						TransferImageData<Bytes_1>(isWrite, xoffset, yoffset, zoffset, width, height, depth, pixelPitch, pixelHeight, getPitch(), getHeight(), input, output);
						break;
					case GL_RG8:
					case GL_RG8I:
					case GL_RG8_SNORM:
					...
					case GL_LUMINANCE_ALPHA:
					case GL_LUMINANCE8_ALPHA8_EXT:
						TransferImageData<Bytes_2>(isWrite, xoffset, yoffset, zoffset, width, height, depth, pixelPitch, pixelHeight, getPitch(), getHeight(), input, output);
						break;
					case GL_RGB8:
					case GL_RGB8I:
					case GL_RGB8_SNORM:
					case GL_RGB:
					case GL_RGB_INTEGER:
						TransferImageData<ByteRGB>(isWrite, xoffset, yoffset, zoffset, width, height, depth, pixelPitch, pixelHeight, getPitch(), getHeight(), input, output);
						break;
					case GL_RGBA8:
					case GL_RGBA8I:
					case GL_RGBA8_SNORM:
					...
					case GL_BGRA_EXT:
					case GL_BGRA8_EXT:
						TransferImageData<Bytes_4>(isWrite, xoffset, yoffset, zoffset, width, height, depth, pixelPitch, pixelHeight, getPitch(), getHeight(), input, output);
						break;
					default: UNREACHABLE(format);
					}
					break;
				case GL_UNSIGNED_BYTE:
					...
					case GL_RGB:
					case GL_RGB_INTEGER:
						TransferImageData<UByteRGB>(isWrite, xoffset, yoffset, zoffset, width, height, depth, pixelPitch, pixelHeight, getPitch(), getHeight(), input, output);
						break;
					case GL_RGBA8:
					case GL_RGBA8UI:
					case GL_RGBA8_SNORM:
					...
					case GL_BGRA8_EXT:
						...
						break;
					case GL_SRGB8:
						TransferImageData<SRGB>(isWrite, xoffset, yoffset, zoffset, width, height, depth, pixelPitch, pixelHeight, getPitch(), getHeight(), input, output);
						break;
					case GL_SRGB8_ALPHA8:
						TransferImageData<SRGBA>(isWrite, xoffset, yoffset, zoffset, width, height, depth, pixelPitch, pixelHeight, getPitch(), getHeight(), input, output);
						break;
					default: UNREACHABLE(format);
					}
					break;
				case GL_UNSIGNED_SHORT_5_6_5:
					...
					case GL_RGB565:
					case GL_RGB:
						TransferImageData<RGB565>(isWrite, xoffset, yoffset, zoffset, width, height, depth, pixelPitch, pixelHeight, getPitch(), getHeight(), input, output);
						break;
					default: UNREACHABLE(format);
					}
					break;
				case GL_UNSIGNED_SHORT_4_4_4_4:
					...
					case GL_RGBA4:
					case GL_RGBA:
						TransferImageData<RGBA4444>(isWrite, xoffset, yoffset, zoffset, width, height, depth, pixelPitch, pixelHeight, getPitch(), getHeight(), input, output);
						break;
					default: UNREACHABLE(format);
					}
					break;
				case GL_UNSIGNED_SHORT_5_5_5_1:
					...
					case GL_RGB5_A1:
					case GL_RGBA:
						TransferImageData<RGBA5551>(isWrite, xoffset, yoffset, zoffset, width, height, depth, pixelPitch, pixelHeight, getPitch(), getHeight(), input, output);
						break;
					default: UNREACHABLE(format);
					}
					break;
				case GL_UNSIGNED_INT_10F_11F_11F_REV:
					...
					case GL_RGB:
						TransferImageData<R11G11B10F>(isWrite, xoffset, yoffset, zoffset, width, height, depth, pixelPitch, pixelHeight, getPitch(), getHeight(), input, output);
						break;
					default: UNREACHABLE(format);
					}
					break;
				case GL_UNSIGNED_INT_5_9_9_9_REV:
					...
					case GL_RGB9_E5:
					case GL_RGB:
						TransferImageData<RGB9E5>(isWrite, xoffset, yoffset, zoffset, width, height, depth, pixelPitch, pixelHeight, getPitch(), getHeight(), input, output);
						break;
					default: UNREACHABLE(format);
					}
					break;
				case GL_UNSIGNED_INT_2_10_10_10_REV:
					...
					{
					case GL_RGB10_A2UI:
						TransferImageData<RGB10A2UI>(isWrite, xoffset, yoffset, zoffset, width, height, depth, pixelPitch, pixelHeight, getPitch(), getHeight(), input, output);
						break;
					case GL_RGB10_A2:
					case GL_RGBA:
					case GL_RGBA_INTEGER:
				...
				case GL_FLOAT:
					...
					case GL_LUMINANCE_ALPHA:
					case GL_LUMINANCE_ALPHA32F_EXT:
						TransferImageData<Bytes_8>(isWrite, xoffset, yoffset, zoffset, width, height, depth, pixelPitch, pixelHeight, getPitch(), getHeight(), input, output);
						break;
					case GL_RED:
					case GL_R32F:
						TransferImageData<Bytes_4>(isWrite, xoffset, yoffset, zoffset, width, height, depth, pixelPitch, pixelHeight, getPitch(), getHeight(), input, output);
					...
					case GL_RGB:
					case GL_RGB32F:
						TransferImageData<FloatRGB>(isWrite, xoffset, yoffset, zoffset, width, height, depth, pixelPitch, pixelHeight, getPitch(), getHeight(), input, output);
						break;
					case GL_RGBA:
					case GL_RGBA32F:
						TransferImageData<Bytes_16>(isWrite, xoffset, yoffset, zoffset, width, height, depth, pixelPitch, pixelHeight, getPitch(), getHeight(), input, output);
						break;
					case GL_DEPTH_COMPONENT:
					case GL_DEPTH_COMPONENT32F:
						TransferImageData<Bytes_4>(isWrite, xoffset, yoffset, zoffset, width, height, depth, pixelPitch, pixelHeight, getPitch(), getHeight(), input, output);
				...
				case GL_HALF_FLOAT_OES:
					...
					case GL_RGB:
					case GL_RGB16F:
						TransferImageData<HalfFloatRGB>(isWrite, xoffset, yoffset, zoffset, width, height, depth, pixelPitch, pixelHeight, getPitch(), getHeight(), input, output);
						break;
					case GL_RGBA:
					case GL_RGBA16F:
						TransferImageData<Bytes_8>(isWrite, xoffset, yoffset, zoffset, width, height, depth, pixelPitch, pixelHeight, getPitch(), getHeight(), input, output);
				...
				case GL_SHORT:
					...
					case GL_RGB:
					case GL_RGB_INTEGER:
						TransferImageData<ShortRGB>(isWrite, xoffset, yoffset, zoffset, width, height, depth, pixelPitch, pixelHeight, getPitch(), getHeight(), input, output);
						break;
					case GL_RGBA16I:
					case GL_RGBA:
					case GL_RGBA_INTEGER:
				...
				case GL_UNSIGNED_SHORT:
					...
					case GL_RGB:
					case GL_RGB_INTEGER:
						TransferImageData<UShortRGB>(isWrite, xoffset, yoffset, zoffset, width, height, depth, pixelPitch, pixelHeight, getPitch(), getHeight(), input, output);
						break;
					case GL_RGBA16UI:
					case GL_RGBA:
					case GL_RGBA_INTEGER:
					...
					case GL_DEPTH_COMPONENT:
					case GL_DEPTH_COMPONENT16:
						TransferImageData<D16>(isWrite, xoffset, yoffset, zoffset, width, height, depth, pixelPitch, pixelHeight, getPitch(), getHeight(), input, output);
						break;
					default: UNREACHABLE(format);
					}
					break;
				case GL_INT:
					...
					case GL_RGB:
					case GL_RGB_INTEGER:
						TransferImageData<IntRGB>(isWrite, xoffset, yoffset, zoffset, width, height, depth, pixelPitch, pixelHeight, getPitch(), getHeight(), input, output);
						break;
					case GL_RGBA32I:
					case GL_RGBA:
					case GL_RGBA_INTEGER:
				...
				case GL_UNSIGNED_INT:
					...
					case GL_RGB:
					case GL_RGB_INTEGER:
						TransferImageData<UIntRGB>(isWrite, xoffset, yoffset, zoffset, width, height, depth, pixelPitch, pixelHeight, getPitch(), getHeight(), input, output);
						break;
					case GL_RGBA32UI:
					case GL_RGBA:
					case GL_RGBA_INTEGER:
					...
					case GL_DEPTH_COMPONENT32_OES:
					case GL_DEPTH_COMPONENT:
						TransferImageData<D32>(isWrite, xoffset, yoffset, zoffset, width, height, depth, pixelPitch, pixelHeight, getPitch(), getHeight(), input, output);
						break;
					default: UNREACHABLE(format);
					}
					break;
				case GL_UNSIGNED_INT_24_8_OES:
					transferD24S8ImageData(isWrite, xoffset, yoffset, zoffset, width, height, depth, pixelPitch, pixelHeight, input, output);
					break;
				case GL_FLOAT_32_UNSIGNED_INT_24_8_REV:
					transferD32FS8ImageData(isWrite, xoffset, yoffset, zoffset, width, height, depth, pixelPitch, pixelHeight, input, output);
					break;
				default: UNREACHABLE(type);
				}
			}
	...
	void Image::transferD24S8ImageData(bool isWriteToTexture, GLint xoffset, GLint yoffset, GLint zoffset, GLsizei width, GLsizei height, GLsizei depth, int inputPitch, int inputHeight, const void *input, void *buffer)
	{
		if (!isWriteToTexture)
		{
			UNIMPLEMENTED();
		...
		TransferImageData<D24>(isWriteToTexture, xoffset, yoffset, zoffset, width, height, depth, inputPitch, inputHeight, getPitch(), getHeight(), input, buffer);

		unsigned char *stencil = reinterpret_cast<unsigned char*>(lockStencil(0, 0, 0, sw::PUBLIC));

		if(stencil)
		{
			TransferImageData<S8>(isWriteToTexture, xoffset, yoffset, zoffset, width, height, depth, inputPitch, inputHeight, getStencilPitchB(), getHeight(), input, stencil);

			unlockStencil();
		}
	}

	void Image::transferD32FS8ImageData(bool isWriteToTexture, GLint xoffset, GLint yoffset, GLint zoffset, GLsizei width, GLsizei height, GLsizei depth, int inputPitch, int inputHeight, const void *input, void *buffer)
	{
		if (!isWriteToTexture)
		{
			UNIMPLEMENTED();
		...
		TransferImageData<D32F>(isWriteToTexture, xoffset, yoffset, zoffset, width, height, depth, inputPitch, inputHeight, getPitch(), getHeight(), input, buffer);

		unsigned char *stencil = reinterpret_cast<unsigned char*>(lockStencil(0, 0, 0, sw::PUBLIC));

		if(stencil)
		{
			TransferImageData<S24_8>(isWriteToTexture, xoffset, yoffset, zoffset, width, height, depth, inputPitch, inputHeight, getStencilPitchB(), getHeight(), input, stencil);
