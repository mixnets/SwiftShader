...
namespace
{
	...
	void TransferImageData(egl::Image::TransferType transfer, GLint xoffset, GLint yoffset, GLint zoffset, GLsizei width, GLsizei height, GLsizei depth, int inputPitch, int inputHeight, int destPitch, GLsizei destHeight, const void *input, void *output)
	{
		for(int z = 0; z < depth; ++z)
		{
			const unsigned char *inputStart = static_cast<const unsigned char*>(input) + ((z + (transfer == egl::Image::TransferType::WriteTex ? 0 : zoffset)) * inputPitch * inputHeight);
			unsigned char *destStart = static_cast<unsigned char*>(output) + ((z + (transfer == egl::Image::TransferType::WriteTex ? zoffset : 0)) * destPitch * destHeight);
			for(int y = 0; y < height; ++y)
			{
...
namespace egl
{
	...
	void Image::transferImageData(TransferType transferType, Context *context, GLint xoffset, GLint yoffset, GLint zoffset, GLsizei width, GLsizei height, GLsizei depth, GLenum format, GLenum type, const UnpackInfo& unpackInfo, void *pixels)
	{
		...
		{
			void *buffer =  lock(0, 0, transferType == TransferType::WriteTex ? sw::LOCK_WRITEONLY : sw::LOCK_READONLY);

			if(buffer)
			{
				const void* input = transferType == TransferType::WriteTex ? pixels : buffer;
				void* output = transferType == TransferType::WriteTex ? buffer : pixels;
				switch(type)
				{
				case GL_BYTE:
					switch(format)
					...
					case GL_LUMINANCE:
					case GL_LUMINANCE8_EXT:
						TransferImageData<Bytes_1>(transferType, xoffset, yoffset, zoffset, width, height, depth, pixelPitch, pixelHeight, getPitch(), getHeight(), input, output);
						break;
					case GL_RG8:
					case GL_RG8I:
					case GL_RG8_SNORM:
					...
					case GL_LUMINANCE_ALPHA:
					case GL_LUMINANCE8_ALPHA8_EXT:
						TransferImageData<Bytes_2>(transferType, xoffset, yoffset, zoffset, width, height, depth, pixelPitch, pixelHeight, getPitch(), getHeight(), input, output);
						break;
					case GL_RGB8:
					case GL_RGB8I:
					case GL_RGB8_SNORM:
					case GL_RGB:
					case GL_RGB_INTEGER:
						TransferImageData<ByteRGB>(transferType, xoffset, yoffset, zoffset, width, height, depth, pixelPitch, pixelHeight, getPitch(), getHeight(), input, output);
						break;
					case GL_RGBA8:
					case GL_RGBA8I:
					case GL_RGBA8_SNORM:
					...
					case GL_BGRA_EXT:
					case GL_BGRA8_EXT:
						TransferImageData<Bytes_4>(transferType, xoffset, yoffset, zoffset, width, height, depth, pixelPitch, pixelHeight, getPitch(), getHeight(), input, output);
						break;
					default: UNREACHABLE(format);
					}
					break;
				case GL_UNSIGNED_BYTE:
					...
					case GL_RGB:
					case GL_RGB_INTEGER:
						TransferImageData<UByteRGB>(transferType, xoffset, yoffset, zoffset, width, height, depth, pixelPitch, pixelHeight, getPitch(), getHeight(), input, output);
						break;
					case GL_RGBA8:
					case GL_RGBA8UI:
					case GL_RGBA8_SNORM:
					...
					case GL_BGRA8_EXT:
						...
						break;
					case GL_SRGB8:
						TransferImageData<SRGB>(transferType, xoffset, yoffset, zoffset, width, height, depth, pixelPitch, pixelHeight, getPitch(), getHeight(), input, output);
						break;
					case GL_SRGB8_ALPHA8:
						TransferImageData<SRGBA>(transferType, xoffset, yoffset, zoffset, width, height, depth, pixelPitch, pixelHeight, getPitch(), getHeight(), input, output);
						break;
					default: UNREACHABLE(format);
					}
					break;
				case GL_UNSIGNED_SHORT_5_6_5:
					...
					case GL_RGB565:
					case GL_RGB:
						TransferImageData<RGB565>(transferType, xoffset, yoffset, zoffset, width, height, depth, pixelPitch, pixelHeight, getPitch(), getHeight(), input, output);
						break;
					default: UNREACHABLE(format);
					}
					break;
				case GL_UNSIGNED_SHORT_4_4_4_4:
					...
					case GL_RGBA4:
					case GL_RGBA:
						TransferImageData<RGBA4444>(transferType, xoffset, yoffset, zoffset, width, height, depth, pixelPitch, pixelHeight, getPitch(), getHeight(), input, output);
						break;
					default: UNREACHABLE(format);
					}
					break;
				case GL_UNSIGNED_SHORT_5_5_5_1:
					...
					case GL_RGB5_A1:
					case GL_RGBA:
						TransferImageData<RGBA5551>(transferType, xoffset, yoffset, zoffset, width, height, depth, pixelPitch, pixelHeight, getPitch(), getHeight(), input, output);
						break;
					default: UNREACHABLE(format);
					}
					break;
				case GL_UNSIGNED_INT_10F_11F_11F_REV:
					...
					case GL_RGB:
						TransferImageData<R11G11B10F>(transferType, xoffset, yoffset, zoffset, width, height, depth, pixelPitch, pixelHeight, getPitch(), getHeight(), input, output);
						break;
					default: UNREACHABLE(format);
					}
					break;
				case GL_UNSIGNED_INT_5_9_9_9_REV:
					...
					case GL_RGB9_E5:
					case GL_RGB:
						TransferImageData<RGB9E5>(transferType, xoffset, yoffset, zoffset, width, height, depth, pixelPitch, pixelHeight, getPitch(), getHeight(), input, output);
						break;
					default: UNREACHABLE(format);
					}
					break;
				case GL_UNSIGNED_INT_2_10_10_10_REV:
					...
					{
					case GL_RGB10_A2UI:
						TransferImageData<RGB10A2UI>(transferType, xoffset, yoffset, zoffset, width, height, depth, pixelPitch, pixelHeight, getPitch(), getHeight(), input, output);
						break;
					case GL_RGB10_A2:
					case GL_RGBA:
					case GL_RGBA_INTEGER:
				...
				case GL_FLOAT:
					...
					case GL_LUMINANCE_ALPHA:
					case GL_LUMINANCE_ALPHA32F_EXT:
						TransferImageData<Bytes_8>(transferType, xoffset, yoffset, zoffset, width, height, depth, pixelPitch, pixelHeight, getPitch(), getHeight(), input, output);
						break;
					case GL_RED:
					case GL_R32F:
						TransferImageData<Bytes_4>(transferType, xoffset, yoffset, zoffset, width, height, depth, pixelPitch, pixelHeight, getPitch(), getHeight(), input, output);
					...
					case GL_RGB:
					case GL_RGB32F:
						TransferImageData<FloatRGB>(transferType, xoffset, yoffset, zoffset, width, height, depth, pixelPitch, pixelHeight, getPitch(), getHeight(), input, output);
						break;
					case GL_RGBA:
					case GL_RGBA32F:
						TransferImageData<Bytes_16>(transferType, xoffset, yoffset, zoffset, width, height, depth, pixelPitch, pixelHeight, getPitch(), getHeight(), input, output);
						break;
					case GL_DEPTH_COMPONENT:
					case GL_DEPTH_COMPONENT32F:
						TransferImageData<Bytes_4>(transferType, xoffset, yoffset, zoffset, width, height, depth, pixelPitch, pixelHeight, getPitch(), getHeight(), input, output);
				...
				case GL_HALF_FLOAT_OES:
					...
					case GL_RGB:
					case GL_RGB16F:
						TransferImageData<HalfFloatRGB>(transferType, xoffset, yoffset, zoffset, width, height, depth, pixelPitch, pixelHeight, getPitch(), getHeight(), input, output);
						break;
					case GL_RGBA:
					case GL_RGBA16F:
						TransferImageData<Bytes_8>(transferType, xoffset, yoffset, zoffset, width, height, depth, pixelPitch, pixelHeight, getPitch(), getHeight(), input, output);
				...
				case GL_SHORT:
					...
					case GL_RGB:
					case GL_RGB_INTEGER:
						TransferImageData<ShortRGB>(transferType, xoffset, yoffset, zoffset, width, height, depth, pixelPitch, pixelHeight, getPitch(), getHeight(), input, output);
						break;
					case GL_RGBA16I:
					case GL_RGBA:
					case GL_RGBA_INTEGER:
				...
				case GL_UNSIGNED_SHORT:
					...
					case GL_RGB:
					case GL_RGB_INTEGER:
						TransferImageData<UShortRGB>(transferType, xoffset, yoffset, zoffset, width, height, depth, pixelPitch, pixelHeight, getPitch(), getHeight(), input, output);
						break;
					case GL_RGBA16UI:
					case GL_RGBA:
					case GL_RGBA_INTEGER:
					...
					case GL_DEPTH_COMPONENT:
					case GL_DEPTH_COMPONENT16:
						TransferImageData<D16>(transferType, xoffset, yoffset, zoffset, width, height, depth, pixelPitch, pixelHeight, getPitch(), getHeight(), input, output);
						break;
					default: UNREACHABLE(format);
					}
					break;
				case GL_INT:
					...
					case GL_RGB:
					case GL_RGB_INTEGER:
						TransferImageData<IntRGB>(transferType, xoffset, yoffset, zoffset, width, height, depth, pixelPitch, pixelHeight, getPitch(), getHeight(), input, output);
						break;
					case GL_RGBA32I:
					case GL_RGBA:
					case GL_RGBA_INTEGER:
				...
				case GL_UNSIGNED_INT:
					...
					case GL_RGB:
					case GL_RGB_INTEGER:
						TransferImageData<UIntRGB>(transferType, xoffset, yoffset, zoffset, width, height, depth, pixelPitch, pixelHeight, getPitch(), getHeight(), input, output);
						break;
					case GL_RGBA32UI:
					case GL_RGBA:
					case GL_RGBA_INTEGER:
					...
					case GL_DEPTH_COMPONENT32_OES:
					case GL_DEPTH_COMPONENT:
						TransferImageData<D32>(transferType, xoffset, yoffset, zoffset, width, height, depth, pixelPitch, pixelHeight, getPitch(), getHeight(), input, output);
						break;
					default: UNREACHABLE(format);
					}
					break;
