...
namespace
{
	...
	void TransferImageRow<ByteRGB>(egl::Image::TransferType transfer, const unsigned char *source, unsigned char *dest, GLint xoffset, GLsizei width)
	{
		...
		{
			dest[destChannels * x + 0] = source[x * sourceChannels + 0];
			dest[destChannels * x + 1] = source[x * sourceChannels + 1];
			dest[destChannels * x + 2] = source[x * sourceChannels + 2];
			if (isWrite)
			{
				dest[destChannels * x + 3] = 0x7F;
			}
		}
	}
	...
	void TransferImageRow<ShortRGB>(egl::Image::TransferType transfer, const unsigned char *source, unsigned char *dest, GLint xoffset, GLsizei width)
	{
		...
		for(int x = 0; x < width; x++)
		{
			destS[destChannels * x + 0] = sourceS[x * sourceChannels + 0];
			destS[destChannels * x + 1] = sourceS[x * sourceChannels + 1];
			destS[destChannels * x + 2] = sourceS[x * sourceChannels + 2];
			if (isWrite)
			{
				destS[destChannels * x + 3] = 0x7FFF;
			}
		}
	}
	...
	{
		const bool isWrite = transfer == egl::Image::TransferType::WriteTex;
		const int *sourceI = reinterpret_cast<const int*>(source);
		int *destI = reinterpret_cast<int*>(dest);
		unsigned int sourceChannels;
		unsigned int destChannels;
		if (isWrite)
		{
		...
		for(int x = 0; x < width; x++)
		{
			destI[destChannels * x + 0] = sourceI[x * sourceChannels + 0];
			destI[destChannels * x + 1] = sourceI[x * sourceChannels + 1];
			destI[destChannels * x + 2] = sourceI[x * sourceChannels + 2];
			if (isWrite)
			{
				destI[destChannels * x + 3] = 0x7FFFFFFF;
			}
		}
	}
	...
	void TransferImageRow<UIntRGB>(egl::Image::TransferType transfer, const unsigned char *source, unsigned char *dest, GLint xoffset, GLsizei width)
	{
		...
		for(int x = 0; x < width; x++)
		{
			...
			if (isWrite)
			{
				destI[destChannels * x + 3] = 0xFFFFFFFF;
			}
		}
	}
	...
	void TransferImageRow<FloatRGB>(egl::Image::TransferType transfer, const unsigned char *source, unsigned char *dest, GLint xoffset, GLsizei width)
	{
		...
		for(int x = 0; x < width; x++)
		{
			destF[destChannels * x + 0] = sourceF[x * sourceChannels + 0];
			destF[destChannels * x + 1] = sourceF[x * sourceChannels + 1];
			destF[destChannels * x + 2] = sourceF[x * sourceChannels + 2];
			if (isWrite)
			{
				destF[destChannels * x + 3] = 1.0f;
			}
		}
	}
	...
	void TransferImageRow<HalfFloatRGB>(egl::Image::TransferType transfer, const unsigned char *source, unsigned char *dest, GLint xoffset, GLsizei width)
	{
		...
		for(int x = 0; x < width; x++)
		{
			destH[destChannels * x + 0] = sourceH[x * sourceChannels + 0];
			destH[destChannels * x + 1] = sourceH[x * sourceChannels + 1];
			destH[destChannels * x + 2] = sourceH[x * sourceChannels + 2];
			if (isWrite)
			{
				destH[destChannels * x + 3] = 0x3C00; // SEEEEEMMMMMMMMMM, S = 0, E = 15, M = 0: 16bit flpt representation of 1
			}
		}
	}
