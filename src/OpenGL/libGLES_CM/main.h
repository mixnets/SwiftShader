...
{
public:
	static LibGLES_CM *getSingleton();

	virtual egl::Context *createContext(const egl::Config *config, const egl::Context *shareCOntext);
	virtual __eglMustCastToProperFunctionPointerType getProcAddress(const char *proc);

	virtual egl::Image *createBackBuffer(int width, int height, const egl::Config *config);
	virtual egl::Image *createDepthStencil(unsigned int, unsigned int, sw::Format format, int samples, bool );
	virtual sw::FrameBuffer *createFrameBuffer(EGLNativeDisplayType display, EGLNativeWindowType window, int width, int height);
