...
namespace es1
{
	...
	void Device::clearDepth(float z)
	{
		...
		depthBuffer->clearDepthBuffer(z, clearRect.x0, clearRect.y0, clearRect.width(), clearRect.height());
	}
	...
	void Device::clearStencil(unsigned int stencil, unsigned int mask)
	{
		...
		stencilBuffer->clearStencilBuffer(stencil, mask, clearRect.x0, clearRect.y0, clearRect.width(), clearRect.height());
	}
