...
bool Texture::copy(egl::Image *source, const sw::SliceRect &sourceRect, GLenum destFormat, GLint xoffset, GLint yoffset, GLint zoffset, egl::Image *dest)
{
    ...
    sw::SliceRect destRect = {xoffset, yoffset, xoffset + (sourceRect.rect.x1 - sourceRect.rect.x0), yoffset + (sourceRect.rect.y1 - sourceRect.rect.y0), zoffset};
...
void Texture2D::copyImage(GLint level, GLenum format, GLint x, GLint y, GLsizei width, GLsizei height, Framebuffer *source)
{
    ...
    if(width != 0 && height != 0)
    {
		...
		sourceRect.rect.clip(0, 0, source->getColorbuffer()->getWidth(), source->getColorbuffer()->getHeight());
...
void Texture3D::copyImage(GLint level, GLenum format, GLint x, GLint y, GLint z, GLsizei width, GLsizei height, GLsizei depth, Framebuffer *source)
{
	...
	{
		for(GLint sliceZ = 0; sliceZ < depth; ++sliceZ)
		{
			sw::SliceRect sourceRect = {x, y, x + width, y + height, z + sliceZ};
			sourceRect.rect.clip(0, 0, source->getColorbuffer()->getWidth(), source->getColorbuffer()->getHeight());
