...
bool Texture::copy(egl::Image *source, const sw::SliceRect &sourceRect, GLenum destFormat, GLint xoffset, GLint yoffset, GLint zoffset, egl::Image *dest)
{
	...
	bool success = device->stretchRect(source->get(), &sourceRect, dest->get(), &destRect, Device::ALL_BUFFERS);
...
void Texture2D::setImage(GLint level, GLsizei width, GLsizei height, GLenum format, GLenum type, const egl::Image::UnpackInfo& unpackInfo, const void *pixels)
{
	...
	image[level] = 0;//////new egl::Image(this, width, height, format, type);
...
void Texture2D::setCompressedImage(GLint level, GLenum format, GLsizei width, GLsizei height, GLsizei imageSize, const void *pixels)
{
	...
	image[level] = 0;//////new egl::Image(this, width, height, sizedInternalFormat, GL_UNSIGNED_BYTE);
(void)sizedInternalFormat;
	if(!image[level])
	{
		return error(GL_OUT_OF_MEMORY);
	}
...
void Texture2D::generateMipmaps()
{
	...
	for(unsigned int i = 1; i <= q; i++)
	{
		...
		image[i] = 0;/////new egl::Image(this, std::max(image[0]->getWidth() >> i, 1), std::max(image[0]->getHeight() >> i, 1), image[0]->getFormat(), image[0]->getType());
		...
		getDevice()->stretchRect(image[i - 1]->get(), 0, image[i]->get(), 0, Device::ALL_BUFFERS | Device::USE_FILTER);
	}
}
...
void TextureCubeMap::setCompressedImage(GLenum target, GLint level, GLenum format, GLsizei width, GLsizei height, GLsizei imageSize, const void *pixels)
{
	...
	GLenum sizedInternalFormat = GetSizedInternalFormat(format, GL_UNSIGNED_BYTE);
	image[face][level] = 0;////new egl::Image(this, width, height, sizedInternalFormat, GL_UNSIGNED_BYTE);
(void)sizedInternalFormat;
	if(!image[face][level])
	{
		return error(GL_OUT_OF_MEMORY);
...
void TextureCubeMap::setImage(GLenum target, GLint level, GLsizei width, GLsizei height, GLenum format, GLenum type, const egl::Image::UnpackInfo& unpackInfo, const void *pixels)
{
	...
	image[face][level] = 0;////new egl::Image(this, width, height, format, type);
...
void TextureCubeMap::generateMipmaps()
{
	...
	for(unsigned int f = 0; f < 6; f++)
	{
		for(unsigned int i = 1; i <= q; i++)
		{
			...
			image[f][i] = 0;//new egl::Image(this, std::max(image[0][0]->getWidth() >> i, 1), std::max(image[0][0]->getHeight() >> i, 1), image[0][0]->getFormat(), image[0][0]->getType());
			...
			getDevice()->stretchRect(image[f][i - 1]->get(), 0, image[f][i]->get(), 0, Device::ALL_BUFFERS | Device::USE_FILTER);
		}
	}
}
...
void Texture3D::setImage(GLint level, GLsizei width, GLsizei height, GLsizei depth, GLenum format, GLenum type, const egl::Image::UnpackInfo& unpackInfo, const void *pixels)
{
	...
	image[level] = 0;////new egl::Image(this, width, height, depth, format, type);
...
void Texture3D::setCompressedImage(GLint level, GLenum format, GLsizei width, GLsizei height, GLsizei depth, GLsizei imageSize, const void *pixels)
{
	...
	GLenum sizedInternalFormat = GetSizedInternalFormat(format, GL_UNSIGNED_BYTE);
	image[level] = 0;///new egl::Image(this, width, height, depth, sizedInternalFormat, GL_UNSIGNED_BYTE);
(void)sizedInternalFormat;
	if(!image[level])
	{
		return error(GL_OUT_OF_MEMORY);
...
void Texture3D::generateMipmaps()
{
	...
	for(unsigned int i = 1; i <= q; i++)
	{
		...
		image[i] = 0;//new egl::Image(this, std::max(image[0]->getWidth() >> i, 1), std::max(image[0]->getHeight() >> i, 1), std::max(image[0]->getDepth() >> i, 1), image[0]->getFormat(), image[0]->getType());
		...
		getDevice()->stretchCube(image[i - 1]->get(), image[i]->get());
	}
}
...
void Texture2DArray::generateMipmaps()
{
	...
	for(unsigned int i = 1; i <= q; i++)
	{
		...
		image[i] = 0;//new egl::Image(this, w, h, depth, image[0]->getFormat(), image[0]->getType());
		...
		for(int z = 0; z < depth; ++z)
		{
			...
			getDevice()->stretchRect(image[i - 1]->get(), &srcRect, image[i]->get(), &dstRect, Device::ALL_BUFFERS | Device::USE_FILTER);
		}
	}
}
...
egl::Image *createBackBuffer(int width, int height, const egl::Config *config)
{
	if(config)
	{
		return 0;//new egl::Image(width, height, config->mRenderTargetFormat, config->mSamples, false);
	}
...
//	case sw::FORMAT_D32_LOCKABLE:
	...
	egl::Image *surface = 0;///new egl::Image(width, height, format, multiSampleDepth, lockable);
