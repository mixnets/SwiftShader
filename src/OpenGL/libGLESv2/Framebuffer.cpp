...
GLenum Framebuffer::getImplementationColorReadType(int clientVersion) const
{
	Renderbuffer *colorbuffer = getReadColorbuffer();
	...
	if(colorbuffer)
	{
		...
		case GL_R16F:           return clientVersion >= 3 ? GL_HALF_FLOAT : GL_HALF_FLOAT_OES;
		case GL_RG16F:          return clientVersion >= 3 ? GL_HALF_FLOAT : GL_HALF_FLOAT_OES;
		case GL_R11F_G11F_B10F: return GL_UNSIGNED_INT_10F_11F_11F_REV;
		case GL_RGB16F:         return clientVersion >= 3 ? GL_HALF_FLOAT : GL_HALF_FLOAT_OES;
		case GL_RGBA16F:        return clientVersion >= 3 ? GL_HALF_FLOAT : GL_HALF_FLOAT_OES;
