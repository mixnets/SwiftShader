...
GLuint Context::createTransformFeedback()
{
	...
	mTransformFeedbackMap[handle] = nullptr;
...
TransformFeedback *Context::getTransformFeedback(GLuint transformFeedback) const
{
	...
	return (transformFeedbackObject == mTransformFeedbackMap.end()) ? nullptr : transformFeedbackObject->second;
}
                              ...
                              GLbitfield mask)
{
	...
	if(partialBufferCopy && (blitDepthBuffer || blitStencilBuffer))
	{
		ERR("Only whole-buffer depth and stencil blits are supported by this implementation.");
		return error(GL_INVALID_OPERATION);   // Only whole-buffer copies are permitted
	}

    if(blitRenderTarget || blitDepthBuffer || blitStencilBuffer)
    {
		if(flipX) swap(destRect.x0, destRect.x1);
		if(flipy) swap(destRect.y0, destRect.y1);
        ...
        if(blitDepthBuffer)
        {
			egl::Image *readDepthBuffer = readFramebuffer->getDepthBuffer();
            egl::Image *drawDepthBuffer = drawFramebuffer->getDepthBuffer();

            bool success = device->stretchRect(readDepthBuffer, nullptr, drawDepthBuffer, nullptr, false);

			readDepthBuffer->release();
			drawDepthBuffer->release();
        ...
        {
			egl::Image *readStencilBuffer = readFramebuffer->getStencilBuffer();
            egl::Image *drawStencilBuffer = drawFramebuffer->getStencilBuffer();

            bool success = device->stretchRect(readStencilBuffer, nullptr, drawStencilBuffer, nullptr, false);

			readStencilBuffer->release();
			drawStencilBuffer->release();
