...
void Context::clearDepthBuffer(GLint drawbuffer, const GLfloat *value)
{
	if(device && mState.depthMask && !mState.rasterizerDiscardEnabled)
	{
		...
		depthbuffer->clearDepthBuffer(depth, clearRect.x0, clearRect.y0, clearRect.width(), clearRect.height());
...
void Context::clearStencilBuffer(GLint drawbuffer, const GLint *value)
{
	if(device && mState.stencilWritemask && !mState.rasterizerDiscardEnabled)
	{
		...
		stencilbuffer->clearStencilBuffer(stencil, static_cast<unsigned char>(mState.stencilWritemask), clearRect.x0, clearRect.y0, clearRect.width(), clearRect.height());
...
void Context::clearDepthStencilBuffer(GLint drawbuffer, GLfloat depth, GLint stencil)
{
	if(device && (mState.depthMask || mState.stencilWritemask) && !mState.rasterizerDiscardEnabled)
	{
		...
		{
			depthstencil->clearStencilBuffer(stencil, static_cast<unsigned char>(mState.stencilWritemask), clearRect.x0, clearRect.y0, clearRect.width(), clearRect.height());
		}

		if(mState.depthMask)
		{
			depthstencil->clearDepthBuffer(depth, clearRect.x0, clearRect.y0, clearRect.width(), clearRect.height());
		}
