...
GLenum VertexDataManager::prepareVertexData(GLint start, GLsizei count, TranslatedAttribute *translated)
{
	...
	const VertexArray *vertexArray = mContext->getCurrentVertexArray();
        ...
        if(program->getAttributeStream(i) != -1)
        {
            if(attrib.mArrayEnabled)
            {
                ...
                if(staticBuffer)
                {
					...
					translated[i].offset = start * attrib.stride() + attrib.mOffset;
					translated[i].stride = attrib.stride();
                }
                else
                {
                    unsigned int streamOffset = writeAttributeData(mStreamingBuffer, start, count, attrib);
					...
					translated[i].stride = attrib.typeSize();
                }
