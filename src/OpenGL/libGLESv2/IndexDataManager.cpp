...
int recomputePrimitiveCount(GLenum mode, GLsizei count, std::vector<GLsizei>& restartIndices, unsigned int* primitiveCount)
{
	size_t numRestarts = restartIndices.size();
	*primitiveCount = 0;
...
GLenum IndexDataManager::prepareIndexData(GLenum mode, GLenum type, GLuint start, GLuint end, GLsizei count, Buffer *buffer, const void *indices, TranslatedIndexData *translated, bool primitiveRestart)
{
	...
	std::vector<GLsizei> restartIndices;
	computeRange(type, indices, count, &translated->minIndex, &translated->maxIndex, primitiveRestart ? &restartIndices : nullptr);
