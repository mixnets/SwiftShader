...
void computeRange(const IndexType *indices, GLsizei count, GLuint *minIndex, GLuint *maxIndex, bool primitiveRestart, const IndexType primitiveRestartIndex)
{
	if(!primitiveRestart)
	{
		*minIndex = indices[0];
	...
	{
		if(primitiveRestart && indices[i] == primitiveRestartIndex)
		{
			continue;
		}
...
void computeRange(GLenum type, const void *indices, GLsizei count, GLuint *minIndex, GLuint *maxIndex, bool primitiveRestart)
{
	if(type == GL_UNSIGNED_BYTE)
	{
		computeRange(static_cast<const GLubyte*>(indices), count, minIndex, maxIndex, primitiveRestart, GLubyte(-1));
	}
	else if(type == GL_UNSIGNED_INT)
	{
		computeRange(static_cast<const GLuint*>(indices), count, minIndex, maxIndex, primitiveRestart, GLuint(-1));
	}
	else if(type == GL_UNSIGNED_SHORT)
	{
		computeRange(static_cast<const GLushort*>(indices), count, minIndex, maxIndex, primitiveRestart, GLushort(-1));
	}
	else UNREACHABLE(type);
}
