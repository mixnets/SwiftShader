                ...
                GLint border, GLenum format, GLenum type, const GLvoid* data)
{
	...
	if(context)
	{
		...
		if(target == GL_TEXTURE_2D)
		{
			...
			texture->setImage(level, width, height, sizedInternalFormat, type, context->getUnpackInfo(), data);
		}
		else
		{
			es2::TextureCubeMap *texture = context->getTextureCubeMap();
			...
			texture->setImage(target, level, width, height, sizedInternalFormat, type, context->getUnpackInfo(), data);
		}
	}
}
                   ...
                   GLenum format, GLenum type, const GLvoid* data)
{
	...
	if(context)
	{
		if(target == GL_TEXTURE_2D)
		{
			...
			texture->subImage(level, xoffset, yoffset, width, height, format, type, context->getUnpackInfo(), data);
		}
		else if(es2::IsCubemapTextureTarget(target))
		{
			es2::TextureCubeMap *texture = context->getTextureCubeMap();
			...
			texture->subImage(target, level, xoffset, yoffset, width, height, format, type, context->getUnpackInfo(), data);
		}
		else UNREACHABLE(target);
	}
}
...
void TexSubImage3DOES(GLenum target, GLint level, GLint xoffset, GLint yoffset, GLint zoffset, GLsizei width, GLsizei height, GLsizei depth, GLenum format, GLenum type, const void *data)
{
	...
	if(context)
	{
		...
		texture->subImage(level, xoffset, yoffset, zoffset, width, height, depth, format, type, context->getUnpackInfo(), data);
	}
}
