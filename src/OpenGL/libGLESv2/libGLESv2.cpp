...
static bool validateColorBufferFormat(GLenum textureFormat, GLenum colorbufferFormat)
{
	...
	case GL_DEPTH_STENCIL_OES:
		return error(GL_INVALID_OPERATION), false;
	default:
		return error(GL_INVALID_ENUM, textureFormat), false;
	}
	return true;
}
...
void AttachShader(GLuint program, GLuint shader)
{
	...
	if(context)
	{
		...
		if(!programObject)
		{
			...
			{
				return error(GL_INVALID_VALUE, program);
			}
		}
		...
		if(!shaderObject)
		{
			...
			{
				return error(GL_INVALID_VALUE, shader);
			}
		}
...
void BindBuffer(GLenum target, GLuint buffer)
{
	...
	if(context)
	{
		...
		case GL_COPY_READ_BUFFER:
			...
			else return error(GL_INVALID_ENUM, target);
...
GLuint CreateShader(GLenum type)
{
	...
	if(context)
	{
		...
		default:
			return error(GL_INVALID_ENUM, type), 0;
		}
	}
...
void FramebufferRenderbuffer(GLenum target, GLenum attachment, GLenum renderbuffertarget, GLuint renderbuffer)
{
	...
	if(context)
	{
		...
		case GL_DEPTH_STENCIL_ATTACHMENT:
			...
			else return error(GL_INVALID_ENUM, attachment);
		default:
			return error(GL_INVALID_ENUM, attachment);
		}
	}
...
void GetActiveAttrib(GLuint program, GLuint index, GLsizei bufsize, GLsizei *length, GLint *size, GLenum *type, GLchar *name)
{
	...
	if(bufsize < 0)
	{
		return error(GL_INVALID_VALUE, bufsize);
	}
...
void GetAttachedShaders(GLuint program, GLsizei maxcount, GLsizei* count, GLuint* shaders)
{
	...
	if(maxcount < 0)
	{
		return error(GL_INVALID_VALUE, maxcount);
	}
...
int GetAttribLocation(GLuint program, const GLchar* name)
{
	...
	if(context)
	{
		...
		if(!programObject)
		{
			...
			{
				return error(GL_INVALID_VALUE, program), -1;
			}
		}
...
void GetBufferParameteriv(GLenum target, GLenum pname, GLint* params)
{
	...
	if(context)
	{
		...
		case GL_BUFFER_ACCESS_FLAGS:
			...
			else return error(GL_INVALID_ENUM, pname);
...
void GetShaderPrecisionFormat(GLenum shadertype, GLenum precisiontype, GLint* range, GLint* precision)
{
	...
	case GL_FRAGMENT_SHADER:
		break;
	default:
		return error(GL_INVALID_ENUM, shadertype);
	}
	...
	case GL_HIGH_INT:
		...
		*precision = 0;
		break;
	default:
		return error(GL_INVALID_ENUM, precisiontype);
	}
}
...
void GetVertexAttribfv(GLuint index, GLenum pname, GLfloat* params)
{
	...
	if(context)
	{
		...
		default: return error(GL_INVALID_ENUM, pname);
		}
	}
}
...
GLboolean IsEnabled(GLenum cap)
{
	...
	if(context)
	{
		...
		case GL_PRIMITIVE_RESTART_FIXED_INDEX:
			...
			else return error(GL_INVALID_ENUM, cap), false;
		...
		default:
			return error(GL_INVALID_ENUM, cap), false;
		}
	}
...
void RenderbufferStorageMultisample(GLenum target, GLsizei samples, GLenum internalformat, GLsizei width, GLsizei height)
{
	...
	if(context)
	{
		...
		else error(GL_INVALID_ENUM, internalformat);
	}
}
...
void SetFenceNV(GLuint fence, GLenum condition)
{
	...
	{
		return error(GL_INVALID_ENUM, condition);
	}
...
void ShaderBinary(GLsizei n, const GLuint* shaders, GLenum binaryformat, const GLvoid* binary, GLsizei length)
{
	...
	return error(GL_INVALID_ENUM, binaryformat);
}
...
void UniformMatrix2fv(GLint location, GLsizei count, GLboolean transpose, const GLfloat* value)
{
	...
	if(context)
	{
		...
		{
			return error(GL_INVALID_VALUE, transpose);
		}
...
void VertexAttribPointer(GLuint index, GLint size, GLenum type, GLboolean normalized, GLsizei stride, const GLvoid* ptr)
{
	...
	case GL_UNSIGNED_INT_2_10_10_10_REV:
		...
		else return error(GL_INVALID_ENUM, type);
	case GL_INT:
	case GL_UNSIGNED_INT:
	case GL_HALF_FLOAT:
		if(clientVersion >= 3)
...
static void BlitFramebuffer(GLint srcX0, GLint srcY0, GLint srcX1, GLint srcY1, GLint dstX0, GLint dstY0, GLint dstX1, GLint dstY1, GLbitfield mask, GLenum filter, bool allowPartialDepthStencilBlit)
{
	...
	case GL_NEAREST:
		break;
	default:
		return error(GL_INVALID_ENUM, filter);
	}
	...
	if(context)
	{
		...
		for(unsigned int i = 0; i < (unsigned)n; i++)
		{
			...
			case GL_COLOR_ATTACHMENT15_EXT:
				...
				break;
			default:
				return error(GL_INVALID_ENUM, bufs[i]);
			}
		}
