...
namespace es2
{
	...
	bool Program::linkValidateTransformFeedback()
	{
			...
			for(const glsl::Varying varying : vertexShader->varyings)
			{
				if(tfVaryingName == varying.name)
				{
					...
					if(transformFeedbackBufferMode == GL_SEPARATE_ATTRIBS &&
					   componentCount > MAX_TRANSFORM_FEEDBACK_SEPARATE_COMPONENTS)
					{
						appendToInfoLog("Transform feedback varying's %s components (%d) exceed the maximum separate components (%d).",
						                varying.name.c_str(), componentCount, MAX_TRANSFORM_FEEDBACK_SEPARATE_COMPONENTS);
						return false;
					}
		...
		if(transformFeedbackBufferMode == GL_INTERLEAVED_ATTRIBS &&
		   totalComponents > MAX_TRANSFORM_FEEDBACK_INTERLEAVED_COMPONENTS)
		{
	...
	void Program::link()
	{
		...
		if(!linkValidateTransformFeedback())
		{
			return;
		}
