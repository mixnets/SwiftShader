		...
		for(const auto &uniform : shader->activeUniforms)
		{
			...
			if(uniform.blockId >= 0)
			{
				...
				if(activeUniformBlocks[uniform.blockId].dataSize > MAX_UNIFORM_BLOCK_SIZE)
				{
					if(shader->getType() == GL_VERTEX_SHADER)
					{
						appendToInfoLog("Vertex shader active uniform block (%s) exceeds GL_MAX_UNIFORM_BLOCK_SIZE (%d)", uniformBlockName, MAX_UNIFORM_BLOCK_SIZE);
						return false;
					}
					else if(shader->getType() == GL_FRAGMENT_SHADER)
					{
						appendToInfoLog("Fragment shader active uniform block (%s) exceeds GL_MAX_UNIFORM_BLOCK_SIZE (%d)", uniformBlockName, MAX_UNIFORM_BLOCK_SIZE);
						return false;
					}
					else UNREACHABLE(shader->getType());
				}
