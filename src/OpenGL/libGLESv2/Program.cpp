...
namespace es2
{
	...
	bool Program::linkTransformFeedback()
	{
		...
		for(const std::string &indexedTfVaryingName : transformFeedbackVaryings)
		{
			...
			std::string tfVaryingName = es2::ParseUniformName(indexedTfVaryingName, &arrayIndex);
			bool hasSubscript = (arrayIndex != GL_INVALID_INDEX);
			int subscript = static_cast<int>(arrayIndex);
