...
void GL_APIENTRY glReadBuffer(GLenum src)
{
	...
	case GL_COLOR_ATTACHMENT15:
		if(context)
		{
			...
			if(context->getIntegerv(GL_MAX_COLOR_ATTACHMENTS, &maxColorAttachments))
			{
				int attachment = src - GL_COLOR_ATTACHMENT0;
...
{
	TRACE("(GLenum pname = 0x%X, GLuint index = %d, GLint* params = 0x%0.8p)",
		  target, index, data);
