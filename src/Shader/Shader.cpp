...
namespace sw
{
	...
	void Shader::analyzeLimits()
	{
		...
		struct FunctionInfo
		{
			...
			std::vector<FunctionID> calls; // What this function calls.
		...
		for(size_t i = 0; i < instruction.size(); i++)
		{
			...
			switch (inst->opcode)
			{
				...
				case OPCODE_CALLNZ:
				{
					...
					FunctionID id = inst->dst.label;
					functions[currentFunc].calls.push_back(id);
		...
		// stripped in earlier stages=). Unreachable functions may be
		// code generated, but their own limits are not considered below,
		// potentially causing OOB indexing in later stages.
		...
		traverse = [&](FunctionID id) -> Limits
		{
			...
			{
				auto calleeLi = traverse(callee);
				limits.loops = std::max(limits.loops, calleeLi.loops);
				limits.ifs = std::max(limits.ifs, calleeLi.ifs);
				limits.stack = std::max(limits.stack, calleeLi.stack);
			}
			visited.erase(id);

			limits.loops += func.limits.loops;
			limits.ifs += func.limits.ifs;
			limits.stack += func.limits.stack;
			return limits;
		};
