...
namespace sw
{
	...
	void PixelProgram::sampleTexture(Vector4f &c, const Src &sampler, Float4 &u, Float4 &v, Float4 &w, Float4 &q, Vector4f &dsx, Vector4f &dsy, Vector4f &offset, SamplerMethod method, unsigned int function)
	{
		Vector4f tmp;

		if(sampler.type == Shader::PARAMETER_SAMPLER && sampler.rel.type == Shader::PARAMETER_VOID)
		{
			sampleTexture(tmp, sampler.index, u, v, w, q, dsx, dsy, offset, method, function);
		}
		else
		{
			...
			for(int i = 0; i < TEXTURE_IMAGE_UNITS; i++)
			{
				if(shader->usesSampler(i))
				{
					If(index == i)
					{
						sampleTexture(tmp, i, u, v, w, q, dsx, dsy, offset, method, function);
	...
	void PixelProgram::sampleTexture(Vector4f &c, int samplerIndex, Float4 &u, Float4 &v, Float4 &w, Float4 &q, Vector4f &dsx, Vector4f &dsy, Vector4f &offset, SamplerMethod method, unsigned int function)
	{
		#if PERF_PROFILE
			Long texTime = Ticks();
		#endif
		...
		if((function & Project) == 0)
		{
			sampler[samplerIndex]->sampleTexture(texture, c, u, v, w, q, dsx, dsy, offset, function, method);
		}
		else
		{
			...
			sampler[samplerIndex]->sampleTexture(texture, c, u_q, v_q, w_q, q, dsx, dsy, offset, function, method);
		}
