...
namespace sw
{
	...
	void PixelProgram::applyShader(Int cMask[4])
	{
		...
		{
			if(word.size() < 5 + 1)   // Header and first inststruction word.
			{
				assert(false);
				return;
			}
			...
			assert(word[4] == 0);   // Reserved (instruction schema).
		}
	...
	breakWhile3:
		...
		//	spv::Op op = opcode(word[i]);
			...
			//	spv::Op op = opcode(word[i]);
				...
				assert(wordCount(word[i - 1]) == 1);   // OpFunctionEnd is a single word.
			}
		}
