...
namespace sw
{
		...
		for(size_t i = 0; i < shader->getLength(); i++)
		{
			...
			if(dst.type != Shader::PARAMETER_VOID && dst.type != Shader::PARAMETER_LABEL && opcode != Shader::OPCODE_TEXKILL && opcode != Shader::OPCODE_NOP)
			{
				...
				if(instruction->isPredicated())
				{
					...
					case Shader::PARAMETER_COLOROUT:
						...
						else
						{
							...
							if(dst.x) pDst.x = oC[dst.index + a].x;
							if(dst.y) pDst.y = oC[dst.index + a].y;
							if(dst.z) pDst.z = oC[dst.index + a].z;
							if(dst.w) pDst.w = oC[dst.index + a].w;
						}
						break;
				...
				case Shader::PARAMETER_COLOROUT:
					...
					{
						broadcastColor0 = false;
						Int4 a = gatherAddress(dst.rel) + dst.index;
	...
	Vector4f PixelProgram::fetchRegister(const Src &src, unsigned int offset)
	{
		...
		case Shader::PARAMETER_COLOROUT:
			...
			{
				reg = oC[i + gatherAddress(src.rel)];
			}
			break;
		case Shader::PARAMETER_DEPTHOUT:
			reg.x = oDepth;
	...
	Int4 PixelProgram::gatherAddress(const Shader::Relative &rel)
	{
	...
	//	case Shader::PARAMETER_CONST:    a = *Pointer<Float>(uniformAddress(src.bufferIndex, rel.index) + component * sizeof(float)); break;
		case Shader::PARAMETER_MISCTYPE:
			switch(rel.index)
			{
			case Shader::VPosIndex:  a = vPos.x;  break;
