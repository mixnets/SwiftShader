...
namespace sw
{
	...
	void PixelProgram::sampleTexture(Vector4f &c, const Src &sampler, Float4 &u, Float4 &v, Float4 &w, Float4 &q, Vector4f &dsx, Vector4f &dsy, Vector4f &offset, bool project, bool isOffset, SamplerMethod method)
	{
		Vector4f tmp;

		if(sampler.type == Shader::PARAMETER_SAMPLER && sampler.rel.type == Shader::PARAMETER_VOID)
		{
			sampleTexture(tmp, sampler.index, u, v, w, q, dsx, dsy, offset, project, isOffset, method);
		}
		else
		{
			...
			for(int i = 0; i < TEXTURE_IMAGE_UNITS; i++)
			{
				if(shader->usesSampler(i))
				{
					If(index == i)
					{
						sampleTexture(tmp, i, u, v, w, q, dsx, dsy, offset, project, isOffset, method);
	...
	void PixelProgram::sampleTexture(Vector4f &c, int stage, Float4 &u, Float4 &v, Float4 &w, Float4 &q, Vector4f &dsx, Vector4f &dsy, Vector4f &offset, bool project, bool isOffset, SamplerMethod method)
	{
		#if PERF_PROFILE
			Long texTime = Ticks();
		#endif
		...
		if(!project)
		{
			sampler[stage]->sampleTexture(texture, c, u, v, w, q, dsx, dsy, offset, isOffset, method);
		}
		else
		{
			...
			sampler[stage]->sampleTexture(texture, c, u_q, v_q, w_q, q, dsx, dsy, offset, isOffset, method);
		}
	...
	{
		sampleTexture(dst, src1, src0.x, src0.y, src0.z, src0.w, src0, src0, Vector4f(), project, false, bias ? Bias : Implicit);
	}

	void PixelProgram::TEXOFFSET(Vector4f &dst, Vector4f &src0, const Src &src1, Vector4f &src2, bool project, bool bias)
	{
		sampleTexture(dst, src1, src0.x, src0.y, src0.z, src0.w, src0, src0, src2, project, true, bias ? Bias : Implicit);
	}

	void PixelProgram::TEXLDL(Vector4f &dst, Vector4f &src0, const Src &src1, Vector4f &offset, bool project, bool bias)
	{
		sampleTexture(dst, src1, src0.x, src0.y, src0.z, src0.w, src0, src0, offset, project, true, Lod);
	}
	...
	{
		sampleTexture(dst, src1, src0.x, src0.y, src0.z, src0.w, src2, src3, Vector4f(), false, false, Grad);
	}

	void PixelProgram::TEXGRAD(Vector4f &dst, Vector4f &src0, const Src& src1, Vector4f &src2, Vector4f &src3, Vector4f &offset)
	{
		sampleTexture(dst, src1, src0.x, src0.y, src0.z, src0.w, src2, src3, offset, false, true, Grad);
	}

	void PixelProgram::TEXLDD(Vector4f &dst, Vector4f &src0, const Src &src1, Vector4f &src2, Vector4f &src3, bool project)
	{
		sampleTexture(dst, src1, src0.x, src0.y, src0.z, src0.w, src2, src3, Vector4f(), project, false, Grad);
	}

	void PixelProgram::TEXLDL(Vector4f &dst, Vector4f &src0, const Src &src1, bool project)
	{
		sampleTexture(dst, src1, src0.x, src0.y, src0.z, src0.w, src0, src0, Vector4f(), project, false, Lod);
	}
