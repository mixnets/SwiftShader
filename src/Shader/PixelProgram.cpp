...
namespace sw
{
	...
	Vector4f PixelProgram::sampleTexture(int samplerIndex, Vector4f &uvwq, Float4 &bias, Vector4f &dsx, Vector4f &dsy, Vector4f &offset, SamplerFunction function)
	{
		...
		Vector4f &c = sampler[samplerIndex]->sampleTexture(texture, uvwq.x, uvwq.y, uvwq.z, uvwq.w, bias, dsx, dsy, offset, function);
	...
	{
		dst = sampleTexture(src1, src0, src0.x, src0, src0, src0, Lod);
	}
