...
namespace sw
{
	...
	void PixelProgram::sampleTexture(Vector4f &c, int samplerIndex, Vector4f &uvwq, Vector4f &dsx, Vector4f &dsy, Vector4f &offset, SamplerMethod method, unsigned int options)
	{
		...
		if(!(options & Project))
		{
			sampler[samplerIndex]->sampleTexture(texture, c, uvwq, dsx, dsy, offset, options, method);
		}
		else
		{
			...
			sampler[samplerIndex]->sampleTexture(texture, c, uvwq_q, dsx, dsy, offset, options, method);
		}
