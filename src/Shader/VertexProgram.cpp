...
namespace sw
{
	...
	void VertexProgram::TEX(Vector4f &dst, Vector4f &src0, const Src &src1)
	{
		Vector4f coord = src0;
		coord.w = Float4(0.0f);
		sampleTexture(dst, src1, coord, a0, a0, src0, Lod, None);
	}
	...
	void VertexProgram::TEXOFFSET(Vector4f &dst, Vector4f &src0, const Src& src1, Vector4f &src2)
	{
		Vector4f coord = src0;
		coord.w = Float4(0.0f);
		sampleTexture(dst, src1, coord, a0, a0, src2, Lod, Offset);
	}
	...
	void VertexProgram::TEXGRAD(Vector4f &dst, Vector4f &src0, const Src& src1, Vector4f &src2, Vector4f &src3)
	{
		Vector4f coord = src0;
		coord.w = Float4(0.0f);
		sampleTexture(dst, src1, coord, src2, src3, src0, Grad, None);
	}
	...
	void VertexProgram::TEXGRAD(Vector4f &dst, Vector4f &src0, const Src& src1, Vector4f &src2, Vector4f &src3, Vector4f &offset)
	{
		Vector4f coord = src0;
		coord.w = Float4(0.0f);
		sampleTexture(dst, src1, coord, src2, src3, offset, Grad, Offset);
	}
	...
	void VertexProgram::sampleTexture(Vector4f &c, const Src &s, Vector4f &uvwq, Vector4f &dsx, Vector4f &dsy, Vector4f &offset, SamplerMethod method, unsigned int options)
	{
		...
		if(s.type == Shader::PARAMETER_SAMPLER && s.rel.type == Shader::PARAMETER_VOID)
		{
			...
			sampler[s.index]->sampleTexture(texture, tmp, uvwq, dsx, dsy, offset, options, method);
		}
		else
		{
			...
			for(int i = 0; i < VERTEX_TEXTURE_IMAGE_UNITS; i++)
			{
				if(shader->usesSampler(i))
				{
					If(index == i)
					{
						...
						sampler[i]->sampleTexture(texture, tmp, uvwq, dsx, dsy, offset, options, method);
