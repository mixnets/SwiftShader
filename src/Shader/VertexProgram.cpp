...
namespace sw
{
	...
	void VertexProgram::TEXLOD(Vector4f &dst, Vector4f &src0, const Src& src1, Vector4f &offset, Float4 &lod)
	{
	...
	Vector4f VertexProgram::sampleTexture(const Src &s, Vector4f &uvwq, Vector4f &dsx, Vector4f &dsy, Vector4f &offset, SamplerFunction function)
	{
		...
		if(s.type == Shader::PARAMETER_SAMPLER && s.rel.type == Shader::PARAMETER_VOID)
		{
			...
			tmp = SamplerCore(constants, state.sampler[s.index]).sampleTexture(texture, uvwq.x, uvwq.y, uvwq.z, uvwq.w, uvwq.x, dsx, dsy, offset, function);
		}
		else
		{
			...
			for(int i = 0; i < VERTEX_TEXTURE_IMAGE_UNITS; i++)
			{
				if(shader->usesSampler(i))
				{
					If(index == i)
					{
						...
						tmp = SamplerCore(constants, state.sampler[i]).sampleTexture(texture, uvwq.x, uvwq.y, uvwq.z, uvwq.w, uvwq.x, dsx, dsy, offset, function);
