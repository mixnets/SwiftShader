				...
				//	case Shader::PARAMETER_OUTPUT:
						...
						else
						{
							...
							else if(dst.rel.type == Shader::PARAMETER_LOOP)
							{
								...
								if(dst.x) pDst.x = o[dst.index + aL].x;
								if(dst.y) pDst.y = o[dst.index + aL].y;
								if(dst.z) pDst.z = o[dst.index + aL].z;
								if(dst.w) pDst.w = o[dst.index + aL].w;
							}
							else
							{
								Int a = relativeAddress(dst);
			...
			//	case Shader::PARAMETER_OUTPUT:
					...
					else
					{
						...
						else if(dst.rel.type == Shader::PARAMETER_LOOP)
						{
							...
							if(dst.x) o[dst.index + aL].x = d.x;
							if(dst.y) o[dst.index + aL].y = d.y;
							if(dst.z) o[dst.index + aL].z = d.z;
							if(dst.w) o[dst.index + aL].w = d.w;
						}
						else
						{
							Int a = relativeAddress(dst);
