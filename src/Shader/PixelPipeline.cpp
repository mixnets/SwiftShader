...
namespace sw
{
	...
	void PixelPipeline::applyShader(Int cMask[4])
	{
		...
		for(size_t i = 0; i < shader->getLength(); i++)
		{
			...
			Float4 u = version < 0x0104 ? this->v[2 + dst.index].x : this->v[2 + src0.index].x;
			Float4 v = version < 0x0104 ? this->v[2 + dst.index].y : this->v[2 + src0.index].y;
			Float4 s = version < 0x0104 ? this->v[2 + dst.index].z : this->v[2 + src0.index].z;
			Float4 t = version < 0x0104 ? this->v[2 + dst.index].w : this->v[2 + src0.index].w;
			...
			case Shader::OPCODE_TEX:
				...
				else if(version == 0x0104)
				{
					if(src0.type == Shader::PARAMETER_TEXTURE)
					{
						if((src0.swizzle & 0x30) == 0x20)   // .xyz
						{
							TEX(d, u, v, s, dst.index, src0.modifier == Shader::MODIFIER_DZ || src0.modifier == Shader::MODIFIER_DW);
						}
						else   // .xyw
						{
							TEX(d, u, v, t, dst.index, src0.modifier == Shader::MODIFIER_DZ || src0.modifier == Shader::MODIFIER_DW);
						}
					}
					else
					{
				...
				break;
			case Shader::OPCODE_TEXBEM:       TEXBEM(d, s0, u, v, s, dst.index);                                             break;
			case Shader::OPCODE_TEXBEML:      TEXBEML(d, s0, u, v, s, dst.index);                                            break;
			case Shader::OPCODE_TEXREG2AR:    TEXREG2AR(d, s0, dst.index);                                                   break;
			case Shader::OPCODE_TEXREG2GB:    TEXREG2GB(d, s0, dst.index);                                                   break;
			case Shader::OPCODE_TEXM3X2PAD:   TEXM3X2PAD(u, v, s, s0, 0, src0.modifier == Shader::MODIFIER_SIGN);            break;
			case Shader::OPCODE_TEXM3X2TEX:   TEXM3X2TEX(d, u, v, s, dst.index, s0, src0.modifier == Shader::MODIFIER_SIGN); break;
			case Shader::OPCODE_TEXM3X3PAD:   TEXM3X3PAD(u, v, s, s0, pad++ % 2, src0.modifier == Shader::MODIFIER_SIGN);    break;
			case Shader::OPCODE_TEXM3X3TEX:   TEXM3X3TEX(d, u, v, s, dst.index, s0, src0.modifier == Shader::MODIFIER_SIGN); break;
			case Shader::OPCODE_TEXM3X3SPEC:  TEXM3X3SPEC(d, u, v, s, dst.index, s0, s1);                                    break;
			case Shader::OPCODE_TEXM3X3VSPEC: TEXM3X3VSPEC(d, u, v, s, dst.index, s0);                                       break;
			case Shader::OPCODE_CND:          CND(d, s0, s1, s2);                                                               break;
			case Shader::OPCODE_TEXREG2RGB:   TEXREG2RGB(d, s0, dst.index);                                                  break;
			case Shader::OPCODE_TEXDP3TEX:    TEXDP3TEX(d, u, v, s, dst.index, s0);                                          break;
			case Shader::OPCODE_TEXM3X2DEPTH: TEXM3X2DEPTH(d, u, v, s, s0, src0.modifier == Shader::MODIFIER_SIGN);          break;
			case Shader::OPCODE_TEXDP3:       TEXDP3(d, u, v, s, s0);                                                        break;
			case Shader::OPCODE_TEXM3X3:      TEXM3X3(d, u, v, s, s0, src0.modifier == Shader::MODIFIER_SIGN);               break;
	...
	{
		Float4 u = this->v[2 + coordinates].x;
		Float4 v = this->v[2 + coordinates].y;
		Float4 w = this->v[2 + coordinates].z;
		Float4 q = this->v[2 + coordinates].w;
		...
		sampleTexture(c, stage, u, v, w, q, project);
	}
	...
	void PixelPipeline::TEXM3X3VSPEC(Vector4s &dst, Float4 &u, Float4 &v, Float4 &s, int stage, Vector4s &src0)
	{
		TEXM3X3PAD(u, v, s, src0, 2, false);

		Float4 E[3];   // Eye vector

		E[0] = this->v[2 + stage - 2].w;
		E[1] = this->v[2 + stage - 1].w;
		E[2] = this->v[2 + stage - 0].w;
