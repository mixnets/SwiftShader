			...
			*Pointer<Float4>(primitive + OFFSET(Primitive,yQuad), 16) = yQuad;
			...
			if(state.interpolateZ)
			{
				...
				*Pointer<Float4>(primitive + OFFSET(Primitive, z.B), 16) = Float4(B);
				...
				Float bias = 0.0f;  // TODO(b/155302798): Optimize 0 += x;
			...
			//	if(state.applyConstantDepthBias)
				{
					...
					//	Breakpoint();
						...
						float x = 1.0;
						float y = std::nextafter(1.0f, 2.0f);

						float d = y - x;

						constexpr int ee = (0x34000000 >> 23) - 127;
			...
			//	if(state.applySlopeDepthBias)
				{
					...
					bias += m * *Pointer<Float>(data + OFFSET(DrawData, slopeDepthBias));
				}
			...
			/*	if(state.applyDepthBiasClamp)
				...
				C += bias;  // TODO(b/155302798): Optimize x += 0;
	
				*Pointer<Float4>(primitive + OFFSET(Primitive, z.C), 16) = Float4(C);
				*Pointer<Float4>(primitive + OFFSET(Primitive, bias), 16) = Float4(bias);
			}
