...
namespace sw
{
	...
	void VertexRoutine::computeClipFlags()
	{
		...
		{
			Int4 min1 = CmpNLE(-o[pos].w, o[pos].z);
			clipFlags |= *Pointer<Int>(constants + OFFSET(Constants,min1) + SignMask(min1) * 4);
		}
		else
		{
			Int4 min0 = CmpNLE(Float4(0.0f), o[pos].z);
			clipFlags |= *Pointer<Int>(constants + OFFSET(Constants,min0) + SignMask(min0) * 4);
		}
