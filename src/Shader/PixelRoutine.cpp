...
namespace sw
{
	...
	void PixelRoutine::ps_1_x(Registers &r, Int cMask[4])
	{
		...
		//	#endif
			...
			if(src0.type != Shader::PARAMETER_VOID) s0 = regs(r, src0);
			if(src1.type != Shader::PARAMETER_VOID) s1 = regs(r, src1);
			if(src2.type != Shader::PARAMETER_VOID) s2 = regs(r, src2);
	...
	Vector4s PixelRoutine::regs(Registers &r, const Src &src)
	{
		Vector4s *reg;
		int i = src.index;
