...
namespace sw
{
	...
	void SamplerCore::sampleTexture(Pointer<Byte> &texture, Vector4f &c, Float4 &u, Float4 &v, Float4 &w, Float4 &q, Vector4f &dsx, Vector4f &dsy, Vector4f &offset, bool hasOffset, SamplerMethod method)
	{
		#if PERF_PROFILE
			AddAtomic(Pointer<Long>(&profiler.texOperations), 4);
	...
	{
		Int4 ioffset = Min(Max(As<Int4>(offset), Int4(MIN_PROGRAM_TEXEL_OFFSET)), Int4(MAX_PROGRAM_TEXEL_OFFSET));
		...
		case AddressingMode::ADDRESSING_WRAP:
			tmp = (tmp + Int4(whd) * Int4(-MIN_PROGRAM_TEXEL_OFFSET)) % Int4(whd);
			break;
