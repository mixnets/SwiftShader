...
namespace sw
{
	...
	void SamplerCore::sampleTexture(Pointer<Byte> &texture, Vector4s &c, Float4 &u, Float4 &v, Float4 &w, Float4 &q, Vector4f &dsx, Vector4f &dsy, SamplerMethod method, bool texelFetch)
	{
		sampleTexture(texture, c, u, v, w, q, dsx, dsy, dsx, false, method, true, texelFetch);
	}

	void SamplerCore::sampleTexture(Pointer<Byte> &texture, Vector4s &c, Float4 &u, Float4 &v, Float4 &w, Float4 &q, Vector4f &dsx, Vector4f &dsy, Vector4f &offset, bool hasOffset, SamplerMethod method, bool fixed12, bool texelFetch)
	{
		#if PERF_PROFILE
		...
		else
		{
			...
			if(!hasFloatTexture())
			{
				sampleFilter(texture, c, uuuu, vvvv, wwww, offset, hasOffset, texelFetch, lod, anisotropy, uDelta, vDelta, face, method);
			}
			else
			{
				Vector4f cf;

				sampleFloatFilter(texture, cf, uuuu, vvvv, wwww, offset, hasOffset, texelFetch, lod, anisotropy, uDelta, vDelta, face, method);
	...
	void SamplerCore::sampleTexture(Pointer<Byte> &texture, Vector4f &c, Float4 &u, Float4 &v, Float4 &w, Float4 &q, Vector4f &dsx, Vector4f &dsy, Vector4f &offset, SamplerMethod method, bool hasOffset, bool texelFetch)
	{
		#if PERF_PROFILE
			AddAtomic(Pointer<Long>(&profiler.texOperations), 4);
		...
		else
		{
			if(hasFloatTexture())   // FIXME: Mostly identical to integer sampling
			{
				...
				sampleFloatFilter(texture, c, uuuu, vvvv, wwww, offset, hasOffset, texelFetch, lod, anisotropy, uDelta, vDelta, face, method);
			}
			else
			{
				Vector4s cs;

				sampleTexture(texture, cs, u, v, w, q, dsx, dsy, offset, hasOffset, method, false, texelFetch);
	...
	void SamplerCore::sampleFilter(Pointer<Byte> &texture, Vector4s &c, Float4 &u, Float4 &v, Float4 &w, Vector4f &offset, bool hasOffset, bool texelFetch, Float &lod, Float &anisotropy, Float4 &uDelta, Float4 &vDelta, Int face[4], SamplerMethod method)
	{
		sampleAniso(texture, c, u, v, w, offset, hasOffset, texelFetch, lod, anisotropy, uDelta, vDelta, face, false, method);
		...
		if(state.mipmapFilter > MIPMAP_POINT)
		{
			...
			sampleAniso(texture, cc, u, v, w, offset, hasOffset, texelFetch, lod, anisotropy, uDelta, vDelta, face, true, method);
	...
	void SamplerCore::sampleAniso(Pointer<Byte> &texture, Vector4s &c, Float4 &u, Float4 &v, Float4 &w, Vector4f &offset, bool hasOffset, bool texelFetch, Float &lod, Float &anisotropy, Float4 &uDelta, Float4 &vDelta, Int face[4], bool secondLOD, SamplerMethod method)
	{
		if(state.textureFilter != FILTER_ANISOTROPIC || method == Lod)
		{
			sampleQuad(texture, c, u, v, w, offset, hasOffset, texelFetch, lod, face, secondLOD);
		}
		else
		{
			...
			Do
			{
				sampleQuad(texture, c, u0, v0, w, offset, hasOffset, texelFetch, lod, face, secondLOD);
	...
	void SamplerCore::sampleQuad(Pointer<Byte> &texture, Vector4s &c, Float4 &u, Float4 &v, Float4 &w, Vector4f &offset, bool hasOffset, bool texelFetch, Float &lod, Int face[4], bool secondLOD)
	{
		if(state.textureType != TEXTURE_3D)
		{
			sampleQuad2D(texture, c, u, v, w, offset, hasOffset, texelFetch, lod, face, secondLOD);
		}
		else
		{
			sample3D(texture, c, u, v, w, offset, hasOffset, texelFetch, lod, secondLOD);
		}
	}

	void SamplerCore::sampleQuad2D(Pointer<Byte> &texture, Vector4s &c, Float4 &u, Float4 &v, Float4 &w, Vector4f &offset, bool hasOffset, bool texelFetch, Float &lod, Int face[4], bool secondLOD)
	{
		int componentCount = textureComponentCount();
		...
		{
			sampleTexel(c, uuuu, vvvv, wwww, offset, hasOffset, texelFetch, mipmap, buffer);
		}
		else
		{
			Vector4s c0;
			...
			sampleTexel(c0, uuuu0, vvvv0, wwww, offset, hasOffset, texelFetch, mipmap, buffer);
			sampleTexel(c1, uuuu1, vvvv0, wwww, offset, hasOffset, texelFetch, mipmap, buffer);
			sampleTexel(c2, uuuu0, vvvv1, wwww, offset, hasOffset, texelFetch, mipmap, buffer);
			sampleTexel(c3, uuuu1, vvvv1, wwww, offset, hasOffset, texelFetch, mipmap, buffer);
	...
	void SamplerCore::sample3D(Pointer<Byte> &texture, Vector4s &c_, Float4 &u_, Float4 &v_, Float4 &w_, Vector4f &offset, bool hasOffset, bool texelFetch, Float &lod, bool secondLOD)
	{
		int componentCount = textureComponentCount();
		...
		if(state.textureFilter == FILTER_POINT || texelFetch)
		{
			sampleTexel(c_, uuuu, vvvv, wwww, offset, hasOffset, texelFetch, mipmap, buffer);
		}
		else
		{
			Vector4s c[2][2][2];
			...
			for(int i = 0; i < 2; i++)
			{
				for(int j = 0; j < 2; j++)
				{
					for(int k = 0; k < 2; k++)
					{
						sampleTexel(c[i][j][k], u[i][j][k], v[i][j][k], s[i][j][k], offset, hasOffset, texelFetch, mipmap, buffer);
	...
	void SamplerCore::sampleFloatFilter(Pointer<Byte> &texture, Vector4f &c, Float4 &u, Float4 &v, Float4 &w, Vector4f &offset, bool hasOffset, bool texelFetch, Float &lod, Float &anisotropy, Float4 &uDelta, Float4 &vDelta, Int face[4], SamplerMethod method)
	{
		sampleFloatAniso(texture, c, u, v, w, offset, hasOffset, texelFetch, lod, anisotropy, uDelta, vDelta, face, false, method);
		...
		if(state.mipmapFilter > MIPMAP_POINT)
		{
			...
			sampleFloatAniso(texture, cc, u, v, w, offset, hasOffset, texelFetch, lod, anisotropy, uDelta, vDelta, face, true, method);
	...
	void SamplerCore::sampleFloatAniso(Pointer<Byte> &texture, Vector4f &c, Float4 &u, Float4 &v, Float4 &w, Vector4f &offset, bool hasOffset, bool texelFetch, Float &lod, Float &anisotropy, Float4 &uDelta, Float4 &vDelta, Int face[4], bool secondLOD, SamplerMethod method)
	{
		if(state.textureFilter != FILTER_ANISOTROPIC || method == Lod)
		{
			sampleFloat(texture, c, u, v, w, offset, hasOffset, texelFetch, lod, face, secondLOD);
		}
		else
		{
			Int a = RoundInt(anisotropy);
			...
			Do
			{
				sampleFloat(texture, c, u0, v0, w, offset, hasOffset, texelFetch, lod, face, secondLOD);
	...
	void SamplerCore::sampleFloat(Pointer<Byte> &texture, Vector4f &c, Float4 &u, Float4 &v, Float4 &w, Vector4f &offset, bool hasOffset, bool texelFetch, Float &lod, Int face[4], bool secondLOD)
	{
		if(state.textureType != TEXTURE_3D)
		{
			sampleFloat2D(texture, c, u, v, w, offset, hasOffset, texelFetch, lod, face, secondLOD);
		}
		else
		{
			sampleFloat3D(texture, c, u, v, w, offset, hasOffset, texelFetch, lod, secondLOD);
		}
	}

	void SamplerCore::sampleFloat2D(Pointer<Byte> &texture, Vector4f &c, Float4 &u, Float4 &v, Float4 &w, Vector4f &offset, bool hasOffset, bool texelFetch, Float &lod, Int face[4], bool secondLOD)
	{
		int componentCount = textureComponentCount();
		bool gather = state.textureFilter == FILTER_GATHER;
		...
		if(state.textureFilter == FILTER_POINT || texelFetch)
		{
			sampleTexel(c, uuuu, vvvv, wwww, offset, hasOffset, texelFetch, w, mipmap, buffer);
		}
		else
		{
			Vector4f c0;
			...
			sampleTexel(c0, uuuu0, vvvv0, wwww, offset, hasOffset, texelFetch, w, mipmap, buffer);
			sampleTexel(c1, uuuu1, vvvv0, wwww, offset, hasOffset, texelFetch, w, mipmap, buffer);
			sampleTexel(c2, uuuu0, vvvv1, wwww, offset, hasOffset, texelFetch, w, mipmap, buffer);
			sampleTexel(c3, uuuu1, vvvv1, wwww, offset, hasOffset, texelFetch, w, mipmap, buffer);
	...
	void SamplerCore::sampleFloat3D(Pointer<Byte> &texture, Vector4f &c, Float4 &u, Float4 &v, Float4 &w, Vector4f &offset, bool hasOffset, bool texelFetch, Float &lod, bool secondLOD)
	{
		int componentCount = textureComponentCount();
		...
		else
		{
			...
			sampleTexel(c0, uuuu0, vvvv0, wwww0, offset, hasOffset, texelFetch, w, mipmap, buffer);
			sampleTexel(c1, uuuu1, vvvv0, wwww0, offset, hasOffset, texelFetch, w, mipmap, buffer);
			sampleTexel(c2, uuuu0, vvvv1, wwww0, offset, hasOffset, texelFetch, w, mipmap, buffer);
			sampleTexel(c3, uuuu1, vvvv1, wwww0, offset, hasOffset, texelFetch, w, mipmap, buffer);
			sampleTexel(c4, uuuu0, vvvv0, wwww1, offset, hasOffset, texelFetch, w, mipmap, buffer);
			sampleTexel(c5, uuuu1, vvvv0, wwww1, offset, hasOffset, texelFetch, w, mipmap, buffer);
			sampleTexel(c6, uuuu0, vvvv1, wwww1, offset, hasOffset, texelFetch, w, mipmap, buffer);
			sampleTexel(c7, uuuu1, vvvv1, wwww1, offset, hasOffset, texelFetch, w, mipmap, buffer);
	...
	void SamplerCore::computeLod(Pointer<Byte> &texture, Float &lod, Float &anisotropy, Float4 &uDelta, Float4 &vDelta, Float4 &uuuu, Float4 &vvvv, const Float &lodBias, Vector4f &dsx, Vector4f &dsy, SamplerMethod method)
	{
		...
		{
			lod = lodBias + *Pointer<Float>(texture + OFFSET(Texture,LOD)); //FIXME: Use baseLevel
		}

		lod = Max(lod, 0.0f);
		lod = Min(lod, Float(MIPMAP_LEVELS - 2));   // Trilinear accesses lod+1

		//FIXME: clamp lod between minLod and maxLod
	}
	...
	Short4 SamplerCore::applyOffset(Short4 &uvw, Float4 &offset, const Int4 &whd, AddressingMode mode, bool wrap)
	{
	...
	void SamplerCore::computeIndices(Int index[4], Short4 uuuu, Short4 vvvv, Short4 wwww, Vector4f &offset, bool hasOffset, bool texelFetch, const Pointer<Byte> &mipmap)
	{
		Short4 uuu2;
		...
		else
		{
			...
			if(hasOffset)
			{
				uuuu = applyOffset(uuuu, offset.x, Int4(*Pointer<UShort4>(mipmap + OFFSET(Mipmap, width))), state.addressingModeU, texelFetch ? false : true);
				vvvv = applyOffset(vvvv, offset.y, Int4(*Pointer<UShort4>(mipmap + OFFSET(Mipmap, height))), state.addressingModeV, texelFetch ? false : true);
			}
			uuu2 = uuuu;
			uuuu = As<Short4>(UnpackLow(uuuu, vvvv));
			uuu2 = As<Short4>(UnpackHigh(uuu2, vvvv));
		...
		if((state.textureType == TEXTURE_3D) || (state.textureType == TEXTURE_2D_ARRAY))
		{
			if(state.textureType != TEXTURE_2D_ARRAY)
			{
				...
				if(hasOffset)
				{
					wwww = applyOffset(wwww, offset.z, Int4(*Pointer<UShort4>(mipmap + OFFSET(Mipmap, depth))), state.addressingModeW, texelFetch ? false : true);
				}
			}
			Short4 www2 = wwww;
	...
	void SamplerCore::sampleTexel(Vector4s &c, Short4 &uuuu, Short4 &vvvv, Short4 &wwww, Vector4f &offset, bool hasOffset, bool texelFetch, Pointer<Byte> &mipmap, Pointer<Byte> buffer[4])
	{
		Int index[4];

		computeIndices(index, uuuu, vvvv, wwww, offset, hasOffset, texelFetch, mipmap);
		...
		else if(hasYuvFormat())
		{
			...
			computeIndices(index, uuuu, vvvv, wwww, offset, hasOffset, texelFetch, mipmap + sizeof(Mipmap));
	...
	void SamplerCore::sampleTexel(Vector4f &c, Short4 &uuuu, Short4 &vvvv, Short4 &wwww, Vector4f &offset, bool hasOffset, bool texelFetch, Float4 &z, Pointer<Byte> &mipmap, Pointer<Byte> buffer[4])
	{
		Int index[4];
