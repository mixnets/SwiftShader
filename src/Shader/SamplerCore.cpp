...
namespace sw
{
	...
	{
		sampleTexture(texture, c, u, v, w, q, dsx, dsy, dsx, Implicit, None, true);
	}

	void SamplerCore::sampleTexture(Pointer<Byte> &texture, Vector4s &c, Float4 &u, Float4 &v, Float4 &w, Float4 &q, Vector4f &dsx, Vector4f &dsy, Vector4f &offset, SamplerMethod method, SamplerOption option, bool fixed12)
	{
		#if PERF_PROFILE
		...
		else
		{
			...
			if(!hasFloatTexture())
			{
				sampleFilter(texture, c, uuuu, vvvv, wwww, offset, lod, anisotropy, uDelta, vDelta, face, method, option);
			}
			else
			{
				Vector4f cf;

				sampleFloatFilter(texture, cf, uuuu, vvvv, wwww, offset, lod, anisotropy, uDelta, vDelta, face, method, option);
	...
	void SamplerCore::sampleTexture(Pointer<Byte> &texture, Vector4f &c, Float4 &u, Float4 &v, Float4 &w, Float4 &q, Vector4f &dsx, Vector4f &dsy, Vector4f &offset, SamplerMethod method, SamplerOption option)
	{
		#if PERF_PROFILE
			AddAtomic(Pointer<Long>(&profiler.texOperations), 4);
		...
		else
		{
			if(hasFloatTexture())   // FIXME: Mostly identical to integer sampling
			{
				...
				sampleFloatFilter(texture, c, uuuu, vvvv, wwww, offset, lod, anisotropy, uDelta, vDelta, face, method, option);
			}
			else
			{
				Vector4s cs;

				sampleTexture(texture, cs, u, v, w, q, dsx, dsy, offset, method, option, false);
	...
	void SamplerCore::sampleFilter(Pointer<Byte> &texture, Vector4s &c, Float4 &u, Float4 &v, Float4 &w, Vector4f &offset, Float &lod, Float &anisotropy, Float4 &uDelta, Float4 &vDelta, Int face[4], SamplerMethod method, SamplerOption option)
	{
		sampleAniso(texture, c, u, v, w, offset, lod, anisotropy, uDelta, vDelta, face, false, method, option);
		...
		if(state.mipmapFilter > MIPMAP_POINT)
		{
			...
			sampleAniso(texture, cc, u, v, w, offset, lod, anisotropy, uDelta, vDelta, face, true, method, option);
	...
	void SamplerCore::sampleAniso(Pointer<Byte> &texture, Vector4s &c, Float4 &u, Float4 &v, Float4 &w, Vector4f &offset, Float &lod, Float &anisotropy, Float4 &uDelta, Float4 &vDelta, Int face[4], bool secondLOD, SamplerMethod method, SamplerOption option)
	{
		if(state.textureFilter != FILTER_ANISOTROPIC || method == Lod || method == Fetch)
		{
			sampleQuad(texture, c, u, v, w, offset, lod, face, secondLOD, method, option);
		}
		else
		{
			...
			Do
			{
				sampleQuad(texture, c, u0, v0, w, offset, lod, face, secondLOD, method, option);
	...
	void SamplerCore::sampleQuad(Pointer<Byte> &texture, Vector4s &c, Float4 &u, Float4 &v, Float4 &w, Vector4f &offset, Float &lod, Int face[4], bool secondLOD, SamplerMethod method, SamplerOption option)
	{
		if(state.textureType != TEXTURE_3D)
		{
			sampleQuad2D(texture, c, u, v, w, offset, lod, face, secondLOD, method, option);
		}
		else
		{
			sample3D(texture, c, u, v, w, offset, lod, secondLOD, method, option);
		}
	}

	void SamplerCore::sampleQuad2D(Pointer<Byte> &texture, Vector4s &c, Float4 &u, Float4 &v, Float4 &w, Vector4f &offset, Float &lod, Int face[4], bool secondLOD, SamplerMethod method, SamplerOption option)
	{
		int componentCount = textureComponentCount();
		...
		{
			sampleTexel(c, uuuu, vvvv, wwww, offset, mipmap, buffer, method, option);
		}
		else
		{
			Vector4s c0;
			...
			sampleTexel(c0, uuuu0, vvvv0, wwww, offset, mipmap, buffer, method, option);
			sampleTexel(c1, uuuu1, vvvv0, wwww, offset, mipmap, buffer, method, option);
			sampleTexel(c2, uuuu0, vvvv1, wwww, offset, mipmap, buffer, method, option);
			sampleTexel(c3, uuuu1, vvvv1, wwww, offset, mipmap, buffer, method, option);
	...
	void SamplerCore::sample3D(Pointer<Byte> &texture, Vector4s &c_, Float4 &u_, Float4 &v_, Float4 &w_, Vector4f &offset, Float &lod, bool secondLOD, SamplerMethod method, SamplerOption option)
	{
		int componentCount = textureComponentCount();
		...
		if(state.textureFilter == FILTER_POINT || texelFetch)
		{
			sampleTexel(c_, uuuu, vvvv, wwww, offset, mipmap, buffer, method, option);
		}
		else
		{
			Vector4s c[2][2][2];
			...
			for(int i = 0; i < 2; i++)
			{
				for(int j = 0; j < 2; j++)
				{
					for(int k = 0; k < 2; k++)
					{
						sampleTexel(c[i][j][k], u[i][j][k], v[i][j][k], s[i][j][k], offset, mipmap, buffer, method, option);
	...
	void SamplerCore::sampleFloatFilter(Pointer<Byte> &texture, Vector4f &c, Float4 &u, Float4 &v, Float4 &w, Vector4f &offset, Float &lod, Float &anisotropy, Float4 &uDelta, Float4 &vDelta, Int face[4], SamplerMethod method, SamplerOption option)
	{
		sampleFloatAniso(texture, c, u, v, w, offset, lod, anisotropy, uDelta, vDelta, face, false, method, option);
		...
		if(state.mipmapFilter > MIPMAP_POINT)
		{
			...
			sampleFloatAniso(texture, cc, u, v, w, offset, lod, anisotropy, uDelta, vDelta, face, true, method, option);
	...
	void SamplerCore::sampleFloatAniso(Pointer<Byte> &texture, Vector4f &c, Float4 &u, Float4 &v, Float4 &w, Vector4f &offset, Float &lod, Float &anisotropy, Float4 &uDelta, Float4 &vDelta, Int face[4], bool secondLOD, SamplerMethod method, SamplerOption option)
	{
		if (state.textureFilter != FILTER_ANISOTROPIC || method == Lod || method == Fetch)
		{
			sampleFloat(texture, c, u, v, w, offset, lod, face, secondLOD, method, option);
		}
		else
		{
			Int a = RoundInt(anisotropy);
			...
			Do
			{
				sampleFloat(texture, c, u0, v0, w, offset, lod, face, secondLOD, method, option);
	...
	void SamplerCore::sampleFloat(Pointer<Byte> &texture, Vector4f &c, Float4 &u, Float4 &v, Float4 &w, Vector4f &offset, Float &lod, Int face[4], bool secondLOD, SamplerMethod method, SamplerOption option)
	{
		if(state.textureType != TEXTURE_3D)
		{
			sampleFloat2D(texture, c, u, v, w, offset, lod, face, secondLOD, method, option);
		}
		else
		{
			sampleFloat3D(texture, c, u, v, w, offset, lod, secondLOD, method, option);
		}
	}

	void SamplerCore::sampleFloat2D(Pointer<Byte> &texture, Vector4f &c, Float4 &u, Float4 &v, Float4 &w, Vector4f &offset, Float &lod, Int face[4], bool secondLOD, SamplerMethod method, SamplerOption option)
	{
		int componentCount = textureComponentCount();
		bool gather = state.textureFilter == FILTER_GATHER;
		...
		if(state.textureFilter == FILTER_POINT || texelFetch)
		{
			sampleTexel(c, uuuu, vvvv, wwww, offset, w, mipmap, buffer, method, option);
		}
		else
		{
			Vector4f c0;
			...
			sampleTexel(c0, uuuu0, vvvv0, wwww, offset, w, mipmap, buffer, method, option);
			sampleTexel(c1, uuuu1, vvvv0, wwww, offset, w, mipmap, buffer, method, option);
			sampleTexel(c2, uuuu0, vvvv1, wwww, offset, w, mipmap, buffer, method, option);
			sampleTexel(c3, uuuu1, vvvv1, wwww, offset, w, mipmap, buffer, method, option);
	...
	void SamplerCore::sampleFloat3D(Pointer<Byte> &texture, Vector4f &c, Float4 &u, Float4 &v, Float4 &w, Vector4f &offset, Float &lod, bool secondLOD, SamplerMethod method, SamplerOption option)
	{
		int componentCount = textureComponentCount();
		...
		else
		{
			...
			sampleTexel(c0, uuuu0, vvvv0, wwww0, offset, w, mipmap, buffer, method, option);
			sampleTexel(c1, uuuu1, vvvv0, wwww0, offset, w, mipmap, buffer, method, option);
			sampleTexel(c2, uuuu0, vvvv1, wwww0, offset, w, mipmap, buffer, method, option);
			sampleTexel(c3, uuuu1, vvvv1, wwww0, offset, w, mipmap, buffer, method, option);
			sampleTexel(c4, uuuu0, vvvv0, wwww1, offset, w, mipmap, buffer, method, option);
			sampleTexel(c5, uuuu1, vvvv0, wwww1, offset, w, mipmap, buffer, method, option);
			sampleTexel(c6, uuuu0, vvvv1, wwww1, offset, w, mipmap, buffer, method, option);
			sampleTexel(c7, uuuu1, vvvv1, wwww1, offset, w, mipmap, buffer, method, option);
	...
	void SamplerCore::computeIndices(Int index[4], Short4 uuuu, Short4 vvvv, Short4 wwww, Vector4f &offset, const Pointer<Byte> &mipmap, SamplerMethod method, SamplerOption option)
	{
		Short4 uuu2;

		bool texelFetch = (method == Fetch);
		bool hasOffset = (option == Offset);
	...
	void SamplerCore::sampleTexel(Vector4s &c, Short4 &uuuu, Short4 &vvvv, Short4 &wwww, Vector4f &offset, Pointer<Byte> &mipmap, Pointer<Byte> buffer[4], SamplerMethod method, SamplerOption option)
	{
		Int index[4];

		computeIndices(index, uuuu, vvvv, wwww, offset, mipmap, method, option);
		...
		else if(hasYuvFormat())
		{
			...
			computeIndices(index, uuuu, vvvv, wwww, offset, mipmap + sizeof(Mipmap), method, option);
	...
	void SamplerCore::sampleTexel(Vector4f &c, Short4 &uuuu, Short4 &vvvv, Short4 &wwww, Vector4f &offset, Float4 &z, Pointer<Byte> &mipmap, Pointer<Byte> buffer[4], SamplerMethod method, SamplerOption option)
	{
		Int index[4];
