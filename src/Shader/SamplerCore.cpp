...
namespace sw
{
	...
	Vector4f SamplerCore::sampleTexture(Pointer<Byte> &texture, Float4 &u, Float4 &v, Float4 &w, Float4 &q, Float4 &q2, Vector4f &dsx, Vector4f &dsy, Vector4f &offset, SamplerFunction function)
	{
		Vector4f c;
		...
		else
		{
			...
			if(hasFloatTexture() || hasUnnormalizedIntegerTexture() || forceFloatFiltering)   // FIXME: Mostly identical to integer sampling
			{
				...
				if(state.compare != COMPARE_BYPASS && state.textureType == TEXTURE_CUBE && function == Bias)
				{
					lodBias = q2.x;
					qqqq = q;
				}
				else if(state.compare != COMPARE_BYPASS && state.textureType == TEXTURE_2D_ARRAY && function == Lod)
				{
					lodBias = Float(0.0f);
					qqqq = q;
				}
