...
namespace sw
{
	...
	Vector4s SamplerCore::sampleTexture(Pointer<Byte> &texture, Float4 &u, Float4 &v, Float4 &w, Float4 &q, Float4 &bias, Vector4f &dsx, Vector4f &dsy, Vector4f &offset, SamplerFunction function, bool fixed12)
	{
		...
		else if((state.compare == COMPARE_ALWAYS) || (state.compare == COMPARE_NEVER))
		{
			c.x = (state.compare == COMPARE_ALWAYS) ?
		...
		else
		{
			...
			if(state.textureType != TEXTURE_3D)
			{
				...
				{
					Float4 lodX, lodY, lodZ, M;
					cubeFace(face, uuuu, vvvv, lodX, lodY, lodZ, u, v, w, M);
					computeLodCube(texture, lod, lodX, lodY, lodZ, bias.x, dsx, dsy, M, function);
				}
			}
			else
			{
	...
	Vector4f SamplerCore::sampleTexture(Pointer<Byte> &texture, Float4 &u, Float4 &v, Float4 &w, Float4 &q, Float4 &bias, Vector4f &dsx, Vector4f &dsy, Vector4f &offset, SamplerFunction function)
	{
		...
		{
			c.x = (state.compare == COMPARE_ALWAYS) ? Float4(1.0f) : Float4(0.0f);
			c.y = Float4(0.0f);
			c.z = Float4(0.0f);
			c.w = Float4(1.0f);
		}
	...
	void SamplerCore::computeLodCube(Pointer<Byte> &texture, Float &lod, Float4 &u, Float4 &v, Float4 &s, const Float &lodBias, Vector4f &dsx, Vector4f &dsy, Float4 &M, SamplerFunction function)
	{
		if(function != Lod && function != Fetch)
		{
	...
	void SamplerCore::cubeFace(Int face[4], Float4 &U, Float4 &V, Float4 &lodX, Float4 &lodY, Float4 &lodZ, Float4 &x, Float4 &y, Float4 &z, Float4 &M)
	{
	...
	Vector4f SamplerCore::sampleTexel(Int4 &uuuu, Int4 &vvvv, Int4 &wwww, Float4 &z, Pointer<Byte> &mipmap, Pointer<Byte> buffer[4], SamplerFunction function)
	{
		...
		if(hasFloatTexture() || has32bitIntegerTextureComponents())
		{
			...
			if(state.compare != COMPARE_BYPASS)
			{
				...
				case COMPARE_ALWAYS:       // Already handled
				case COMPARE_NEVER:        // Already handled
				default:                   ASSERT(false);
				}
