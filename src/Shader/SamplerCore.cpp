...
namespace sw
{
	...
	void SamplerCore::sampleQuad2D(Pointer<Byte> &texture, Vector4s &c, Float4 &u, Float4 &v, Float &lod, Int face[4], bool secondLOD)
	{
		...
		if((state.textureFilter == FILTER_MIN_POINT_MAG_LINEAR) || (state.textureFilter == FILTER_MIN_LINEAR_MAG_POINT))
		{
			If(useFilter(lod))
			{
				sampleQuad2DFilter(c, uuuu, vvvv, mipmap, buffer);
			}
			Else
			{
	...
	void SamplerCore::sampleQuad2DFilter(Vector4s &c, Short4 &uuuu, Short4 &vvvv, Pointer<Byte> &mipmap, Pointer<Byte> buffer[4])
	{
		int componentCount = textureComponentCount();
	...
	void SamplerCore::sampleFloat2D(Pointer<Byte> &texture, Vector4f &c, Float4 &u, Float4 &v, Float4 &z, Float &lod, Int face[4], bool secondLOD)
	{
		...
		if((state.textureFilter == FILTER_MIN_POINT_MAG_LINEAR) || (state.textureFilter == FILTER_MIN_LINEAR_MAG_POINT))
		{
			If(useFilter(lod))
			{
				sampleFloat2DFilter(c, uuuu, vvvv, z, mipmap, buffer);
			}
			Else
			{
	...
	void SamplerCore::sampleFloat2DFilter(Vector4f &c, Short4 &uuuu, Short4 &vvvv, Float4 &z, Pointer<Byte> &mipmap, Pointer<Byte> buffer[4])
	{
		int componentCount = textureComponentCount();
		bool gather = state.textureFilter == FILTER_GATHER;
