...
namespace sw
{
	...
	void SamplerCore::sampleQuad(Pointer<Byte> &texture, Vector4s &c, Float4 &u, Float4 &v, Float4 &w, Vector4f &offset, Float &lod, Int face[4], bool secondLOD, SamplerFunction function)
	{
		if(state.textureType != TEXTURE_3D)
		{
			c = sampleQuad2D(texture, u, v, w, offset, lod, face, secondLOD, function);
		}
		else
		{
	...
	//	Short4 vvvv1 = vvvv + *Pointer<Short4>(mipmap + OFFSET(Mipmap,vHalf));
		...
		Float4 f0 = As<Float4>(rgba_x);
		Float4 f1 = As<Float4>(rgba_y);
		Float4 f2 = As<Float4>(rgba_z);
		Float4 f3 = As<Float4>(rgba_w);
		...
		Int4 xxxx = MulAdd(xx, As<Short8>(f0v));

		Short8 yy = MulAdd(rgba_y, As<SByte16>(f0u));
		Int4 yyyy = MulAdd(yy, As<Short8>(f0v));

		Short8 zz = MulAdd(rgba_z, As<SByte16>(f0u));
		Int4 zzzz = MulAdd(zz, As<Short8>(f0v));
	...
	Vector4s SamplerCore::sampleQuad2D(Pointer<Byte> &texture, Float4 &u, Float4 &v, Float4 &w, Vector4f &offset, Float &lod, Int face[4], bool secondLOD, SamplerFunction function)
	{
		...
		{
			return sampleTexel(uuuu, vvvv, wwww, offset, mipmap, buffer, function);
		}
		else
		{
