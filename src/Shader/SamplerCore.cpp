...
namespace sw
{
	...
	void SamplerCore::sampleFloat2D(Pointer<Byte> &texture, Vector4f &c, Float4 &u, Float4 &v, Float4 &w, Vector4f &offset, Float &lod, Int face[4], bool secondLOD, SamplerFunction function)
	{
		...
		address(w, z0, z0, fv, mipmap, offset.z, filter, OFFSET(Mipmap, depth), FILTER_POINT, state.addressingModeW, function);
		address(v, y0, y1, fv, mipmap, offset.y, filter, OFFSET(Mipmap, height), state.textureFilter, state.addressingModeV, function);
		address(u, x0, x1, fu, mipmap, offset.x, filter, OFFSET(Mipmap, width), state.textureFilter, state.addressingModeU, function);

		if(state.textureFilter == FILTER_POINT || (function == Fetch))
		{
			y0 *= *Pointer<Int4>(mipmap + OFFSET(Mipmap, pitchP), 16);
			if(hasDepth())
			{
	...
	void SamplerCore::sampleFloat3D(Pointer<Byte> &texture, Vector4f &c, Float4 &u, Float4 &v, Float4 &w, Vector4f &offset, Float &lod, bool secondLOD, SamplerFunction function)
	{
		...
		address(u, x0, x1, fu, mipmap, offset.x, filter, OFFSET(Mipmap, width), state.textureFilter, state.addressingModeU, function);
		address(v, y0, y1, fv, mipmap, offset.y, filter, OFFSET(Mipmap, height), state.textureFilter, state.addressingModeV, function);
		address(w, z0, z1, fw, mipmap, offset.z, filter, OFFSET(Mipmap, depth), state.textureFilter, state.addressingModeW, function);
	...
	{
		UInt4 indices = uuuu + (hasDepth() ? vvvv + wwww : vvvv);
	...
	void SamplerCore::address(Float4 &uvw, Int4& xyz0, Int4& xyz1, Float4& f, Pointer<Byte>& mipmap, Float4 &texOffset, Int4 &filter, int whd, FilterType filterType, AddressingMode addressingMode, SamplerFunction function)
	{
		if(addressingMode == ADDRESSING_LAYER && state.textureType != TEXTURE_2D_ARRAY)
		{
			return; // Unused
		...
		else
		{
			...
			if(filterType != FILTER_POINT) // Compute 2nd coordinate, if needed
			{
				bool gather = state.textureFilter == FILTER_GATHER;
...
//	}
	...
	bool SamplerCore::hasDepth() const
	{
