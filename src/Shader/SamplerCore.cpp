...
namespace sw
{
	...
	void SamplerCore::sampleQuad(Pointer<Byte> &texture, Vector4s &c, Float4 &u, Float4 &v, Float4 &w, Vector4f &offset, Float &lod, Int face[4], bool secondLOD, SamplerMethod method, SamplerOption option)
	{
		...
		{
			sample3D(texture, c, u, v, w, offset, lod, secondLOD, option);
		}
	}
	...
	void SamplerCore::sampleQuad2D(Pointer<Byte> &texture, Vector4s &c, Float4 &u, Float4 &v, Float4 &w, Vector4f &offset, Float &lod, Int face[4], bool secondLOD, SamplerMethod method, SamplerOption option)
	{
		...
		{
			sampleTexel(c, uuuu, vvvv, wwww, offset, mipmap, buffer, option);
		}
		else
		{
			Vector4s c0;
			...
			sampleTexel(c0, uuuu0, vvvv0, wwww, offset, mipmap, buffer, option);
			sampleTexel(c1, uuuu1, vvvv0, wwww, offset, mipmap, buffer, option);
			sampleTexel(c2, uuuu0, vvvv1, wwww, offset, mipmap, buffer, option);
			sampleTexel(c3, uuuu1, vvvv1, wwww, offset, mipmap, buffer, option);
	...
	void SamplerCore::sample3D(Pointer<Byte> &texture, Vector4s &c_, Float4 &u_, Float4 &v_, Float4 &w_, Vector4f &offset, Float &lod, bool secondLOD, SamplerOption option)
	{
		int componentCount = textureComponentCount();
		...
		bool texelFetch = (option & Fetch) != 0;
		...
		if(state.textureFilter == FILTER_POINT || texelFetch)
		{
			sampleTexel(c_, uuuu, vvvv, wwww, offset, mipmap, buffer, option);
		}
		else
		{
			Vector4s c[2][2][2];
			...
			for(int i = 0; i < 2; i++)
			{
				for(int j = 0; j < 2; j++)
				{
					for(int k = 0; k < 2; k++)
					{
						sampleTexel(c[i][j][k], u[i][j][k], v[i][j][k], s[i][j][k], offset, mipmap, buffer, option);
	...
	{
		if (state.textureFilter != FILTER_ANISOTROPIC || method == Lod || option & Fetch)
		{
			sampleFloat(texture, c, u, v, w, offset, lod, face, secondLOD, option);
		}
		else
		{
			Int a = RoundInt(anisotropy);
			...
			Do
			{
				sampleFloat(texture, c, u0, v0, w, offset, lod, face, secondLOD, option);
	...
	void SamplerCore::sampleFloat(Pointer<Byte> &texture, Vector4f &c, Float4 &u, Float4 &v, Float4 &w, Vector4f &offset, Float &lod, Int face[4], bool secondLOD, SamplerOption option)
	{
		if(state.textureType != TEXTURE_3D)
		{
			sampleFloat2D(texture, c, u, v, w, offset, lod, face, secondLOD, option);
		}
		else
		{
			sampleFloat3D(texture, c, u, v, w, offset, lod, secondLOD, option);
		}
	}

	void SamplerCore::sampleFloat2D(Pointer<Byte> &texture, Vector4f &c, Float4 &u, Float4 &v, Float4 &w, Vector4f &offset, Float &lod, Int face[4], bool secondLOD, SamplerOption option)
	{
		int componentCount = textureComponentCount();
		bool gather = state.textureFilter == FILTER_GATHER;
		...
		if(state.textureFilter == FILTER_POINT || texelFetch)
		{
			sampleTexel(c, uuuu, vvvv, wwww, offset, w, mipmap, buffer, option);
		}
		else
		{
			Vector4f c0;
			...
			sampleTexel(c0, uuuu0, vvvv0, wwww, offset, w, mipmap, buffer, option);
			sampleTexel(c1, uuuu1, vvvv0, wwww, offset, w, mipmap, buffer, option);
			sampleTexel(c2, uuuu0, vvvv1, wwww, offset, w, mipmap, buffer, option);
			sampleTexel(c3, uuuu1, vvvv1, wwww, offset, w, mipmap, buffer, option);
	...
	void SamplerCore::sampleFloat3D(Pointer<Byte> &texture, Vector4f &c, Float4 &u, Float4 &v, Float4 &w, Vector4f &offset, Float &lod, bool secondLOD, SamplerOption option)
	{
		int componentCount = textureComponentCount();
		...
		else
		{
			...
			sampleTexel(c0, uuuu0, vvvv0, wwww0, offset, w, mipmap, buffer, option);
			sampleTexel(c1, uuuu1, vvvv0, wwww0, offset, w, mipmap, buffer, option);
			sampleTexel(c2, uuuu0, vvvv1, wwww0, offset, w, mipmap, buffer, option);
			sampleTexel(c3, uuuu1, vvvv1, wwww0, offset, w, mipmap, buffer, option);
			sampleTexel(c4, uuuu0, vvvv0, wwww1, offset, w, mipmap, buffer, option);
			sampleTexel(c5, uuuu1, vvvv0, wwww1, offset, w, mipmap, buffer, option);
			sampleTexel(c6, uuuu0, vvvv1, wwww1, offset, w, mipmap, buffer, option);
			sampleTexel(c7, uuuu1, vvvv1, wwww1, offset, w, mipmap, buffer, option);
	...
	void SamplerCore::computeIndices(Int index[4], Short4 uuuu, Short4 vvvv, Short4 wwww, Vector4f &offset, const Pointer<Byte> &mipmap, SamplerOption option)
	{
		Short4 uuu2;

		bool texelFetch = (option & Fetch) != 0;
		bool hasOffset = (option & Offset) != 0;
	...
	void SamplerCore::sampleTexel(Vector4s &c, Short4 &uuuu, Short4 &vvvv, Short4 &wwww, Vector4f &offset, Pointer<Byte> &mipmap, Pointer<Byte> buffer[4], SamplerOption option)
	{
		Int index[4];

		computeIndices(index, uuuu, vvvv, wwww, offset, mipmap, option);
		...
		else if(hasYuvFormat())
		{
			...
			computeIndices(index, uuuu, vvvv, wwww, offset, mipmap + sizeof(Mipmap), option);
	...
	void SamplerCore::sampleTexel(Vector4f &c, Short4 &uuuu, Short4 &vvvv, Short4 &wwww, Vector4f &offset, Float4 &z, Pointer<Byte> &mipmap, Pointer<Byte> buffer[4], SamplerOption option)
	{
		Int index[4];
