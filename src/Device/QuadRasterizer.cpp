...
void QuadRasterizer::rasterize(Int &yMin, Int &yMax)
{
	...
	Do
	{
		...
		auto oy = SIMD::Float([](int i) { return float((i & 0x2) >> 1 | (i & 0x8) >> 2); });  // 0, 0, 1, 1, 0, 0, 1, 1, 2, 2, 3, 3, 2, 2, 3, 3
		fy = SIMD::Float(Float(y)) + oy - SIMD::Float(*Pointer<Float>(primitive + OFFSET(Primitive, y0)));
		...
		If(x0 < x1)
		{
			if(interpolateW())
			{
				Dw = SIMD::Float(*Pointer<Float>(primitive + OFFSET(Primitive, w.C))) + fy * SIMD::Float(*Pointer<Float>(primitive + OFFSET(Primitive, w.B)));
			}
			...
			if(spirvShader)
			{
					...
					if(spirvShader->inputs[interfaceInterpolant].Type != SpirvShader::ATTRIBTYPE_UNUSED)
					{
						...
						if(!spirvShader->inputs[interfaceInterpolant].Flat)
						{
							Dv[interfaceInterpolant] +=
							    fy * SIMD::Float(*Pointer<Float>(primitive + OFFSET(Primitive, V[packedInterpolant].B)));
						}
						packedInterpolant++;
					}
				}
				...
				for(unsigned int i = 0; i < state.numClipDistances; i++)
				{
					DclipDistance[i] = SIMD::Float(*Pointer<Float>(primitive + OFFSET(Primitive, clipDistance[i].C))) +
					                   fy * SIMD::Float(*Pointer<Float>(primitive + OFFSET(Primitive, clipDistance[i].B)));
				}
				...
				for(unsigned int i = 0; i < state.numCullDistances; i++)
				{
					DcullDistance[i] = SIMD::Float(*Pointer<Float>(primitive + OFFSET(Primitive, cullDistance[i].C))) +
					                   fy * SIMD::Float(*Pointer<Float>(primitive + OFFSET(Primitive, cullDistance[i].B)));
				}
			}
