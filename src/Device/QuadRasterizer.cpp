	...
	Do
	{
		...
		if(interpolateZ())
		{
			for(unsigned int q = 0; q < state.multiSampleCount; q++)
			{
				...
				Dz[q] = SIMD::Float(*Pointer<Float>(primitive + OFFSET(Primitive, z_.C))) + y * SIMD::Float(*Pointer<Float>(primitive + OFFSET(Primitive, z_.B)));
			}
		}
		...
		If(x0 < x1)
		{
			if(interpolateW())
			{
				Dw = SIMD::Float(*Pointer<Float>(primitive + OFFSET(Primitive, w_.C))) + yyyy * SIMD::Float(*Pointer<Float>(primitive + OFFSET(Primitive, w_.B)));
			}
			...
			if(spirvShader)
			{
				...
				for(int interfaceInterpolant = 0; interfaceInterpolant < MAX_INTERFACE_COMPONENTS; interfaceInterpolant++)
				{
					...
					Dv[interfaceInterpolant] = *Pointer<Float>(primitive + OFFSET(Primitive, V_[packedInterpolant].C));
					if(!spirvShader->inputs[interfaceInterpolant].Flat)
					{
						Dv[interfaceInterpolant] +=
						    yyyy * SIMD::Float(*Pointer<Float>(primitive + OFFSET(Primitive, V_[packedInterpolant].B)));
					}
					packedInterpolant++;
				}
				...
				{
					DclipDistance[i] = SIMD::Float(*Pointer<Float>(primitive + OFFSET(Primitive, clipDistance_[i].C))) +
					                   yyyy * SIMD::Float(*Pointer<Float>(primitive + OFFSET(Primitive, clipDistance_[i].B)));
				}

				for(unsigned int i = 0; i < state.numCullDistances; i++)
				{
					DcullDistance[i] = SIMD::Float(*Pointer<Float>(primitive + OFFSET(Primitive, cullDistance_[i].C))) +
					                   yyyy * SIMD::Float(*Pointer<Float>(primitive + OFFSET(Primitive, cullDistance_[i].B)));
				}
			}
