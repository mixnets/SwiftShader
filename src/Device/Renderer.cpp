...
namespace sw
{
	...
	void Renderer::draw(const sw::Context* context, VkIndexType indexType, unsigned int count, int baseVertex, TaskEvents *events, bool update)
	{
		...
		draw->primitiveCount = count;
		draw->primitivesPerBatch = BatchSize / ms;
	...
	void DrawCall::teardown()
	{
		...
		vertexRoutine->unbind();
		setupRoutine->unbind();
		pixelRoutine->unbind();
	}

	void DrawCall::run(const yarn::Loan<DrawCall>& call, yarn::Ticket::Queue* tickets)
	{
		call->setup();

		call->numBatches = (call->primitiveCount + call->primitivesPerBatch - 1) / call->primitivesPerBatch;
		call->numBatchWorkers = std::min<unsigned int>(call->numBatches, BatchData::MaxInstances);
		call->batchTickets = tickets->takeN(call->numBatches);
		auto callFinishedTicket = tickets->take();

		for (unsigned int batchWorker = 0; batchWorker < call->numBatchWorkers; batchWorker++)
		{
			auto batchData = call->batchDataPool->borrow();

			yarn::schedule([=] {
				for (unsigned int batch = batchWorker; batch < call->numBatches; batch += call->numBatchWorkers)
				{
					auto ticket = call->batchTickets[batch];
					defer(ticket.done());

					int batchFirstPrimitive = batch * call->primitivesPerBatch;
					int batchPrimitiveCount = std::min(batchFirstPrimitive + call->primitivesPerBatch, call->primitiveCount) - batchFirstPrimitive;
					...
					{
						SCOPED_EVENT("VERTEX draw %d, batch %d", id, batch);
						...
						vertexTask.primitiveStart = batchFirstPrimitive;
						vertexTask.vertexCount = batchPrimitiveCount * 3;
						if (vertexTask.vertexCache.drawCall != call->id)
						{
							vertexTask.vertexCache.clear();
							vertexTask.vertexCache.drawCall = call->id;
						}

						{
							SCOPED_EVENT("vertices");
							call->vertexPointer(&triangleBatch.front().v0, &triangleIndices[0][0], &vertexTask, call->data);
						}

						if (call->setupState.rasterizerDiscard)
						{
							return;
						}

						{
							...
							visible = call->setupPrimitives(&triangleBatch, &primitiveBatch, call.get(), batchPrimitiveCount);
						}
						if (visible <= 0)
						{
							return;
					...
					{
						SCOPED_EVENT("PIXEL draw %d, batch %d", id, batch);
						if (call->topology != VK_PRIMITIVE_TOPOLOGY_POINT_LIST)
						{
							yarn::WaitGroup clusterWG(ClusterCount);
							for (int cluster = 0; cluster < ClusterCount; cluster++)
							{
								yarn::schedule([=] {
									defer(clusterWG.done());
									SCOPED_EVENT("draw %d, batch %d, cluster %d", id, batch, cluster);
									auto& primitiveBatch = batchData->primitives;
									call->pixelPointer(&primitiveBatch.front(), visible, cluster, call->data);
									});
							}
							clusterWG.wait();
						}
		...
		yarn::schedule([call, callFinishedTicket] {
			callFinishedTicket.wait();
			call->teardown();
		});
	}
