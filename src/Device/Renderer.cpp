...
namespace sw
{
	...
	void Renderer::draw(const sw::Context* context, VkIndexType indexType, unsigned int count, int baseVertex, TaskEvents *events, bool update)
	{
		...
		DrawCall::run(draw, &tickets, clusterQueues);
	}
	...
	{
		unsigned int numPrimitivesPerChunk = batch->numPrimitives;

		switch(draw->topology)
		{
		case VK_PRIMITIVE_TOPOLOGY_POINT_LIST:
			numPrimitivesPerChunk = 64;
			break;
		default:
			break;
		}

		yarn::storage::Vector<int, 32> numVisible;
		auto numChunks = (batch->numPrimitives + numPrimitivesPerChunk - 1) / numPrimitivesPerChunk;
		numVisible.resize(numChunks);
		yarn::parallelize(batch->numPrimitives, numPrimitivesPerChunk, [=, &numVisible] (size_t id, size_t first, size_t count) {
			SCOPED_EVENT("PRIMITIVES draw %d batch %d [%d-%d]", draw->id, batch->id, first, first+count);
			auto triangles = &batch->triangles[first];
			auto primitives = &batch->primitives[first];
			numVisible[id] += draw->setupPrimitives(triangles, primitives, draw, count);
		});

		// Defrag any gaps.
		unsigned int totalNumVisible = 0;
		for (size_t i = 0; i < numChunks; i++)
		{
			auto src = &batch->primitives[i*numPrimitivesPerChunk];
			auto dst = &batch->primitives[totalNumVisible];
			auto cnt = numVisible[i];
			if (src != dst)
			{
				memcpy(dst, src, cnt * sizeof(Primitive));
			}
			totalNumVisible += cnt;
		}
		batch->numVisible = totalNumVisible;
	}
	...
	void DrawCall::processPixels(const yarn::Loan<DrawCall>& draw, const yarn::Loan<BatchData>& batch, const std::shared_ptr<yarn::Finally>& finally)
	{
		for (int cluster = 0; cluster < MaxClusterCount; cluster++)
		{
			batch->clusterTickets[cluster].onCall([draw, batch, cluster, finally]
			{
