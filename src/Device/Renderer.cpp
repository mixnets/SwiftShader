			...
			yarn::schedule([&, batch, batchData, ticket] {
...

				{
...

					{
						...
						vertexPointer(&triangleBatch.front().v0, &triangleIndices[0][0], &vertexTask, data);
					}
...

					{
						...
						visible = setupPrimitives(&triangleBatch, &primitiveBatch, this, batchPrimitiveCount);
					}
					if(visible <= 0)
					{
						return;
				...
				{
					SCOPED_EVENT("draw %d, batch %d", id, batch);
					if (this->topology != VK_PRIMITIVE_TOPOLOGY_POINT_LIST)
					{
						yarn::WaitGroup clusterWG(ClusterCount);
						for (int cluster = 0; cluster < ClusterCount; cluster++)
						{
							yarn::schedule([=, &primitiveBatch] {
								SCOPED_EVENT("draw %d, batch %d, cluster %d", id, batch, cluster);
								defer(clusterWG.done());
								pixelPointer(&primitiveBatch.front(), visible, cluster, data);
							});
						}
						clusterWG.wait();
					}
