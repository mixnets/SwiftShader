                    ...
                    vk::Pipeline::PushConstantStorage const &pushConstants, bool update)
{
	...
	if(update)
	{
		...
		vertexRoutine = vertexProcessor.routine(vertexState, pipelineState.getPipelineLayout(), vertexShader, inputs.descriptorSets);
		setupRoutine = setupProcessor.routine(setupState);
		pixelRoutine = pixelProcessor.routine(pixelState, pipelineState.getPipelineLayout(), fragmentShader, inputs.descriptorSets);
	}
	...
	draw->descriptorSetObjects = inputs.descriptorSetObjects;
	...
	data->descriptorSets = inputs.descriptorSets;
	data->descriptorDynamicOffsets = inputs.descriptorDynamicOffsets;

	for(int i = 0; i < MAX_INTERFACE_COMPONENTS / 4; i++)
	{
		data->input[i] = inputs.stream[i].buffer;
		data->robustnessSize[i] = inputs.stream[i].robustnessSize;
		data->stride[i] = inputs.stream[i].vertexStride;
	}
