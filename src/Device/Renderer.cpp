...
namespace sw
{
	...
	void Renderer::draw(const sw::Context* context, VkIndexType indexType, unsigned int count, int baseVertex, TaskEvents *events, bool update)
	{
		...
		draw->batchTickets.resize(0);
		draw->batchTickets.reserve(draw->numBatches);
		tickets.takeN(draw->numBatches, [&](yarn::Ticket&& ticket) { draw->batchTickets.emplace_back(std::move(ticket)); });
	...
	void DrawCall::run(const yarn::Loan<DrawCall>& draw)
	{
		...
		std::vector<yarn::Loan<BatchData>> batchDatas;
		batchDatas.reserve(draw->numBatchWorkers);
		{
			SCOPED_EVENT("batchDataPool->borrowN()");
			draw->batchDataPool->borrowN(draw->numBatchWorkers, [&](yarn::Loan<BatchData>&& batchData) {
				batchDatas.emplace_back(batchData);
			});
		}

		for (unsigned int batchWorker = 0; batchWorker < draw->numBatchWorkers; batchWorker++)
		{
			auto batch = std::move(batchDatas[batchWorker]);
			batch->draw = draw;
