...
namespace sw
{
	...
	void DrawCall::run(const yarn::Loan<DrawCall>& draw, yarn::Ticket::Queue* tickets)
	{
		draw->setup();
		...
		for (unsigned int batchId = 0; batchId < numBatches; batchId++)
		{
			...
			yarn::schedule([draw, batch, ticket] {
				...
				processPixels(draw, batch, ticket);
			});
		}

		auto ticket = tickets->take();
		yarn::schedule([ticket, draw] {
			ticket.wait();
	...
	{
		SCOPED_EVENT("VERTEX draw %d, batch %d", draw->id, batch->id);
	...
	void DrawCall::processPixels(yarn::Loan<DrawCall> draw, yarn::Loan<BatchData> batch, yarn::Ticket ticket)
	{
		yarn::schedule([draw, batch, ticket] {
			auto clusterCount = calcNumClusters(batch.get());

			ticket.wait();

			for (int cluster = 1; cluster < clusterCount; cluster++)
			{
				yarn::schedule([draw, batch, cluster, clusterCount, ticket] {
					SCOPED_EVENT("PIXEL draw %d, batch %d, cluster %d", draw->id, batch->id, cluster);
					draw->pixelPointer(&batch->primitives.front(), batch->numVisible, cluster, clusterCount, draw->data);
				});
			}

			SCOPED_EVENT("PIXEL draw %d, batch %d, cluster %d", draw->id, batch->id, 0);
			draw->pixelPointer(&batch->primitives.front(), batch->numVisible, 0, clusterCount, draw->data);
		});
	}

	int DrawCall::calcNumClusters(BatchData* batch)
	{
		int pixels = calcNumPixels(batch);
		auto count = ceilPow2((pixels + PixelsPerCluster - 1) / PixelsPerCluster);
		return std::min<int>(count, MaxClusterCount);
	}

	int DrawCall::calcNumPixels(BatchData* batch)
	{
		int pixels = 0;
		for (unsigned int i = 0; i < batch->numPrimitives; i++)
		{
			auto &primitive = batch->primitives[i];
			for (int y = primitive.yMin; y < primitive.yMax; y++)
			{
				auto &outline = primitive.outline[y];
				pixels += outline.right - outline.left;
			}
		}
		return pixels;
	}
