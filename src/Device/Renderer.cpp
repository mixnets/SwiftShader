...
namespace sw
{
	...
	void DrawCall::run()
	{
		...
		yarn::TicketQueue tickets;
		yarn::WaitGroup batchWG(numBatches);
		...
		for (unsigned int batch = 0; batch < numBatches; batch++)
		{
			...
			yarn::schedule([&, batch, batchData, ticket] {
				defer(batchWG.done());
				defer(ticket.done());
				...
				{
					SCOPED_EVENT("draw %d, batch %d", id, batch);
					...
					{
						SCOPED_EVENT("setupPrimitives");
						visible = setupPrimitives(batchData.get(), this, batchPrimitiveCount);
					}
					if(visible <= 0)
					{
						return;
