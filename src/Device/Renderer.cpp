...
void Renderer::draw(const vk::GraphicsPipeline *pipeline, const vk::DynamicState &dynamicState, unsigned int count, int baseVertex,
                    ...
                    const VkOffset2D &framebufferOffset, vk::Pipeline::PushConstantStorage const &pushConstants, bool update)
{
	if(count == 0) { return; }
	...
	// Scissor
	{
		...
		int x0 = framebufferOffset.x;
		int y0 = framebufferOffset.y;
		int x1 = x0 + framebufferExtent.width;
		int y1 = y0 + framebufferExtent.height;
