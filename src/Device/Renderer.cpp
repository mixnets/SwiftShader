...
namespace sw
{
	...
	void DrawCall::run(const yarn::Loan<DrawCall>& draw, yarn::Ticket::Queue* tickets, yarn::Ticket::Queue clusterQueues[MaxClusterCount])
	{
		...
		yarn::schedule([draw, ticket, clusterQueues] {
			for (int i = 0; i < MaxClusterCount; i++)
			{
				clusterQueues[i].take().wait();
			}
	...
	void DrawCall::processPixels(yarn::Loan<DrawCall> draw, yarn::Loan<BatchData> batch)
	{
		for (int cluster = 0; cluster < MaxClusterCount; cluster++)
		{
			batch->clusterTickets[cluster].onCall([draw, batch, cluster]
			{
