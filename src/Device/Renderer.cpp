...
namespace sw
{
	...
	template<typename T>
	inline bool setBatchIndices(unsigned int batch[128][3], VkPrimitiveTopology topology, const T* indices, unsigned int start, unsigned int primitiveCount)
	{
	...
	void Renderer::processPrimitiveVertices(int unit, unsigned int start, unsigned int primitiveCount, unsigned int loop, int thread)
	{
		...
		else
		{
			...
			case VK_INDEX_TYPE_UINT16:
				if(!setBatchIndices(batch, topology, static_cast<const uint16_t*>(indices), start, primitiveCount))
				{
					return;
				}
				break;
			case VK_INDEX_TYPE_UINT32:
				if(!setBatchIndices(batch, topology, static_cast<const uint32_t*>(indices), start, primitiveCount))
				{
					return;
				}
				break;
		...
		task->vertexCount = primitiveCount * 3;
