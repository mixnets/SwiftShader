...
namespace sw
{
	...
	void DrawCall::run(const yarn::Loan<DrawCall>& call, yarn::Ticket::Queue* tickets)
	{
		...
		auto numBatches = (call->primitiveCount + call->primitivesPerBatch - 1) / call->primitivesPerBatch;

		constexpr int batchWorkers = 8;

		auto batchTickets = tickets->takeN(numBatches);

		for (unsigned int batchWorker = 0; batchWorker < batchWorkers; batchWorker++)
		{
			auto batchData = call->batchDataPool->borrow();

			yarn::schedule([=] {
				for (unsigned int batch = batchWorker; batch < numBatches; batch += batchWorkers)
				{
					auto ticket = batchTickets[batch];
					defer(ticket.done());
