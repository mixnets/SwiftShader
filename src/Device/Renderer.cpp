...
namespace sw
{
	...
	void DrawCall::processPrimitives(DrawCall* draw, BatchData* batch)
	{
		...
		std::atomic<int> numVisible = { 0 };
		yarn::parallelize(batch->numPrimitives, numPrimitivesPerChunk, [=, &numVisible] (size_t first, size_t count) {
			...
			numVisible += draw->setupPrimitives(triangles, primitives, draw, count);
		});
		batch->numVisible = numVisible;
	}
