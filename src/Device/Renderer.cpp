...
namespace sw
{
	...
	void DrawCall::run(const yarn::Loan<DrawCall>& draw)
	{
		...
		for (unsigned int batchId = 0; batchId < numBatches; batchId++)
		{
			...
			yarn::schedule([draw, batch, finally] {
				auto ticket = std::move(draw->batchTickets[batch->id]);
				...
				yarn::schedule([draw, batch, ticket, finally] {
					ticket.wait();
					processPixels(draw.get(), batch.get());
				});
			});
		}
	...
	void DrawCall::processPixels(DrawCall* draw, BatchData* batch)
	{
		SCOPED_EVENT("PIXEL");
