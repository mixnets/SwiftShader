...
namespace sw
{
	...
	static constexpr int BatchSize = 512;
	static constexpr int PixelsPerCluster = 512;
	static constexpr int MaxClusterCount = 32;
	...
	struct DrawCall
	{
		struct BatchData
		{
			static constexpr int MaxInstances = 4;
		...
		static void run(const yarn::Loan<DrawCall>& draw, yarn::Ticket::Queue* tickets);
		static void processVertices(DrawCall* draw, BatchData* batch);
		static void processPrimitives(DrawCall* draw, BatchData* batch);
		static void processPixels(yarn::Loan<DrawCall> draw, yarn::Loan<BatchData> batch, yarn::Ticket ticket);
		static int calcNumClusters(BatchData* batch);
		static int calcNumPixels(BatchData* batch);
		void setup();
		void teardown();
