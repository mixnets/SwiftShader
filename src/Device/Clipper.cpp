...
void clipNear(sw::Polygon &polygon, bool negativeOneToOne)
{
	...
	for(int i = 0; i < polygon.n; i++)
	{
		...
		float di = V[i]->z + (negativeOneToOne ? V[i]->w : 0.0f);
		float dj = V[j]->z + (negativeOneToOne ? V[j]->w : 0.0f);
...
bool Clipper::Clip(Polygon &polygon, int clipFlagsOr, const DrawCall &draw)
{
	...
	{
		if(clipFlagsOr & CLIP_NEAR) clipNear(polygon, draw.depthClipState.negativeOneToOne);
		if(polygon.n >= 3)
		{
