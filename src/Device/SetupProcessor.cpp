...
SetupProcessor::State SetupProcessor::update(const sw::Context *context) const
{
	...
	state.fixedPointDepthBuffer = context->depthBuffer && context->depthBuffer->getFormat(VK_IMAGE_ASPECT_DEPTH_BIT);
	...
	state.interpolateZ = true;  //context->depthBufferActive() || vPosZW;
