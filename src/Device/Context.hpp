...
struct IndexBuffer
{
	void setBinding(const VkIndexType indexType, Buffer *buffer, const VkDeviceSize offset);
	inline VkIndexType getIndexType() const { return indexType; }
	void getIndexBuffers(VkPrimitiveTopology topology, uint32_t count, uint32_t first, bool indexed, bool hasPrimitiveRestartEnable, std::vector<std::pair<uint32_t, void *>> *indexBuffers);
...
struct Inputs
{
	...
	void setVertexInputBinding(uint32_t binding, vk::Buffer *buffer, const VkDeviceSize offset);
