...
namespace sw
{
	...
	{
		if(dest->getFormat(subresourceRange.aspectMask) == VK_FORMAT_UNDEFINED)
		{
			return;
		}
		...
		State state(format, dest->getFormat(subresourceRange.aspectMask), dest->getSampleCountFlagBits(), { 0xF });
		...
		for(; subresLayers.mipLevel <= lastMipLevel; subresLayers.mipLevel++)
		{
			...
			BlitData data =
			{
				...
				dest->rowPitchBytes(subresourceRange.aspectMask, subresLayers.mipLevel),   // dPitchB
				dest->slicePitchBytes(subresourceRange.aspectMask, subresLayers.mipLevel), // dSliceB
	...
	bool Blitter::fastClear(void *pixel, VkFormat format, vk::Image *dest, const VkImageSubresourceRange& subresourceRange, const VkRect2D* renderArea)
	{
		...
		switch(dest->getFormat(subresourceRange.aspectMask))
		{
		...
		for(; subresLayers.mipLevel <= lastMipLevel; subresLayers.mipLevel++)
		{
			int rowPitchBytes = dest->rowPitchBytes(subresourceRange.aspectMask, subresLayers.mipLevel);
			int slicePitchBytes = dest->slicePitchBytes(subresourceRange.aspectMask, subresLayers.mipLevel);
			VkExtent3D extent = dest->getMipLevelExtent(subresLayers.mipLevel);
			if(!renderArea)
			{
				area.extent.width = extent.width;
			...
			for(subresLayers.baseArrayLayer = subresourceRange.baseArrayLayer; subresLayers.baseArrayLayer <= lastLayer; subresLayers.baseArrayLayer++)
			{
				for(uint32_t depth = 0; depth < extent.depth; depth++)
				{
					...
					for(int j = 0; j < dest->getSampleCountFlagBits(); j++)
					{
						...
						switch(Surface::bytes(dest->getFormat(subresourceRange.aspectMask)))
						{
						case 2:
	...
	void Blitter::blit(vk::Image *src, vk::Image *dst, const VkImageBlit& region, VkFilter filter)
	{
		...
		State state(src->getFormat(), dst->getFormat(), dst->getSampleCountFlagBits(),
		...
		data.sPitchB = src->rowPitchBytes(region.srcSubresource.aspectMask, region.srcSubresource.mipLevel);
		data.dPitchB = dst->rowPitchBytes(region.dstSubresource.aspectMask, region.dstSubresource.mipLevel);
		data.dSliceB = dst->slicePitchBytes(region.dstSubresource.aspectMask, region.dstSubresource.mipLevel);
		...
		VkOffset3D srcOffset = region.srcOffsets[0];
		VkOffset3D dstOffset = region.dstOffsets[0];

		for(int i = 0; i < numSlices; i++)
		{
			data.source = src->getTexelPointer(srcOffset, region.srcSubresource);
			data.dest = dst->getTexelPointer(dstOffset, region.srcSubresource);
			blitFunction(&data);
			srcOffset.z++;
			dstOffset.z++;
		}
