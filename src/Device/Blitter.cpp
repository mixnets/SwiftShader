...
namespace sw
{
	...
	void Blitter::clear(void *pixel, vk::Format format, vk::Image *dest, const vk::Format& viewFormat, const VkImageSubresourceRange& subresourceRange, const VkRect2D* renderArea)
	{
		...
		RoutineSPtr blitRoutine = getBlitRoutine(state);
		if(!blitRoutine)
		{
			return;
		}
	...
	RoutineSPtr Blitter::getBlitRoutine(const State &state)
	{
	...
	RoutineSPtr Blitter::getCornerUpdateRoutine(const State &state)
	{
		std::unique_lock<std::mutex> lock(cornerUpdateMutex);
		RoutineSPtr cornerUpdateRoutine = cornerUpdateCache.query(state);
	...
	RoutineSPtr Blitter::generateCornerUpdate(const State& state)
	{
	...
	void Blitter::updateBorders(vk::Image* image, const VkImageSubresourceLayers& subresourceLayers)
	{
		...
		RoutineSPtr cornerUpdateRoutine = getCornerUpdateRoutine(state);
		if(!cornerUpdateRoutine)
		{
			return;
		}
