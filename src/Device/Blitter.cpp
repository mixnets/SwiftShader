...
void Blitter::clear(const void *pixel, vk::Format format, vk::Image *dest, const vk::Format &viewFormat, const VkImageSubresourceRange &subresourceRange, const VkRect2D *renderArea)
{
	...
	for(; subres.mipLevel <= lastMipLevel; subres.mipLevel++)
	{
		...
		BlitData data = {
			...
			format.bytes32(),                                  // sPitchB
			dest->rowPitchBytes32(aspect, subres.mipLevel),    // dPitchB
			0,                                                 // sSliceB (unused in clear operations)
			dest->slicePitchBytes32(aspect, subres.mipLevel),  // dSliceB
...
void Blitter::blit(const vk::Image *src, vk::Image *dst, VkImageBlit2KHR region, VkFilter filter)
{
	...
	BlitData data = {
		nullptr,                                                            // source
		nullptr,                                                            // dest
		src->rowPitchBytes32(srcAspect, region.srcSubresource.mipLevel),    // sPitchB
		dst->rowPitchBytes32(dstAspect, region.dstSubresource.mipLevel),    // dPitchB
		src->slicePitchBytes32(srcAspect, region.srcSubresource.mipLevel),  // sSliceB
		dst->slicePitchBytes32(dstAspect, region.dstSubresource.mipLevel),  // dSliceB
...
void Blitter::updateBorders(const vk::Image *image, const VkImageSubresource &subresource)
{
	...
	CubeBorderData data = {
		...
		image->rowPitchBytes32(aspect, subresource.mipLevel),
