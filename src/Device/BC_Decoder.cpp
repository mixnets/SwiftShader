...
namespace
{
	...
	struct BC_color
	{
		void decode(unsigned char* dst, int x, int y, int dstW, int dstH, int dstPitch, int dstBpp, bool hasAlphaChannel, bool hasSeparateAlpha, bool isSigned) const
		{
			Color c[4];
			c[0].extract565(c0, isSigned);
			c[1].extract565(c1, isSigned);
	...
	private:
		struct Color
		{
			...
			void extract565(const unsigned int c565, bool isSigned)
			{
	...
	private:
		unsigned char c0() const
		{
			return static_cast<unsigned char>(data & 0xFF);
		}

		unsigned char c1() const
		{
			return static_cast<unsigned char>((data & 0xFF00) >> 8);
		}

		signed char c0s() const
		{
			return static_cast<signed char>(data & 0xFF);
		}

		signed char c1s() const
		{
			return static_cast<signed char>((data & 0xFF00) >> 8);
		}
...
bool BC_Decoder::Decode(const unsigned char* src, unsigned char* dst, int w, int h, int dstW, int dstH, int dstPitch, int dstBpp, int n, bool isNoAlphaU)
{
	...
	static_assert(sizeof(BC_channel) == 8, "BC_channel must be 16 bytes");
	static_assert(sizeof(BC_alpha) == 8, "BC_alpha must be 16 bytes");
	...
	case 1: // BC1
		{
			...
			for(int y = 0; y < h; y += BlockHeight, dst += dy)
			{
				...
				{
					color->decode(dstRow, x, y, dstW, dstH, dstPitch, dstBpp, isAlpha, false, isSigned);
				}
			}
		}
		break;
	case 2: // BC2
		{
			...
			for(int y = 0; y < h; y += BlockHeight, dst += dy)
			{
				...
				{
					color->decode(dstRow, x, y, dstW, dstH, dstPitch, dstBpp, isAlpha, true, isSigned);
	...
	case 5: // BC5
		...
		break;
	case 6: // BC6H, unimplemented
	case 7: // BC7, unimplemented
	default:
		return false;
	}
