...
namespace sw
{
	...
	private:
		...
		RoutineSPtr getBlitRoutine(const State &state);
		RoutineSPtr generate(const State &state);
		RoutineSPtr getCornerUpdateRoutine(const State &state);
		RoutineSPtr generateCornerUpdate(const State& state);
