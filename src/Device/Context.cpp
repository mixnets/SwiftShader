...
void IndexBuffer::setBinding(const VkIndexType indexType, vk::Buffer *buffer, const VkDeviceSize offset)
{
...
void IndexBuffer::getIndexBuffers(VkPrimitiveTopology topology, uint32_t count, uint32_t first, bool indexed, bool hasPrimitiveRestartEnable, std::vector<std::pair<uint32_t, void *>> *indexBuffers)
{
	if(indexed)
	{
...
void Inputs::setVertexInputBinding(uint32_t binding, vk::Buffer *buffer, const VkDeviceSize offset)
{
	vertexInputBindings[binding] = { buffer, offset };
}
    ...
    , robustBufferAccess(robustBufferAccess)
{
	...
	depthBias = (rasterizationState->depthBiasEnable != VK_FALSE) ? rasterizationState->depthBiasConstantFactor : 0.0f;
...
const GraphicsState GraphicsState::combineStates(const DynamicState &dynamicState) const
{
	...
	{
		combinedState.depthBias = dynamicState.depthBiasConstantFactor;
