...
void IndexBuffer::getIndexBuffers(VkPrimitiveTopology topology, uint32_t count, uint32_t first, bool indexed, const VertexInputBinding &binding, VkIndexType type, bool hasPrimitiveRestartEnable, std::vector<std::pair<uint32_t, void *>> *indexBuffers)
{
	if(indexed)
	{
		...
		if(hasPrimitiveRestartEnable)
		{
			switch(type)
			{
				...
				case VK_INDEX_TYPE_UINT32:
					...
					break;
				default:
					UNSUPPORTED("VkIndexType %d", int(type));
			}
		}
		else
		{
