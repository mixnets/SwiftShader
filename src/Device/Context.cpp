...
void Inputs::setVertexInputBinding(const VertexInputBinding bindings[], const DynamicState &dynamicState)
{
	...
	if(dynamicStateFlags.dynamicVertexInput || dynamicStateFlags.dynamicVertexInputBindingStride)
	{
		...
		{
			sw::Stream &input = stream[i];
			const auto &desc = dynamicState.vertexInputBindings[input.binding];

			input.vertexStride = desc.inputRate == VK_VERTEX_INPUT_RATE_VERTEX ? desc.stride : 0;
			input.instanceStride = desc.inputRate == VK_VERTEX_INPUT_RATE_INSTANCE ? desc.stride : 0;
		}
	}
}
