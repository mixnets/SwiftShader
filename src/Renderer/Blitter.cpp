...
namespace sw
{
	...
	bool Blitter::write(Float4 &c, Pointer<Byte> element, Format format, const BlitterOptions& options)
	{
		...
		case FORMAT_R8UI:
		case FORMAT_R8:
			if(useR) { *Pointer<Byte>(element) = Byte(RoundInt(Float(c.x))); }
			break;
		case FORMAT_A16B16G16R16I:
			if(useRGBA)
			{
		...
		case FORMAT_A16B16G16R16:
			...
			{
				if(useR) { *Pointer<UShort>(element) = UShort(RoundInt(Float(c.x))); }
				if(useG) { *Pointer<UShort>(element + 2) = UShort(RoundInt(Float(c.y))); }
				if(useB) { *Pointer<UShort>(element + 4) = UShort(RoundInt(Float(c.z))); }
				if(useA) { *Pointer<UShort>(element + 6) = UShort(RoundInt(Float(c.w))); }
			}
			break;
		case FORMAT_X16B16G16R16UI:
			if(useRGBA)
		...
		case FORMAT_A32B32G32R32UI:
			...
			{
				if(useR) { *Pointer<UInt>(element) = As<UInt>(RoundInt(Float(c.x))); }
				if(useG) { *Pointer<UInt>(element + 4) = As<UInt>(RoundInt(Float(c.y))); }
				if(useB) { *Pointer<UInt>(element + 8) = As<UInt>(RoundInt(Float(c.z))); }
				if(useA) { *Pointer<UInt>(element + 12) = As<UInt>(RoundInt(Float(c.w))); }
			}
			break;
		case FORMAT_X32B32G32R32UI:
			if(useRGBA)
	...
	bool Blitter::write(Int4 &c, Pointer<Byte> element, Format format, const BlitterOptions& options)
	{
		...
		case FORMAT_R16I:
			...
			break;
		case FORMAT_A16B16G16R16UI:
			if(useA) { *Pointer<UShort>(element + 6) = UShort(Extract(c, 3)); }
		case FORMAT_X16B16G16R16UI:
			if(useA && (format != FORMAT_A16B16G16R16UI))
			{
				*Pointer<UShort>(element + 6) = UShort(0xFFFF);
			}
			if(useB) { *Pointer<UShort>(element + 4) = UShort(Extract(c, 2)); }
		case FORMAT_G16R16UI:
			if(useG) { *Pointer<UShort>(element + 2) = UShort(Extract(c, 1)); }
		case FORMAT_R16UI:
			if(useR) { *Pointer<UShort>(element) = UShort(Extract(c, 0)); }
			break;
		case FORMAT_A32B32G32R32I:
			if(useRGBA)
			{
