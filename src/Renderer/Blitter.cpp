...
namespace sw
{
	...
	bool Blitter::write(Float4 &c, Pointer<Byte> element, Format format, const BlitterOptions& options)
	{
		...
		case FORMAT_A8R8G8B8:
			...
			{
				if(useB) { *Pointer<Byte>(element + 0) = Byte(RoundUInt(Float(c.z))); }
		...
		case FORMAT_A8B8G8R8:
			...
			{
				if(useR) { *Pointer<Byte>(element + 0) = Byte(RoundUInt(Float(c.x))); }
				if(useG) { *Pointer<Byte>(element + 1) = Byte(RoundUInt(Float(c.y))); }
		...
		case FORMAT_A32B32G32R32UI:
			...
			{
				if(useR) { *Pointer<UInt>(element) = RoundUInt(Float(c.x)); }
				if(useG) { *Pointer<UInt>(element + 4) = RoundUInt(Float(c.y)); }
				if(useB) { *Pointer<UInt>(element + 8) = RoundUInt(Float(c.z)); }
				if(useA) { *Pointer<UInt>(element + 12) = RoundUInt(Float(c.w)); }
			}
			break;
		case FORMAT_X32B32G32R32UI:
			if(useRGBA)
	...
	bool Blitter::write(Int4 &c, Pointer<Byte> element, Format format, const BlitterOptions& options)
	{
		...
		case FORMAT_R8I:
			...
			break;
		case FORMAT_A8B8G8R8UI:
			if(useA) { *Pointer<Byte>(element + 3) = Byte(As<UInt>(Extract(c, 3))); }
		case FORMAT_X8B8G8R8UI:
			if(useA && (format != FORMAT_A8B8G8R8UI))
			{
				*Pointer<Byte>(element + 3) = Byte(0xFF);
			}
			if(useB) { *Pointer<Byte>(element + 2) = Byte(As<UInt>(Extract(c, 2))); }
		case FORMAT_G8R8UI:
			if(useG) { *Pointer<Byte>(element + 1) = Byte(As<UInt>(Extract(c, 1))); }
		case FORMAT_R8UI:
			if(useR) { *Pointer<Byte>(element) = Byte(As<UInt>(Extract(c, 0))); }
			break;
		case FORMAT_A16B16G16R16I:
