...
namespace sw
{
	...
	void Blitter::write(Float4 &c, Pointer<Byte> element, Format format, const Blitter::Options& options)
	{
	...
	void Blitter::write(Int4 &c, Pointer<Byte> element, Format format, const Blitter::Options& options)
	{
		bool writeR = (options & WRITE_RED) == WRITE_RED;
		bool writeG = (options & WRITE_GREEN) == WRITE_GREEN;
		bool writeB = (options & WRITE_BLUE) == WRITE_BLUE;
