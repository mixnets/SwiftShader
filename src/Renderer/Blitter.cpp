...
namespace sw
{
	...
	bool Blitter::write(Float4 &c, Pointer<Byte> element, Format format, const BlitterOptions& options)
	{
		...
		case FORMAT_A8B8G8R8I:
		case FORMAT_A8B8G8R8I_SNORM:
			if(useA) { *Pointer<SByte>(element + 3) = SByte(RoundInt(Float(c.w))); }
		case FORMAT_X8B8G8R8I:
		case FORMAT_X8B8G8R8I_SNORM:
			...
			if(useB) { *Pointer<SByte>(element + 2) = SByte(RoundInt(Float(c.z))); }
		case FORMAT_G8R8I:
		case FORMAT_G8R8I_SNORM:
			if(useG) { *Pointer<SByte>(element + 1) = SByte(RoundInt(Float(c.y))); }
		case FORMAT_R8I:
		case FORMAT_R8I_SNORM:
			if(useR) { *Pointer<SByte>(element) = SByte(RoundInt(Float(c.x))); }
			break;
		case FORMAT_A8B8G8R8UI:
			if(useA) { *Pointer<Byte>(element + 3) = Byte(RoundUInt(Float(c.w))); }
		case FORMAT_X8B8G8R8UI:
		...
		case FORMAT_A16B16G16R16I:
			...
			{
				if(useR) { *Pointer<Short>(element) = Short(RoundInt(Float(c.x))); }
				if(useG) { *Pointer<Short>(element + 2) = Short(RoundInt(Float(c.y))); }
				if(useB) { *Pointer<Short>(element + 4) = Short(RoundInt(Float(c.z))); }
				if(useA) { *Pointer<Short>(element + 6) = Short(RoundInt(Float(c.w))); }
			}
			break;
		case FORMAT_X16B16G16R16I:
			if(useRGBA)
		...
		case FORMAT_A16B16G16R16:
			...
			{
				if(useR) { *Pointer<UShort>(element) = UShort(RoundUInt(Float(c.x))); }
				if(useG) { *Pointer<UShort>(element + 2) = UShort(RoundUInt(Float(c.y))); }
				if(useB) { *Pointer<UShort>(element + 4) = UShort(RoundUInt(Float(c.z))); }
				if(useA) { *Pointer<UShort>(element + 6) = UShort(RoundUInt(Float(c.w))); }
			}
			break;
		case FORMAT_X16B16G16R16UI:
			if(useRGBA)
		...
		case FORMAT_R5G6B5:
			if(useR && useG && useB)
			{
				*Pointer<UShort>(element) = UShort(RoundUInt(Float(c.z)) |
				                                  (RoundUInt(Float(c.y)) << UInt(5)) |
				                                  (RoundUInt(Float(c.x)) << UInt(11)));
			}
			else
			{
				...
				*Pointer<UShort>(element) = (*Pointer<UShort>(element) & UShort(unmask)) | 
				                            (UShort(RoundUInt(Float(c.z)) |
				                                   (RoundUInt(Float(c.y)) << UInt(5)) |
				                                   (RoundUInt(Float(c.x)) << UInt(11))) & UShort(mask));
			}
			break;
		default:
			return false;
	...
	bool Blitter::write(Int4 &c, Pointer<Byte> element, Format format, const BlitterOptions& options)
	{
		...
		{
		case FORMAT_A8B8G8R8I:
			if(useA) { *Pointer<SByte>(element + 3) = SByte(Extract(c, 3)); }
		case FORMAT_X8B8G8R8I:
			if(useA && (format != FORMAT_A8B8G8R8I))
			{
				*Pointer<SByte>(element + 3) = SByte(0x7F);
			}
			if(useB) { *Pointer<SByte>(element + 2) = SByte(Extract(c, 2)); }
		case FORMAT_G8R8I:
			if(useG) { *Pointer<SByte>(element + 1) = SByte(Extract(c, 1)); }
		case FORMAT_R8I:
			if(useR) { *Pointer<SByte>(element) = SByte(Extract(c, 0)); }
			break;
		case FORMAT_A8B8G8R8UI:
			if(useA) { *Pointer<Byte>(element + 3) = Byte(Extract(c, 3)); }
		case FORMAT_X8B8G8R8UI:
			if(useA && (format != FORMAT_A8B8G8R8UI))
			{
				*Pointer<Byte>(element + 3) = Byte(0xFF);
			}
			if(useB) { *Pointer<Byte>(element + 2) = Byte(Extract(c, 2)); }
		case FORMAT_G8R8UI:
			if(useG) { *Pointer<Byte>(element + 1) = Byte(Extract(c, 1)); }
		case FORMAT_R8UI:
			if(useR) { *Pointer<Byte>(element) = Byte(Extract(c, 0)); }
			break;
		case FORMAT_A16B16G16R16I:
			if(useA) { *Pointer<Short>(element + 6) = Short(Extract(c, 3)); }
		case FORMAT_X16B16G16R16I:
			if(useA && (format != FORMAT_A16B16G16R16I))
			{
				*Pointer<Short>(element + 6) = Short(0x7FFF);
			}
			if(useB) { *Pointer<Short>(element + 4) = Short(Extract(c, 2)); }
		case FORMAT_G16R16I:
			if(useG) { *Pointer<Short>(element + 2) = Short(Extract(c, 1)); }
		case FORMAT_R16I:
			if(useR) { *Pointer<Short>(element) = Short(Extract(c, 0)); }
			break;
		case FORMAT_A16B16G16R16UI:
			if(useA) { *Pointer<UShort>(element + 6) = UShort(As<UInt>(Extract(c, 3))); }
		case FORMAT_X16B16G16R16UI:
			if(useA && (format != FORMAT_A16B16G16R16UI))
			{
				*Pointer<UShort>(element + 6) = UShort(0xFFFF);
			}
			if(useB) { *Pointer<UShort>(element + 4) = UShort(As<UInt>(Extract(c, 2))); }
		case FORMAT_G16R16UI:
			if(useG) { *Pointer<UShort>(element + 2) = UShort(As<UInt>(Extract(c, 1))); }
		case FORMAT_R16UI:
			if(useR) { *Pointer<UShort>(element) = UShort(As<UInt>(Extract(c, 0))); }
			break;
		case FORMAT_A32B32G32R32I:
			if(useRGBA)
			{
