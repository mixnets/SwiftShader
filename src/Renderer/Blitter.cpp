...
namespace sw
{
	...
	bool Blitter::blitReactor(Surface *source, const SliceRect &sourceRect, Surface *dest, const SliceRect &destRect, bool filter)
	{
		...
		state.sourceFormat = source->getFormat(true);
		state.destFormat = dest->getFormat(true);
		state.filter = filter;
		...
		data.source = source->lock(0, 0, sourceRect.slice, sw::LOCK_READONLY, sw::PUBLIC, true);
		data.dest = dest->lock(0, 0, destRect.slice, sw::LOCK_WRITEONLY, sw::PUBLIC, true);
		data.sPitchB = source->getPitchB(true);
		data.dPitchB = dest->getPitchB(true);
