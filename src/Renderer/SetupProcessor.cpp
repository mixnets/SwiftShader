			...
			for(int interpolant = 0; interpolant < 10; interpolant++)
			{
				for(int component = 0; component < 4; component++)
				{
					...
					if(semantic.active())
					{
						...
						case Shader::USAGE_COLOR:    flat = flatShading || !semantic.smooth; break;
						}
			...
			for(int interpolant = 0; interpolant < 10; interpolant++)
			{
				for(int component = 0; component < 4; component++)
				{
					...
					case Shader::USAGE_TEXCOORD:
						...
						state.gradient[interpolant][component].flat = (point && !sprite) || !semantic.smooth;
						break;
					case Shader::USAGE_COLOR:
						state.gradient[interpolant][component].attribute = D0 + semantic.index;
						state.gradient[interpolant][component].flat = flatShading || !semantic.smooth;
						break;
					default:
						ASSERT(false);
					}
