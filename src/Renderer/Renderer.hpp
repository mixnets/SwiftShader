...
namespace sw
{
	...
	struct DrawData
	{
		...
		struct VS
		{
			...
			void* u[MAX_UNIFORM_BUFFER_BINDINGS];
			int4 i[16];
			bool b[16];
		};
	...
	public:
		...
		virtual void clear(void* pixel, Format format, Surface *dest, const SliceRect &dRect, const BlitterOptions& options);
		virtual void blit(Surface *source, const SliceRect &sRect, Surface *dest, const SliceRect &dRect, const BlitterOptions& options);
