...
namespace sw
{
	...
	public:
		...
		{
		public:
			Buffer(int width, int height, int depth, Format format, bool target)
			{
				this->buffer = NULL;
				this->width = width;
				this->height = height;
				this->depth = depth;
				this->bytes = Surface::bytes(format);
				this->pitchB = Surface::pitchB(width, format, target);
				this->pitchP = Surface::pitchP(width, format, target);
				this->sliceB = Surface::sliceB(width, height, format, target);
				this->sliceP = Surface::sliceP(width, height, format, target);
				this->format = format;
				this->lock = LOCK_UNLOCKED;
				this->dirty = false;
			}
