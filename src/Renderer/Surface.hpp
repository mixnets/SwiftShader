...
namespace sw
{
	...
	protected:
		...
		Surface(Resource *texture, int width, int height, int depth, Format format, bool lockable, bool renderTarget, int pitchP = 0, int border = 0);
	...
	public:
		...
		void computeCornerInternal(int x0, int y0, int x1, int y1);
		...
		static unsigned int size(int width, int height, int border, int depth, Format format);   // FIXME: slice * depth
