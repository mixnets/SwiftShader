...
namespace sw
{
	...
	void Renderer::draw(DrawType drawType, unsigned int indexOffset, unsigned int count, bool update)
	{
		...
		for(int q = 0; q < ss; q++)
		{
			...
			for(int sampler = 0; sampler < TEXTURE_IMAGE_UNITS + VERTEX_TEXTURE_IMAGE_UNITS; sampler++)
			{
	...
	void Renderer::finishRendering(Task &pixelTask)
	{
		...
		if(ref == 0)
		{
			...
			if(ref == 0)
			{
				...
				for(int i = 0; i < TEXTURE_IMAGE_UNITS + VERTEX_TEXTURE_IMAGE_UNITS; i++)
				{
					if(draw.texture[i])
					{
	...
	{
		ASSERT(sampler < (TEXTURE_IMAGE_UNITS + VERTEX_TEXTURE_IMAGE_UNITS));
	...
	{
		ASSERT(sampler < (TEXTURE_IMAGE_UNITS + VERTEX_TEXTURE_IMAGE_UNITS) && face < 6 && level < MIPMAP_LEVELS);
