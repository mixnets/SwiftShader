...
namespace sw
{
	...
	void Renderer::draw(DrawType drawType, unsigned int indexOffset, unsigned int count, bool update, bool primitiveRestart, unsigned int indexCount)
	{
		...
		if(primitiveRestart && context->indexBuffer)
		{
	...
	void Renderer::advanceToNextBatch(DrawCall *draw, int firstPrimitive)
	{
		...
		for(int i = draw->primitiveRestartDataIndex; i < draw->primitiveRestartData->data.size() && draw->primitiveRestartData->data[i].second < firstPrimitive; i++)
		{
	...
	template <typename T> void Renderer::processINDEXEDsimple(DrawCall *draw, T *indexBegin, T *index, unsigned int batch[][3], unsigned int triangleCount, int primitiveRestartDataIndex)
	{
	...
	template <typename T> void Renderer::processINDEXEDLINELOOP(DrawCall *draw, T *indexBegin, T *index, unsigned int batch[][3], unsigned int triangleCount, unsigned int startPrimitive, int primitiveRestartDataIndex)
	{
		if(!draw->primitiveRestart)
		{
	...
	template <typename T> void Renderer::processINDEXEDTRIANGLESTRIP(DrawCall *draw, T *indexBegin, T *index, unsigned int batch[][3], unsigned int triangleCount, unsigned int startPrimitive, int primitiveRestartDataIndex)
	{
		if(!draw->primitiveRestart)
		{
			for(unsigned int i = 0; i < triangleCount; i++)
	...
	template <typename T> void Renderer::processINDEXEDTRIANGLEFAN(DrawCall *draw, T *indexBegin, T *index, unsigned int batch[][3], unsigned int triangleCount, unsigned int startPrimitive, int primitiveRestartDataIndex)
	{
		if(!draw->primitiveRestart)
		{
			for(unsigned int i = 0; i < triangleCount; i++)
		...
		case DRAW_INDEXEDLINESTRIP8:
			{
				...
				processINDEXEDsimple(draw, (const unsigned char*)indices, index, batch, triangleCount, primitiveProgress[unit].primitiveRestartDataIndex);
			}
			break;
		...
		case DRAW_INDEXEDLINESTRIP16:
			{
				...
				processINDEXEDsimple(draw, (const unsigned short*)indices, index, batch, triangleCount, primitiveProgress[unit].primitiveRestartDataIndex);
			}
			break;
		...
		case DRAW_INDEXEDLINESTRIP32:
			{
				...
				processINDEXEDsimple(draw, (const unsigned int*)indices, index, batch, triangleCount, primitiveProgress[unit].primitiveRestartDataIndex);
			}
			break;
