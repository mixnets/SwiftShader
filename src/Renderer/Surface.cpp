...
namespace sw
{
	...
	Surface::Surface(int width, int height, int depth, Format format, void *pixels, int pitch, int slice, LockResourceId id, ThreadAnalyzer * ta) : lockable(true), renderTarget(false)
	{
		resource = new Resource(0, id, ta);
		hasParent = false;
		ownExternal = false;
		depth = max(1, depth);
	...
	Surface::Surface(Resource *texture, int width, int height, int depth, Format format, bool lockable, bool renderTarget, LockResourceId id, ThreadAnalyzer * ta) : lockable(lockable), renderTarget(renderTarget)
	{
		resource = texture ? texture : new Resource(0, id, ta);
		hasParent = texture != 0;
		ownExternal = true;
		depth = max(1, depth);
