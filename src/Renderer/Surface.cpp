...
namespace sw
{
	...
	Surface::Surface(Resource *texture, int width, int height, int depth, Format format, bool lockable, bool renderTarget, Buffer *pInternal, Buffer *pExternal) : lockable(lockable), renderTarget(renderTarget)
	{
		...
		if (pExternal) {
			...
			external = new Buffer(width, height, depth, format, renderTarget && !texture);
		...
		else
		{
			...
			internal = new Buffer(width, height, depth, internalFormat, !!renderTarget);
			internalToFree = internal;
		}
		stencil = new Buffer(width, height, depth, FORMAT_S8, !!renderTarget);
	...
	Surface::~Surface()
	{
		...
		if (internalToFree && (internal->buffer != external->buffer))
		{
		...
		if (externalToFree) {
			delete externalToFree;
			externalToFree = NULL;
		}
		external = 0;
		if (internalToFree) {
			delete internalToFree;
			internalToFree = NULL;
		}
		internal = 0;
