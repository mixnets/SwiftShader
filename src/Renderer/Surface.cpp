...
namespace sw
{
	...
	Surface::Surface(Resource *texture, int width, int height, int depth, Format format, bool lockable, bool renderTarget, int pitch) : lockable(lockable), renderTarget(renderTarget)
	{
		...
		internal.format = selectInternalFormat(format);
		internal.bytes = bytes(internal.format);
		internal.pitchB = !pitch ? pitchB(internal.width, internal.format, renderTarget) : pitch;
		internal.pitchP = !pitch ? pitchP(internal.width, internal.format, renderTarget) : (pitch / internal.bytes);
