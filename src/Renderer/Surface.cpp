...
namespace sw
{
	...
	void Surface::convertIfRequired(Color<float>& color, bool intSrc, bool signedIntSrc, bool intDst, bool signedIntDst)
	{
		if(intSrc && !intDst)
		{
			if(signedIntSrc)
			{
				color.r = static_cast<float>(*reinterpret_cast<int*>(&color.r));
				color.g = static_cast<float>(*reinterpret_cast<int*>(&color.g));
				color.b = static_cast<float>(*reinterpret_cast<int*>(&color.b));
				color.a = static_cast<float>(*reinterpret_cast<int*>(&color.a));
			}
			else
			{
				color.r = static_cast<float>(*reinterpret_cast<unsigned int*>(&color.r));
				color.g = static_cast<float>(*reinterpret_cast<unsigned int*>(&color.g));
				color.b = static_cast<float>(*reinterpret_cast<unsigned int*>(&color.b));
				color.a = static_cast<float>(*reinterpret_cast<unsigned int*>(&color.a));
			}
		}
		else if(!intSrc && intDst)
		{
			if(signedIntDst)
			{
				color.r = I_as_F(static_cast<int>(color.r));
				color.g = I_as_F(static_cast<int>(color.g));
				color.b = I_as_F(static_cast<int>(color.b));
				color.a = I_as_F(static_cast<int>(color.a));
			}
			else
			{
				color.r = I_as_F(static_cast<unsigned int>(color.r));
				color.g = I_as_F(static_cast<unsigned int>(color.g));
				color.b = I_as_F(static_cast<unsigned int>(color.b));
				color.a = I_as_F(static_cast<unsigned int>(color.a));
			}
		}
	}
	...
	{
		bool intSrc = isNonNormalizedInteger(source.format);
		bool signedIntSrc = isSignedNonNormalizedInteger(source.format);
		bool intDst = isNonNormalizedInteger(destination.format);
		bool signedIntDst = isSignedNonNormalizedInteger(destination.format);
			...
			for(int y = 0; y < height; y++)
			{
				...
				else
				{
					...
					for(int x = 0; x < width; x++)
					{
						...
						convertIfRequired(color, intSrc, signedIntSrc, intDst, signedIntDst);
