	...
	class Blitter
	{
		struct Options
		{
			...
			Options(bool filter, bool useStencil, bool convertSRGB)
				: writeMask(0xF), clearOperation(false), filter(filter), useStencil(useStencil), convertSRGB(convertSRGB), allowOutOfBoundsSourceCoords(false) {}
			Options(unsigned int writeMask)
				: writeMask(writeMask), clearOperation(true), filter(false), useStencil(false), convertSRGB(true), allowOutOfBoundsSourceCoords(false) {}
			...
			bool useStencil : 1;
			bool convertSRGB : 1;
			bool allowOutOfBoundsSourceCoords : 1;
		};
