...
namespace sw
{
	...
	void QuadRasterizer::generate()
	{
		...
		primitive = Pointer<Byte>(arg(0));
		Int count(arg(1));
		cluster = Int(arg(2));
		data = Pointer<Byte>(arg(3));
	...
	void QuadRasterizer::rasterize(Int &yMin, Int &yMax)
	{
		...
		for(int index = 0; index < RENDERTARGETS; index++)
		{
			...
			{
				cBuffer[index] = *Pointer<Pointer<Byte> >(data + OFFSET(DrawData,colorBuffer[index])) + yMin * *Pointer<Int>(this->data + OFFSET(DrawData,colorPitchB[index]));
			}
		}

		if(state.depthTestActive)
		{
			zBuffer = *Pointer<Pointer<Byte> >(data + OFFSET(DrawData,depthBuffer)) + yMin * *Pointer<Int>(this->data + OFFSET(DrawData,depthPitchB));
		}

		if(state.stencilActive)
		{
			sBuffer = *Pointer<Pointer<Byte> >(data + OFFSET(DrawData,stencilBuffer)) + yMin * *Pointer<Int>(this->data + OFFSET(DrawData,stencilPitchB));
		}
		...
		Do
		{
			...
			if(interpolateZ())
			{
				for(unsigned int q = 0; q < state.multiSample; q++)
				{
					...
					Dz[q] = *Pointer<Float4>(primitive + OFFSET(Primitive,z.C), 16) + y * *Pointer<Float4>(this->primitive + OFFSET(Primitive,z.B), 16);
				}
			}
