...
void XcbSurfaceKHR::getSurfaceCapabilities(VkSurfaceCapabilitiesKHR *pSurfaceCapabilities) const
{
	...
	auto geom = xcb_get_geometry_reply(connection, xcb_get_geometry(connection, window), nullptr);
...
{
 	auto gc = xcb_generate_id(connection);

	uint32_t values[2] = {0, 0xffffffff};
 	xcb_create_gc(connection, gc, window, XCB_GC_FOREGROUND | XCB_GC_BACKGROUND, values);
...
void XcbSurfaceKHR::detachImage(PresentImage* image)
{
	...
	{
		xcb_free_gc(connection, it->second);
