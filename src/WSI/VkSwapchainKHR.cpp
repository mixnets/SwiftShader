...
void SwapchainKHR::resetImages()
{
	for(uint32_t i = 0; i < images.size(); i++)
	{
		...
		images[i].imageMemory = VK_NULL_HANDLE;
		images[i].imageStatus = NONEXISTENT;
	}
}
...
VkResult SwapchainKHR::createImages(VkDevice device)
{
	...
	VkResult status;
	for(imageCount = 0; imageCount < images.size(); imageCount++)
	{
		PresentImage& currentImage = images[imageCount];
