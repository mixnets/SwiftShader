	...
	public:
		RGB9E5(float rgb[3])
		{
			...
			else  // Fast implementation.
			{
				...
				// smaller than 0.
				float range = bit_cast<float>(bit_cast<unsigned int>(max_c * 2.0f) & 0xFF800000);

				// Extract the 9 mantissa bits with exposed leading 1 (or denormalization),
				// and 5 exponent bits.
				unsigned int R9 = (bit_cast<unsigned int>(red_r + range) >> (23 - N)) & 0x1FF;
				unsigned int G9 = (bit_cast<unsigned int>(green_r + range) >> (23 - N)) & 0x1FF;
				unsigned int B9 = (bit_cast<unsigned int>(blue_r + range) >> (23 - N)) & 0x1FF;
				unsigned int E5 = (bit_cast<unsigned int>(range) >> 23) - 127 + B;
			}
		}
