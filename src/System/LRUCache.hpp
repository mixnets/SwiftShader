...
public:
	...
	// get() looks up the cache entry with the given key.
...
private:
	...
	// find() returns the Entry* for with the given key or nullptr.
	// find() performs this by casting the Key pointer to a Keyed pointer for
	// searching the std::unordered_set.
	//
	// This unpleasantness is to avoid having a duplicate Key held by both an
