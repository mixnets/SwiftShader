...
struct CacheTraits<LRUCache<KEY, DATA, HASH>>
{
	...
	static Data get(Cache &cache, const Key &key) { return cache.get(key); }
	static void add(Cache &cache, const Key &key, const Data &data) { cache.add(key, data); }
	static ConstIterator begin(const Cache &cache) { return cache.begin(); }
	static ConstIterator end(const Cache &cache) { return cache.end(); }
...
struct CacheTraits<std::map<KEY, DATA, COMPARE, ALLOCATOR>>
{
	using Key = KEY;
	using Data = DATA;
	using Cache = std::map<KEY, DATA, COMPARE, ALLOCATOR>;
...
template<typename KEY, typename DATA, typename COMPARE, typename ALLOCATOR>
struct CacheTraits<std::unordered_map<KEY, DATA, COMPARE, ALLOCATOR>>
{
	using Key = KEY;
	using Data = DATA;
	using Cache = std::unordered_map<KEY, DATA, COMPARE, ALLOCATOR>;
	using ConstIterator = typename Cache::const_iterator;

	template<typename V>
	using WithData = std::unordered_map<KEY, V, COMPARE, ALLOCATOR>;
...
////////////////////////////////////////////////////////////////////////////////
// SyncCache is transforms a thread-unsafe cache or map of type CACHE into one
...
private:
	...
	using Cache = typename CacheTraits<CACHE>::template WithData<std::shared_ptr<Entry>>;
	using Traits = CacheTraits<Cache>;

	Cache cache GUARDED_BY(mutex);
...
void SyncCache<CACHE>::foreach(Callback &&callback) const
{
	...
	for(auto it = Traits::begin(cache); it != Traits::end(cache); ++it)
	{
...
{
	marl::lock lock(mutex);
	other.foreach([this](const Key &key, const Data &data) {
		auto entry = std::make_shared<Entry>();
		entry->ready.signal();
		entry->data = data;
		Traits::add(cache, key, entry);
