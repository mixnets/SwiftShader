...
struct CacheTraits<std::unordered_map<KEY, DATA, COMPARE, KEY_EQUAL, ALLOCATOR>>
{
	using Key = KEY;
	using Data = DATA;
	using Cache = std::unordered_map<KEY, DATA, COMPARE, KEY_EQUAL, ALLOCATOR>;
	using ConstIterator = typename Cache::const_iterator;
...
private:
	...
	using Cache = typename CacheTraits<CACHE>::template WithData<EntryLoan>;
