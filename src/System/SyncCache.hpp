...
struct CacheTraits<LRUCache<KEY, DATA, HASH>>
{
	...
	static Data lookup(Cache &cache, const Key &key) { return cache.lookup(key); }
...
struct CacheMapBaseTraits
{
	...
	static Data lookup(Cache &cache, const Key &key)
	{
...
// SyncCache provides basic foreach(), lookup() and add() methods for querying
// and adding to the cache, as well as a lookupOrCreate() method for performing
// both in one atomic operation. lookupOrCreate() will ensure that the cache
// entry will only be created once even if multiple concurrent calls are made
// with the same key.
////////////////////////////////////////////////////////////////////////////////
...
public:
	...
	// lookup() queries the cache for the entry with the given key.
	...
	// lookupOrCreate() looks for a cache entry with the given key, returning
	// the associated entry's data if found. If the cache does not hold an entry
	// with the given key, and this is the first thread to call lookupOrCreate()
	// for the given key, then create is called and the returned value is added
	// to the cache with the given key. If lookupOrCreate() is called by another
	...
	inline Data lookupOrCreate(const Key &key, Create &&create);
...
template<typename CACHE>
typename SyncCache<CACHE>::Data SyncCache<CACHE>::lookup(const Key &key)
{
	EntryLoan entry;
	{
		marl::lock lock(mutex);
		entry = Traits::lookup(cache, key);
		if(!entry.get())
		{
...
template<typename CACHE>
template<typename Create>
typename SyncCache<CACHE>::Data SyncCache<CACHE>::lookupOrCreate(const Key &key, Create &&create)
{
	EntryLoan entry;
	bool doCreate = false;
	{
