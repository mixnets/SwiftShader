...
MemFdLinux::~MemFdLinux()
{
	close();
}

void MemFdLinux::importFd(int fd)
{
	close();
	this->fd = fd;
}

int MemFdLinux::exportFd() const
{
	if (fd < 0)
	{
		return -1;
...
bool MemFdLinux::allocate(const char* name, size_t size)
{
	close();
	...
	{
		sw::trace("ftruncate() returned %d: %s", errno, strerror(errno));
		::close(fd);
		fd = -1;
		return false;
	}
