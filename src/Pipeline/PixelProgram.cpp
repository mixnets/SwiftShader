...
void PixelProgram::setBuiltins(Int &x, Int &y, SIMD::Float (&z)[4], SIMD::Float &w, Int cMask[4], const SampleSet &samples)
{
	...
	routine.fragCoord[0] = SIMD::Float(Float(x)) + SIMD::Float([x0, x1](int i) { return (i & 1) ? x1 : x0; });
	...
	routine.windowSpacePosition[0] = SIMD::Int(x) + SIMD::Int([](int i) { return i & 1; });
	...
	routine.pointCoord[0] = SIMD::Float(0.5f) + pointSizeInv * (((SIMD::Float(Float(x)) + SIMD::Float([](int i) { return (i & 1) ? 1.0f : 0.0f; })) - SIMD::Float(*Pointer<Float>(primitive + OFFSET(Primitive, x0)))));
