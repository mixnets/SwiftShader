...
void PixelProgram::setBuiltins(Int &x, Int &y, Float4 (&z)[4], Float4 &w, Int cMask[4])
{
	...
	routine.setInputBuiltin(spirvShader, spv::BuiltInPointCoord, [&](const SpirvShader::BuiltinMapping &builtin, Array<SIMD::Float> &value) {
		...
		value[builtin.FirstComponent + 0] = SIMD::Float(0.5f, 1.5f, 0.5f, 1.5f) * SIMD::Float(pointSizeFactor) +
		                                    SIMD::Float(0.5f + pointSizeFactor * (Float(x) - *Pointer<Float>(primitive + OFFSET(Primitive, pointCoordX))));
		value[builtin.FirstComponent + 1] = SIMD::Float(0.5f, 0.5f, 1.5f, 1.5f) * SIMD::Float(pointSizeFactor) +
		                                    SIMD::Float(0.5f + pointSizeFactor * (Float(y) - *Pointer<Float>(primitive + OFFSET(Primitive, pointCoordY))));
	});
