...
SIMD::Int PixelProgram::maskAny(Int cMask[4], const SampleSet &samples)
{
	...
	// Convert to Booleans
	std::vector<int> laneBits;
	for(int i = 0; i < SIMD::Width; i++) { laneBits.push_back(1 << i); }  // 1, 2, 4, 8, ...
	SIMD::Int mask(maskUnion);
	mask = CmpNEQ(mask & SIMD::Int(laneBits), 0);
	return mask;
}
	...
	for(int index = 0; index < MAX_COLOR_BUFFERS; index++)
	{
		...
		case VK_FORMAT_A2R10G10B10_UNORM_PACK32:
			for(unsigned int q : samples)
			{
				...
				/////////////				ASSERT(SIMD::Width == 4);
				for(int lane128 = 0; lane128 < 1 /*SIMD::Width / 4*/; lane128++)
				{
					Vector4s color;
					...
					writeColor(index, buffer, x + lane128 * 2, color, sMask[q], zMask[q], (cMask[q] >> (lane128 * 8)) & 0xF);
