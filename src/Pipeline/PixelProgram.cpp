	...
	for(int index = 0; index < RENDERTARGETS; index++)
	{
		...
		switch(format)
		{
			...
			case VK_FORMAT_A2R10G10B10_UNORM_PACK32:
				...
				{
					Pointer<Byte> buffer = cBuffer[index];
					Vector4s color;
					...
					alphaBlend(index, buffer, color, x);
					writeColor(index, buffer, x, color, sMask[0], zMask[0], cMask[0]);
				}
				else if(state.multiSampleCount == 4)
				{
					...
					Int4 c0 = Int4(c4.xxxx);
					Int4 c1 = Int4(c4.yyyy);
					Int4 c2 = Int4(c4.zzzz);
					Int4 c3 = Int4(c4.wwww);
					...
					*Pointer<Int4>(buffer + 16) = merge(c3old, c3, mask3);
					...
					UNSUPPORTED("state.multiSampleCount %d", int(state.multiSampleCount));
				break;
