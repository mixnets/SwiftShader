...
struct SpirvShader::Impl::Group
{
	    ...
	    APPLY &&apply)
	{
		...
		{
			auto mask = As<SIMD::UInt>(state->storesAndAtomicsMask());
...
SpirvShader::EmitResult SpirvShader::EmitGroupNonUniform(InsnIterator insn, EmitState *state) const
{
	...
	switch(insn.opcode())
	{
		case spv::OpGroupNonUniformElect:
		{
			...
			SIMD::Int mask = state->storesAndAtomicsMask();
		...
		case spv::OpGroupNonUniformAll:
		{
			...
			dst.move(0, AndAll(predicate.UInt(0) | ~As<SIMD::UInt>(state->storesAndAtomicsMask())));
			break;
		}
		...
		{
			GenericValue predicate(this, state, insn.word(4));
			dst.move(0, OrAll(predicate.UInt(0) & As<SIMD::UInt>(state->storesAndAtomicsMask())));
			break;
		}
		...
		case spv::OpGroupNonUniformAllEqual:
		{
			...
			SIMD::UInt active = As<SIMD::UInt>(state->storesAndAtomicsMask());
		...
		case spv::OpGroupNonUniformBallot:
		{
			...
			GenericValue predicate(this, state, insn.word(4));
			dst.move(0, SIMD::Int(SignMask(state->storesAndAtomicsMask() & predicate.Int(0))));
