...
//
// Necessary because BinaryOperation<> below needs to use SpirvShader private
// types (e.g. InsnIterator or EmitState and won't compile without it.
//
// The alternative would be to place the template in the class' private section
// in the the header file, which seems un-necessary and ugly.
//
...
SpirvShader::EmitResult SpirvShader::EmitGroupNonUniform(InsnIterator insn, EmitState *state) const
{
	...
	{
		using Type = SIMD::Int;
		SpirvShaderGroupOps::BinaryOperation(
				this, insn, state, dst,
				Type(0),
