...
SpirvShader::EmitResult SpirvShader::EmitDot(InsnIterator insn, EmitState *state) const
{
	...
	case spv::OpSDotAccSat:
		dst.move(0, SDotAccSat(lhsType.componentCount, lhs, rhs, Operand(this, state, insn.word(5))));
		break;
	case spv::OpUDotAccSat:
		dst.move(0, UDotAccSat(lhsType.componentCount, lhs, rhs, Operand(this, state, insn.word(5))));
		break;
	case spv::OpSUDotAccSat:
		dst.move(0, SUDotAccSat(lhsType.componentCount, lhs, rhs, Operand(this, state, insn.word(5))));
		break;
	default:
...
SIMD::Int SpirvShader::SDotAccSat(unsigned numComponents, Operand const &x, Operand const &y, Operand const &accum) const
{
	if(numComponents == 1)  // 4x8bit packed
	{
		SIMD::Int d(0);
		...
		d += accum.Int(0);  // FIXME: saturated add
		return d;
	}
	else
	{
...
SIMD::UInt SpirvShader::UDotAccSat(unsigned numComponents, Operand const &x, Operand const &y, Operand const &accum) const
{
	if(numComponents == 1)  // 4x8bit packed
	{
		SIMD::UInt d(0);
		...
		d += accum.UInt(0);  // FIXME: saturated add
		return d;
	}
	else
	{
...
SIMD::Int SpirvShader::SUDotAccSat(unsigned numComponents, Operand const &x, Operand const &y, Operand const &accum) const
{
	if(numComponents == 1)  // 4x8bit packed
	{
		SIMD::Int d(0);
