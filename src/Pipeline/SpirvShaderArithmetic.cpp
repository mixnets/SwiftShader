...
SIMD::Float SpirvShader::Dot(unsigned numComponents, Operand const &x, Operand const &y)
{
...
RValue<SIMD::Int> SpirvShader::AddSat(RValue<SIMD::Int> a, RValue<SIMD::Int> b)
{
	SIMD::Int sum = a + b;
...
RValue<SIMD::UInt> SpirvShader::AddSat(RValue<SIMD::UInt> a, RValue<SIMD::UInt> b)
{
	SIMD::UInt sum = a + b;

	// Overflow happened if the sum of unsigned integers is smaller than any of the 2 numbers being added
	SIMD::UInt oob = CmpLT(sum, a);  // Automatically sets oob to UINT_MAX when true
