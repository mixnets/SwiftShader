...
void SpirvEmitter::SpirvEmitter::EmitMatrixTimesVector(SpirvShader::InsnIterator insn)
{
	auto &type = shader.getType(insn.resultTypeId());
	auto &dst = createIntermediate(insn.resultId(), type.componentCount);
	auto lhs = Operand(shader, *this, insn.word(3));
...
void SpirvEmitter::SpirvEmitter::EmitTranspose(SpirvShader::InsnIterator insn)
{
	auto &type = shader.getType(insn.resultTypeId());
	auto &dst = createIntermediate(insn.resultId(), type.componentCount);
