...
SpirvShader::EmitResult SpirvShader::EmitImageRead(InsnIterator insn, EmitState *state) const
{
	...
	switch(vkFormat)
	{
		...
		case VK_FORMAT_A2B10G10R10_UNORM_PACK32:
			dst.move(0, SIMD::Float(packed[0] & SIMD::Int(0x00003FF)) * SIMD::Float(1.0f / 0x00003FF));
			dst.move(1, SIMD::Float(packed[0] & SIMD::Int(0x00FFC00)) * SIMD::Float(1.0f / 0x00FFC00));
			dst.move(2, SIMD::Float(packed[0] & SIMD::Int(0x3FF0000)) * SIMD::Float(1.0f / 0x3FF0000));
			dst.move(3, SIMD::Float(As<SIMD::UInt>(packed[0]) >> 30) * SIMD::Float(1.0f / 0x3));
			break;
		case VK_FORMAT_A2R10G10B10_UNORM_PACK32:
			dst.move(0, SIMD::Float(packed[0] & SIMD::Int(0x3FF0000)) * SIMD::Float(1.0f / 0x3FF0000));
			dst.move(1, SIMD::Float(packed[0] & SIMD::Int(0x00FFC00)) * SIMD::Float(1.0f / 0x00FFC00));
			dst.move(2, SIMD::Float(packed[0] & SIMD::Int(0x00003FF)) * SIMD::Float(1.0f / 0x00003FF));
			dst.move(3, SIMD::Float(As<SIMD::UInt>(packed[0]) >> 30) * SIMD::Float(1.0f / 0x3));
			break;
