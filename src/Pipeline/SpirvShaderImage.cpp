...
SpirvShader::EmitResult SpirvShader::EmitImageWrite(const ImageInstruction &instruction, EmitState *state) const
{
	...
	if(ptr.isBasePlusOffset)
	{
		...
		Pointer<Byte> samplerDescriptor = getSamplerDescriptor(descriptor, instruction, state);

		if(imageFormat == VK_FORMAT_UNDEFINED)  // spv::ImageFormatUnknown
		{
			Pointer<Byte> samplerFunction = lookupSamplerFunction(descriptor, samplerDescriptor, instruction, state);
	...
	else
	{
		for(int j = 0; j < SIMD::Width; j++)
		{
			...
			Pointer<Byte> samplerDescriptor = getSamplerDescriptor(descriptor, instruction, j, state);
			if(imageFormat == VK_FORMAT_UNDEFINED)  // spv::ImageFormatUnknown
			{
				Pointer<Byte> samplerFunction = lookupSamplerFunction(descriptor, samplerDescriptor, instruction, state);
