...
void SpirvShader::EmitImageSampleUnconditional(Array<SIMD::Float> &out, ImageInstruction instruction, InsnIterator insn, EmitState *state) const
{
	...
	uint32_t instructionPosition = insn.distanceFrom(this->begin());  //////////// move up
...
void SpirvShader::EmitImageWriteUnconditional(ImageInstruction instruction, InsnIterator insn, EmitState *state) const
{
	...
	Pointer<Byte> imageDescriptor = state->getPointer(imageId).base;  // vk::StorageImageDescriptor*
...
SpirvShader::EmitResult SpirvShader::EmitImageWrite(InsnIterator insn, EmitState *state) const
{
	...
	SIMD::Int packed[4];
	int texelSize = 0;
	auto format = static_cast<spv::ImageFormat>(imageType.definition.word(8));  //////////////// SpirvFormatToVulkanFormat
	switch(format)
	{
