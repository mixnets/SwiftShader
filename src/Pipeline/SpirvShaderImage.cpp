...
SIMD::Pointer SpirvShader::GetTexelAddress(EmitState const *state, SIMD::Pointer ptr, Operand const &coordinate, Type const &imageType, Pointer<Byte> descriptor, int texelSize, Object::ID sampleId, bool useStencilAspect, OutOfBoundsBehavior outOfBoundsBehavior) const
{
	...
	bool definedOobBehavior = (outOfBoundsBehavior == OutOfBoundsBehavior::Nullify);
...
SpirvShader::EmitResult SpirvShader::EmitImageRead(InsnIterator insn, EmitState *state) const
{
	...
	//
	// While we could be using OutOfBoundsBehavior::RobustBufferAccess for read operation is from buffer resources,
