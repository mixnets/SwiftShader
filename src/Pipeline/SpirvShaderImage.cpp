...
SpirvShader::EmitResult SpirvShader::EmitImageRead(InsnIterator insn, EmitState *state) const
{
	...
	else if(texelSize == 2)  // TODO: 16-bit gather
	{
		...
		for(int i = 0; i < SIMD::Width; i++)
		{
			...
			{
				packed[i] = Insert(packed[i], Int(*Pointer<Short>(texelPtr.base + Extract(offsets, i))), i);
			}
		}
	}
	else if(texelSize == 1)  // TODO: 8-bit gather
	{
		SIMD::Int offsets = texelPtr.offsets();
		...
		for(int i = 0; i < SIMD::Width; i++)
		{
			If(Extract(mask, i) != 0)  // TODO: mask[i]
			{
				packed[i] = Insert(packed[i], Int(*Pointer<Byte>(texelPtr.base + Extract(offsets, i))), i);
			}
		}
	}
	else
