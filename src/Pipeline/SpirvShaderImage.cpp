...
EmitState::ImageInstruction::ImageInstruction(SpirvShader::InsnIterator insn, const SpirvShader &spirv, const EmitState &state)
    ...
    , position(insn.distanceFrom(spirv.begin()))
{
	...
	else
	{
		...
		else
		{
			...
			SpirvShader::Object::ID sampledImageId = insn.word(3);
	...
	const SpirvShader::Type &imageObjectType = spirv.getObjectType(imageId);
	const SpirvShader::Type &imageReferenceType = (imageObjectType.opcode() == spv::OpTypeSampledImage)
	                                                  ? spirv.getType(imageObjectType.definition.word(2))
	                                                  : imageObjectType;
	const SpirvShader::Type &imageType = ((imageReferenceType.opcode() == spv::OpTypePointer)
	...
	const SpirvShader::Object &coordinateObject = spirv.getObject(coordinateId);
	const SpirvShader::Type &coordinateType = spirv.getType(coordinateObject);
...
EmitState::ImageInstructionSignature EmitState::ImageInstruction::parseVariantAndMethod(SpirvShader::InsnIterator insn)
{
...
uint32_t EmitState::ImageInstruction::getImageOperandsIndex(SpirvShader::InsnIterator insn)
{
	switch(insn.opcode())
	{
...
uint32_t EmitState::ImageInstruction::getImageOperandsMask(SpirvShader::InsnIterator insn)
{
	uint32_t operandsIndex = getImageOperandsIndex(insn);
...
EmitState::EmitResult EmitState::EmitImageQuerySizeLod(SpirvShader::InsnIterator insn)
{
	auto &resultTy = shader.getType(insn.resultTypeId());
	auto imageId = SpirvShader::Object::ID(insn.word(3));
	auto lodId = SpirvShader::Object::ID(insn.word(4));
...
EmitState::EmitResult EmitState::EmitImageQuerySize(SpirvShader::InsnIterator insn)
{
	auto &resultTy = shader.getType(insn.resultTypeId());
	auto imageId = SpirvShader::Object::ID(insn.word(3));
	auto lodId = SpirvShader::Object::ID(0);
...
void EmitState::GetImageDimensions(const SpirvShader::Type &resultTy, SpirvShader::Object::ID imageId, SpirvShader::Object::ID lodId, Intermediate &dst) const
{
...
EmitState::EmitResult EmitState::EmitImageQueryLevels(SpirvShader::InsnIterator insn)
{
	auto &resultTy = shader.getType(insn.resultTypeId());
...
EmitState::EmitResult EmitState::EmitImageQuerySamples(SpirvShader::InsnIterator insn)
{
	auto &resultTy = shader.getType(insn.resultTypeId());
	ASSERT(resultTy.componentCount == 1);
	auto imageId = SpirvShader::Object::ID(insn.word(3));
...
EmitState::EmitResult EmitState::EmitImageTexelPointer(const ImageInstruction &instruction)
{
	...
	Pointer<Byte> descriptor = getPointer(instruction.imageId).getUniformPointer();  // vk::StorageImageDescriptor*
	...
	auto ptr = GetTexelAddress(instruction, descriptor, uvwa, sample, imageFormat, robustness, routine);

	createPointer(instruction.resultId, ptr);

	return EmitResult::Continue;
}

EmitState::EmitResult EmitState::EmitSampledImage(SpirvShader::InsnIterator insn)
{
	SpirvShader::Object::ID resultId = insn.word(2);
	SpirvShader::Object::ID imageId = insn.word(3);
	SpirvShader::Object::ID samplerId = insn.word(4);
...
EmitState::EmitResult EmitState::EmitImage(SpirvShader::InsnIterator insn)
{
	SpirvShader::Object::ID resultId = insn.word(2);
	SpirvShader::Object::ID imageId = insn.word(3);
