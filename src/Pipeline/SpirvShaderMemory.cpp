...
SpirvShader::EmitResult SpirvShader::EmitLoad(InsnIterator insn, EmitState *state) const
{
	...
	auto ptr = GetPointerToData(pointerId, 0, state, false);
		...
		case spv::StorageClassWorkgroup:
			{
				...
				auto ptr = GetPointerToData(resultId, 0, state, false);
...
SpirvShader::EmitResult SpirvShader::EmitCopyMemory(InsnIterator insn, EmitState *state) const
{
	...
	auto dstPtr = GetPointerToData(dstPtrId, 0, state, false);
	auto srcPtr = GetPointerToData(srcPtrId, 0, state, false);
...
SIMD::Pointer SpirvShader::GetPointerToData(Object::ID id, SIMD::Int arrayIndex, EmitState const *state, bool nonUniform) const
{
	auto routine = state->routine;
	...
	case Object::Kind::DescriptorSet:
		{
			...
			if(!nonUniform)
			{
				Int arrayIdx = Extract(arrayIndex, 0);
				Int descriptorOffset = bindingOffset + descriptorSize * arrayIdx;
				Assert(set.getBase() != Pointer<Byte>(nullptr));
				Pointer<Byte> descriptor = set.getBase() + descriptorOffset;  // BufferDescriptor* or inline uniform block
			...
			{
				SIMD::Int descriptorOffset = bindingOffset + descriptorSize * arrayIndex;
				...
				if(!set.isBasePlusOffset)
				{
					auto setPointers = set.getPointers();
				...
				if(routine->pipelineLayout->isDescriptorDynamic(d.DescriptorSet, d.Binding))
				{
					SIMD::Int dynamicOffsetIndex = SIMD::Int(routine->pipelineLayout->getDynamicOffsetIndex(d.DescriptorSet, d.Binding) + arrayIndex);
