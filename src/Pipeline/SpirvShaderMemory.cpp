	...
	case Object::Kind::DescriptorSet:
		{
			...
			SIMD::Pointer descriptor = SIMD::Pointer(set.base, set.limit(), descriptorOffset);  // BufferDescriptor* or inline uniform block
			...
			Int4 activeLaneMask = state->activeLaneMask();
			Pointer<Byte> base;
			If(Extract(activeLaneMask, 0) != 0)
			{
				base = *Pointer<Pointer<Byte>>(descriptor.base + Extract(descriptor.dynamicOffsets, 0) + OFFSET(vk::BufferDescriptor, base));
			}
			Else If(Extract(activeLaneMask, 1) != 0)
			{
				base = *Pointer<Pointer<Byte>>(descriptor.base + Extract(descriptor.dynamicOffsets, 1) + OFFSET(vk::BufferDescriptor, base));
			}
			Else If(Extract(activeLaneMask, 2) != 0)
			{
				base = *Pointer<Pointer<Byte>>(descriptor.base + Extract(descriptor.dynamicOffsets, 2) + OFFSET(vk::BufferDescriptor, base));
			}
			Else If(Extract(activeLaneMask, 3) != 0)
			{
				base = *Pointer<Pointer<Byte>>(descriptor.base + Extract(descriptor.dynamicOffsets, 3) + OFFSET(vk::BufferDescriptor, base));
			}
			Else
			{
				base = rr::Pointer<Byte>(nullptr);
			}

			Int4 offset = (descriptor + OFFSET(vk::BufferDescriptor, offset)).Load<Int4>(robustness, activeLaneMask);
			Int4 size = (descriptor + OFFSET(vk::BufferDescriptor, sizeInBytes)).Load<Int4>(robustness, activeLaneMask);
			Int4 robustnessSize = (descriptor + OFFSET(vk::BufferDescriptor, robustnessSize)).Load<Int4>(robustness, activeLaneMask);
			Int4 limit = 0;

			if(routine->pipelineLayout->isDescriptorDynamic(d.DescriptorSet, d.Binding))
			{
				Int4 dynamicOffsetIndex = routine->pipelineLayout->getDynamicOffsetIndex(d.DescriptorSet, d.Binding) + arrayIndex;
				...
				limit = Min(size, robustnessSize - dynamicOffsets);
			}
			else
			{
				limit = size;
			...
			Int scalarLimit = Max(Extract(limit, 0), Max(Extract(limit, 1), Max(Extract(limit, 2), Extract(limit, 3))));

			return SIMD::Pointer(base, scalarLimit, offset);
		}
