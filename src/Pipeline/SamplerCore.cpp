...
Vector4s SamplerCore::sampleTexel(Short4 &uuuu, Short4 &vvvv, Short4 &wwww, const Short4 &layerIndex, Vector4i &offset, const Int4 &sample, Pointer<Byte> &mipmap, Pointer<Byte> buffer)
{
	...
	if(isYcbcrFormat())
	{
		...
		else
		{
			...
			{
				y = y * Float4(static_cast<float>(0x7FFF));
				u = u * Float4(static_cast<float>(0x7FFF));
				v = v * Float4(static_cast<float>(0x7FFF));
			...
			else
			{
				...
				r = r * Float4(static_cast<float>(0x7FFF));
				g = g * Float4(static_cast<float>(0x7FFF));
				b = b * Float4(static_cast<float>(0x7FFF));
