...
Vector4f SamplerCore::sampleFloat2D(Pointer<Byte> &texture, Float4 &u, Float4 &v, Float4 &w, const Float4 &a, Float4 &dRef, Vector4i &offset, const Int4 &sample, Float &lod, bool secondLOD, SamplerFunction function)
{
	...
	if(!gather)
	{
		address(u, x0, x1, fu, mipmap, Int4(Int(offset.x.x)), filter, OFFSET(Mipmap, width), state.addressingModeU, function);
		address(v, y0, y2, fv, mipmap, Int4(Int(offset.y.x)), filter, OFFSET(Mipmap, height), state.addressingModeV, function);
	...
	{
		Int4 _;
		address(u, x0, _, fu, mipmap, Int4(Int(offset.x.x)), filter, OFFSET(Mipmap, width), state.addressingModeU, function);
		address(u, x1, _, fu, mipmap, Int4(Int(offset.x.y)), filter, OFFSET(Mipmap, width), state.addressingModeU, function);
		address(u, x2, _, fu, mipmap, Int4(Int(offset.x.z)), filter, OFFSET(Mipmap, width), state.addressingModeU, function);
		address(u, x3, _, fu, mipmap, Int4(Int(offset.x.w)), filter, OFFSET(Mipmap, width), state.addressingModeU, function);

		address(v, y0, _, fv, mipmap, Int4(Int(offset.y.x)), filter, OFFSET(Mipmap, height), state.addressingModeV, function);
		address(v, y1, _, fv, mipmap, Int4(Int(offset.y.y)), filter, OFFSET(Mipmap, height), state.addressingModeV, function);
		address(v, y2, _, fv, mipmap, Int4(Int(offset.y.z)), filter, OFFSET(Mipmap, height), state.addressingModeV, function);
		address(v, y3, _, fv, mipmap, Int4(Int(offset.y.w)), filter, OFFSET(Mipmap, height), state.addressingModeV, function);
	}
