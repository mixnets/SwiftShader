...
Vector4f SamplerCore::sampleTexture(Pointer<Byte> &texture, Float4 uvwa[4], Float4 &dRef, Float &&lodOrBias, Float4 &dsx, Float4 &dsy, Vector4i &offset, Int4 &sample, SamplerFunction function)
{
	...
	const sw::float4 compScale = getComponentScale(true);
	int componentIndex = getComponentIndex(function);
	int numComponents = (componentIndex < 0) ? 4 : textureComponentCount();
	...
	if(use32BitFiltering)
	{
		...
		if(!hasFloatTexture() && !hasUnnormalizedIntegerTexture() && !state.compareEnable)
		{
			...
			{
				c[component] *= Float4(1.0f / compScale[(componentIndex < 0) ? component : componentIndex]);
			}
		}
	}
	else  // 16-bit filtering.
	{
		...
		for(int component = 0; component < numComponents; component++)
		{
			c[component] = Float4(hasUnsignedTextureComponent(component) ? As<UShort4>(cs[component]) : cs[component]) * Float4(1.0f / compScale[(componentIndex < 0) ? component : componentIndex]);
		}
	}
...
Vector4f SamplerCore::replaceBorderTexel(const Vector4f &c, Int4 valid)
{
	...
	const sw::float4 scaleComp = getComponentScale(scaled);
...
sw::float4 SamplerCore::getComponentScale(bool scaled) const
{
	...
	const sw::int4 shift = sw::int4(std::max(16 - bits.x, 0), std::max(16 - bits.y, 0), std::max(16 - bits.z, 0),
	                                std::max(16 - bits.w, 0));
	sw::float4 scaleComp = scaled ? sw::float4(static_cast<uint16_t>(scale.x) << shift.x,
...
int SamplerCore::getComponentIndex(const SamplerFunction &function) const
{
	if(function.method == Gather)
	{
		VkComponentSwizzle swizzle = gatherSwizzle();
