...
Vector4f SamplerCore::sampleTexture(Pointer<Byte> &texture, Float4 uvwa[4], Float4 &dRef, Float &&lodOrBias, Float4 &dsx, Float4 &dsy, Vector4i &offset, Int4 &sampleId, SamplerFunction function)
{
	Vector4f c;
	...
	{
		c = sampleFloatFilter(texture, uuuu, vvvv, wwww, cubeArrayCoord, dRef, offset, sampleId, lod, anisotropy, uDelta, vDelta, function);
	...
	{
		Vector4s cs = sampleFilter(texture, uuuu, vvvv, wwww, cubeArrayCoord, offset, sampleId, lod, anisotropy, uDelta, vDelta, function);
...
Vector4s SamplerCore::sampleFilter(Pointer<Byte> &texture, Float4 &u, Float4 &v, Float4 &w, const Float4 &cubeArrayCoord, Vector4i &offset, const Int4 &sampleId, Float &lod, Float &anisotropy, Float4 &uDelta, Float4 &vDelta, SamplerFunction function)
{
	Vector4s c = sampleAniso(texture, u, v, w, cubeArrayCoord, offset, sampleId, lod, anisotropy, uDelta, vDelta, false, function);
	...
	{
		Vector4s cc = sampleAniso(texture, u, v, w, cubeArrayCoord, offset, sampleId, lod, anisotropy, uDelta, vDelta, true, function);
...
Vector4s SamplerCore::sampleAniso(Pointer<Byte> &texture, Float4 &u, Float4 &v, Float4 &w, const Float4 &cubeArrayCoord, Vector4i &offset, const Int4 &sampleId, Float &lod, Float &anisotropy, Float4 &uDelta, Float4 &vDelta, bool secondLOD, SamplerFunction function)
{
	Vector4s c;

	if(state.textureFilter != FILTER_ANISOTROPIC || function == Lod || function == Fetch)
	{
		c = sampleQuad(texture, u, v, w, cubeArrayCoord, offset, sampleId, lod, secondLOD, function);
	}
	else
	{
		...
		Do
		{
			c = sampleQuad(texture, u0, v0, w, cubeArrayCoord, offset, sampleId, lod, secondLOD, function);
...
Vector4s SamplerCore::sampleQuad(Pointer<Byte> &texture, Float4 &u, Float4 &v, Float4 &w, const Float4 &cubeArrayCoord, Vector4i &offset, const Int4 &sampleId, Float &lod, bool secondLOD, SamplerFunction function)
{
	if(state.textureType != VK_IMAGE_VIEW_TYPE_3D)
	{
		return sampleQuad2D(texture, u, v, w, cubeArrayCoord, offset, sampleId, lod, secondLOD, function);
	}
	else
	{
...
Vector4s SamplerCore::sampleQuad2D(Pointer<Byte> &texture, Float4 &u, Float4 &v, Float4 &w, const Float4 &cubeArrayCoord, Vector4i &offset, const Int4 &sampleId, Float &lod, bool secondLOD, SamplerFunction function)
{
	Vector4s c;
...
Vector4s SamplerCore::sample3D(Pointer<Byte> &texture, Float4 &u_, Float4 &v_, Float4 &w_, Vector4i &offset, const Int4 &sampleId, Float &lod, bool secondLOD, SamplerFunction function)
{
	Vector4s c_;
	...
	if(state.textureFilter == FILTER_POINT)
	{
		c_ = sampleTexel(uuuu, vvvv, wwww, offset, mipmap, 0, sampleId, buffer, function);
	}
	else
	{
		Vector4s c[2][2][2];
		...
		for(int i = 0; i < 2; i++)
		{
			for(int j = 0; j < 2; j++)
			{
				for(int k = 0; k < 2; k++)
				{
					c[i][j][k] = sampleTexel(u[i][j][k], v[i][j][k], s[i][j][k], offset, mipmap, 0, sampleId, buffer, function);
...
Vector4f SamplerCore::sampleFloatFilter(Pointer<Byte> &texture, Float4 &u, Float4 &v, Float4 &w, const Float4 &cubeArrayCoord, Float4 &dRef, Vector4i &offset, const Int4 &sampleId, Float &lod, Float &anisotropy, Float4 &uDelta, Float4 &vDelta, SamplerFunction function)
{
	Vector4f c = sampleFloatAniso(texture, u, v, w, cubeArrayCoord, dRef, offset, sampleId, lod, anisotropy, uDelta, vDelta, false, function);
	...
	if(state.mipmapFilter == MIPMAP_LINEAR)
	{
		Vector4f cc = sampleFloatAniso(texture, u, v, w, cubeArrayCoord, dRef, offset, sampleId, lod, anisotropy, uDelta, vDelta, true, function);
...
Vector4f SamplerCore::sampleFloatAniso(Pointer<Byte> &texture, Float4 &u, Float4 &v, Float4 &w, const Float4 &cubeArrayCoord, Float4 &dRef, Vector4i &offset, const Int4 &sampleId, Float &lod, Float &anisotropy, Float4 &uDelta, Float4 &vDelta, bool secondLOD, SamplerFunction function)
{
	Vector4f c;

	if(state.textureFilter != FILTER_ANISOTROPIC || function == Lod || function == Fetch)
	{
		c = sampleFloat(texture, u, v, w, cubeArrayCoord, dRef, offset, sampleId, lod, secondLOD, function);
	}
	else
	{
		Int a = RoundInt(anisotropy);
		...
		Do
		{
			c = sampleFloat(texture, u0, v0, w, cubeArrayCoord, dRef, offset, sampleId, lod, secondLOD, function);
...
Vector4f SamplerCore::sampleFloat(Pointer<Byte> &texture, Float4 &u, Float4 &v, Float4 &w, const Float4 &cubeArrayCoord, Float4 &dRef, Vector4i &offset, const Int4 &sampleId, Float &lod, bool secondLOD, SamplerFunction function)
{
	if(state.textureType != VK_IMAGE_VIEW_TYPE_3D)
	{
		return sampleFloat2D(texture, u, v, w, cubeArrayCoord, dRef, offset, sampleId, lod, secondLOD, function);
	}
	else
	{
...
Vector4f SamplerCore::sampleFloat2D(Pointer<Byte> &texture, Float4 &u, Float4 &v, Float4 &w, const Float4 &cubeArrayCoord, Float4 &dRef, Vector4i &offset, const Int4 &sampleId, Float &lod, bool secondLOD, SamplerFunction function)
{
	Vector4f c;
	...
	{
		c = sampleTexel(x0, y0, z0, dRef, mipmap, cubeArrayId, sampleId, buffer, function);
	}
	else
	{
		y1 *= pitchP;

		Vector4f c00 = sampleTexel(x0, y0, z0, dRef, mipmap, cubeArrayId, sampleId, buffer, function);
		Vector4f c10 = sampleTexel(x1, y0, z0, dRef, mipmap, cubeArrayId, sampleId, buffer, function);
		Vector4f c01 = sampleTexel(x0, y1, z0, dRef, mipmap, cubeArrayId, sampleId, buffer, function);
		Vector4f c11 = sampleTexel(x1, y1, z0, dRef, mipmap, cubeArrayId, sampleId, buffer, function);
...
Vector4f SamplerCore::sampleFloat3D(Pointer<Byte> &texture, Float4 &u, Float4 &v, Float4 &w, Vector4i &offset, const Int4 &sampleId, Float &lod, bool secondLOD, SamplerFunction function)
{
	Vector4f c;
	...
	if(state.textureFilter == FILTER_POINT || (function == Fetch))
	{
		c = sampleTexel(x0, y0, z0, w, mipmap, 0, sampleId, buffer, function);
	}
	else
	{
		y1 *= pitchP;
		...
		Vector4f c000 = sampleTexel(x0, y0, z0, w, mipmap, 0, sampleId, buffer, function);
		Vector4f c100 = sampleTexel(x1, y0, z0, w, mipmap, 0, sampleId, buffer, function);
		Vector4f c010 = sampleTexel(x0, y1, z0, w, mipmap, 0, sampleId, buffer, function);
		Vector4f c110 = sampleTexel(x1, y1, z0, w, mipmap, 0, sampleId, buffer, function);
		Vector4f c001 = sampleTexel(x0, y0, z1, w, mipmap, 0, sampleId, buffer, function);
		Vector4f c101 = sampleTexel(x1, y0, z1, w, mipmap, 0, sampleId, buffer, function);
		Vector4f c011 = sampleTexel(x0, y1, z1, w, mipmap, 0, sampleId, buffer, function);
		Vector4f c111 = sampleTexel(x1, y1, z1, w, mipmap, 0, sampleId, buffer, function);
...
void SamplerCore::computeIndices(UInt index[4], Short4 uuuu, Short4 vvvv, Short4 wwww, Vector4i &offset, const Pointer<Byte> &mipmap, const Short4 &cubeArrayId, const Int4 &sampleId, SamplerFunction function)
{
...
Vector4s SamplerCore::sampleTexel(Short4 &uuuu, Short4 &vvvv, Short4 &wwww, Vector4i &offset, Pointer<Byte> &mipmap, const Short4 &cubeArrayId, const Int4 &sampleId, Pointer<Byte> buffer, SamplerFunction function)
{
	Vector4s c;
...
void SamplerCore::address(const Float4 &uvw, Int4 &xyz0, Int4 &xyz1, Float4 &f, Pointer<Byte> &mipmap, Int4 &offset, Int4 &filter, int whd, AddressingMode addressingMode, SamplerFunction function)
{
	...
	else if(addressingMode == ADDRESSING_LAYER)  // Note: Offset does not apply to array layers
	{
		...
		xyz0 = As<Int4>(Min(As<UInt4>(x), maxXYZ));
	}
	...
	else
	{
		...
		else if(function.offset)
		{
			switch(addressingMode)
			{
				...
				case ADDRESSING_CLAMP:
					xyz0 = As<Int4>(Min(As<UInt4>(xyz0), maxXYZ));
					xyz1 = As<Int4>(Min(As<UInt4>(xyz1), maxXYZ));
					break;
				default:  // Wrap
					xyz0 = mod(xyz0, dim);
					xyz1 = mod(xyz1, dim);
