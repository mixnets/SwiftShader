	...
	if(use32BitFiltering)
	{
		...
		if(!hasFloatTexture() && !hasUnnormalizedIntegerTexture() && !state.compareEnable)
		{
			...
			case VK_FORMAT_R16G16B16A16_UNORM:
				c.x = Max(c.x * Float4(1.0f / 0xFFFF), Float4(-1.0f));
				c.y = Max(c.y * Float4(1.0f / 0xFFFF), Float4(-1.0f));
				c.z = Max(c.z * Float4(1.0f / 0xFFFF), Float4(-1.0f));
				c.w = Max(c.w * Float4(1.0f / 0xFFFF), Float4(-1.0f));
				break;
...
Vector4f SamplerCore::replaceBorderTexel(const Vector4f &c, Int4 valid)
{
	...
	const sw::float4 scaleComp = scaled ? sw::float4(static_cast<uint16_t>(scale.x) << shift.x, static_cast<uint16_t>(scale.y) << shift.y,
