...
namespace sw
{
	...
	void SamplerCore::computeIndices(UInt index[4], Short4 uuuu, Short4 vvvv, Short4 wwww, Vector4f &offset, Short4 valid, const Pointer<Byte> &mipmap, SamplerFunction function)
	{
		if(borderModeActive())
		{
			uuuu &= valid;
	...
	Vector4s SamplerCore::sampleTexel(Short4 &uuuu, Short4 &vvvv, Short4 &wwww, Vector4f &offset, Pointer<Byte> &mipmap, Pointer<Byte> buffer[4], SamplerFunction function)
	{
		...
		if(borderModeActive())
		{
			b = CmpGT(hasThirdCoordinate() ? (uuuu | vvvv | wwww) : (uuuu | vvvv), Short4(-1));
		}

		UInt index[4];
		computeIndices(index, uuuu, vvvv, wwww, offset, b, mipmap, function);
		...
		if(hasYuvFormat())
		{
			...
			computeIndices(index, uuuu, vvvv, wwww, offset, wwww, mipmap + sizeof(Mipmap), function);
	...
	Vector4f SamplerCore::sampleTexel(Int4 &uuuu, Int4 &vvvv, Int4 &wwww, Float4 &z, Pointer<Byte> &mipmap, Pointer<Byte> buffer[4], SamplerFunction function)
	{
		...
		if(borderModeActive())
		{
			border = CmpNLT(hasThirdCoordinate() ? (uuuu | vvvv | wwww) : (uuuu | vvvv), Int4(0));
		}
