...
Vector4f SamplerCore::sampleTexture(Pointer<Byte> &texture, Float4 uvwa[4], Float4 &dRef, Float &&lodOrBias, Float4 &dsx, Float4 &dsy, Vector4i &offset, Int4 &sample, SamplerFunction function)
{
	...
	sw::float4 compScale = getComponentScale(true);
	bool componentIndexOverride = false;
	int componentIndex = 0;
	int numComponents = textureComponentCount();
	...
	if(function == Gather)
	{
		...
		case VK_COMPONENT_SWIZZLE_A:
			...
			componentIndexOverride = true;
			componentIndex = swizzle - VK_COMPONENT_SWIZZLE_R;
			numComponents = 4;
			break;
		default:
			break;
	...
	if(use32BitFiltering)
	{
		...
		if(!hasFloatTexture() && !hasUnnormalizedIntegerTexture() && !state.compareEnable)
		{
			...
			{
				c[component] *= Float4(1.0f / compScale[componentIndexOverride ? componentIndex : component]);
			}
		}
	}
	else  // 16-bit filtering.
	{
		...
		for(int component = 0; component < numComponents; component++)
		{
			c[component] = Float4(As<UShort4>(cs[component])) * Float4(1.0f / compScale[componentIndexOverride ? componentIndex : component]);
		}
	}
...
Vector4f SamplerCore::replaceBorderTexel(const Vector4f &c, Int4 valid)
{
	...
	sw::float4 scaleComp = getComponentScale(scaled);
