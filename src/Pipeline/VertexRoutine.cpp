...
namespace sw
{
	...
	void VertexRoutine::generate()
	{
		...
		{
			UInt index = *batch;
			UInt tagIndex = index & VertexCache::TAG_MASK;

			If(tagCache[tagIndex] != index)
			{
				readInput(batch);
				program(batch);
				computeClipFlags();
	...
	void VertexRoutine::writeCache(Pointer<Byte> &vertexCache, Pointer<UInt> &tagCache, Pointer<UInt> &batch)
	{
		...
		// Write the vertices out in reverse order to ensure the first one is now guaranteed to be in the cache.
		...
		*Pointer<Int>(vertexCache + sizeof(Vertex) * cacheIndex2 + OFFSET(Vertex,clipFlags)) = (clipFlags >> 16) & 0x0000000FF;   // TODO: unsigned char Vertex::clipFlags
		...
		*Pointer<Float4>(vertexCache + sizeof(Vertex) * cacheIndex0 + OFFSET(Vertex,projected), 16) = proj.x;
		...
		tagCache[index3 & VertexCache::TAG_MASK] = index3;
		tagCache[index2 & VertexCache::TAG_MASK] = index2;
		tagCache[index1 & VertexCache::TAG_MASK] = index1;
		tagCache[index0 & VertexCache::TAG_MASK] = index0;
	}
