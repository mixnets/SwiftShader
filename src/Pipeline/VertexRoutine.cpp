...
namespace sw
{
	                                   ...
	                                   bool robustBufferAccess, UInt & robustnessSize)
	{
		...
		Pointer<Byte> source0;
		Pointer<Byte> source1;
		Pointer<Byte> source2;
		Pointer<Byte> source3;
		...
		if (robustBufferAccess)
		{
			...
			source0 = IfThenElse(limits.x <= robustnessSize, buffer + offsets.x, zeroSource);
			source1 = IfThenElse(limits.y <= robustnessSize, buffer + offsets.y, zeroSource);
			source2 = IfThenElse(limits.z <= robustnessSize, buffer + offsets.z, zeroSource);
			source3 = IfThenElse(limits.w <= robustnessSize, buffer + offsets.w, zeroSource);
		}
		else
		{
			source0 = buffer + offsets.x;
			source1 = buffer + offsets.y;
			source2 = buffer + offsets.z;
			source3 = buffer + offsets.w;
		}
