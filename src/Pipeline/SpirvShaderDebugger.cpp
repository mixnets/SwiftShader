...
struct SpirvShader::Impl::Debugger
{
	...
	void exposeVariable(const SpirvShader* shader, const Key& key, debug::Scope* scope, Object::ID id, EmitState *state) const;
...
void SpirvShader::Impl::Debugger::process(const SpirvShader *shader, const InsnIterator &insn, EmitState *state, Pass pass)
{
	...
	case OpenCLDebugInfo100DebugDeclare:
		defineOrEmit(insn, pass, [&](debug::Declare* decl) {
			...
			exposeVariable(shader, decl->local->name.c_str(), decl->local->parent, decl->variable, state);
		});
		break;
...
template<typename Key>
void SpirvShader::Impl::Debugger::exposeVariable(const SpirvShader* shader, const Key& key, debug::Scope* scope, Object::ID id, EmitState *state) const
{
	...
	for (int lane = 0; lane < SIMD::Width; lane++) {
		exposeVariable(shader, Group::localsLane(dbgState, scope, lane), lane, key, id, state);
...
void SpirvShader::dbgExposeIntermediate(Object::ID id, EmitState *state) const
{
	...
	dbg->exposeVariable(this, id, nullptr, id, state);
}
