...
private:
	...
	// isDebugInstruction() returns true if the given id was registered as
	// a debug instruction with add() or addNone().
	bool isDebugInstruction(debug::Object::ID id) const;
	...
	switch(extInstIndex)
	{
		...
		case OpenCLDebugInfo100DebugTypeFunction:
			defineOrEmit(insn, pass, [&](debug::FunctionType *type) {
				...
				if(isDebugInstruction(insn.word(6)))
				{
...
bool SpirvShader::Impl::Debugger::isDebugInstruction(debug::Object::ID id) const
{
	return objects.find(debug::Object::ID(id.value())) != objects.end();
}
