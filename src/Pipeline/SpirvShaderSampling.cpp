	...
	rr::Function<Void(Pointer<Byte>, Pointer<SIMD::Float>, Pointer<SIMD::Float>, Pointer<Byte>)> function;
	{
		   ...
		   samplerFunction.method == Bias || samplerFunction.method == Fetch)
		{
			...
			Do
			{
				...
				SIMD::Float4 sample = s.sampleTextureW(texture, uvwa, dRef, lod[i], dPdx, dPdy, offset, sampleId);
		...
		{
			scalar::Float lod = lodOrBias.x;
			SIMD::Float4 sample = s.sampleTextureW(texture, uvwa, dRef, lod, (dsx[0]), (dsy[0]), offset, sampleId);
