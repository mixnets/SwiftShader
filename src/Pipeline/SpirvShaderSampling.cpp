...
{
	return getOrCreateImageSampler(inst, imageDescriptor->imageViewId, sampler ? sampler->id : 0, imageDescriptor->device);
}

SpirvShader::ImageSampler *SpirvShader::getOrCreateImageSampler(uint32_t inst, uint32_t imageViewId, uint32_t samplerId, const vk::Device *device)
{
	ImageInstruction instruction(inst);
	ASSERT(imageViewId != 0 && (samplerId != 0 || instruction.samplerMethod == Fetch));
	ASSERT(device);

	vk::Device::SamplingRoutineCache::Key key = { inst, imageViewId, samplerId };
	...
	auto createSamplingRoutine = [&device](const vk::Device::SamplingRoutineCache::Key &key) {
		...
		if(vkSamplerState)
		{
			...
			if(imageViewData.multipleMipLevels ||
