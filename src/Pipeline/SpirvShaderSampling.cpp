...
static SIMD::Pointer GetTexelAddress(SpirvShader::ImageInstructionSignature instruction, const Sampler &samplerState, Pointer<Byte> descriptor, SIMD::Float coordinate[4], const SIMD::Int sampleIdx, int texelSize, bool useStencilAspect, OutOfBoundsBehavior outOfBoundsBehavior)
{
	//auto routine = state->routine;
	bool isArrayed = samplerState.isArrayed();
	/////auto dim = spv::Dim2D;  //////////////////// static_cast<spv::Dim>(imageType.definition.word(3));
	int dims = samplerState.dimensionality();
