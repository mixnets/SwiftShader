	...
	Function<Void(Pointer<Byte>, Pointer<Byte>, Pointer<SIMD::Float>, Pointer<SIMD::Float>, Pointer<Byte>)> function;
	{
		...
		if(samplerFunction.method == Lod || samplerFunction.method == Grad)  // TODO(b/133868964): Also handle divergent Bias and Fetch with Lod.
		{
			...
			For(Int i = 0, i < SIMD::Width, i++)
			{
				...
				bleh.x = Pointer<Float>(&dsx.x)[i];
				bleh.y = Pointer<Float>(&dsx.y)[i];
				bleh.z = Pointer<Float>(&dsx.z)[i];

				blah.x = Pointer<Float>(&dsy.x)[i];
				blah.y = Pointer<Float>(&dsy.y)[i];
				blah.z = Pointer<Float>(&dsy.z)[i];
				...
				Vector4f sample = s.sampleTexture(texture, sampler, uvw[0], uvw[1], uvw[2], q, lod[i], bleh, blah, offset, samplerFunction);
