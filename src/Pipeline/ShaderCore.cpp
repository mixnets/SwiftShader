...
Float4 Sin(RValue<Float4> x, bool relaxedPrecision)
{
...
Float4 Cos(RValue<Float4> x, bool relaxedPrecision)
{
	const Float4 q = 0.25f;
	const Float4 pi2 = 1 / (2 * 3.1415926535f);
...
Float4 Tan(RValue<Float4> x, bool relaxedPrecision)
{
...
static Float4 Asin_4_terms(RValue<Float4> x)
{
	...
	Float4 absx = Abs(x);
	return As<Float4>(As<Int4>(half_pi - Sqrt(1.0f - absx, false) * (a0 + absx * (a1 + absx * (a2 + absx * a3)))) ^
...
static Float4 Asin_8_terms(RValue<Float4> x)
{
	...
	Float4 absx = Abs(x);
	return As<Float4>(As<Int4>(half_pi - Sqrt(1.0f - absx, false) * (a0 + absx * (a1 + absx * (a2 + absx * (a3 + absx * (a4 + absx * (a5 + absx * (a6 + absx * a7)))))))) ^
	                  (As<Int4>(x) & Int4(0x80000000)));
}
...
Float4 Atan(RValue<Float4> x, bool relaxedPrecision)
{
	Float4 absx = Abs(x);
...
Float4 Atan2(RValue<Float4> y, RValue<Float4> x, bool relaxedPrecision)
{
...
Float4 Exp2(RValue<Float4> x, bool relaxedPrecision)
{
...
Float4 Log2(RValue<Float4> x, bool relaxedPrecision)
{
	Float4 x0;
	Float4 x1;
	Float4 x2;
...
Float4 Exp(RValue<Float4> x, bool relaxedPrecision)
{
	return sw::Exp2(1.44269504f * x, relaxedPrecision);  // 1/ln(2)
}

Float4 Log(RValue<Float4> x, bool relaxedPrecision)
{
...
Float4 Sinh(RValue<Float4> x, bool relaxedPrecision)
{
	return (sw::Exp(x, relaxedPrecision) - sw::Exp(-x, relaxedPrecision)) * 0.5f;
}

Float4 Cosh(RValue<Float4> x, bool relaxedPrecision)
{
	return (sw::Exp(x, relaxedPrecision) + sw::Exp(-x, relaxedPrecision)) * 0.5f;
}

Float4 Tanh(RValue<Float4> x, bool relaxedPrecision)
{
