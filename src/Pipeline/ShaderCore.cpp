...
{
	SIMD::Float log = sw::Log2(x, relaxedPrecision);
	log *= y;
...
{
	SIMD::Float e_x = sw::Exp(x, relaxedPrecision);
	SIMD::Float e_minus_x = sw::Exp(-x, relaxedPrecision);
...
{
	return SIMD::Float();
	////rr::Sqrt(x);  // TODO(b/222218659): Optimize for relaxed precision.
}
...
Float4 r11g11b10Unpack(UInt r11g11b10bits)
{
	...
	return Float4();
	////As<Float4>(halfToFloatBits(halfBits));
}
...
UInt r11g11b10Pack(const Float4 &value)
{
	...
	auto halfBits = UInt4();
	////floatToHalfBits(As<UInt4>(Max(value, Float4(0.0f))), true);
...
{
	return ((sw::SIMD::UInt(1) << bits) & CmpLT(bits, sw::SIMD::UInt(32)));
}
...
Pointer::Pointer(std::array<rr::Pointer<Byte>, 8> pointers)
    : pointers(pointers)
    , isBasePlusOffset(false)
{}
