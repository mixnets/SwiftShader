...
//using rr::SIMD::Extract128;
//using rr::SIMD::Insert128;
...
static RValue<Float> Atan_01(Float x)
{
	// From 4.4.49, page 81 of the Handbook of Mathematical Functions, by Milton Abramowitz and Irene Stegun
	const Float a2(-0.3333314528f);
	const Float a4(0.1999355085f);
	const Float a6(-0.1420889944f);
	const Float a8(0.1065626393f);
	const Float a10(-0.0752896400f);
	const Float a12(0.0429096138f);
	const Float a14(-0.0161657367f);
	const Float a16(0.0028662257f);
	Float x2 = x * x;
...
static RValue<Float> Sin5(Float x)
{
	// A * x^5 + B * x^3 + C * x
...
RValue<Float> Sin(RValue<Float> x, bool relaxedPrecision)
{
	const Float q = 0.25f;
	const Float pi2 = 1 / (2 * 3.1415926535f);

	// Range reduction and mirroring
	Float x_2 = MulAdd(x, -pi2, q);
	Float z = q - Abs(x_2 - Round(x_2));

	return Sin5(z);
}

RValue<Float> Cos(RValue<Float> x, bool relaxedPrecision)
{
	const Float q = 0.25f;
	const Float pi2 = 1 / (2 * 3.1415926535f);
...
RValue<Float> Tan(RValue<Float> x, bool relaxedPrecision)
{
	return Sin(x, relaxedPrecision) / Cos(x, relaxedPrecision);
}

static RValue<Float> Asin_4_terms(RValue<Float> x)
{
	...
	Float absx = Abs(x);
	return As<Float>(As<Int>(half_pi - Sqrt<Highp>(1.0f - absx) * (a0 + absx * (a1 + absx * (a2 + absx * a3)))) ^
	                 (As<Int>(x) & Int(0x80000000)));
}

static RValue<Float> Asin_8_terms(RValue<Float> x)
{
	...
	const Float a6(0.006700901f);
	const Float a7(-0.0012624911f);
	Float absx = Abs(x);
	return As<Float>(As<Int>(half_pi - Sqrt<Highp>(1.0f - absx) * (a0 + absx * (a1 + absx * (a2 + absx * (a3 + absx * (a4 + absx * (a5 + absx * (a6 + absx * a7)))))))) ^
	                 (As<Int>(x) & Int(0x80000000)));
}

RValue<Float> Asin(RValue<Float> x, bool relaxedPrecision)
{
...
RValue<Float> Acos(RValue<Float> x, bool relaxedPrecision)
{
	// pi/2 - arcsin(x)
	return 1.57079632e+0f - Asin_4_terms(x);
}

RValue<Float> Atan(RValue<Float> x, bool relaxedPrecision)
{
	Float absx = Abs(x);
	Int O = CmpNLT(absx, 1.0f);
	Float y = As<Float>((O & As<Int>(1.0f / absx)) | (~O & As<Int>(absx)));  // FIXME: Vector select

	const Float half_pi(1.57079632f);
	Float theta = Atan_01(y);
	return As<Float>(((O & As<Int>(half_pi - theta)) | (~O & As<Int>(theta))) ^  // FIXME: Vector select
	                 (As<Int>(x) & Int(0x80000000)));
}

RValue<Float> Atan2(RValue<Float> y, RValue<Float> x, bool relaxedPrecision)
{
	const Float pi(3.14159265f);             // pi
	const Float minus_pi(-3.14159265f);      // -pi
	const Float half_pi(1.57079632f);        // pi/2
	const Float quarter_pi(7.85398163e-1f);  // pi/4

	// Rotate to upper semicircle when in lower semicircle
	Int S = CmpLT(y, 0.0f);
	Float theta = As<Float>(S & As<Int>(minus_pi));
	Float x0 = As<Float>((As<Int>(y) & Int(0x80000000)) ^ As<Int>(x));
	Float y0 = Abs(y);

	// Rotate to right quadrant when in left quadrant
	Int Q = CmpLT(x0, 0.0f);
	theta += As<Float>(Q & As<Int>(half_pi));
	Float x1 = As<Float>((Q & As<Int>(y0)) | (~Q & As<Int>(x0)));   // FIXME: Vector select
	Float y1 = As<Float>((Q & As<Int>(-x0)) | (~Q & As<Int>(y0)));  // FIXME: Vector select

	// Mirror to first octant when in second octant
	Int O = CmpNLT(y1, x1);
	Float x2 = As<Float>((O & As<Int>(y1)) | (~O & As<Int>(x1)));  // FIXME: Vector select
	Float y2 = As<Float>((O & As<Int>(x1)) | (~O & As<Int>(y1)));  // FIXME: Vector select

	// Approximation of atan in [0..1]
	Int zero_x = CmpEQ(x2, 0.0f);
	Int inf_y = IsInf(y2);  // Since x2 >= y2, this means x2 == y2 == inf, so we use 45 degrees or pi/4
	Float atan2_theta = Atan_01(y2 / x2);
	theta += As<Float>((~zero_x & ~inf_y & ((O & As<Int>(half_pi - atan2_theta)) | (~O & (As<Int>(atan2_theta))))) |  // FIXME: Vector select
	                   (inf_y & As<Int>(quarter_pi)));
	...
	Int precision_loss = S & Q & O & ~inf_y;

	return As<Float>((precision_loss & As<Int>(-atan2_theta)) | (~precision_loss & As<Int>(theta)));  // FIXME: Vector select
}

// TODO(chromium:1299047)
static RValue<Float> Exp2_legacy(RValue<Float> x0)
{
	Int i = RoundInt(x0 - 0.5f);
	Float ii = As<Float>((i + Int(127)) << 23);

	Float f = x0 - Float(i);
	Float ff = As<Float>(Int(0x3AF61905));
	ff = ff * f + As<Float>(Int(0x3C134806));
	ff = ff * f + As<Float>(Int(0x3D64AA23));
	ff = ff * f + As<Float>(Int(0x3E75EAD4));
	ff = ff * f + As<Float>(Int(0x3F31727B));
	ff = ff * f + 1.0f;

	return ii * ff;
}

RValue<Float> Exp2(RValue<Float> x, bool relaxedPrecision)
{
	// Clamp to prevent overflow past the representation of infinity.
	Float x0 = x;
	x0 = Min(x0, 128.0f);
	x0 = Max(x0, As<Float>(Int(0xC2FDFFFF)));  // -126.999992
	...
	if(!relaxedPrecision)  // highp
	{
		...
		const Float e = -3.0684753e-1f;

		Float r = MulAdd(MulAdd(MulAdd(MulAdd(a, f, b), f, c), f, d), f, e);
		...
		Float y = MulAdd(r, f, x0);
		Int i = Int(y * (1 << 23)) + (127 << 23);
	...
	else  // RelaxedPrecision / mediump
	{
		...
		Float r = MulAdd(MulAdd(a, f, b), f, c);

		// bit_cast<float>(int(x * 2^23)) is a piecewise linear approximation of 2^x.
		// See "Fast Exponential Computation on SIMD Architectures" by Malossi et al.
		Float y = MulAdd(r, f, x0);
		Int i = Int(MulAdd((1 << 23), y, (127 << 23)));

		return As<Float>(i);
	}
}

RValue<Float> Log2_legacy(RValue<Float> x)
{
	Float x1 = As<Float>(As<Int>(x) & Int(0x7F800000));
	x1 = As<Float>(As<UInt>(x1) >> 8);
	x1 = As<Float>(As<Int>(x1) | As<Int>(Float(1.0f)));
	x1 = (x1 - 1.4960938f) * 256.0f;
	Float x0 = As<Float>((As<Int>(x) & Int(0x007FFFFF)) | As<Int>(Float(1.0f)));

	Float x2 = MulAdd(MulAdd(9.5428179e-2f, x0, 4.7779095e-1f), x0, 1.9782813e-1f);
	Float x3 = MulAdd(MulAdd(MulAdd(1.6618466e-2f, x0, 2.0350508e-1f), x0, 2.7382900e-1f), x0, 4.0496687e-2f);

	x1 += (x0 - 1.0f) * (x2 / x3);

	Int pos_inf_x = CmpEQ(As<Int>(x), Int(0x7F800000));
	return As<Float>((pos_inf_x & As<Int>(x)) | (~pos_inf_x & As<Int>(x1)));
}

RValue<Float> Log2(RValue<Float> x, bool relaxedPrecision)
{
	if(SWIFTSHADER_LEGACY_PRECISION)  // TODO(chromium:1299047)
	{
	...
	if(!relaxedPrecision)  // highp
	{
		...
		// Handle log2(inf) = inf.
		y = As<Float>(As<Int>(y) | (CmpEQ(im, 0x7F800000) & As<Int>(Float::infinity())));
		...
		Float z = MulAdd(MulAdd(MulAdd(MulAdd(MulAdd(MulAdd(MulAdd(a, m, b), m, c), m, d), m, e), m, f), m, g), m, h);
	...
	else  // RelaxedPrecision / mediump
	{
		...
		// approximation of log2(). Scale to the radix and subtract exponent bias.
		Int im = As<Int>(x);
		Float y = MulAdd(Float(im), (1.0f / (1 << 23)), -127.0f);
		...
		// Add a polynomial approximation of log2(m+1)-m to the result's mantissa.
		const Float a = 2.8017103e-22f;
		const Float b = -8.373131e-15f;
		const Float c = 5.0615534e-8f;

		Float f = MulAdd(MulAdd(a, m, b), m, c);

		return MulAdd(f, m, y);
	}
}

RValue<Float> Exp(RValue<Float> x, bool relaxedPrecision)
{
	return Exp2(1.44269504f * x, relaxedPrecision);  // 1/ln(2)
}

RValue<Float> Log(RValue<Float> x, bool relaxedPrecision)
{
	return 6.93147181e-1f * Log2(x, relaxedPrecision);  // ln(2)
}

RValue<Float> Pow(RValue<Float> x, RValue<Float> y, bool relaxedPrecision)
{
	Float log = Log2(x, relaxedPrecision);
	log *= y;
	return Exp2(log, relaxedPrecision);
}

RValue<Float> Sinh(RValue<Float> x, bool relaxedPrecision)
{
	return (Exp(x, relaxedPrecision) - Exp(-x, relaxedPrecision)) * 0.5f;
}

RValue<Float> Cosh(RValue<Float> x, bool relaxedPrecision)
{
	return (Exp(x, relaxedPrecision) + Exp(-x, relaxedPrecision)) * 0.5f;
}

RValue<Float> Tanh(RValue<Float> x, bool relaxedPrecision)
{
	Float e_x = Exp(x, relaxedPrecision);
	Float e_minus_x = Exp(-x, relaxedPrecision);
	return (e_x - e_minus_x) / (e_x + e_minus_x);
}

RValue<Float> Asinh(RValue<Float> x, bool relaxedPrecision)
{
	return Log(x + Sqrt(x * x + 1.0f, relaxedPrecision), relaxedPrecision);
}

RValue<Float> Acosh(RValue<Float> x, bool relaxedPrecision)
{
	return Log(x + Sqrt(x + 1.0f, relaxedPrecision) * Sqrt(x - 1.0f, relaxedPrecision), relaxedPrecision);
}

RValue<Float> Atanh(RValue<Float> x, bool relaxedPrecision)
{
...
RValue<Float> reciprocal(RValue<Float> x, bool pp, bool exactAtPow2)
{
	return Rcp(x, pp, exactAtPow2);
}

RValue<Float> reciprocalSquareRoot(RValue<Float> x, bool absolute, bool pp)
{
	Float abs = x;
...
RValue<Float4> Pow(RValue<Float4> x, RValue<Float4> y, bool relaxedPrecision)
{
	...
	xx = Insert128(xx, x, 0);
	yy = Insert128(yy, y, 0);
	return Extract128(SIMD::Pow(xx, yy, relaxedPrecision), 0);
}
...
UInt4 floatToHalfBits(UInt4 floatBits, bool storeInUpperBits)
{
