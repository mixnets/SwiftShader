...
RValue<SIMD::Float> Ldexp(RValue<SIMD::Float> significand, RValue<SIMD::Int> exponent)
{
	...
	// Multiply the input value by the two powers to get the final number.
	// Note that multiplying fltA and fltB together may result in an overflow,
	// so ensure that significand is multiplied by fltA, *then* the result of that with fltB.
