...
namespace sw
{
			  ...
			  serialID{serialCounter++}, modes{}
	{
		...
		for (auto insn : *this)
		{
			...
			case spv::OpConstantComposite:
			{
				...
				auto objectId = ObjectID(insn.word(2));
	...
	void SpirvShader::ProcessExecutionMode(InsnIterator insn)
	{
		...
		case spv::ExecutionModeLocalSize:
			...
			modes.WorkgroupSizeX = modes.LocalSizeX;
			modes.WorkgroupSizeY = modes.LocalSizeY;
			modes.WorkgroupSizeZ = modes.LocalSizeZ;
			break;
	...
	void SpirvShader::EmitLoad(InsnIterator insn, SpirvRoutine *routine) const
	{
		...
		BranchDivergent(routine, pointer.kind == Object::Kind::Value, [&]()
		{
			...
			auto offsets = pointer.kind == Object::Kind::Value ?
					As<SIMD::Int>(routine->getIntermediate(pointerId)[0]) :
