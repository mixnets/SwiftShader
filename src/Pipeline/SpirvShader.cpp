...
namespace sw
{
	...
	void SpirvShader::EvalSpecConstantOp(InsnIterator insn)
	{
		...
		case spv::OpUGreaterThanEqual:
		case spv::OpSGreaterThanEqual:
			return EvalSpecConstantBinaryOp(insn);
		...
		case spv::OpLogicalNot:
		case spv::OpQuantizeToF16:
			return EvalSpecConstantUnaryOp(insn);
		...
		default:
			UNIMPLEMENTED("EvalSpecConstantOp op: %s", OpcodeName(opcode).c_str());
		}
	}
	...
	void SpirvShader::EvalSpecConstantUnaryOp(InsnIterator insn)
	{
		...
		for (auto i = 0u; i < size; i++)
		{
			...
			default:
				UNIMPLEMENTED("EvalSpecConstantBinaryOp op: %s", OpcodeName(opcode).c_str());
			}
		}
	}
