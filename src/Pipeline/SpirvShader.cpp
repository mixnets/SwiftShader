...
namespace sw
{
			  ...
			  serialID{serialCounter++}, modes{}
	{
		...
		for (auto insn : *this)
		{
			...
			case spv::OpLabel:
				...
				currentBlock = SpirvBlock::ID(insn.word(1));
				blockStart = insn;
				break;
			...
			case spv::OpUnreachable:
			{
				...
				blocks[currentBlock] = SpirvBlock(blockStart, blockEnd);
				currentBlock = SpirvBlock::ID(0);
			...
			std::function<void(SpirvScope*)> emitFalse) const
	{
		...
		for (auto &itIm : trueScope.intermediates)
		{
			...
			// 	for (auto i = 0u; i < type.sizeInComponents; i++)
			// 	{
			// 		selIm.emplace(i, As<SIMD::Float>(trueIm[i]));
			// 	}
			// }
		}
		...
		// 		for (auto i = 0u; i < type.sizeInComponents; i++)
		// 		{
		// 			selIm.emplace(i, As<SIMD::Float>(falseIm[i]));
		// 		}
		// 	}
		// }
	}
