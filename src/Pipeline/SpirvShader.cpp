	...
	void SpirvShader::emit(SpirvRoutine *routine) const
	{
		for (auto insn : *this)
		{
			...
			case spv::OpCompositeInsert:
			{
				...
				auto firstNewComponent = WalkLiteralAccessChain(insn.word(1), insn.wordCount() - 5, insn.wordPointer(5));
				auto & dst = routine->getIntermediate(insn.word(2));
				...
				if (srcObject.kind == Object::Kind::Constant)
				{
					...
					for (auto i = firstNewComponent + newPartObject.sizeInComponents; i < type.sizeInComponents; i++)
						dst.emplace(i, RValue<Float4>(src[i]));
				}
				else
				{
				...
				if (newPartObject.kind == Object::Kind::Constant)
				{
					...
					for (auto i = 0u; i < newPartObject.sizeInComponents; i++)
			...
			case spv::OpCompositeExtract:
			{
				...
				auto compositeTypeId = compositeObject.definition.word(1);
