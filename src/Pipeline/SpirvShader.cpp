...
namespace sw
{
	...
	{
		// Walk an explicitly laid out storage class. `offset` is in sizeof(float) units.
		ApplyDecorationsForId(&d, id);
		...
		case spv::OpTypeMatrix:
			for (auto i = 0u; i < type.definition.word(3); i++)
			{
				...
				if (!d.HasMatrixStride) {
					// TODO: propagate matrix strides correctly through OpAccessChain. Until then,
					// use the std140 minimum stride (8 for columns containing 2 elements; 16 for columns containing 3 or 4)
					d.MatrixStride = getType(type.definition.word(2)).sizeInComponents > 2 ? 16 : 8;
				}
