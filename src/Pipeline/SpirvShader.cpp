...
namespace sw
{
	...
	bool SpirvShader::IsStorageLaneReplicated(spv::StorageClass storageClass)
	{
		switch (storageClass)
		{
		case spv::StorageClassUniform:
	...
	void SpirvShader::EmitLoad(InsnIterator insn, SpirvRoutine *routine) const
	{
		...
		bool laneReplicated = IsStorageLaneReplicated(pointerBaseTy.storageClass);
		...
		if (pointer.kind == Object::Kind::Value)
		{
			...
			for (auto i = 0u; i < objectTy.sizeInComponents; i++)
			{
				...
				for (int j = 0; j < SIMD::Width; j++)
				{
					...
					if (!laneReplicated) { offset = offset * SIMD::Width + j; }
					v = Insert(v, ptrBase[offset], j);
				}
				dst.emplace(i, v);
			}
		}
		else if (!laneReplicated)
		{
