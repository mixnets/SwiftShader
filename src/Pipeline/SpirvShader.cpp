...
namespace sw
{
	namespace SIMD
	{
		...
		T Load(Pointer ptr, Int mask, bool atomic /* = false */, std::memory_order order /* = std::memory_order_relaxed */)
		{
			...
			T out;
			mask &= CmpLT(ptr.offset, SIMD::Int(ptr.limit)); // Disable OOB reads.
			If(!ptr.uniform || AnyFalse(mask))
			{
				...
				for (int i = 0; i < SIMD::Width; i++)
				{
					If(Extract(mask, i) != 0)
					{
						rr::Int offset = Extract(ptr.offset, i);
			...
			Else
			{
				out = Load(rr::Pointer<T>(ptr.base), sizeof(float), atomic, order);
			}
			return out;
		}
		...
		void Store(Pointer ptr, T val, Int mask, bool atomic /* = false */, std::memory_order order /* = std::memory_order_relaxed */)
		{
			...
			Else
			{
				Store(val, rr::Pointer<T>(ptr.base), sizeof(float), atomic, order);
			}
		}
