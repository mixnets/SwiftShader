	...
	void SpirvShader::EmitBinaryOp(InsnIterator insn, SpirvRoutine *routine) const
	{
		...
		for (auto i = 0u; i < lhsType.sizeInComponents; i++)
		{
			...
			case spv::OpSMulExtended:
			{
				...
				for (int j = 0; j < SIMD::Width; j++)
				{
					high = Insert(high, As<Float>(MulHigh(
							Extract(As<SIMD::Int>(lhs), j),
							Extract(As<SIMD::Int>(rhs), j))), j);
				}
				dst.emplace(i + lhsType.sizeInComponents, high);
				break;
			}
			case spv::OpUMulExtended:
			{
				...
				{
					high = Insert(high, As<Float>(MulHigh(
							As<UInt>(Extract(As<SIMD::Int>(lhs), j)),
							As<UInt>(Extract(As<SIMD::Int>(rhs), j)))), j);
				}
				dst.emplace(i + lhsType.sizeInComponents, high);
				break;
			}
