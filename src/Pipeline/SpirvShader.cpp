...
namespace sw
{
	...
	void SpirvShader::EmitBlock(Block::ID id, EmitState *state) const
	{
		...
		{
			case Block::Simple:
			case Block::StructuredBranch:
			case Block::UnstructuredBranch:
			case Block::StructuredSwitch:
			case Block::UnstructuredSwitch:
				if (id != mainBlockId)
				{
					// Emit all incoming blocks and set the activeLaneMask.
					Intermediate activeLaneMask(1);
					activeLaneMask.emplace(0, SIMD::Int(0));
	...
	void SpirvShader::EmitLoop(EmitState *state) const
	{
		...
		// Branch to loop header or merge block.
	...
	Intermediate SpirvShader::getPhiValue(InsnIterator insn, EmitState *state) const
	{
		...
		for (uint32_t w = 3; w < insn.wordCount(); w += 2)
		{
			...
			for (uint32_t i = 0; i < type.sizeInComponents; i++)
			{
				...
				out.replace(i, first ? inMasked : (out.Int(i) | inMasked));
			}
			first = false;
		}
	...
	SpirvShader::EmitResult SpirvShader::EmitPhi(InsnIterator insn, EmitState *state) const
	{
		...
		auto vals = getPhiValue(insn, state);
