...
SIMD::Pointer SpirvShader::WalkExplicitLayoutAccessChain(Object::ID baseId, Object::ID elemId, const Span &indexIds, const EmitState *state) const
{
	...
	OffsetToElem(ptr, elemId, d.ArrayStride, state);
...
SIMD::Pointer SpirvShader::WalkAccessChain(Object::ID baseId, Object::ID elemId, const Span &indexIds, EmitState const *state) const
{
...
SpirvShader::EmitResult SpirvShader::EmitAccessChain(InsnIterator insn, EmitState *state) const
{
	...
	Object::ID elemId = (insn.opcode() == spv::OpPtrAccessChain) ? insn.word(4) : 0;
	...
	{
		auto ptr = WalkExplicitLayoutAccessChain(baseId, elemId, Span(insn, indexId, insn.wordCount() - indexId), state);
		state->createPointer(resultId, ptr);
	}
	else
	{
		auto ptr = WalkAccessChain(baseId, elemId, Span(insn, indexId, insn.wordCount() - indexId), state);
		state->createPointer(resultId, ptr);
	}
	...
	case Object::Kind::Pointer:
		{
			...
			state->assignPhysicalStoragePointer(insn.resultId(),
			                                    SIMD::Pointer::IfThenElse(cond.Int(0), lhs, rhs));
		}
		break;
	default:
		{
