...
namespace sw
{
	namespace SIMD
	{
		...
		T Load(Pointer ptr, OutOfBoundsBehavior robust, Int mask, bool atomic /* = false */, std::memory_order order /* = std::memory_order_relaxed */, int alignment /* = sizeof(float) */)
		{
			...
			if (ptr.isStaticAllInBounds(sizeof(float), robust))
			{
			...
			else
			{
				...
				case OutOfBoundsBehavior::UndefinedValue:
					mask &= ptr.isInBounds(sizeof(float), robust);  // Disable out-of-bounds reads.
					break;
		...
		void Store(Pointer ptr, T val, OutOfBoundsBehavior robust, Int mask, bool atomic /* = false */, std::memory_order order /* = std::memory_order_relaxed */)
		{
			...
			case OutOfBoundsBehavior::UndefinedValue:
				mask &= ptr.isInBounds(sizeof(float), robust);  // Disable out-of-bounds writes.
				break;
			case OutOfBoundsBehavior::UndefinedBehavior:
				// Nothing to do. Application/compiler must guarantee no out-of-bounds accesses.
				break;
			...
			if (!atomic && order == std::memory_order_relaxed)
			{
				if (ptr.hasStaticEqualOffsets())
				{
					...
					else If (AnyTrue(mask))  // TODO(b/128636885): Fold into If(!robust || AnyTrue(mask))
					{
						*rr::Pointer<EL>(ptr.base + ptr.staticOffsets[0], alignment) = As<EL>(scalarVal);
					}
				}
	...
	SpirvShader::EmitResult SpirvShader::EmitImageRead(InsnIterator insn, EmitState *state) const
	{
		...
		auto robust = state->getOutOfBoundsBehavior(spv::StorageClassUniform);
