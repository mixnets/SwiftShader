		...
		for (auto insn : *this)
		{
			...
			case spv::OpVariable:
			{
				...
				case Object::Kind::PhysicalPointer:
				{
					...
					ASSERT(d.Binding >= 0);
					Pointer<Byte> setPtr = routine->descriptorSets[d.DescriptorSet];
					Pointer<Byte> bindingPtr = Pointer<Byte>(setPtr + d.Binding * sizeof(VkDescriptorBufferInfo));
					Pointer<Byte> buffer = *Pointer<Pointer<Byte>>(bindingPtr + OFFSET(VkDescriptorBufferInfo, buffer));
					Int offset = *Pointer<Int>(bindingPtr + OFFSET(VkDescriptorBufferInfo, offset));
			...
			case spv::OpTypePointer:
			case spv::OpTypeFunction:
				break; // Handled by earlier passes.
			default:
