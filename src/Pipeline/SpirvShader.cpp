	...
	else
	{
		if(rendering)
		{
			...
			for(auto i = 0u; i < rendering->colorAttachmentCount; i++)
			{
				...
				// > fragment output state, the value of renderPass is VK_NULL_HANDLE,
				// > and VkRenderingInputAttachmentIndexInfoKHR is included,
				// > VkRenderingInputAttachmentIndexInfoKHR::colorAttachmentCount must
				// > be equal to VkPipelineRenderingCreateInfo::colorAttachmentCount
				auto attachmentIndex = inputAttachmentMapping ? inputAttachmentMapping->pColorAttachmentInputIndices[i] : i;
				if(attachmentIndex != VK_ATTACHMENT_UNUSED)
				{
					inputAttachmentFormats[attachmentIndex] = rendering->pColorAttachmentFormats[attachmentIndex];
				}
			}
