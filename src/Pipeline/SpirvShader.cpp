...
namespace sw
{
	...
	std::pair<Pointer<Byte>, SIMD::Int> SpirvShader::WalkExplicitLayoutAccessChain(Object::ID id, uint32_t numIndices, uint32_t const *indexIds, SpirvRoutine *routine) const
	{
		...
		switch (baseObject.kind)
		{
			...
			case Object::Kind::DescriptorSet:
			{
				...
				ASSERT(getObject(indexIds[0]).kind == Object::Kind::Constant); // TODO: Dynamic lookup
				size_t bindingOffset = setLayout->getBindingOffset(d.Binding, GetConstantInt(indexIds[0]));
				...
				if (setLayout->isBindingDynamic(d.Binding))
				{
					...
					Pointer<Byte> binding = routine->descriptorSetBindings + d.DescriptorSet * sizeof(vk::DescriptorSet::Binding); // vk::DescriptorSetBinding*
				...
				numIndices--; indexIds++; typeId = getType(typeId).element;
				break;
			}
	...
	SpirvShader::EmitResult SpirvShader::EmitVariable(InsnIterator insn, EmitState *state) const
	{
		...
		case spv::StorageClassStorageBuffer:
		{
			...
			routine->physicalPointers[resultId] = set;
			break;
		}
		case spv::StorageClassPushConstant:
		{
