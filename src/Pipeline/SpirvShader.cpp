...
namespace sw
{
	...
	void SpirvShader::emit(SpirvRoutine *routine) const
	{
		for (auto insn : *this)
		{
			...
			case spv::OpLoad:
			{
				// TODO untangle: allocated objects currently treated the same as ssavalues.

				// Dereference a pointer
				auto &object = getObject(insn.word(2));
				auto &type = getObject(insn.word(1));
				auto &pointer = getObject(insn.word(3));
				routine->createLvalue(insn.word(2), type.sizeInComponents);
				...
				if (object.storageClass == spv::StorageClassImage ||
					object.storageClass == spv::StorageClassUniform ||
					object.storageClass == spv::StorageClassUniformConstant)
				{
				...
				if (object.kind == Object::Kind::Value)
				{
