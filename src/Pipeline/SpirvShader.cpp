...
namespace sw
{
	...
	SpirvShader::EmitResult SpirvShader::EmitImageSample(ImageInstruction instruction, InsnIterator insn, EmitState *state) const
	{
		...
		bool hasSampler = (sampledImage.opcode() == spv::OpSampledImage) ||
			(sampledImage.opcode() == spv::OpLoad && sampledImage.type == spv::OpTypeSampledImage);
		auto samplerDescriptor = hasSampler ?
