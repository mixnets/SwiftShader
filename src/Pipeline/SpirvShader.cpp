...
namespace sw
{
	...
	{
		// Produce a *byte* offset into external memory
		...
		for (auto i = 0u; i < numIndexes; i++)
		{
			...
			case spv::OpTypeRuntimeArray:
			{
				...
				if (obj.kind == Object::Kind::Constant)
					constantOffset += d.ArrayStride/4 * GetConstantInt(indexIds[i]);
				else
					dynamicOffset += SIMD::Int(d.ArrayStride / 4) * routine->getIntermediate(indexIds[i]).Int(0);
				typeId = type.element;
				break;
			}
			case spv::OpTypeMatrix:
			{
				...
				auto & obj = getObject(indexIds[i]);
				if (obj.kind == Object::Kind::Constant)
					constantOffset += d.MatrixStride/4 * GetConstantInt(indexIds[i]);
				else
					dynamicOffset += SIMD::Int(d.MatrixStride / 4) * routine->getIntermediate(indexIds[i]).Int(0);
				typeId = type.element;
				break;
			}
			case spv::OpTypeVector:
