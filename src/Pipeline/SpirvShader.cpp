...
SpirvShader::Operand::Operand(const SpirvShader *shader, const EmitState *state, SpirvShader::Object::ID objectId)
    : Operand(state, objectId, shader->getObject(objectId))
{}
...
SpirvShader::Operand::Operand(const EmitState *state, SpirvShader::Object::ID objectId, const Object &object)
    : type(object.type)
    , constant(reinterpret_cast<uint32_t *>(object.constantValue.get()))
    , intermediate(object.kind == SpirvShader::Object::Kind::Intermediate ? &state->getIntermediate(objectId) : nullptr)
{
