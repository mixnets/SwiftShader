				...
				case Object::Kind::Variable:
				{
					...
					case spv::StorageClassUniform:
					{
						...
						if (d.BufferBlock)
						{
						    ...
						    Int buffer = *Pointer<Int>(bindingPtr + OFFSET(VkDescriptorBufferInfo, buffer));
							Int offset = *Pointer<Int>(bindingPtr + OFFSET(VkDescriptorBufferInfo, offset));
							Int address = buffer + offset;
							Float4 v;
							for (int j = 0; j < 4; j++)
							{
								v = Insert(v, Float(address), j);
							}
							auto &dst = routine->getValue(resultId);
							dst[0] = v;
						} else {
							UNIMPLEMENTED("Uniform type implemented");
						}
						break;
					}
					default:
						UNIMPLEMENTED("Variable storage class %d not yet implemented", (int)objectTy.storageClass);
					}
					break;
				}
				default:
