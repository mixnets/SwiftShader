...
namespace sw
{
			  ...
			  serialID{serialCounter++}, modes{}
	{
		...
		for (auto insn : *this)
		{
			...
			case spv::OpTypeFunction:
			{
				...
				{
					auto storageClass = static_cast<spv::StorageClass>(insn.word(2));
					auto elementTypeId = insn.word(3);
					auto &elementTy = getType(elementTypeId);
					type.element = elementTypeId;
					type.isBuiltInBlock = elementTy.isBuiltInBlock;
	...
	void SpirvShader::ProcessInterfaceVariable(Object &object)
	{
		...
		{
			builtinInterface[d->second.BuiltIn] = {resultId, 0, objectTy.sizeInComponents};
		}
		else
		{
	...
	void SpirvShader::emitProlog(SpirvRoutine *routine) const
	{
		...
		for (auto insn : *this)
		{
			...
			case spv::OpVariable:
			{
				...
				if (objectTy.sizeInComponents > 0)
				{
					routine->createLvalue(insn.word(2), objectTy.sizeInComponents);
				}
				break;
			}
			default:
		...
		for (auto insn : *this)
		{
			...
			case spv::OpVariable:
			{
				...
				case Object::Kind::PhysicalPointer:
				{
					...
					auto setLayout = vk::Cast(pipelineLayout->getDescriptorSetLayout(d.DescriptorSet));
					...
					Pointer<Byte> set = routine->descriptorSets[d.DescriptorSet];
					Pointer<Byte> binding = Pointer<Byte>(set + bindingOffset);
					Pointer<Byte> buffer = *Pointer<Pointer<Byte>>(binding + OFFSET(VkDescriptorBufferInfo, buffer));
					Int offset = *Pointer<Int>(binding + OFFSET(VkDescriptorBufferInfo, offset));
					Pointer<Byte> address = buffer + offset;
					routine->physicalPointers[resultId] = address;
					RR_WATCH(d.DescriptorSet, d.Binding, set, binding, buffer, offset, address);
					break;
				}
				case Object::Kind::Variable:
				{
					// TODO
					break;
				}
			...
			case spv::OpLoad:
			{
				...
				RR_WATCH(objectId.value(), pointerId.value());
				RR_WATCH(pointer.kind == Object::Kind::PhysicalPointer);
				if (pointer.kind == Object::Kind::PhysicalPointer)
				{
					auto &ptr = routine->getPhysicalPointer(pointerId);
					Pointer<Float> floatPtr = As<Pointer<Float>>(ptr);
					RR_WATCH(floatPtr);
					Float f = *floatPtr;
					dst.emplace(0, Float4(f));
					break;
				}
			...
			case spv::OpAccessChain:
			{
				...
				if (base.kind == Object::Kind::PhysicalPointer)
				{
					routine->physicalPointers[objectId] = routine->physicalPointers[baseId];
					break;
				}
			...
			case spv::OpStore:
			{
				...
				if (pointer.kind == Object::Kind::PhysicalPointer)
				{
					auto &addr = routine->physicalPointers[pointerId];
					auto dst = Pointer<Float>(addr);
