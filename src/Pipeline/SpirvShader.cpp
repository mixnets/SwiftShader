...
SIMD::Pointer SpirvShader::WalkExplicitLayoutAccessChain(Object::ID baseId, const Span &indexIds, const EmitState *state, bool nonUniform) const
{
	...
	SIMD::Pointer ptr = GetPointerToData(baseId, arrayIndex, state, nonUniform);
		...
		case spv::OpTypeRuntimeArray:
			{
				...
				if(getType(baseObject).storageClass == spv::StorageClassUniformConstant)
				{
					...
					else
					{
						...
						if(nonUniform)
						{
							ptr.offsetBase(SIMD::Int(descriptorSize * intermediate));
						}
						else
						{
							ptr.offsetBase(Int(descriptorSize * Extract(intermediate, 0)));
						}
					}
				}
				else
...
SpirvShader::EmitResult SpirvShader::EmitAccessChain(InsnIterator insn, EmitState *state) const
{
	...
	{
		auto ptr = WalkExplicitLayoutAccessChain(baseId, Span(insn, 4, insn.wordCount() - 4), state, nonUniform);
