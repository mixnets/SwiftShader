...
{
	if(descriptorDecorations.find(pointerId) != descriptorDecorations.end())
	{
		const auto &d = descriptorDecorations.at(pointerId);
...
{
	EmitState state(routine, entryPoint, activeLaneMask, storesAndAtomicsMask, descriptorSets, multiSampleCount, executionModel);
