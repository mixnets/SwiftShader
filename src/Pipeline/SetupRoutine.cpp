		...
		Function<Int(Pointer<Byte>, Pointer<Byte>, Pointer<Byte>, Pointer<Byte>)> function;
		{
			...
			If(m != 0 || Bool(!triangle))   // Clipped triangle; reproject
			{
				...
				Do
				{
					...
					Y[i] = RoundInt(*Pointer<Float>(data + OFFSET(DrawData,Y0xF)) + v.y * rhw * *Pointer<Float>(data + OFFSET(DrawData,HxW)));
			...
			{
				yMin = (yMin + *Pointer<Int>(constants + OFFSET(Constants, yMinMultiSampleOffset))) >> subPixB;
				yMax = (yMax + *Pointer<Int>(constants + OFFSET(Constants, yMaxMultiSampleOffset))) >> subPixB;
			}
			else
			{
