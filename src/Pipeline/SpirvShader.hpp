...
namespace sw
{
	...
	public:
		...
		~Intermediate()
		{
			for (auto i = 0u; i < size; i++)
				reinterpret_cast<Ty *>(&contents[i])->~Ty();
		}

		template<typename ...Args>
		void emplace(uint32_t n, Args&&... args)
		{
			new (&contents[n]) Ty(std::forward<Args>(args)...);
		}

		Ty const & operator[](uint32_t n) const
		{
			return *reinterpret_cast<Ty const *>(&contents[n]);
		}
	...
	private:
		using ContentsType = std::aligned_storage<sizeof(Ty), alignof(Ty)>;
