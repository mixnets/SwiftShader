...
namespace sw
{
	...
	{
		Zero,                // Loads become zero, stores are elided.
		RobustBufferAccess,  // As defined by the Vulkan spec (access anywhere within bounds, or zeroing).
		UndefinedValue,      // Loaded value can be anything. Not secure. No program termination.
	...
	namespace SIMD
	{
		...
		template<typename T>
		void Store(Pointer ptr, RValue<T> val, OutOfBoundsBehavior robust, Int mask, bool atomic = false, std::memory_order order = std::memory_order_relaxed)
		{
