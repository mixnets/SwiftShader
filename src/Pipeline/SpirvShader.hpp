...
namespace sw
{
	...
	public:
		...
		class TypeOrObjectID : public ID<TypeOrObject>
		{
		public:
			typedef std::hash<ID<TypeOrObject>> Hash;

			inline TypeOrObjectID(uint32_t id) : ID(id) {}
			inline TypeOrObjectID(TypeID id) : ID(id.value()) {}
			inline TypeOrObjectID(ObjectID id) : ID(id.value()) {}
	...
	public:
		...
		std::unordered_map<SpirvShader::ObjectID, std::unique_ptr<Value>> lvalues;
