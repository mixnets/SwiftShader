...
sw::SIMD::Float Interpolate(const sw::SIMD::Float &x, const sw::SIMD::Float &y, const sw::SIMD::Float &rhw,
                            ...
                            const sw::SpirvRoutine::InputData &inputData)
{
                                     ...
                                     uint32_t component, EmitState *state, InterpolationType type) const
{
	...
	if(ptr.hasDynamicOffsets)
	{
		...
		return ::Interpolate(x, y, rhw, A, B, C, state->routine->inputsData[packedInterpolant]);
	}
	else
	{
	...
	return SpirvRoutine::interpolateAtXY(x, y, rhw, planeEquation, state->routine->inputsData[packedInterpolant]);
}

SIMD::Float SpirvRoutine::interpolateAtXY(const SIMD::Float &x, const SIMD::Float &y, const SIMD::Float &rhw, Pointer<Byte> planeEquation, const InputData &inputData)
{
	SIMD::Float A;
	SIMD::Float B;
	...
	return ::Interpolate(x, y, rhw, A, B, C, inputData);
}
