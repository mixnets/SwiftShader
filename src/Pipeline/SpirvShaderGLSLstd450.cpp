...
SpirvShader::EmitResult SpirvShader::EmitExtGLSLstd450(InsnIterator insn, EmitState *state) const
{
	...
	switch(extInstIndex)
	{
		...
		case GLSLstd450Distance:
		{
			...
			for(auto i = 1u; i < p0.sizeInComponents; i++)
			{
		...
		case GLSLstd450Modf:
		{
			...
			Intermediate whole(type.sizeInComponents);
		...
		case GLSLstd450ModfStruct:
		{
			...
			for(auto i = 0u; i < val.sizeInComponents; i++)
			{
				auto wholeAndFrac = Modf(val.Float(i));
				dst.move(i, wholeAndFrac.second);
				dst.move(val.sizeInComponents + i, wholeAndFrac.first);
			}
			break;
		}
		...
		case GLSLstd450Frexp:
		{
			...
			Intermediate exp(type.sizeInComponents);
		...
		case GLSLstd450FrexpStruct:
		{
			...
			for(auto i = 0u; i < val.sizeInComponents; i++)
			{
				auto significandAndExponent = Frexp(val.Float(i));
				dst.move(i, significandAndExponent.first);
				dst.move(val.sizeInComponents + i, significandAndExponent.second);
			}
			break;
		}
		...
		case GLSLstd450Determinant:
		{
			...
			switch(mat.sizeInComponents)
			{
				...
				case 16:  // 4x4
					...
					break;
				default:
					UNREACHABLE("GLSLstd450Determinant can only operate with square matrices. Got %d elements", int(mat.sizeInComponents));
			}
			break;
		}
		case GLSLstd450MatrixInverse:
		{
			...
			switch(mat.sizeInComponents)
			{
				...
				default:
					UNREACHABLE("GLSLstd450MatrixInverse can only operate with square matrices. Got %d elements", int(mat.sizeInComponents));
			}
			break;
		}
		case GLSLstd450IMix:
