...
{
	Int4 oneOut = ~CmpLT(src, Float4(1.0f));
	Int4 zeroOut = CmpGT(Float4(0.0f), dst);
	return As<Float4>(~zeroOut & ((oneOut &
	                               As<Int4>(Float4(1.0f))) |
	                              (~oneOut &
...
{
	Int4 oneOut = ~CmpLT(dst, Float4(1.0f));
	Int4 zeroOut = ~CmpGT(src, Float4(0.0f));
	return As<Float4>(
	    (oneOut &
	     As<Int4>(Float4(1.0f))) |
	    (~oneOut & ~zeroOut &
...
void PixelRoutine::computeAdvancedBlendMode(int index, Vector4f &srcColor, Vector4f &dstColor)
{
	...
	ASSERT(state.blendState[index].blendOverlap == VK_BLEND_OVERLAP_UNCORRELATED_EXT);
