...
void PixelRoutine::quad(Pointer<Byte> cBuffer[RENDERTARGETS], Pointer<Byte> &zBuffer, Pointer<Byte> &sBuffer, Int cMask[4], Int &x, Int &y)
{
	...
	bool shaderContainsInterpolation = spirvShader && spirvShader->getModes().ContainsInterpolation;
	if(!perSampleRendering && spirvShader)
	{
		...
		// GLSL 4.50 interpolation instructions also require the sampleRateShading feature.
		perSampleRendering |= shaderContainsInterpolation;
	}
	unsigned int numSampleRenders = perSampleRendering ? state.multiSampleCount : 1;

	for(unsigned int i = 0; i < numSampleRenders; ++i)
	{
		int sampleId = perSampleRendering ? i : -1;
		unsigned int msLoopInit = perSampleRendering ? sampleId : 0;
		unsigned int msLoopEnd = perSampleRendering ? sampleId + 1 : state.multiSampleCount;

		for(unsigned int q = msLoopInit; q < msLoopEnd; q++)
		{
			zMask[q] = cMask[q];
			sMask[q] = cMask[q];
		}
		...
		If(depthPass || Bool(!earlyDepthTest))
		{
			...
			if(interpolateW())
			{
				...
				if(state.centroid || shaderContainsInterpolation)
				{
					rhwCentroid = reciprocal(SpirvRoutine::interpolateXY(XXXX, YYYY, rhwCentroid, primitive + OFFSET(Primitive, w), false, false));
				}
			}
			...
			if(spirvShader)
			{
				...
				{
					routine.multiSampleCount = state.multiSampleCount;
					routine.primitive = primitive;

					routine.x = xxxx;
					routine.y = yyyy;
					routine.rhw = rhw;

					routine.xCentroid = XXXX;
					routine.yCentroid = YYYY;
					routine.rhwCentroid = rhwCentroid;
				}

				if(perSampleRendering && (state.multiSampleCount > 1))
				{
					...
					if(input.Type != SpirvShader::ATTRIBTYPE_UNUSED)
					{
						if(input.Centroid && state.enableMultiSampling)
						{
							routine.inputs[interpolant] =
							    SpirvRoutine::interpolateXY(XXXX, YYYY, rhwCentroid,
							                                primitive + OFFSET(Primitive, V[interpolant]),
							                                input.Flat, !input.NoPerspective);
						}
						else if(perSampleRendering)
						{
							routine.inputs[interpolant] =
							    SpirvRoutine::interpolateXY(xxxx, yyyy, rhw,
							                                primitive + OFFSET(Primitive, V[interpolant]),
							                                input.Flat, !input.NoPerspective);
						}
						else
				...
				for(uint32_t i = 0; i < state.numClipDistances; i++)
				{
					...
					for(auto ms = msLoopInit; ms < msLoopEnd; ms++)
					{
...
void PixelRoutine::alphaToCoverage(Int cMask[4], const Float4 &alpha, int sampleId)
{
	...
	unsigned int msLoopInit = (sampleId >= 0) ? sampleId : 0;
	unsigned int msLoopEnd = (sampleId >= 0) ? sampleId + 1 : state.multiSampleCount;
