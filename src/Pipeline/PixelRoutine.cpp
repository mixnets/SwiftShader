...
void PixelRoutine::writeColor(int index, const Pointer<Byte> &cBuffer, const Int &x, Vector4s &current, const Int &sMask, const Int &zMask, const Int &cMask)
{
	...
	int writeMask = colorWriteMask(index);
...
int PixelRoutine::colorWriteMask(int index)
{
	vk::Format format = state.colorFormat[index];
	...
	case VK_FORMAT_B8G8R8A8_SRGB:
		// BGRA: swap R and B
		return (writeMask & 0x0000000A) |
		       (writeMask & 0x00000001) << 2 |
		       (writeMask & 0x00000004) >> 2;
	case VK_FORMAT_R4G4B4A4_UNORM_PACK16:
	case VK_FORMAT_R5G5B5A1_UNORM_PACK16:
		// ABGR: reverse the channel order
		return (writeMask & 0x00000001) << 3 |
		       (writeMask & 0x00000002) << 1 |
		       (writeMask & 0x00000004) >> 1 |
		       (writeMask & 0x00000008) >> 3;
	case VK_FORMAT_B4G4R4A4_UNORM_PACK16:
	case VK_FORMAT_B5G5R5A1_UNORM_PACK16:
		// ARGB: move A in front of RGB
		return (writeMask & 0x00000007) << 1 |
		       (writeMask & 0x00000008) >> 3;
	default:
