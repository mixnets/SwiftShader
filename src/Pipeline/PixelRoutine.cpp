	...
	for(unsigned int i = 0; i < numSampleRenders; i++)
	{
		...
		If(depthPass || Bool(!earlyDepthTest))
		{
			...
			If(alphaPass)
			{
				...
				occlusionSampleCount(zMask, sMask);
			}
		}

		writeStencil(sBuffer, x, sMask, zMask, cMask);
	}
}

void PixelRoutine::stencilTest(const Pointer<Byte> &sBuffer, const Int &x, Int sMask[4])
{
...
void PixelRoutine::writeDepth(Pointer<Byte> &zBuffer, const Int &x, const Int zMask[4])
{
...
void PixelRoutine::occlusionSampleCount(const Int zMask[4], const Int sMask[4])
{
...
void PixelRoutine::writeStencil(Pointer<Byte> &sBuffer, const Int &x, const Int sMask[4], const Int zMask[4], const Int cMask[4])
{
	if(!state.stencilActive)
	{
		return;
