	...
	for(unsigned int i = 0; i < numSampleRenders; ++i)
	{
		...
		if(earlyDepthTest)
		{
			for(unsigned int q = sampleLoopInit; q < sampleLoopEnd; q++)
			{
				...
				depthBoundsTest(zBuffer, q, x, z[q], zMask[q], cMask[q]);
			}
		}
...
void PixelRoutine::depthBoundsTest16(const Pointer<Byte> &zBuffer, int q, const Int &x, RValue<Float4> z, Int &zMask, Int &cMask)
{
	Pointer<Byte> buffer = zBuffer + 2 * x;
	Int pitch = *Pointer<Int>(data + OFFSET(DrawData, depthPitchB));
...
void PixelRoutine::depthBoundsTest32F(const Pointer<Byte> &zBuffer, int q, const Int &x, RValue<Float4> z, Int &zMask, Int &cMask)
{
	Pointer<Byte> buffer = zBuffer + 4 * x;
	Int pitch = *Pointer<Int>(data + OFFSET(DrawData, depthPitchB));
	...
	Int4 zTest = Int4(CmpLE(Float4(state.minDepthBounds), zValue) & CmpGE(Float4(state.maxDepthBounds), zValue));
	if(!state.depthTestActive)
	{
...
void PixelRoutine::depthBoundsTest(const Pointer<Byte> &zBuffer, int q, const Int &x, RValue<Float4> z, Int &zMask, Int &cMask)
{
	if(state.depthBoundsTestActive)
	{
		if(state.depthFormat == VK_FORMAT_D16_UNORM)
		{
			depthBoundsTest16(zBuffer, q, x, z, zMask, cMask);
		}
		else
		{
			depthBoundsTest32F(zBuffer, q, x, z, zMask, cMask);
		}
	}
}
