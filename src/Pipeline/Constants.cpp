...
namespace sw
{
	...
	Constants::Constants()
	{
		...
		constexpr float4 X[4] = {
			sw::replicate(samplePositions[0][0] - 0.5f), // -0.125
			sw::replicate(samplePositions[1][0] - 0.5f), // +0.375
			sw::replicate(samplePositions[2][0] - 0.5f), // -0.375
			sw::replicate(samplePositions[3][0] - 0.5f), // +0.125
		};

		constexpr float4 Y[4] = {
			sw::replicate(-(samplePositions[0][1] - 0.5f)), // +0.375
			sw::replicate(-(samplePositions[1][1] - 0.5f)), // +0.125
			sw::replicate(-(samplePositions[2][1] - 0.5f)), // -0.125
			sw::replicate(-(samplePositions[3][1] - 0.5f)), // -0.375
		};
		...
		const int Xf[4] = { toFP(X[2][0], subPixB), toFP(X[1][0], subPixB), toFP(X[3][0], subPixB), toFP(X[0][0], subPixB) };
		const int Yf[4] = { toFP(Y[2][0], subPixB), toFP(Y[1][0], subPixB), toFP(Y[3][0], subPixB), toFP(Y[0][0], subPixB) };
		...
		// large enough (+/- max range - epsilon) to include sample points
		{
			...
			yMinMultiSampleOffset = toFP(1, subPixB) - toFP(yMax, subPixB) - 1;
			yMaxMultiSampleOffset = toFP(1, subPixB) + toFP(yMax, subPixB) - 1;
		}
