...
void WaitForDebugger(HINSTANCE instance)
{
    if(!IsDebuggerPresent())
    {
        HRSRC res = FindResource(instance, MAKEINTRESOURCE(IDD_DIALOG1), RT_DIALOG);
		DLGTEMPLATE* pTemplate = (DLGTEMPLATE*)LoadResource(instance, res);
		DialogBoxIndirect(instance, pTemplate, NULL, DebuggerWaitDialogProc);
    }
}
