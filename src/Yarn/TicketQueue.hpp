...
class Ticket
{
    ...
    struct Record
    {
        ...
        ConditionVariable calledCondVar;
        std::mutex mutex;
        Task onCall; // guarded by mutex
        bool called; // guarded by mutex
    };
...
void Ticket::wait() const
{
    ...
    record->calledCondVar.wait(lock, [this] { return record->called; });
}
