...
public:
    ...
    template <typename F>
    inline void borrowN(size_t n, const F& f) const;
...
template <typename T, int N, PoolPolicy POLICY>
typename FixedSizePool<T, N, POLICY>::Loan FixedSizePool<T, N, POLICY>::borrow() const
{
    Loan out;
    borrowN(1, [&](Loan&& loan) { out = std::move(loan); });
    return out;
}

template <typename T, int N, PoolPolicy POLICY>
template <typename F>
void FixedSizePool<T, N, POLICY>::borrowN(size_t n, const F& f) const
{
...
template <typename T, int N, PoolPolicy POLICY>
std::pair<typename FixedSizePool<T, N, POLICY>::Loan, bool> FixedSizePool<T, N, POLICY>::tryBorrow() const
{
    std::unique_lock<std::mutex> lock(storage->mutex);
...
template <typename T, PoolPolicy POLICY>
template <typename F>
inline void UnboundedPool<T, POLICY>::borrowN(size_t n, const F& f) const
{
    std::unique_lock<std::mutex> lock(storage->mutex);
    for (size_t i = 0; i < n; i++)
    {
