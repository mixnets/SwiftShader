...
public:
    ...
    template <typename F>
    inline void borrowN(size_t n, const F& f);
...
template <typename T, int N, PoolPolicy POLICY>
template <typename F>
void FixedSizePool<T, N, POLICY>::borrowN(size_t n, const F& f)
{
...
template <typename T, PoolPolicy POLICY>
template <typename F>
inline void UnboundedPool<T, POLICY>::borrowN(size_t n, const F& f)
{
    std::unique_lock<std::mutex> lock(storage->mutex);
    for (size_t i = 0; i < n; i++)
    {
