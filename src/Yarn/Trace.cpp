...
Trace* Trace::get()
{
    ...
    static auto trace = new Trace(file);
    return trace;
...
{
    auto idx = eventQueueWriteIdx++ % NumEventQueues;
    auto &queue = eventQueues[idx];
    std::unique_lock<std::mutex> lock(queue.mutex);
    auto notify = queue.data.size() == 0;
    queue.data.push(event);
    lock.unlock();
...
{
    auto idx = eventQueueReadIdx++ % NumEventQueues;
    auto &queue = eventQueues[idx];
    std::unique_lock<std::mutex> lock(queue.mutex);
    queue.condition.wait(lock, [&queue] { return queue.data.size() > 0; });
    auto event = queue.data.front();
    queue.data.pop();
    return event;
}
