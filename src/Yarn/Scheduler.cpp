...
void Scheduler::enqueue(Task&& task)
{
    ...
    {
        size_t n = numSpinningWorkers.load();
...
bool Scheduler::beginSpinning(int workerId)
{
    for (int i = 0; i < 8; i++)
    {
        auto n = numSpinningWorkers.load();
        if (n < int(spinningWorkers.size()) && numSpinningWorkers.compare_exchange_weak(n, n + 1))
        {
            spinningWorkers[n] = workerId;
            return true;
        }
    }
    return false;
...
void Scheduler::endSpinning()
{
    auto n = --numSpinningWorkers;
    YARN_ASSERT(n >= 0, "numSpinningWorkers out of sync");
}
...
void Scheduler::Worker::waitForWork(std::unique_lock<std::mutex> &lock)
{
    ...
    if (work.num == 0)
    {
        if (scheduler->beginSpinning(id))
        {
            defer(scheduler->endSpinning());
            TRACE("SPIN DOWN");
            lock.unlock();
            spin();
            lock.lock();
