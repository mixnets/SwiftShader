...
void Scheduler::enqueue(Task&& task)
{
    while(true)
    {
        ...
        if (numThreads > 0)
        {
            ...
            if (auto thread = workerThreads[idx].load())
            {
...
Scheduler::Worker::Worker(Scheduler *scheduler, Mode mode, uint32_t id) : id(id), mode(mode), scheduler(scheduler)
{
    ...
    case Mode::MultiThreaded:
        thread = std::thread([=]
        {
            ...
            // static auto numCPUs = Thread::getNumCPUs();
            // static auto cpuBase = rand() % numCPUs;
            // std::bitset<1024> mask;
            // mask.set((id + cpuBase) % numCPUs);
