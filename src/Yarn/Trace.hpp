...
private:
    ...
    static constexpr unsigned int NumEventQueues = 32;
    EventQueue eventQueues[NumEventQueues];
    std::atomic<unsigned int> eventQueueWriteIdx;
    unsigned int eventQueueReadIdx;
...
#define SCOPED_EVENT(...) yarn::Trace::ScopedEvent YARN_CONCAT(scoped_event, __LINE__)(__VA_ARGS__);
#define BEGIN_ASYNC_EVENT(id, ...) do { if (auto t = yarn::Trace::get()) { t->beginAsyncEvent(id, __VA_ARGS__); } } while(false);
#define END_ASYNC_EVENT(id, ...) do { if (auto t = yarn::Trace::get()) { t->endAsyncEvent(id, __VA_ARGS__); } } while(false);
#define SCOPED_ASYNC_EVENT(id, ...) yarn::Trace::ScopedAsyncEvent YARN_CONCAT(defer_, __LINE__)(id, __VA_ARGS__);
#define NAME_THREAD(...) do { if (auto t = yarn::Trace::get()) { t->nameThread(__VA_ARGS__); } } while(false);

#else // YARN_TRACE_ENABLED

#define SCOPED_EVENT(...)
#define BEGIN_ASYNC_EVENT(id, ...)
#define END_ASYNC_EVENT(id, ...)
#define SCOPED_ASYNC_EVENT(id, ...)
#define NAME_THREAD(...)
